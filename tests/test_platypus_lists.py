...
from reportlab.platypus import (SimpleDocTemplate, Table, ListFlowable, ListItem,
...
parasitic gap construction.''',
    '''Blah blah blah blah blah blah discipline?... naked? ... With a melon!?
blah blah blah blah blah Very silly indeed Mr. Nesbitt has learned the first
lesson of 'Not Being Seen', not to stand up. blah blah blah Would you like
a twist of lemming sir?.''',
    '''Blah blah blah multidisciplinary blah blah blah blah blah blah blah
blah blah blah blah. Blah blah blah conceptualize blah contribution blah blah
blah blah blah blah blah blah blah blah blah blah proactive. Blah blah blah
blah blah blah proactive blah mastery learning blah blah blah blah blah
projection Total Quality Management blah.''',
    '''Blah Archer IV blah blah blah blah blah blah blah asteroid field USS
Enterprise quantum flux blah blah Pacifica blah blah blah blah blah asteroid
field. Blah blah K'Vort Class Bird-of-Prey battle bridge blah blah blah Bolian
blah blah Dr. Pulaski blah blah blah blah.''',
    '''Blah blah blah Rexx blah RFC822-compliant blah blah ...went into "yo-yo
mode" blah blah blah blah blah blah security blah DOS Unix blah blah blah. Blah
blah blah blah blah blah blah blah blah blah blah blah blah blah blah blah blah
blah blah Virtual Reality Modeling Language blah blah blah blah blah.''',
]
...
class ListsTestCase(unittest.TestCase):
    ...
    def test1(self):
        ...
        data = [[str(i+1), Paragraph("xx " * (i % 10), styleSheet["BodyText"]),
                 Paragraph(("blah "*(i % 40)), normal)] for i in range(5)]
        data1 = [[str(i+1), Paragraph(["zz ", "yy "][i]*(i+3),
                                      styleSheet["BodyText"]),
                  Paragraph(("duh  "*(i+3)), normal)] for i in range(2)]
        OL = ListFlowable([
            ...
            ListFlowable([
                ...
                Paragraph(TEXTS[0], normal), ], bulletType='i',),
        ...
        story.append(Paragraph(
            "Now try a list with a very long URL in it.  Without splitting the " +
            "long word it used to be that this can push out the right page margin",
            normal))
        OL = ListFlowable([
            Paragraph(TEXTS[1], normal),
            Paragraph('''For details about pairing the smart card reader with
the Android device, refer to the baiMobile specification:
<a href="http://www.biometricassociates.com/downloads/user-guides/baiMobile-3000MP-User-Guide-for-Android-v2.0.pdf" color="blue">http://www.biometricassociates.com/downloads/user-guides/make-the-url-even-longer/baiMobile-3000MP-User-Guide-for-Android-v2.0.pdf</a>.''',  # noqa
                      normal),
        ...
        story.append(Paragraph(
            "Same as above with a simple paragraph for the long word", normal))
        OL = ListFlowable([
            Paragraph(TEXTS[1], normal),
            Paragraph('''For details about pairing the smart card reader with the
Android device, refer to the baiMobile specification:
http://www.biometricassociates.com/downloads/user-guides/make-the-url-even-longer/baiMobile-3000MP-User-Guide-for-Android-v2.0.pdf.''', normal),  # noqa
            Paragraph(TEXTS[1], normal),
            ])
        story.append(OL)

        story.append(Paragraph(
            "Same as above with a simple unicode paragraph for the long word", normal))
        OL = ListFlowable([
            Paragraph(TEXTS[1], normal),
            Paragraph(u'''For details about pairing the smart card reader with the
Android device, refer to the baiMobile specification:
http://www.biometricassociates.com/downloads/user-guides/make-the-url-even-longer/baiMobile-3000MP-User-Guide-for-Android-v2.0.pdf.''', normal),  # noqa
            Paragraph(TEXTS[1], normal),
            ])
        ...
        story.append(
            ListFlowable([
                ...
                Paragraph("Level 0.4", normal), ],
                         bulletType='1', start='10',),
            )

        story.append(PageBreak())
        story.append(Paragraph("DDIndenter", style=normal))
        story.append(Paragraph("Coffee", style=bold))
        story.append(DDIndenter(Paragraph("Black hot drink", style=normal),
                                leftIndent=36))
        story.append(Paragraph("Milk", style=bold))
        story.append(DDIndenter(Paragraph("White cold drink", style=normal),
                                leftIndent=36))
        story.append(Paragraph("Whiskey", style=bold))
        story.append(DDIndenter(Paragraph("A nice alcoholic drink", style=normal),
                                leftIndent=36))
        story.append(PageBreak())
        story.append(Paragraph("MultiCol", style=normal))
        RT = ('STARTUP COMPUTERS BLAH BUZZWORD STARTREK ' +
              'PRINTING PYTHON CHOMSKY CHOMSKY').split()
        for i in range(5):
            ...
            story.append(
                MultiCol([
                    ...
                    [Paragraph(xmlEscape(randomtext.randomText(
                        topic, randint(1, 7))), style=normal) for j in range(np)]
                ], widths=['20%', 3, '80%'],))
