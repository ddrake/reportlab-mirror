...
class USPS_4State(Barcode):
    ...
    def binary(self):
        ...
        if not value:
            ...
            i = 2
            for name, nd in (('special services', 3), ('customer identifier', 6),
                             ('sequence number', 9)):
                j = i
                i += nd
                svalue = tracking[j:i]
    ...
    def codewords(self):
        ...
        if not self._codewords:
            ...
            A, B = divmod(A, 1365)
            assert 0 <= A <= 658, (
                'improper value %s passed to _2codewords A-->%s' %
                (hex(int(value)), A))
    ...
    def characters(self):
        ...
        >>> print(' '.join(hex(c)[2:] for c in USPS_4State('01234567094987654321',
        '01234567891').characters))
    ...
    _bits2bars = 'T', 'D', 'A', 'F'
    horizontalClearZone = property(lambda self: self.scale(
        'hcz', self.dimensions, self.widthScale))
    verticalClearZone = property(lambda self: self.scale(
        'vcz', self.dimensions, self.heightScale))
    ...
    def computeSize(self):
        if not getattr(self, '_sized', None):
            # ws = self.widthScale (assigned but not used)
            # hs = self.heightScale (assigned but not used)
    ...
    def value(self):
        ...
        return ' '.join((tracking[0:2], tracking[2:5], tracking[5:11],
                         tracking[11:])+routing)
    value = property(value, lambda self, value:
                     self.__dict__.__setitem__('tracking', value))
...
def _crc11(value):
    '''
    >>> usps = [USPS_4State('01234567094987654321',x).binary
    for x in ('','01234','012345678','01234567891')]
    >>> print(' '.join(nhex(x) for x in usps))
    0x1122103b5c2004b1 0xd138a87bab5cf3804b1 0x202bdc097711204d21804b1
    0x16907b2a24abc16a2e5c004b1
...
def _initNof13Table(N, lenT):
    ...
    >>> print(' '.join('T[%d]=%d' % (i, T[i])
    for i in (0,1,2,3,4,1271,1272,1284,1285,1286)))
    T[0]=31 T[1]=7936 T[2]=47 T[3]=7808 T[4]=55 T[1271]=6275
    T[1272]=6211 T[1284]=856 T[1285]=744 T[1286]=496
    '''
    T = lenT*[None]
    l = 0  # noqa
    u = lenT-1
