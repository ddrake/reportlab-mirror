...
if sys.version_info < __min_python_version__:
    raise ImportError(
        """reportlab requires %s.%s+; other versions are unsupported.
           If you want to try with other python versions edit line 10
           of reportlab/__init__ to remove this error.""" % (__min_python_version__))
...
try:
    import reportlab.local_rl_mods  # noqa
except ImportError:
    pass

try:
    # application specific modifications can be anywhere on python path
    import reportlab_mods  # noqa
except ImportError:
    try:
        _fake_import(os.path.expanduser(os.path.join('~', '.reportlab_mods')),
                     'reportlab_mods')
