...
class StandardEncryption:
    ...
    def __init__(self, userPassword, ownerPassword=None, canPrint=1, canModify=1,
                 canCopy=1, canAnnotate=1, strength=None):
        '''
        This class defines the encryption properties to be used while creating a pdf
        document.  Once initiated, a StandardEncryption object can be applied to a
        Canvas or a BaseDocTemplate.
        ...
        The boolean flags canPrint, canModify, canCopy, canAnnotate determine wether
        a user can perform the corresponding actions on the pdf when only a user
        password has been supplied.
        If the user supplies the owner password while opening the pdf, all actions
        can be performed regardless of the flags.
        Note that the security provided by these encryption settings (and even more
        so for the flags) is very weak.
        '''
        ...
        self.O = self.U = self.P = self.key = self.OE = self.UE = self.Perms = None  # noqa
    ...
    def prepare(self, document, overrideID=None):
        ...
        if overrideID:
            ...
            # initialize it...
            # externalID = document.ID()  (set but not used)
        ...
        if self.revision == 5:
            ...
            self.O = computeO(self.userPassword, self.ownerPassword, self.revision)  # noqa

            if DEBUG:
                print("self.O (as hex) = %s" % hexText(self.O))

            # print "\nself.O", self.O, repr(self.O)
            self.key = encryptionkey(self.userPassword, self.O, self.P, internalID,
    ...
    def info(self):
        ...
        if not self.prepared:
            raise ValueError("encryption not prepared!")
        return StandardEncryptionDictionary(O=self.O, OE=self.OE, U=self.U, UE=self.UE,
                                            P=self.P, Perms=self.Perms,
                                            revision=self.revision)
...
class StandardEncryptionDictionary(PDFObject):
    ...
    def __init__(self, O, OE, U, UE,  P, Perms, revision):  # noqa
...
def encryptionkey(password, OwnerKey, Permissions, FileId1, revision=None):
    ...
    if revision == 2:
        ...
        key = md5output[:16]
    if DEBUG:
        print('encryptionkey(%s,%s,%s,%s,%s)==>%s' %
              tuple([hexText(str(x)) for x in (password, OwnerKey, Permissions,
                                               FileId1, revision, key)]))
    return key
...
def computeO(userPassword, ownerPassword, revision):
    ...
    if DEBUG:
        print(('PadString=%s\nownerPad=%s\npassword=%s\n' +
               'userPad=%s\ndigest=%s\nrevision=%s') %
              (ascii(PadString), ascii(ownerPad), ascii(password), ascii(userPad),
               ascii(digest), revision))
    if revision == 2:
        O = ArcIV(digest[:5]).encode(userPad)  # noqa
    elif revision == 3:
        for i in range(50):
        ...
        for i in range(20):
            ...
            O = ArcIV(thisKey).encode(O)  # noqa
    if DEBUG:
        print('computeO(%s,%s,%s)==>%s' %
              tuple([hexText(str(x)) for x in (userPassword, ownerPassword,
                                               revision, O)]))
    return O
...
def computeU(encryptionkey, encodestring=PadString, revision=None, documentId=None):
    ...
    if revision == 2:
        ...
        result = tmp
    if DEBUG:
        print('computeU(%s,%s,%s,%s)==>%s' %
              tuple([hexText(str(x)) for x in (encryptionkey, encodestring, revision,
                                               documentId, result)]))
    return result
...
def encodePDF(key, objectNumber, generationNumber, string, revision=None):
    ...
    revision = checkRevision(revision)
    # print 'encodePDF (%s, %d, %d, %s)' %
    #        (hexText(key), objectNumber, generationNumber, string)
    ...
    if DEBUG:
        print('encodePDF(%s,%s,%s,%s,%s)==>%s' %
              tuple([hexText(str(x)) for x in
...
#  I am thinking about this one.  Needs a change to reportlab to work.
def encryptDocTemplate(dt, userPassword, ownerPassword=None,
                       canPrint=1, canModify=1, canCopy=1, canAnnotate=1,
                       strength=40):
...
def encryptPdfInMemory(inputPDF,
    ...
    try:
        ...
        raise ImportError('''reportlab.lib.pdfencrypt.encryptPdfInMemory failed
because rlextra cannot be imported.
See https://www.reportlab.com/downloads''')

    (bboxInfo, pickledForms) = storeFormsInMemory(inputPDF, all=1, BBoxes=1)
    # names = list(bboxInfo.keys())  (assigned but not used)
...
def encryptPdfOnDisk(inputFileName, outputFileName,
    ...
    outputPDF = encryptPdfInMemory(
        inputPDF, userPassword, ownerPassword,
        canPrint, canModify, canCopy, canAnnotate, strength=strength)
...
"""
    ...
    # try:
    # may be required later - eg if called by security.py
    # caller = sys_argv[0]  (set but not used)
    ...
    if len(argv) > 0:
        ...
        binaryrequired = ('-p', 'printable', '-m', 'modifiable', 'copypastable',
                          '-c', 'annotatable', '-a')
            ...
            if thisarg[0] in argv:
                ...
                if thisarg[0] in binaryrequired:
                    if argv[pos+1] not in ('1', '0'):
                        raise ValueError("%s value must be either '1' or '0'!" %
                                         thisarg[1])
                try:
        ...
        if verbose > 4:
            # useful if feeling paranoid and need
            # to double check things at this point...
            print("\ninfile:", infile)
