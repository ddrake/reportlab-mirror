...
from reportlab.platypus.paraparser import (ParaParser, _PCT, _num as _parser_num,
                                           _re_us_value)
...
def split(text, delim=None):
    ...
    return [uword for uword in (_wsc_re_split(text)
                                if delim is None and u'\xa0' in text else
                                text.split(delim))]
...
def _justifyDrawParaLine(tx, offset, extraspace, words, last=0):
    setXPos(tx, offset)
    text = ' '.join(words)
    simple = (getattr(tx, 'preformatted', False) or (-1e-8 < extraspace <= 1e-8) or
              (last and extraspace > -1e-8))
    if not simple:
        nSpaces = len(words)+_nbspCount(text)-1
        simple = nSpaces <= 0
    if simple:
...
def _justifyDrawParaLineRTL(tx, offset, extraspace, words, last=0):
    return (_rightDrawParaLine if last else
            _justifyDrawParaLine)(tx, offset, extraspace, words, last)
...
def _putFragLine(cur_x, tx, line, last, pKind):
    ...
    for i, f in enumerate(words):
        if hasattr(f, 'cbDefn'):
            ...
            if kind == 'img':
                ...
                tx._canvas.drawImage(cbDefn.image, cur_x_s, cur_y+iy0,
                                     w, h, mask='auto')
                cur_x += w
                cur_x_s += w
                ...
                if kind == 'anchor':
                    ...
                    if not func:
                        raise AttributeError("Missing %s callback attribute '%s'" %
                                             (kind, name))
                    tx._canvas._curr_tx_info = dict(tx=tx, cur_x=cur_x, cur_y=cur_y,
                                                    leading=leading, xs=tx.XtraState)
                    try:
            ...
            rise = f.rise
            if i > 0:
                end_x = cur_x_s - (0 if preformatted else
                                   _trailingSpaceLength(words[i-1].text, tx))
            ...
            if LL != f.us_lines:
                ...
                for ll in eL:
                    us_lines[ll] = us_lines[ll], end_x
                for ll in nL:
                    us_lines[ll] = (ll, fontSize, textColor, cur_x_s), fontSize
                LL = f.us_lines
            if LL:
                for ll in LL:
                    l0, fsmax = us_lines[ll]
                    if fontSize > fsmax:
                        us_lines[ll] = l0, fontSize
            ...
            if AL != f.link:
                ...
                for ll in eL:
                    links[ll] = links[ll], end_x
                for ll in nL:
                    links[ll] = (ll, cur_x), nlo, nhi
                AL = f.link
            if AL:
                for ll in AL:
                    l0, lo, hi = links[ll]
                    if nlo < lo or nhi > hi:
                        links[ll] = l0, min(nlo, lo), max(nhi, hi)
    ...
    if LL:
        for ll in LL:
            us_lines[ll] = us_lines[ll], cur_x_s

    if AL:
        for ll in AL:
            links[ll] = links[ll], cur_x_s
...
def _leftDrawParaLineX(tx, offset, line, last=0):
    ...
    if not simple:
        nSpaces = (line.wordCount +
                   sum([_nbspCount(w.text) for w in line.words
                        if not hasattr(w, 'cbDefn')])-1)
        simple = nSpaces <= 0
    if simple:
...
def _justifyDrawParaLineX(tx, offset, line, last=0):
    ...
    setXPos(tx, offset)
    extraSpace = line.extraSpace
    simple = (line.lineBreak or (-1e-8 < extraSpace <= 1e-8) or
              (last and extraSpace > -1e-8))
    if not simple:
        nSpaces = (line.wordCount +
                   sum([_nbspCount(w.text) for w in line.words
...
def _justifyDrawParaLineXRTL(tx, offset, line, last=0):
    return (_rightDrawParaLineX if last else
            _justifyDrawParaLineX)(tx, offset, line, last)
...
class _SHYWord(list):
    ...
    def shyphenate(self, newWidth, maxWidth):
        ...
        for i, (f, t) in enumerate(self[1:]):
            def sW(s):
                return stringWidth(s, f.fontName, f.fontSize)
        ...
        R = ([ww-L[0]+shyLen] +
             ([] if not right else
              [(f, _SHYIndexedStr(right, [_-x for _ in X[j+1:]]))]) + self[i1+1:])
        return _SplitFragSHY(L), _SHYWordHS(R)


class _SplitFragSHY(_SHYWord, _SplitFragHY):
    '''a head split frag that requires removal of a hyphen
       at the end before rejoining'''
...
def _reconstructSplitFrags(f):
    return ((_SHYWordHS if isinstance(f, _HSFrag) else
             _SHYWord) if isinstance(f, _SHYWord) else
            ((_SplitFragLL if isinstance(f, _SplitFragLL) else
              _HSFrag) if isinstance(f, _HSFrag) else list))
...
def _getFragWords(frags, maxWidth=None):
    ...
    if _processed_frags(frags):
        ...
        #    print('%3d: [%d, [%s]](%s)' % (_i,_r[0],', '.
        #     join(('%r' % _ff[1] for _ff in _r[1:])), type(_r)))
        if True:
            for f in frags:
            ...
            for f in frags:
                ...
                if isinstance(f, _SplitFrag):
                    ...
                    if not W:
                        ...
                        # lf not defined yet - was f intended?
                        # if isinstance(lf, _SplitFragHY):
                        if isinstance(f, _SplitFragHY):
                            sty, t = W[-1]
                            Wlen -= stringWidth(t[-1], sty.fontName,
                                                sty.fontSize) + 1e-8
                    ...
                    if isinstance(f, _HSFrag):
                        ...
                        # aR((((_SHYWordHS if isinstance(f,_HSFrag) else _SHYWord)
                    ...
                    if W:
                        # must end a joining
                        aR(_reconstructSplitFrags(f)([Wlen]+W))
                        # aR((((_SHYWordHS if isinstance(lf,_HSFrag) else _SHYWord)
                        #       if isinstance(lf,_SHYWord)
        ...
        for f in frags:
            ...
            if text != '':
                ...
                for w in S[:-1]:
                    ...
                    W.insert(0, n)
                    aR(_SHYWordHS(W) if shyIndices or isinstance(W, _SHYWord) else
                       _HSFrag(W))
                    W = []
                    shyIndices = False
                    n = 0
                ...
                if text and text[-1] in whitespace:
                    W.insert(0, n)
                    aR(_SHYWord(W) if shyIndices or isinstance(W, _SHYWord) else
                       _HSFrag(W))
                    W = []
                    shyIndices = False
                    n = 0
    ...
    # for _i,_r in enumerate(R):
    #    print('%3d: [%d, [%s]](%s)' %
    #          (_i,_r[0],', '.join(('%r' % _ff[1] for _ff in _r[1:])), type(_r)))
    return R
...
uri_pat = re.compile(u'(^(?:[a-z0-9\\.\\-\\+]*)://)(?:\\S+(?::\\S*)?@)?(?:(?:25[0-5]|2[0-4]\\d|[0-1]?\\d?\\d)(?:\\.(?:25[0-5]|2[0-4]\\d|[0-1]?\\d?\\d)){3}|\\[[0-9a-f:\\.]+\\]|([a-z\xa1-\uffff0-9](?:[a-z\xa1-\uffff0-9-]{0,61}[a-z\xa1-\uffff0-9])?(?:\\.(?!-)[a-z\xa1-\uffff0-9-]{1,63}(?<!-))*\\.(?!-)(?:[a-z\xa1-\uffff-]{2,63}|xn--[a-z0-9]{1,59})(?<!-)\\.?|localhost))(?::\\d{2,5})?(?:[/?#][^\\s]*)?\\Z', re.I)  # noqa
...
def _uri_split_pairs(uri):
    ...
    R = (([(scheme, uri)] if scheme and uri else []) +
         list(_slash_parts(uri, scheme, slash)))
    R.reverse()
    return R
...
_hy_letters = u'A-Za-z\xc0-\xd6\xd8-\xf6\xf8-\u024f\u1e80-\u1e85\u1e00-\u1eff\u0410-\u044f\u1e02\u1e03\u1e0a\u1e0b\u1e1e\u1e1f\u1e40\u1e41\u1e56\u1e57\u1e60\u1e61\u1e6a\u1e6b\u1e9b\u1ef2\u1ef3' # noqa
# explicit hyphens
_shy = u'\xad'
_hy_shy = u'-\xad'
...
def _hyGenPair(hyphenator, s, ww, newWidth, maxWidth, fontName, fontSize,
               uriWasteReduce, embeddedHyphenation, hymwl):
    if isBytes(s):
        s = s.decode('utf8')  # only encoding allowed
    ...
    if (hyphenator and
            (_hy_letters_pat.match(s) or
             (_hy_shy_letters_pat.match(s) and u'' not in H))):
    ...
    # even though the above tries for words with '-'
    # it may be that no split ended with '-'
...
def _fragWordSplitRep(FW):
    ...
    '''
    # cc = plen = 0 (plen is defined but not used)
    cc = 0
    X = []
    eX = X.extend
    U = []
...
def _hyGenFragsPair(hyphenator, FW, newWidth, maxWidth, uriWasteReduce,
...
def _hyphenateFragWord(hyphenator, FW, newWidth, maxWidth, uriWasteReduce,
                       embeddedHyphenation, hymwl=hyphenationMinWordLength):
    ww = FW[0]
    if ww == 0:
        return []
    if len(FW) == 2:
        ...
        if isinstance(FW, _SplitFragLL):
            s = _SplitWordLL(s)
        R = _hyGenPair(hyphenator, s, ww, newWidth, maxWidth, f.fontName,
                       f.fontSize, uriWasteReduce, embeddedHyphenation, hymwl)
        if R:
            jc, hylen, hw, tw, h, t = R
            return [(_SplitFragHY if jc else _SplitFragH)([hw+hylen, (f, h+jc)]),
                    (_SplitFragHS if isinstance(FW, _HSFrag) else
                     _SplitFrag)([tw, (f, t)])]
    else:
        R = _hyGenFragsPair(hyphenator, FW, newWidth, maxWidth, uriWasteReduce,
                            embeddedHyphenation, hymwl)
        if R:
            jc, h, t = R
...
def _splitWord(w, lineWidth, maxWidths, lineno, fontName,
               fontSize, encoding='utf8'):
    '''
...
def _yieldBLParaWords(blPara, start, stop):
    R = []
    aR = R.append
    for ll in blPara.lines[start:stop]:
        for w in ll[1]:
...
def _split_blParaSimple(blPara, start, stop):
    ...
    if isinstance(f.words[-1], _SplitWordHY):
        f.words[-1].__class__ = (_SHYSplit if isinstance(f.words[-1], _SHYStr) else
                                 _SplitWordLL)
    return [f]
...
def _split_blParaHard(blPara, start, stop):
    ...
    for ll in lines:
        for w in ll.words:
            f.append(w)
        if ll is not lines[-1]:
            i = len(f)-1
            while (i >= 0 and hasattr(f[i], 'cbDefn') and
                    not getattr(f[i].cbDefn, 'width', 0)):
                i -= 1
            if i >= 0:
                g = f[i]
                if not g.text:
...
def _drawBullet(canvas, offset, cur_y, bulletText, style, rtl):
    ...
    if rtl or style.bulletAnchor != 'start':
        ...
        if isStr(bulletText):
            ...
            if q:
                bulletWidth += 0.5 * stringWidth(decimalSymbol, style.bulletFontName,
            ...
            for f in bulletText:
                ...
                if q:
                    t = t[:t.index(decimalSymbol)]
                    bulletWidth += 0.5 * stringWidth(decimalSymbol, f.fontName,
                                                     f.fontSize)
...
def _handleBulletWidth(bulletText, style, maxWidths):
    ...
    if bulletText:
        if isStr(bulletText):
            bulletWidth = stringWidth(bulletText, style.bulletFontName,
                                      style.bulletFontSize)
        else:
            # it's a list of fragments
            bulletWidth = 0
...
def _do_under_line(i, x1, ws, tx, us_lines):
    ...
    # dw = tx._defaultLineWidth (assigned but not used)
    x2 = (x1 + tx._canvas.stringWidth(
        ' '.join(tx.XtraState.lines[i][1]), tx._fontname, fs) + ws)
    for n, k, c, w, o, r, m, g in us_lines:
        ...
        if not underline:
            dy = -dy
        y = (y0 + r +
             _usConv(('-0.125*L' if underline else '0.25*L') if o == '' else
                     o, values))
        if not c:
            c = tc
        while m > 0:
...
def _do_post_text(tx):
    ...
    if xs.us_lines:
        ...
        for (((n, k, c, w, o, r, m, g),
              fs, tc, x1), fsmax), x2 in sorted(xs.us_lines.values()):
            underline = k == 'underline'
            values['f'] = fs
            values['F'] = fsmax
            lw = _usConv(w, values, default=tx._defaultLineWidth)
            ...
            if not underline:
                dy = -dy
            y = (y0 + r +
                 _usConv(o if o != '' else ('-0.125*L' if underline else '0.25*L'),
                         values))
            # print 'n=%s k=%s x1=%s x2=%s r=%s c=%s w=%r o=%r fs=%r tc=%s y=%s
            #      lw=%r offs=%r' % (n,k,x1,x2,r,(c.hexval() if c else ''),
            #      w,o,fs,tc.hexval(),y,lw,y-y0-r)
            if not c:
                c = tc
            while m > 0:
                tx._do_line(x1, y, x2, y, lw, c)
...
def makeCJKParaLine(U, maxWidth, widthUsed, extraSpace, lineBreak, calcBounds):
    ...
    for u in U:
        ...
        if calcBounds:
            ...
            if getattr(cbDefn, 'width', 0):
                descent, ascent = imgVRange(imgNormV(cbDefn.height, fontSize),
    ...
    return FragLine(kind=1, extraSpace=extraSpace, wordCount=1, words=words[1:],
                    ...
                    maxWidth=maxWidth, currentWidth=widthUsed, lineBreak=lineBreak)
    ...
    while i < nU:
        ...
        if endLine:
            ...
            if not lineBreak:
                ...
                # if this character cannot start a line, wrap it up to this line so
                # it hangs in the right margin. We won't do two or more though -
                # that's unlikely and would result in growing ugliness.
...
class Paragraph(Flowable):
    """ Paragraph(text, style, bulletText=None, caseSensitive=1)
        ...
        caseSensitive set this to 0 if you want the markup tags
        and their attributes to be case-insensitive.
    ...
    def wrap(self, availWidth, availHeight):
        ...
        first_line_width = (availWidth - (leftIndent+style.firstLineIndent) -
                            style.rightIndent)
        ...
        if blPara.kind == 1:
            if autoLeading not in ('', 'off'):
                height = 0
                if autoLeading == 'max':
                    for ll in blPara.lines:
                        height += max(ll.ascent-ll.descent, leading)
                elif autoLeading == 'min':
                    for ll in blPara.lines:
                        height += ll.ascent - ll.descent
                else:
    ...
    def minWidth(self):
        ...
        if nFrags == 1 and not _processed_frags(frags):
            ...
            fS = f.fontSize
            fN = f.fontName
            return max(stringWidth(w, fN, fS)
                       for w in (split(f.text, ' ')
                                 if hasattr(f, 'text') else f.words))
        else:
    ...
    def _split_blParaProcessed(self, blPara, start, stop):
        ...
        if isinstance(F[-1], _SplitFragHY):
            F[-1].__class__ = (_SHYWordHS if isinstance(F[-1], _SHYWord) else
                               _SplitFragLL)
        return F

    def _get_split_blParaFunc(self):
        return (_split_blParaSimple if self.blPara.kind == 0 else
                (_split_blParaHard if not _processed_frags(self.frags)
    ...
    def split(self, availWidth, availHeight):
        ...
        if style.endDots:
            ...
            style1 = style
        P1 = self.__class__(None, style1, bulletText=self.bulletText,
                            frags=func(blPara, 0, s))
        # this is a major hack
        P1.blPara = ParaLines(kind=1, lines=blPara.lines[0:s],
    ...
    def breakLines(self, width):
        ...
        This structure can be used to easily draw paragraphs with the various
        alignments.  You can supply either a single width or a list of widths;
        the latter will have its last item repeated until necessary. A 2-element
        list is useful when there is a different first line indent; a longer list
        could be created to facilitate custom wraps around irregular objects.
        """
        ...
        if hyphenator:
            if isStr(hyphenator):
                ...
                raise ValueError(('hyphenator should be a language spec or a ' +
                                  'callable unicode -->  pairs not %r') % hyphenator)
        else:
            hyphenator = None
        ...
        frags = self.frags
        nFrags = len(frags)
        if (nFrags == 1 and not (style.endDots or hasattr(frags[0], 'cbDefn') or
                                 hasattr(frags[0], 'backColor') or
                                 _processed_frags(frags))):
            f = frags[0]
            fontSize = f.fontSize
            fontName = f.fontName
            ...
            if hasattr(f, 'text'):
                text = strip(f.text)
                if not text:
                    return f.clone(kind=0, lines=[], ascent=ascent, descent=descent,
                                   fontSize=fontSize)
                else:
                    words = split(text)
            else:
            ...
            while words:
                ...
                # print(f's: {currentWidth=} spaceShrink={limWidth-maxWidth}
                #       {newWidth=} {limWidth=} {newWidth>limWidth}
                #        cond={newWidth>limWidth and not (isinstance(word,_SplitWordH)
                #        or forcedSplit)} {word=}')
                if (newWidth > limWidth and
                        not (isinstance(word, _SplitWordH) or forcedSplit)):
                    if isinstance(word, _SHYStr):
                        hsw = word.__shysplit__(
                                fontName, fontSize,
                                currentWidth + spaceWidth + hyw - 1e-8,
                                limWidth, encoding=self.encoding,
                                )
                        if hsw:
                            words[0:0] = hsw
                        ...
                        hsw = _hyphenateWord(
                            hyphenator if hyOk else None, fontName, fontSize, word,
                            wordWidth, newWidth, limWidth,
                    ...
                    if splitLongWords and not (isinstance(word, _SplitWord) or
                        ...
                        if wordWidth > max(maxWidths[nmw:nmw+1]):
                            ...
                            words[0:0] = _splitWord(
                                word, currentWidth+spaceWidth, maxWidths, lineno,
                ...
                if newWidth <= limWidth or not len(cLine) or forcedSplit:
                    ...
                    if word:
                        cLine.append(word)
                    # print(f's: |line|={len(cLine)} {newWidth=}
                    #       spaceShrink={limWidth-maxWidth}')
                    if forcedSplit:
                        forcedSplit = 0
            ...
            return f.clone(kind=0, lines=lines, ascent=ascent, descent=descent,
                           fontSize=fontSize)
        elif nFrags <= 0:
            return ParaLines(kind=0, fontSize=style.fontSize, fontName=style.fontName,
                             textColor=style.textColor, ascent=style.fontSize,
                             descent=-0.2*style.fontSize, lines=[])
        else:
            ...
            while _words:
                ...
                # test to see if this frag is a line break. If it is we will only
                # act on it if the current width is non-negative or the previous
                # thing was a deliberate lineBreak
                ...
                if spaceShrinkage:
                    ...
                    for wi in words:
                        if wi._fkind == _FK_TEXT:
                            ns = wi.text.count(' ')
                            if ns:
                                spaceShrink += ns*stringWidth(' ', wi.fontName,
                                                              wi.fontSize)
                    spaceShrink *= spaceShrinkage
                    limWidth += spaceShrink
                # print(f'c: {currentWidth=} {spaceShrink=} {newWidth=}
                # {limWidth=} {newWidth>(maxWidth+spaceShrink)}
                # cond={not lineBreak and newWidth>limWidth and not
                # isinstance(w,_SplitFragH) and not hasattr(f,"cbDefn")}
                # word={w[1][1]}')
                if (not lineBreak and newWidth > limWidth and
                        not isinstance(w, _SplitFragH) and
                        not hasattr(f, 'cbDefn')):
                    if isinstance(w, _SHYWord):
                        hsw = w.shyphenate(newWidth, limWidth)
                        if hsw:
                            _words[0:0] = hsw
                            _words.insert(1, _InjectedFrag(
                        ...
                        hsw = _hyphenateFragWord(
                            hyphenator if hyOk else None, w, newWidth, limWidth,
                            uriWasteReduce if hyOk else False,
                            embeddedHyphenation and hyOk, hymwl)
                        if hsw:
                            _words[0:0] = hsw
                            ...
                            hsw = _hyphenateFragWord(
                                hyphenator if hyOk else None, w, wordWidth, maxWidth,
                                uriWasteReduce if hyOk else False,
                                embeddedHyphenation and hyOk, hymwl)
                            if hsw:
                                _words[0:0] = [
                    ...
                    if splitLongWords and not isinstance(w, _SplitFrag):
                        ...
                        if wordWidth > max(maxWidths[nmw:nmw+1]):
                            ...
                            _words.insert(
                                1, _InjectedFrag([0, (f.clone(
                                    _fkind=_FK_BREAK, text=''), '')]))
                            FW.pop(-1)  # remove this as we are doing this one again
                            self._splitLongWordCount += 1
                            continue
                endLine = (newWidth > limWidth and n > 0) or lineBreak
                if not endLine:
                    # print(f'c: |line|={len(words)} {newWidth=}
                    #       spaceShrink={limWidth-maxWidth}')
                    if lineBreak:
                    ...
                    spaceWidth = (stringWidth(' ', fontName, fontSize)
                                  if isinstance(w, _HSFrag) else 0)
                    ...
                    if currentWidth > self._width_max:
                        self._width_max = currentWidth
                    # end of line
                    lines.append(FragLine(
                        extraSpace=maxWidth-currentWidth, wordCount=n, lineBreak=llb,
                        words=words, fontSize=maxSize, ascent=maxAscent,
                        descent=minDescent, maxWidth=maxWidth, sFW=sFW))
                    sFW = len(FW)-1
            ...
            if words:
                ...
                lines.append(ParaLines(
                    extraSpace=(maxWidth - currentWidth), wordCount=n,
                    lineBreak=False, words=words, fontSize=maxSize, ascent=maxAscent,
                    descent=minDescent, maxWidth=maxWidth, sFW=sFW))
            self.frags = FW
            return ParaLines(kind=1, lines=lines)
    ...
    def breakLinesCJK(self, maxWidths):
        ...
        if nFrags == 1 and not hasattr(frags[0], 'cbDefn') and not style.endDots:
            ...
            # single frag case
            lines = []
            # lineno = 0 (assigned but not used)
            if hasattr(f, 'text'):
                text = f.text
            else:
            ...
            # the paragraph drawing routine assumes multiple frags per line,
            # so we need an extra list like this
            # [space, [text]]
            wrappedLines = [(sp, [line]) for (sp, line) in lines]
            return f.clone(kind=0, lines=wrappedLines, ascent=f.fontSize,
                           descent=-0.2*f.fontSize)
        elif nFrags <= 0:
            return ParaLines(kind=0, fontSize=style.fontSize, fontName=style.fontName,
        ...
        if nLines > 0:
            ...
            if blPara.kind == 0:
                ...
                if f.us_lines or f.link:  # or style.endDots:
                    ...
                    # if lastLine and style.endDots and dpl!=_rightDrawParaLine:
                ...
                if bulletText:
                    # oo = offset (assigned but not used)
    ...
    def dumpFrags(frags, indent=4, full=False):
        ...
        for i, f in enumerate(frags):
            if full:
                ...
                aR('[%r, %s], #%d %s' % (f[0], ', '.join(
                    ('(%s,%r)' %
                     (fx[0].__class__.__name__, fx[1]) for fx in f[1:])),
                                         i, f.__class__.__name__))
        i = indent*' '
        return i + ('\n'+i).join(R)
...
if __name__ == '__main__':    # NORUNTESTS
    def dumpParagraphLines(P):
        ...
        for ll, line in enumerate(lines):
            ...
            nwords = len(words)
            outw('line%d: %d(%s)\n  ' %
                 (ll, nwords, str(getattr(line, 'wordCount', 'Unknown'))))
    ...
    def dumpParagraphFrags(P):
        ...
        for ll in range(n):
            print("frag%d: '%s' %s" %
                  (ll, frags[ll].text, ' '.join(
                      ['%s=%s' %
                       (k, getattr(frags[ll], k))
                       for k in frags[ll].__dict__ if k != text])))
        ...
        for W in _getFragWords(frags, 360):
            cum += W[0]
            outw("fragword%d: cum=%3d size=%d" % (ll, cum, W[0]))
            for w in W[1:]:
                outw(' (%s)' % fragDump(w))
            print()
            ll += 1
        ...
        if isinstance(P2.frags[0], list):  # noqa
            ...
            def _showWord(w):
                ...
                return ('\x20\x20%s([%s, %s]),' %
                        (w.__class__.__name__, w[0], ', '.join(t.__self__)))
            for _ in P2.frags:  # noqa
                _S(_showWord(_))
            print('from reportlab.platypus.paragraph import _HSFrag, _SplitFragHS, ' +
                  '_SplitFragHY, _SplitFrag, _getFragWords\n' +
                  'from reportlab.platypus.paraparser import ParaFrag\n' +
                  'from reportlab.lib.colors import Color')
            print('\n'.join(('__frag_%s__ = %r' % _ for _ in sorted(_F.values()))))
            # bug? undefined nabe processed_frags
            print('%s=[\n%s\x20\x20]' % (processed_frags, '\n'.join(_S.__self__)))  # noqa
    ...
    if flagged(1):
        text = '''The <font name=courier color=green>CMYK</font> or subtractive method
        follows the way a printer mixes three pigments (cyan, magenta, and yellow) to
        form colors.  Because mixing chemicals is more difficult than combining light
        there is a fourth parameter for darkness.  For example a chemical combination
        of the <font name=courier color=green>CMY</font> pigments generally never
        makes a perfect black -- instead producing a muddy color -- so, to get black
        printers don't use the <font name=courier color=green>CMY</font> pigments but
        use a direct black ink.  Because <font name=courier color=green>CMYK</font>
        maps more directly to the way printer hardware works it may be the case that
        &amp;| &amp; | colors specified in <font name=courier color=green>CMYK</font>
        will provide better fidelity and better control when printed.'''
    ...
    if flagged(2):
        P = Paragraph("""Price<super><font color="red">*</font></super>""",
                      styleSheet['Normal'])
        dumpParagraphFrags(P)
        w, h = P.wrap(24, 200)
        dumpParagraphLines(P)
...
umfassend zu sein."""  # noqa
    ...
    if flagged(6):
        for text in ['''Here comes <FONT FACE="Helvetica" SIZE="14pt">Helvetica 14</FONT> with <STRONG>strong</STRONG> <EM>emphasis</EM>.''',  # noqa
                     '''Here comes <font face="Helvetica" size="14pt">Helvetica 14</font> with <Strong>strong</Strong> <em>emphasis</em>.''',  # noqa
                     '''Here comes <font face="Courier" size="3cm">Courier 3cm</font> and normal again.''',  # noqa
                     ]:
    ...
    if flagged(7):
        text = """<para align="CENTER" fontSize="24" leading="30"><b>Generated by:</b>Dilbert</para>"""  # noqa
        P = Paragraph(text, styleSheet['Code'])
        dumpParagraphFrags(P)
        w, h = P.wrap(6*72, 9.7*72)
        dumpParagraphLines(P)
    ...
    if flagged(8):
        text = """- bullet 0<br/>- bullet 1<br/>- bullet 2<br/>- bullet 3<br/>- bullet 4<br/>- bullet 5"""  # noqa
...
would follow from the assumption that"""  # noqa
        P = Paragraph(text, ParagraphStyle('aaa', parent=styleSheet['Normal'],
                                           align=TA_JUSTIFY))
        dumpParagraphFrags(P)
...
It should be a normal<onDraw name="_indexAdd" label="normal"/> paragraph, set in Courier 12 pt."""  # noqa
        P = Paragraph(text, style=ParagraphStyle('style1', fontName="Courier",
                                                 fontSize=10))
        dumpParagraphFrags(P)
