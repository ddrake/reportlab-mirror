...
__version__ = '3.3.0'
'''Superclass for renderers to factor out common functionality
and default implementations.'''

from reportlab.graphics.shapes import (STATE_DEFAULTS, UserNode, Line, Image, Rect,
                                       Circle, Ellipse, PolyLine, Polygon, Path,
                                       String, Group, Wedge, DirectDraw)
    ...
    def push(self, delta):
        ...
        for key, value in delta.items():
            if key == 'transform':  # do cumulative matrix
                ...
                # print 'statetracker transform = (%0.2f, %0.2f, %0.2f, %0.2f,
                #       %0.2f, %0.2f)' % tuple(newstate['transform'])
...
def testStateTracker():
    ...
    defaults = {'fillColor': None, 'strokeColor': None, 'fontName': None,
...
class Renderer:
    ...
    def undefined(self, operation):
        raise ValueError("%s operation not defined at superclass class=%s" %
        ...
        try:
            ...
            if showBoundary:
                if hasattr(canvas, 'drawBoundary'):
                    canvas.drawBoundary(showBoundary, x, y,
    ...
    _restores = {'stroke': '_stroke', 'stroke_width': '_lineWidth',
                 'stroke_linecap': '_lineCap', 'stroke_linejoin': '_lineJoin',
    ...
    def drawGroup(self, group):
        ...
        for node in group.getContents():
            ...
            # here is where we do derived values - this seems to get everything.
            # Touch wood.
            self.fillDerivedValues(node)
            try:
                if hasattr(node, '_canvas'):
