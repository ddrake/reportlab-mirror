...
__version__ = '3.3.0'
"""
A flowable is a "floating element" in a document whose exact position is determined
by the other elements that precede it, such as a paragraph, a diagram interspersed
between paragraphs, a section header, etcetera.  Examples of non-flowables include
...
at a higher level. The object's draw() method should assume that (0,0) corresponds to
the bottom left corner of the enclosing rectangle that will contain the object. The
attributes vAlign and hAlign may be used by 'packers' as hints as to how the object
should be placed.
...
from reportlab.lib.styles import _baseFontName, ListStyle
...
__all__ = '''AnchorFlowable BalancedColumns BulletDrawer CallerMacro CondPageBreak
             DDIndenter DocAssert DocAssign DocExec DocIf DocPara DocWhile FailOnDraw
             FailOnWrap Flowable FrameBG FrameSplitter HRFlowable Image
             ImageAndFlowables KeepInFrame KeepTogether LIIndenter ListFlowable ListItem
             Macro NullDraw PTOContainer PageBreak PageBreakIfNotEmpty ParagraphAndImage
             Preformatted SetPageTopFlowables SetTopFlowables SlowPageBreak Spacer
             TopPadder TraceInfo UseUpSpace XBox splitLine splitLines'''.split()
...
class Flowable:
    ...
    def getKeepWithNext(self):
        """returns boolean determining whether the next flowable
        should stay with this one"""
    ...
    def getSpaceAfter(self):
        """returns how much space should follow this item if another item
    ...
    def getSpaceBefore(self):
        """returns how much space should precede this item if another item
        precedess on the same page."""
...
def splitLines(lines, maximum_length, split_characters, new_line_characters):
    ...
    for line in lines:
        if len(line) > maximum_length:
            splitLine(line, lines_splitted, maximum_length, split_characters,
                      new_line_characters)
        else:
...
def splitLine(line_to_split, lines_splitted, maximum_length, split_characters,
              new_line_characters):
...
class Preformatted(Flowable):
    ...
    It attempts to display text exactly as you typed it in a fixed width "typewriter"
    font.  By default the line breaks are exactly where you put them, and it will not
    be wrapped.  You can optionally define a maximum line length and the code will be
    wrapped; and extra characters to be inserted at the beginning of each wrapped
    line (e.g. '> ').
    """
    def __init__(self, text, style, bulletText=None, dedent=0, maxLineLength=None,
                 splitChars=None, newLineChars=""):
...
class Image(Flowable):
    """an image (digital picture).  Formats supported by PIL/Java 1.4 (the Python/Java
       Imaging Library are supported. Images as flowables may be aligned horizontally
    ...
    def __init__(self, filename, width=None, height=None, kind='direct',
        ...
        self._dpi = useDPI
        if not fp and os.path.splitext(filename)[1] in ['.jpg', '.JPG',
                                                        '.jpeg', '.JPEG']:
    ...
    def _setup_inner(self):
        ...
        if img:
            ...
            self.imageWidth, self.imageHeight = (self._drawing.width,
...
class PageBreak(UseUpSpace):
    ...
    def __repr__(self):
        return ("%s(%s)" %
                (self.__class__.__name__, repr(self.nextTemplate)
                 if self.nextTemplate else ''))
...
def _flowableSublist(V):
    ...
    assert not [x for x in V if isinstance(x, LCActionFlowable)], \
        ...
        try:
            ...
            self.annotateException('\nraised by class %s(%s)@0x%8.8x wrap\n' %
                                   (self.__class__.__name__,
                                    self.__class__.__module__, id(self)))
        self._H = H
...
class ParagraphAndImage(Flowable):
    ...
    def __init__(self, P, I, xpad=3, ypad=3, side='right'):  # noqa
        self.P = P
        self.I = I  # noqa
        self.xpad = xpad
        self.ypad = ypad
    ...
    def wrap(self, availWidth, availHeight):
        ...
        P.blPara = P.breakLines([first_line_width] +
                                nIW*[intermediate_widths]+[later_widths])
        if self._side == 'left':
    ...
    def split(self, availWidth, availHeight):
        # wI assigned but not used
        P, hI, ypad = self.P, self.hI, self.ypad
    ...
    def drawOn(self, canv, x, y, _sW=0, scale=1.0, content=None, aW=None):
        ...
        for c in content:
            ...
            if fbg and fbg[-1].active:
                ...
                fbgl = bg.left
                fbgr = bg.right
                # bgm = bg.start (assigned but not used)
...
class PTOContainer(_Container, Flowable):
    ...
    def __init__(self, content, trailer=None, header=None):
        I = _PTOInfo(trailer, header)  # noqa
        self._content = C = []
    ...
    def split(self, availWidth, availHeight):
        ...
        if len(SS) > 1:
            ...
            R1 = C[:i]+R1I+T+F
            R2 = Hdr+R2I+C[i:]
        T = R1 + [PTOContainer(R2, [copy(x) for x in I.trailer],
...
class _FindSplitterMixin:
    def _findSplit(self, canv, availWidth, availHeight, mergeSpace=1, obj=None,
                   content=None, paraFix=True):
        '''return max width, required height for a list of flowables F'''
        W = 0
        H = 0
        pS = 0  # sB defined but not used
        atTop = 1
...
class ImageAndFlowables(_Container, _FindSplitterMixin, Flowable):
    ...
    def __init__(self, I, F, imageLeftPadding=0, imageRightPadding=3,  # noqa
    ...
    def wrap(self, availWidth, availHeight):
        ...
        if hasattr(self, '_wrapArgs'):
            if (self._wrapArgs == (availWidth, availHeight) and
                    getattr(I, '_oldDrawSize', None)) is None:
                return self.width, self.height
            self._reset()
            I._unRestrictSize()
    ...
    def split(self, availWidth, availHeight):
        if hasattr(self, '_wrapArgs'):
            I = self._I  # noqa
            if (self._wrapArgs != (availWidth, availHeight) or
                    getattr(I, '_oldDrawSize', None) is not None):
                self._reset()
                I._unRestrictSize()
    ...
    def drawOn(self, canv, x, y, _sW=0):
        ...
        if self.imageHref:
            canv.linkURL(self.imageHref, (Ix, y+self.height-self._itpad-self._hI,
                                          Ix + self._wI, y+self.height), relative=1)
...
class _AbsRect(NullDraw):
    ...
    def __init__(self, x, y, width, height, strokeWidth=0, strokeColor=None,
                 fillColor=None, strokeDashArray=None):
        self._x = x
        self._y = y
        self._width = width
        self._height = height
    ...
    def drawOn(self, canv, x, y, _sW=0):
        if self._width > _FUZZ and self._height > _FUZZ:
            st = (self._strokeColor and self._strokeWidth is not None
            ...
            if st or self._fillColor:
                ...
                canv.rect(self._x, self._y, self._width, self._height,
                          stroke=1 if st else 0, fill=1 if self._fillColor else 0)
                canv.restoreState()
...
class _AbsLine(NullDraw):
    ...
    def __init__(self, x, y, x1, y1, strokeWidth=0, strokeColor=None,
    ...
    def drawOn(self, canv, x, y, _sW=0):
        if (self._strokeColor and self._strokeWidth is not None
                and self._strokeWidth >= 0):
            canv.saveState()
            canv.setStrokeColor(self._strokeColor)
            canv.setLineWidth(self._strokeWidth)
...
class BalancedColumns(_FindSplitterMixin, NullDraw):
    '''combine a list of flowables and an Image'''
    def __init__(self, F, nCols=2, needed=72, spaceBefore=0, spaceAfter=0,
                 showBoundary=None, leftPadding=None, innerPadding=None,
                 rightPadding=None, topPadding=None, bottomPadding=None,
                 name='', endSlack=0.1, boxStrokeColor=None, boxStrokeWidth=0,
                 boxFillColor=None, boxMargin=None, vLinesStrokeColor=None,
                 vLinesStrokeWidth=None):
        self.name = name or 'BalancedColumns-%d' % id(self)
        if nCols < 2:
            raise ValueError('nCols should be at least 2 not %r in %s' %
                             (nCols, self.identitity()))
        self._content = _flowableSublist(F)
        self._nCols = nCols
        self.spaceAfter = spaceAfter
    ...
    def identity(self, maxLen=None):
        return ("<%s nCols=%r at %s%s%s>" %
                (self.__class__.__name__, self._nCols, hex(id(self)), self._frameName(),
                 getattr(self, 'name', '') and
                 (' name="%s"' % getattr(self, 'name', '')) or ''))
    ...
    def _generated_content(self, aW, aH):
        ...
        bpad = (frame._bottomPadding if self._bottomPadding is None else
                self._bottomPadding)
        ...
        def splitFunc(ah, endSlack=0):
            if ah not in _fres:
                ...
                for i in range(nCols):
                    wi, hi, c0, c1 = self._findSplit(canv, cw, ah, content=cn,
                                                     paraFix=False)
                    w = max(w, wi)
                    h = max(h, hi)
                    c.append(c0)
                    if i == icheck:
                        wc, hc, cc0, cc1 = self._findSplit(canv, cw, 2*ah, content=c1,
                                                           paraFix=False)
                        if hc <= (1+endSlack)*ah:
                            c.append(c1)
                            h = ah-1e-6
        ...
        x1 = frame._x1
        y1 = frame._y1
        # fw = frame._width (assigned but not used)
        ...
        showBoundary = (self.showBoundary if self.showBoundary is not None else
                        frame.showBoundary)
        ...
        if T is None:
            raise LayoutError('%s used in non-doctemplate environment' %
                              self.identity())
        ...
        if indenter0:
            G.append(indenter0)
        doBox = (self._boxStrokeColor and self._boxStrokeWidth and
                 self._boxStrokeWidth >= 0) or self._boxFillColor
        doVLines = (self._vLinesStrokeColor and self._vLinesStrokeWidth and
                    self._vLinesStrokeWidth >= 0)
        ...
        if C1:
            G.append(
                BalancedColumns(
                    C1, nCols=nCols, needed=self._needed, spaceBefore=self.spaceBefore,
                    spaceAfter=self.spaceAfter, showBoundary=self.showBoundary,
                    leftPadding=self._leftPadding, innerPadding=self._innerPadding,
                    rightPadding=self._rightPadding, topPadding=self._topPadding,
                    bottomPadding=self._bottomPadding, name=self.name+'-1',
                    endSlack=self.endSlack, boxStrokeColor=self._boxStrokeColor,
                    boxStrokeWidth=self._boxStrokeWidth,
                    boxFillColor=self._boxFillColor, boxMargin=self._boxMargin,
    ...
    def wrap(self, aW, aH):
        ...
        if aH < self.spaceBefore+self._needed-_FUZZ:
            # we are going straight to the nextTemplate
            # with no attempt to modify the frames
            G = [PageBreak(), self]
            H1 = 0
        else:
            if not self_frame:
    ...
    def render(self, canv, frame, fbx, fby, fbw, fbh):
        if abs(fbw) > _FUZZ and abs(fbh) > _FUZZ:
            ...
            if self.fid == id(frame) and self.cid == id(canv) and self.pn == pn:
                ...
                canv.rect(fbx, fby, fbw, fbh, stroke=1 if sc else 0,
                          fill=1 if fbgc else 0)
                canv.restoreState()
...
class FrameBG(AnchorFlowable):
    ...
    if start in ('frame','frame-permanent') then the background is filled from here
    to the bottom of the frame and immediately discarded for the frame case.
    """
    _ZEROSIZE = 1

    def __init__(self, color=None, left=0, right=0, start=True, strokeWidth=None,
                 strokeColor=None, strokeDashArray=None):
        Spacer.__init__(self, 0, 0)
        self.start = start
        if start:
    ...
    def __repr__(self):
        return ("%s(%s)" %
                (self.__class__.__name__,
                 ', '.join(['%s=%r' % (i, getattr(self, i, None))
    ...
    def draw(self):
        ...
        if frame is None:
            return
        if self.start:
            # sc = self.strokeColor (assigned but not used)
            sw = self.strokeWidth
            ...
            frame._frameBGs.append(
                _FBGBag(left=self.left,
                        ...
                        start=(self.start
                               if self.start in ('frame', 'frame-permanent') else None),
...
class FrameSplitter(NullDraw):
    ...
    def wrap(self, aW, aH):
        ...
        from reportlab.platypus.doctemplate import (NextPageTemplate,
                                                    CurrentFrameFlowable, LayoutError)
        G = [NextPageTemplate(self.nextTemplate)]
        if aH < self.gap+self.required-_FUZZ:
            # we are going straight to the nextTemplate with
            # no attempt to modify the frames
            G.append(PageBreak())
        else:
...
class DDIndenter(Flowable):
    ...
    def drawOn(self, canv, x, y, _sW=0):
        self._flowable.drawOn(canv, x+self._leftIndent, y,
...
class LIIndenter(DDIndenter):
    _IndenterAttrs = '''_flowable _bullet _leftIndent _rightIndent width height
                        spaceBefore spaceAfter'''.split()

    def __init__(self, flowable, leftIndent=0, rightIndent=0, bullet=None,
    ...
    def __init__(self,
        ...
        if style:
            if not isinstance(style, ListStyle):
                raise ValueError('%s style argument (%r) not a ListStyle' %
...
class ListFlowable(_Container, Flowable):
    ...
    def __init__(
        ...
        if style:
            if not isinstance(style, ListStyle):
                raise ValueError('%s style argument not a ListStyle' %
                                 self.__class__.__name__)
            self.style = style
            ...
            if auto:
                for v in start:
                    if v not in self._numberStyles:
                        raise ValueError('invalid start=%r or bullettype=%r' %
                                         (start, self._bulletType))
            else:
    ...
    def _makeLIIndenter(self, flowable, bullet, params=None):
        if params:
            ...
            return LIIndenter(flowable, leftIndent, rightIndent, bullet,
    ...
    def _getContent(self):
        ...
        for d, f in self._flowablesIter():
            ...
            if d:
                ...
                if b:
                    if b.bulletType != bt:
                        raise ValueError(('Included LIIndenter bulletType=%s != ' +
                                          'OrderedList bulletType=%s') %
                                         (b.bulletType, bt))
                    value = int(b.value)
                else:
                    f._bullet = self._makeBullet(value,
                                                 params=getattr(f, 'params', None))
                if fparams:
                    f.__dict__['spaceBefore'] = max(f.__dict__.get('spaceBefore', 0),
                                                    spaceBefore)
                aS(f)
                if inc:
                    value += inc
...
class DocIf(DocPara):
    ...
    def wrap(self, aW, aH):
        self.add_content(*self.checkBlock(
            self.blocks[int(bool(self.funcWrap(aW, aH)))]))
        return 0, 0
