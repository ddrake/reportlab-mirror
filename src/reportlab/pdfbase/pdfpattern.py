...
def _patternSequenceCheck(pattern_sequence):
    ...
    for x in pattern_sequence:
        if not isinstance(x, allowedTypes):
            if len(x) != 1:
                raise ValueError(
                    "sequence elts must be strings/bytes/PDFPatternIfs or " +
                    "singletons containing strings: "+ascii(x))
            if not isinstance(x[0], strTypes):
                raise ValueError(
                    "Singletons must contain strings/bytes or " +
                    "PDFObject instances only: "+ascii(x[0]))
...
class PDFPattern(PDFObject):
    ...
    def __init__(self, pattern_sequence, **keywordargs):
        ...
        Keyword arguments can be non-instances which are substituted directly in string
        conversion, or they can be object instances in which case they should be
        pdfdoc.* style objects with a x.format(doc) method.
        Keyword arguments may be set on initialization or subsequently using
        __setitem__, before format.  "constant object" instances can also be inserted
        in the patterns.
        """
