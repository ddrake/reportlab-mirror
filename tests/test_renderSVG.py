...
import unittest
from reportlab.graphics.shapes import (Drawing, String, Group, ArcPath, Rect, Ellipse,
                                       Circle, Polygon)
...
class RenderSvgSimpleTestCase(unittest.TestCase):
    ...
    def test3(self):
        ...
        drawing.add(Rect(
            2.25*cm, 0.1*cm, 1.5*cm, 0.8*cm, rx=0.25*cm, ry=0.25*cm,
            hrefURL="http://en.wikipedia.org/wiki/Rounded_rectangle",
            hrefTitle="Rounded Rectangle", strokeColor=colors.red,
            fillColor=colors.yellow))

        drawing.add(String(
            1*cm, 1*cm, "Hello World!",
            hrefURL="http://en.wikipedia.org/wiki/Hello_world",
            hrefTitle="Why 'Hello World'?",
            fillColor=colors.darkgreen))

        drawing.add(Rect(
            4.5*cm, 0.5*cm, 5*cm, 1*cm,
        ...
        drawing.add(Ellipse(
            7*cm, 1*cm, 0.5*cm, 0.9*cm,
            hrefTitle="Tooltip with no link?",
            strokeColor=colors.black,
            fillColor=colors.black))

        drawing.add(Polygon(
            [4.5*cm, 1.25*cm, 5*cm, 0.1*cm, 4*cm, 0.1*cm],
    ...
    def test4(self):
        ...
        assert t0 == 'foo'+specialChar, (
            "%s should equal %s" % (ascii(t0), ascii('foo'+specialChar)))
        assert t1 == 'bar'
    ...
    <body>
        ...
        <p>We have four SVG diagrams embedded in this page.
        Each is within a cyan-coloured div.
        The first 3 have a native size of 400x200, thus consume
        a height of 200 pixels on
        ...
        <hr>
        <p>Test of resizing:  the ones below are sized 50%, 100%, 150%.
        We did this by explicitly setting
        ...
        <p>Test of resizing again:  the ones below are sized 50%, 100%, 150%
        by setting width only.</p>
        <div style="background-color:cyan">
            <embed src="test_renderSVG_simple_test3.svg" type="image/svg+xml"
            width="150"/>
        </div>
