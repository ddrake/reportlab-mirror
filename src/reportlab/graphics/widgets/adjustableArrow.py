...
from reportlab.lib.validators import (isNumber, isColorOrNone, isBoxAnchor, isBoolean)
...
class AdjustableArrow(Widget):
    ...
    _attrMap = AttrMap(
        ...
        angle=AttrMapValue(
            isNumber,
            desc=('angle of arrow default (0), right True 0 is horizontal ' +
                  'to right else vertical up')),
        )
...
class AdjustableArrowDrawing(_DrawingEditorMixin, Drawing):
    def __init__(self, width=100, height=63, *args, **kw):
        ...
        self._add(self, AdjustableArrow(), name='adjustableArrow',
