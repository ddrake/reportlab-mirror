...
class PDFImage:
    ...
    def _jpg_imagedata(self, imageFile):
        ...
        imagedata = []
        # imagedata.append(
        #     ('BI /Width %d /Height /BitsPerComponent 8 /ColorSpace /%s ' +
        #      '/Filter [/Filter [ /ASCII85Decode /DCTDecode] ID') %
        #     (info[0], info[1], colorSpace))
    ...
    def PIL_imagedata(self):
        ...
        imagedata = ['BI /W %d /H %d /BPC %d /CS /%s /F [%s/Fl] ID' %
                     (imgwidth, imgheight, bpc, colorSpace,
                      rl_config.useA85 and '/A85 ' or '')]
    ...
    def drawInlineImage(self, canvas, preserveAspectRatio=False, anchor='sw',
                        anchorAtXY=False, showBoundary=False, extraReturn=None):
        ...
        x, y, self.width, self.height, scaled = aspectRatioFix(
            preserveAspectRatio, anchor, self.x, self.y, width, height, self.imgwidth,
            self.imgheight, anchorAtXY)
