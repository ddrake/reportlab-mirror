...
from reportlab.lib.validators import (isNumber, isNumberOrNone, isListOfStringsOrNone,
                                      isListOfNumbers, isListOfNumbersOrNone,
                                      isColorOrNone, OneOf, isBoolean, SequenceOf,
                                      isString, EitherOr, Validator, NoneOr,
                                      isNormalDate, isNoneOrCallable)
from reportlab.lib.attrmap import AttrMap, AttrMapValue
from reportlab.lib import normalDate
from reportlab.graphics.shapes import (Drawing, Line, PolyLine, Rect, Group,
                                       STATE_DEFAULTS, _textBoxLimits,
                                       _rotatedBoxLimits)
from reportlab.graphics.widgetbase import Widget, TypedPropertyCollection
from reportlab.graphics.charts.textlabels import (Label, PMVLabel, XLabel,
                                                  DirectDrawFlowable)
...
class AxisLineAnnotation:
    ...
    drawAtLimit True draw line at appropriate limit if its coordinate exceeds
                    the lo, hi range
...
class AxisBackgroundAnnotation:
    '''Create a set of coloured bars on the background of a chart
    using axis ticks as the bar borders
    colors is a set of colors to use for the background bars.
    A colour of None is just a skip.
...
class TickLU:
    ...
    def __getitem__(self, t):
        # accuracy = self.accuracy (asigned but not used)
        for x, v in self.T:
...
class _AxisG(Widget):
    ...
    def _getLineFunc(self, start, end, parent=None):
        ...
        if _3d_dx is not None:
            ...
            return (lambda v, s=start,  e=end,  f=f, _3d_dx=_3d_dx,
                    _3d_dy=_3d_dy: f(v, s, e, _3d_dx=_3d_dx, _3d_dy=_3d_dy))
        else:
            f = self.isYAxis and self._cyLine or self._cxLine
            return lambda v, s=start, e=end, f=f: f(v, s, e)

    def _makeLines(self, g, start, end, strokeColor, strokeWidth,
        ...
        if w and c and self.visibleGrid:
            ...
            if s or e:
                if self.isYAxis:
                    ...
                    offs = self._y
                self._makeLines(g, s-offs, e-offs, c, w, self.gridStrokeDashArray,
                                self.gridStrokeLineJoin, self.gridStrokeLineCap,
                                self.gridStrokeMiterLimit, parent=parent,
                                exclude=exclude,
...
class CategoryAxis(_AxisG):
    ...
    _attrMap = AttrMap(
        ...
        strokeLineCap=AttrMapValue(OneOf(0, 1, 2),
                                   desc="Line cap 0=butt, 1=round & 2=square"),
        strokeLineJoin=AttrMapValue(OneOf(0, 1, 2),
        ...
        gridStrokeDashArray=AttrMapValue(isListOfNumbersOrNone,
                                         desc='Dash array used for grid lines.'),
        gridStrokeLineCap=AttrMapValue(OneOf(0, 1, 2),
                                       desc="Grid Line cap 0=butt, 1=round & 2=square"),
        ...
        gridStart=AttrMapValue(isNumberOrNone,
                               desc='Start of grid lines wrt axis origin'),
        gridEnd=AttrMapValue(isNumberOrNone, desc='End of grid lines wrt axis origin'),
        drawGridLast=AttrMapValue(isBoolean,
                                  desc='if true draw gridlines after everything else.'),
        ...
        reverseDirection=AttrMapValue(isBoolean,
                                      desc='If true reverse category direction.'),
        style=AttrMapValue(OneOf('parallel', 'stacked', 'parallel_3d'),
        ...
        tickStrokeWidth=AttrMapValue(isNumberOrNone,
                                     desc='Width of ticks if specified.'),
        tickStrokeColor=AttrMapValue(isColorOrNone,
                                     desc='Color of ticks if specified.'),
        ...
        loLLen=AttrMapValue(isNumber,
                            desc='extra line length before start of the axis'),
        ...
        innerTickDraw=AttrMapValue(isNoneOrCallable,
                                   desc="Callable to replace _drawInnerTicks"),
        )

    def __init__(self):
        assert self.__class__.__name__ != 'CategoryAxis', (
        ...
        self.gridStrokeMiterLimit = self.strokeMiterLimit = (
            STATE_DEFAULTS['strokeMiterLimit'])
    ...
    def configure(self, multiSeries, barWidth=None):
        ...
        self._barWidth = (barWidth or
                          ((self._length-self.loPad-self.hiPad) /
                           float(self._catCount or 1)))
...
def _assertYAxis(axis):
    assert axis.isYAxis, (
        "Cannot connect to other axes (%s), but Y- ones." %
        axis.__class__.__name__)


def _assertXAxis(axis):
    assert axis.isXAxis, ("Cannot connect to other axes (%s), but X- ones." %
                          axis.__class__.__name__)
...
class _XTicks:
        ...
        if itd:
            ...
            if tW:
                if tU and not tD:
                    ...
                    tU = tW*sW
            self._makeLines(g, tU, -tD, self.actualTickStrokeColor, sW,
                            self.strokeDashArray, self.strokeLineJoin,
    ...
    def makeTicks(self):
        ...
        try:
            ...
            self._drawSubTicks(getattr(self, 'subTickHi', 0),
                               getattr(self, 'subTickLo', 0), g)
            return g
        finally:
            self._y = yold
...
class XCategoryAxis(_XTicks, CategoryAxis):
    ...
    _attrMap = AttrMap(
        BASE=CategoryAxis,
        tickUp=AttrMapValue(
            isNumber, desc='Tick length up the axis.'),
        tickDown=AttrMapValue(
            isNumber, desc='Tick length down the axis.'),
        joinAxisMode=AttrMapValue(
            OneOf('bottom', 'top', 'value', 'points', None),
            desc=("Mode used for connecting axis " +
    ...
    def makeAxis(self):
        ...
        axis = Line(self._x-self.loLLen, self._y,
                    self._x + self._length+self.hiLLen, self._y)
...
class YCategoryAxis(_YTicks, CategoryAxis):
    ...
    _attrMap = AttrMap(
        ...
        tickLeft=AttrMapValue(
            isNumber, desc='Tick length left of the axis.'),
    ...
    def makeAxis(self):
        ...
        axis = Line(self._x, self._y-self.loLLen, self._x,
    ...
    def makeTickLabels(self):
        ...
        if categoryNames is not None:
            ...
            reverseDirection = self.reverseDirection
            barWidth = self._barWidth
            # labels = self.labels  (assigned but not used)
...
class ValueAxis(_AxisG):
    ...
    _attrMap = AttrMap(
        ...
        gridStrokeWidth=AttrMapValue(
            isNumber, desc='Width of grid lines.'),
        gridStrokeColor=AttrMapValue(
            isColorOrNone, desc='Color of grid lines.'),
        ...
        maximumTicks=AttrMapValue(
            isNumber, desc='Maximum number of ticks.'),
        labels=AttrMapValue(
            None, desc='Handle of the axis labels.'),
        labelAxisMode=AttrMapValue(
            OneOf('high', 'low', 'axis'),
            desc="Like joinAxisMode, but for the axis labels"),
        labelTextFormat=AttrMapValue(
            None, desc='Formatting string or function used for axis labels.'),
        labelTextPostFormat=AttrMapValue(
            None, desc='Extra Formatting string.'),
        labelTextScale=AttrMapValue(
            isNumberOrNone, desc='Scaling for label tick values.'),
        valueMin=AttrMapValue(
            isNumberOrNone, desc='Minimum value on axis.'),
        valueMax=AttrMapValue(
            isNumberOrNone, desc='Maximum value on axis.'),
        valueStep=AttrMapValue(
            isNumberOrNone, desc='Step size used between ticks.'),
        valueSteps=AttrMapValue(
            isListOfNumbersOrNone, desc='List of step sizes used between ticks.'),
        avoidBoundFrac=AttrMapValue(
            EitherOr((isNumberOrNone,
                      SequenceOf(isNumber, emptyOK=0, lo=2, hi=2))),
            desc='Fraction of interval to allow above and below.'),
        avoidBoundSpace=AttrMapValue(
            EitherOr((isNumberOrNone, SequenceOf(
                isNumber, emptyOK=0, lo=2, hi=2))),
            desc='Space to allow above and below.'),
        abf_ignore_zero=AttrMapValue(
            EitherOr((NoneOr(isBoolean), SequenceOf(
                isBoolean, emptyOK=0, lo=2, hi=2))),
            desc=('Set to True to make the avoidBoundFrac ' +
                  'calculations treat zero as non-special')),
        ...
        skipEndL=AttrMapValue(OneOf('none', 'start', 'end', 'both'),
                              desc='Skip high/low tick labels'),
        origShiftIPC=AttrMapValue(
            isNumberOrNone, desc='Lowest label shift interval ratio.'),
        origShiftMin=AttrMapValue(
            isNumberOrNone, desc='Minimum amount to shift.'),
        ...
        annotations=AttrMapValue(
            None, desc='list of annotations'),
        loLLen=AttrMapValue(
            isNumber, desc='extra line length before start of the axis'),
        hiLLen=AttrMapValue(
            isNumber, desc='extra line length after end of the axis'),
        subTickNum=AttrMapValue(
            isNumber, desc='Number of axis sub ticks, if >0'),
        subTickLo=AttrMapValue(
            isNumber, desc='sub tick down or left'),
        subTickHi=AttrMapValue(
            isNumber, desc='sub tick up or right'),
        visibleSubTicks=AttrMapValue(
            isBoolean, desc='Display axis sub ticks, if true.'),
        visibleSubGrid=AttrMapValue(
            isBoolean, desc='Display axis sub grid, if true.'),
        subGridStrokeWidth=AttrMapValue(
            isNumber, desc='Width of grid lines.'),
        subGridStrokeColor=AttrMapValue(
            isColorOrNone, desc='Color of grid lines.'),
        ...
        tickStrokeWidth=AttrMapValue(
            isNumber, desc='Width of ticks if specified.'),
    ...
    def __init__(self, **kw):
        assert self.__class__.__name__ != 'ValueAxis', (
    ...
    def _setRange(self, dataSeries):
        ...
        if origShiftMin is not None or origShiftIPC is not None:
            ...
            if (origShiftSpecialValue is None or
                    abs(origShiftSpecialValue-valueMin) < 1e-6):
                if origShiftIPC:
    ...
    def _calcValueStep(self):
        ...
        if self.valueStep is None:
            ...
            rawInterval = rawRange / min(float(self.maximumTicks-1),
        ...
        for i, tick in enumerate(self._tickValues):
            ...
            if f and label.visible:
                ...
                if v is not None:
                    ...
                    # special property to ensure a label doesn't project
                    # beyond the bounds of an x-axis
    ...
    def scale(self, value):
        ...
        # this could be made more efficient by moving the definition
        # of org and sf into the configuration
...
class XValueAxis(_XTicks, ValueAxis):
    ...
    _attrMap = AttrMap(
        ...
        joinAxis=AttrMapValue(
            None, desc='Join both axes if true.'),
        joinAxisMode=AttrMapValue(
            OneOf('bottom', 'top', 'value', 'points', None),
            desc=("Mode used for connecting axis " +
                  "('bottom', 'top', 'value', 'points', None).")),
...
_NDINTM = (1, 2, 3, 6, 12, 24, 60, 120, 180, 240, 300, 360, 420, 480,
           540, 600, 720, 840, 960, 1080, 1200, 2400)
...
class NormalDateXValueAxis(XValueAxis):
    ...
    _attrMap = AttrMap(
        ...
        niceMonth=AttrMapValue(
            isBoolean, desc="Flag for displaying months 'nicely'."),
        ...
        forceDatesEachYear=AttrMapValue(
            isListOfDaysAndMonths, desc=(
                'List of dates in format "31-Dec",' +
                '"1-Jan".  If present they will always be used for tick marks in ' +
                'the current year, rather than the dates chosen by the automatic ' +
                'algorithm. Hyphen compulsory, case of month optional.')),
    ...
    def __init__(self, **kw):
        ...
        self.dayOfWeekName = ['Monday', 'Tuesday', 'Wednesday', 'Thursday',
                              'Friday', 'Saturday', 'Sunday']
        self.monthName = ['January', 'February', 'March', 'April', 'May', 'June',
    ...
    def _dateFormatter(self, v):
        ...
        try:
            normalDate._dayOfWeekName, (
                normalDate._monthName) = self.dayOfWeekName, self.monthName
    ...
    def _xAxisTicker(self, xVals):
        ...
        xLabelW = RBL[1]-RBL[0]
        # xLabelH = RBL[3]-RBL[2]  (assigned but not used)
        ...
        def addTick(i, xVals=xVals, formatter=formatter,
...
class YValueAxis(_YTicks, ValueAxis):
    ...
    _attrMap = AttrMap(
        ...
        tickLeft=AttrMapValue(
            isNumber, desc='Tick length left of the axis.'),
        tickRight=AttrMapValue(
            isNumber, desc='Tick length right of the axis.'),
        joinAxis=AttrMapValue(
            None, desc='Join both axes if true.'),
        joinAxisMode=AttrMapValue(
            OneOf(('left', 'right', 'value', 'points', None)),
    ...
    def makeAxis(self):
        ...
        axis = Line(self._x, self._y-self.loLLen,
                    self._x, self._y + self._length+self.hiLLen)
        axis.strokeColor = self.strokeColor
        axis.strokeWidth = self.strokeWidth
        axis.strokeDashArray = self.strokeDashArray
        g.add(axis)
            ...
            if _allInt(tv):
                ...
                if u == 1:
                    ...
                    if _allInt(fv):
                        ...
                        def fmt(x, uf=uf, fmt='%%.%df%s' % (stvl, s)):
                            return fmt % (x/uf)
...
class AdjYValueAxis(YValueAxis):
    ...
    _attrMap = AttrMap(
        ...
        leftAxisOrigShiftMin=AttrMapValue(
            isNumber, desc='Minimum amount to shift.'),
        ...
        labelVOffset=AttrMapValue(
            isNumber, desc='add this to the labels'),
        )
    ...
    def _rangeAdjust(self):
        ...
        T, L = ticks(self._valueMin, self._valueMax, split=1, n=n,
                     percent=self.leftAxisPercent, grid=valueStep,
...
class LogValueAxis(ValueAxis):
    ...
    def _setRange(self, dataSeries):
        ...
        if valueMin > valueMax:
            raise ValueError(
                # 3 placeholders but 2 substitutions
                '%s: valueMin=%r should not be greater than valueMax=%r!' %  # noqa
...
class LogXValueAxis(LogValueAxis, LogAxisLabellingSetup, XValueAxis):
    ...
    def scale(self, value):
        ...
        return self._x + self._scaleFactor * (math_log10(value) -
                                              math_log10(self._valueMin))
...
class LogYValueAxis(LogValueAxis, LogAxisLabellingSetup, YValueAxis):
    ...
    def scale(self, value):
        ...
        return self._y + self._scaleFactor * (math_log10(value) -
                                              math_log10(self._valueMin))
