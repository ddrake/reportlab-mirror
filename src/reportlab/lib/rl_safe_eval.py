...
class UntrustedAstTransformer(ast.NodeTransformer):
    ...
    def error(self, node, msg):
        raise BadCode('Line %s: %s' %
                      (getattr(node, 'lineno', '??'), msg))
    ...
    def visit_Call(self, node):
        ...
        # needs_wrap defined but not used
        # needs_wrap = False
...
def astFormat(node):
    return ast.dump(copy.deepcopy(node), annotate_fields=True,
                    include_attributes=True, indent=4)
...
def safer_globals(g=None):
    ...
    for name in ('__annotations__', '__doc__', '__loader__', '__name__',
...
class __RL_SAFE_ENV__:
    ...
    def __init__(self, timeout=None, allowed_magic_methods=None):
        ...
        self.allowed_magic_methods = ((
            __allowed_magic_methods__ if allowed_magic_methods is True else
            allowed_magic_methods) if allowed_magic_methods else
            [])
        import builtins
        ...
        __rl_safe_builtins__.update(
            {_: getattr(builtins, _) for _ in
             ('''None False True abs bool callable chr complex divmod float hash hex id
              int isinstance issubclass len oct ord range repr round slice str tuple
              setattr classmethod staticmethod property divmod next object getattr dict
              iter pow list type max min sum enumerate zip hasattr filter map any all
              sorted reversed range set frozenset
        ...
        self.__rl_builtins__ = __rl_builtins__ = {
            _: __rl_missing_func__(_) for _ in dir(builtins)
        ...
        __rl_builtins__['__rl_apply__'] = (lambda func, *args,
                                           **kwds: self.__rl_apply__(func, args, kwds))
    ...
    def __rl_zip__(self, *args):
        return zip(*[self.__rl_args_iter__(self.__rl_getitem__(args, i))
    ...
    def __rl_is_allowed_name__(self, name):
        ...
        if isinstance(name, strTypes):
            if name in __rl_unsafe__ or (
                name.startswith('__') and name != '__'
    ...
    def __rl_apply__(self, func, args, kwds):
        ...
        if obj:
            if isinstance(obj, dict) and func.__name__ in ('pop', 'setdefault',
                                                           'get', 'popitem'):
                self.__rl_is_allowed_name__(args[0])
        return func(*[a for a in self.__rl_getiter__(args)],
    ...
    def __rl_compile__(self, src, fname='<string>', mode="eval",
                       flags=0, inherit=True, visit=None):
        names_seen = {}
        if not visit:
            bcode = compile(src, fname, mode=mode, flags=flags,
                            dont_inherit=not inherit)
        else:
    ...
    def __rl_safe_eval__(self, expr, g, l, mode, timeout=None,  # noqa
                         allowed_magic_methods=None, __frame_depth__=3):
        bcode, ns = self.__rl_compile__(
            expr, fname='<string>', mode=mode, flags=0, inherit=True,
            visit=UntrustedAstTransformer(
                nameIsAllowed=self.__rl_is_allowed_name__).visit)
        if None in (l, g):
        ...
        self.__rl_limit__ = self.__time_time__() + (timeout if timeout is not None else
                                                    self.timeout)
...
class __rl_safe_eval__:
    ...
    def __call__(self, expr, g=None, l=None, timeout=None,  # noqa
                 allowed_magic_methods=None):
        if not self.env:
            self.env = __RL_SAFE_ENV__(timeout=timeout,
                                       allowed_magic_methods=allowed_magic_methods)
        return self.env.__rl_safe_eval__(
            expr, g, l, self.mode, timeout=timeout,
            allowed_magic_methods=allowed_magic_methods,
            __frame_depth__=2)
...
def rl_extended_literal_eval(expr, safe_callables=None, safe_names=None):
    ...
    try:
        # Python 3.4 and up
        ast.NameConstant
        safe_test = (lambda n: isinstance(n, ast.NameConstant) or
                     isinstance(n, ast.Name) and n.id in safe_names)
        safe_extract = (lambda n: n.value if isinstance(n, ast.NameConstant)
                        else safe_names[n.id])
    except AttributeError:
        # Everything before

        def safe_test(n):
            return isinstance(n, ast.Name) and n.id in safe_names
