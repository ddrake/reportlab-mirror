...
def makeA85Image(filename, IMG=None, detectJpeg=False):
    ...
    assert len(raw) == imgwidth * imgheight * _mode2bpp[img.mode], \
...
class _fusc:
    ...
    def encrypt(self, s):
        return self.__rotate(asciiBase85Encode(
            ''.join(map(chr, self.__fusc(list(map(ord, s)))))), self._n)
    ...
    def __fusc(self, s):
        slen = len(s)
        return list(map(lambda x, y: x ^ y, s,
