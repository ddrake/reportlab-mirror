...
class IndexTestCase(unittest.TestCase):
    ...
    def test0(self):
        ...
        for headers in False, True:
            ...
            if headers:
                description = description.replace('index at',
                                                  'index with alphabetic headers at')
            ...
            def addParas(words):
                txt = u' '.join([(len(w) > 5 and u'<index item=%s/>%s' %
                                  (quoteattr(commajoin([w[:2], w[:3], w])), w) or w)
                                 for w in words])
            ...
            addParas([u+w for u in u'E\xc8\xc9\xca\xcb'
                      for w in (u'erily', u'asily')])
            addParas([u+w for u in u'A\xc0\xc4\xc1\xc3\xc2'
                      for w in (u'dvance', u'ttend')])
            addParas([u+w for u in u'O\xd2\xd6\xd3\xd2'
                      for w in (u'rdinary', u'verflow')])
            addParas([u+w for u in u'U\xd9\xdc\xdb'
                      for w in (u'ndertow', u'nbeliever')])
            addParas([u+w for u in u'e\xe8\xea\xeb\xe9'
                      for w in (u'ventide', u'lision')])
            addParas([u+w for u in u'o\xf2\xf5\xf3\xf4'
                      for w in (u'verture', u'ntology')])

            # test ampersand in index term
            txt = ('\nMarks &amp; Spencer - purveyors of fine groceries, underwear ' +
                   'and ampersands - should have their initials displayed however ' +
                   'they were input.\n<index item="M&amp;S,groceries"/><index ' +
                   'item="M&amp;S,underwear"/><index item="M&amp;S,ampersands"/>')
            para = Paragraph(txt, makeBodyStyle())
            story.append(para)
