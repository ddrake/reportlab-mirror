...
# from ast import literal_eval (imported not used)
...
class Color:
    ...
    def __repr__(self):
        return "Color(%s)" % fp_str(*(self.red, self.green,
                                      self.blue, self.alpha)).replace(' ', ',')
...
class CMYKColor(Color):
    ...
    def __init__(self, cyan=0, magenta=0, yellow=0, black=0,
        ...
        spotName is intended for use as an identifier to the renderer not client
        programs.  Density is used to modify the overall amount of ink.
        knockout is a renderer dependent option that determines whether the applied
        colour knocksout (removes) existing colour; None means use the global default.
        """
        self.cyan = cyan
    ...
    def __key__(self):
        ...
        (-1, True)
        """  # noqa
        return (self.cyan, self.magenta, self.yellow, self.black, self.density,
                self.spotName, self.alpha)
...
class PCMYKColor(CMYKColor):
    ...
    def __init__(self, cyan, magenta, yellow, black, density=100, spotName=None,
                 knockout=None, alpha=100):
        CMYKColor.__init__(self, cyan/100., magenta/100., yellow/100., black/100.,
                           spotName, density/100., knockout=knockout, alpha=alpha/100.)
...
class CMYKColorSep(CMYKColor):
    ...
    def __init__(self, cyan=0, magenta=0, yellow=0, black=0,
        ...
        CMYKColor.__init__(self, cyan, magenta, yellow, black, spotName, density,
                           knockout=None, alpha=alpha)
...
class PCMYKColorSep(PCMYKColor, CMYKColorSep):
    ...
    def __init__(self, cyan=0, magenta=0, yellow=0, black=0,
        ...
        PCMYKColor.__init__(self, cyan, magenta, yellow, black, density, spotName,
                            knockout=None, alpha=alpha)
...
def HexColor(val, htmlOnly=False, hasAlpha=False):
    ...
    if isStr(val):
        ...
        if val[:1] == '#':
            val = val[1:]
            b = 16
            # alpha is assigned but not used
            # if len(val) == 8:
            #     alpha = True
        else:
            if htmlOnly:
...
def linearlyInterpolatedColor(c0, c1, x0, x1, x):
    ...
    if x < x0-1e-8 or x > x1+1e-8:  # fudge factor for numerical problems
        raise ValueError("Can't interpolate: x=%f is not between %f and %f!" %
                         (x, x0, x1))
    if x <= x0:
        return c0
    elif x >= x1:
...
class cssParse:
    def pcVal(self, v):
        ...
        try:
            ...
            raise ValueError('bad percentage argument value %r in css color %r' %
                             (v, self.s))
        return c
    ...
    _n_c = dict(pcmyk=(4, 100, True, False), cmyk=(4, 1, True, False),
                hsl=(3, 1, False, True), rgb=(3, 1, False, False))
...
class toColor:
    ...
    This used to accept arbitrary Python expressions, which resulted in increasngly
    devilish CVEs and security holes from tie to time.  In April 2023 we are creating
    explicit, "dumb" parsing code to replace this.  Acceptable patterns are
    ...
    def __call__(self, arg, default=None):
        ...
        if isinstance(arg, (tuple, list)):
            ...
            return (len(arg) == 3 and Color(arg[0], arg[1], arg[2]) or
                    CMYKColor(arg[0], arg[1], arg[2], arg[3]))
        elif isStr(arg):
            arg = asNative(arg)
            C = cssParse(arg)
            if C:
            ...
            # >>> re.compile(r"(Blacker|Whiter)\((\w+)\,\s?([0-9.]+)\)") \
            #               .match(msg).groups()
            # ('Blacker', 'red', '0.5')
            # >>>
            ...
            if rl_config.toColorCanUse == 'rl_safe_eval':
                ...
                if not self._G:
                    C = globals()
                    self._G = {s: C[s] for s in '''Blacker CMYKColor CMYKColorSep Color
                    ColorType HexColor PCMYKColor PCMYKColorSep Whiter
                    _chooseEnforceColorSpace _enforceCMYK _enforceError _enforceRGB
                    _enforceSEP _enforceSEP_BLACK _enforceSEP_CMYK _namedColors _re_css
                    asNative cmyk2rgb cmykDistance color2bw colorDistance cssParse
                    describe fade fp_str getAllNamedColors hsl2rgb hue2rgb isStr
                    linearlyInterpolatedColor literal_eval obj_R_G_B opaqueColor
                    rgb2cmyk setColors toColor toColorOrNone'''.split()}
                G.update(self._G)
                try:
                ...
                C = {k: C[k] for k in '''Blacker CMYKColor CMYKColorSep Color ColorType
                HexColor PCMYKColor PCMYKColorSep Whiter _chooseEnforceColorSpace
                _enforceCMYK _enforceError _enforceRGB _enforceSEP _enforceSEP_BLACK
                _enforceSEP_CMYK _namedColors _re_css asNative cmyk2rgb cmykDistance
                color2bw colorDistance cssParse describe fade fp_str getAllNamedColors
                hsl2rgb hue2rgb linearlyInterpolatedColor obj_R_G_B opaqueColor
                rgb2cmyk setColors toColor toColorOrNone'''.split()
    ...
    while kw and progress:
        ...
        for k, v in kw.items():
            if isinstance(v, (tuple, list)):
                c = [toColor(x, UNDEF=UNDEF) for x in v]
...
def _enforceSEP_BLACK(c):
    ...
    tc = toColor(c)
    if not isinstance(tc, CMYKColorSep):
        if isinstance(tc, Color) and tc.red == tc.blue == tc.green:  # ahah it's a grey
            tc = _CMYK_black.clone(density=1-tc.red)
        # ie some shade of grey
        elif not (isinstance(tc, CMYKColor) and
                  tc.cyan == tc.magenta == tc.yellow == 0):
