...
from reportlab.lib.validators import (isInt, isNumber, isString, isColorOrNone,
                                      isBoolean, EitherOr, isNumberOrNone)
...
class _BarcodeWidget(PlotArea):
    ...
    def draw(self):
        ...
        G = Group()
        self._Gadd = G.add
        self._Gadd(Rect(self.x, self.y, self.width, self.height, fillColor=None,
                        strokeColor=None, strokeWidth=0.0001))
        self._BCC.draw(self)
        del self.canv, self._Gadd
        return G

    def annotate(self, x, y, text, fontName, fontSize, anchor='middle'):
        self._Gadd(String(self.x+x, self.y+y, text, fontName=fontName,
...
BarcodeCode128 = _BCW(
    ...
    "Code128",
    AttrMap(
        BASE=BarcodeI2of5, UNWANTED=('bearers', 'checksum', 'ratio', 'checksum',
                                     'stop')),
...
BarcodeFIM = _BCW(
    ...
    AttrMap(BASE=_BarcodeWidget,
            ...
            quiet=AttrMapValue(
                isBoolean,
                '''(bool, default 0): Whether to include quiet zones in the symbol.'''),
            lquiet=AttrMapValue(
                isNumber,
...
BarcodeUSPS_4State = _BCW(
    ...
    AttrMap(BASE=_BarcodeWidget,
            widthSize=AttrMapValue(
                isNumber,
                '''(float, default 1): the bar width size adjustment
                between 0 and 1.'''),
            heightSize=AttrMapValue(
                isNumber,
                '''(float, default 1): the bar height size adjustment
                between 0 and 1.'''),
            fontName=AttrMapValue(
                isString, desc='human readable font'),
            fontSize=AttrMapValue(isNumber, desc='human readable font size'),
    ...
    _methods=("\n\tdef annotate(self,x,y,text,fontName,fontSize,anchor='middle'):\n" +
              "\t\t_BarcodeWidget.annotate(self,x,y,text,fontName," +
              "fontSize,anchor='start')\n")
)
BarcodeECC200DataMatrix = _BCW(
    ...
    ('JGB 0204H20B012722900021AC35B2100001003241014241014TPS01  WJ067073605GB185 ' +
     'MOUNT PLEASANT MAIL CENTER         EC1A1BB9ZGBREC1A1BB  EC1A1BB  STEST FILE ' +
     '      FOR SPEC                                       ')
)
