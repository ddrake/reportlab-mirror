...
from reportlab.lib.validators import (isNumber, isNumberOrNone, isColorOrNone,
                                      ...
                                      isListOfNumbersOrNone, isStringOrNone, OneOf,
                                      Percentage)
...
class LineChartProperties(PropHolder):
    _attrMap = AttrMap(
        ...
        lineStyle=AttrMapValue(
            ...
            desc="What kind of plot this line is", advancedUsage=1),
        barWidth=AttrMapValue(
            isNumberOrNone, desc="Percentage of available width to be used for a bar",
            advancedUsage=1),
        inFill=AttrMapValue(
...
class AbstractLineChart(PlotArea):
    ...
    def makeSwatchSample(self, rowNo, x, y, width, height):
        ...
        if lineStyle == 'bar':
            ...
            L = Rect(x, y, width, height, strokeWidth=strokeWidth,
                     strokeColor=color, strokeLineCap=0, strokeDashArray=dash,
...
class HorizontalLineChart(LineChart):
    ...
    _attrMap = AttrMap(
        ...
        lineLabelNudge=AttrMapValue(
            isNumber, desc='Distance between a data point and its label.',
            advancedUsage=1),
        ...
        lineLabelArray=AttrMapValue(
            None, desc=('explicit array of line label values, ' +
                        'must match size of data if present.')),
        groupSpacing=AttrMapValue(
            isNumber, desc='? - Likely to disappear.'),
        ...
        valueAxis=AttrMapValue(
            None, desc='Handle of the value axis.'),
        categoryAxis=AttrMapValue(
            None, desc='Handle of the category axis.'),
        ...
        for rowNo, row in enumerate(reversed(P) if self.reversePlotOrder else P):
            ...
            if lineStyle == 'bar':
                ...
                if isinstance(barWidth, Percentage):
                    ...
                    hbw = barWidth*0.5
                for x, y in row:
                    g.add(Rect(x-hbw, min(y, yzero), 2*hbw, abs(y-yzero),
                ...
                if inFill or isinstance(row, FillPairedData):
                    ...
                    if filler:
                        ...
                        inFillG.add(Polygon(
                            fpoints, fillColor=fillColor,
                            strokeColor=(strokeColor if strokeColor == fillColor else
                                         None),
...
class HorizontalLineChart3D(HorizontalLineChart):
    ...
    def makeLines(self):
        ...
        for rowNo in P:
            ...
            # strokeWidth assigned but not used
...
def sampleCandleStick():
    ...
    chart.valueAxis.valueMin = min(min(boxMid), min(hi), min(lo),
                                   min(boxLo), min(boxHi))
    chart.valueAxis.valueMax = max(max(boxMid), max(hi), max(lo),
                                   max(boxLo), max(boxHi))
    lines = chart.lines
    lines[0].strokeColor = None
    I = range(len(boxMid))  # noqa
    chart.data = [boxMid]
    lines[0].symbol = candles = CandleSticks(
        chart=chart, boxFillColor=boxFillColor, boxWidth=boxWidth,
        crossWidth=crossWidth, strokeWidth=candleStrokeWidth,
        strokeColor=candleStrokeColor)
    for i in I:
        candles[i].setProperties(dict(position=i, boxMid=boxMid[i], crossLo=lo[i],
                                      crossHi=hi[i], boxLo=boxLo[i], boxHi=boxHi[i]))
    return d
