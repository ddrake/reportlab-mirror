...
from reportlab.lib.validators import (isString, isNumber, isColorOrNone, isColor,
                                      isInt, isNumberOrNone, isNoneOrString)
...
class SlideBox(Widget):
    ...
    def _getDrawingDimensions(self):
        ...
        ty = (ty+self.topPadding+self.bottomPadding+self.sourceLabelOffset +
    ...
    def draw(self):
        g = Group()
        ys = (self.bottomPadding+(self.triangleHeight/2)+self.sourceLabelOffset +
              self.sourceLabelFontSize)
        if self.background:
        ...
        for f in range(0, self.numberOfBoxes):
            ...
            g.add(Rect(
                x, 0, self.boxWidth, self.boxHeight,
                strokeColor=self.boxOutlineColor,
                strokeWidth=self.boxOutlineWidth,
                fillColor=None))

            g.add(String(
                x+self.boxWidth/2., (self.boxHeight-ascent)/2.,
                text=str(f+1),
