...
from reportlab.lib.utils import (open_for_read, makeFileName, isSeq, isBytes,
                                 isUnicode, _digester, isStr, bytestr,
                                 annotateException, TimeStamp)
from reportlab.lib.rl_accel import (escapePDF, fp_str, asciiBase85Encode,
                                    asciiBase85Decode)
...
# __InternalName__ is a special attribute that can only be
# set by the Document arbitrator
...
# map keyword to min version that supports it
PDF_SUPPORT_VERSION = dict(transparency=(1, 4),)
...
def format(element, document, toplevel=0):
    ...
    if isinstance(element, PDFObject):
        if not toplevel and hasattr(element, __RefOnly__):
            ...
            if (not rl_config.invariant and
                    rl_config.pdfComments and
                    hasattr(element, __Comment__)):
...
class PDFDocument(PDFObject):
    ...
    def __init__(self,
        ...
        # mapping of internal identifier ("Page001") to PDF objectnumber
        # and generation number (34, 0)
    ...
    def ensureMinPdfVersion(self, *keys):
        """Ensure that the pdf version is greater than or equal
           to that specified by the keys"""
        for k in keys:
    ...
    def ID(self):
        ...
        self._ID = (b'\n['+IDs+IDs+b']\n% ReportLab generated PDF document' +
                    b' -- digest (http://www.reportlab.com)\n')
        return self._ID
    ...
    def inPage(self):
        """specify the current object as a page (enables reference binding
           and other page features)"""
    ...
    def inForm(self):
        ...
        # don't need this check anymore since going in a form
        #     pushes old context at canvas level.
        ...
        if psfontname in fm:
            ...
            try:
                ...
                if fontObj._dynamicFont:
                    raise PDFError("getInternalFontName(%s) called for a dynamic font" %
                                   repr(psfontname))
    ...
    def format(self):
        # register the Catalog/INfo and then format the objects one by one until
        # exhausted (possible infinite loop if there is a bug that continually
        # makes new objects/refs...)
        # Prepare encryption
        ...
        # idToNV = self.idToObjectNumberAndVersion (set but not used)
        ...
        lno = len(numbertoid)
        if counter-1 != lno:
            raise ValueError("counter %s doesn't match number to id dictionary %s" %
                             (counter, lno))
        # now add the xref
    ...
    def Reference(self, obj, name=None):
        # ## note references may "grow" during the final formatting pass:
        #   don't use d.keys()!
        ...
        if hasattr(obj, __InternalName__):
            ...
            if intname not in idToObject:
                raise ValueError("object of type %s named as %s, but not registered" %
...
class PDFDictionary(PDFObject):
    ...
    def __init__(self, dict=None):
        """dict should be namestring to value eg "a": 122
           NOT pdfname to value NOT "/a":122"""
        if dict is None:
            self.dict = {}
        else:
            self.dict = dict.copy()
...
class ViewerPreferencesPDFDictionary(CheckedPDFDictionary):
    validate = dict(
        ...
        NonFullScreenPageMode=checkPDFNames(*'UseNone UseOutlines UseThumbs UseOC'
                                            .split()),
...
def teststream(content=None):
    ...
    S = PDFStream(content=content,
                  filters=(rl_config.useA85 and [PDFBase85Encode, PDFZCompress]
                           or [PDFZCompress]))
    # nothing else needed...
...
class PDFCrossReferenceSubsection(PDFObject):
    ...
    def format(self, document):
        """id sequence should represent contiguous object nums else error.
           free numbers not supported (yet)"""
        ...
        for id in idsequence:
            ...
            if num in taken:
                raise ValueError("object number collision %s %s %s" %
                                 (num, repr(id), repr(taken[id])))
            if num > lastentrynumber or num < firstentrynumber:
                raise ValueError("object number %s not in range %s..%s" %
                                 (num, firstentrynumber, lastentrynumber))
            # compute position in list
            # rnum = num-firstentrynumber (assigned but not used)
            taken[num] = id
...
class PDFTrailer(PDFObject):
    def __init__(self, startxref, Size=None, Prev=None, Root=None, Info=None,
                 ID=None, Encrypt=None):
...
class PDFPage(PDFCatalog):
    ...
    def check_format(self, document):
        ...
        if self.Override_default_compilation:
            return
        self.MediaBox = (self.MediaBox or
                         PDFArray(self.Rotate in (90, 270) and
                                  [0, 0, self.pageheight, self.pagewidth]
        ...
        if not self.Contents:
            ...
            if not stream:
                ...
                S = PDFStream()
                if self.compression:
                    S.filters = (rl_config.useA85 and [PDFBase85Encode, PDFZCompress] or
                                 [PDFZCompress])
                S.content = stream
...
class PDFOutlines(PDFObject):
    ...
    def addOutlineEntry(self, destinationname, level=0, title=None, closed=None):
        ...
        # tree = self.buildtree (assigned but not used)
        # adjust currentlevel and stack to match level
        if level > currentlevel:
            if level > currentlevel+1:
                raise ValueError(("can't jump from outline level %s to level %s, " +
                                 "need intermediates (destinationname=%r, title=%r)") %
        ...
        while level < currentlevel:
            ...
            if isinstance(lastinprevious, tuple):
                ...
                raise ValueError("cannot reset existing sections: " +
                                 repr(lastinprevious))
            else:
                name = lastinprevious
                sectionlist = current
    ...
    def translateNames(self, canvas, object):
        ...
        raise TypeError("in outline, destination name must be string: got a %s" %
                        type(object))

    def prepare(self, document, canvas):
        """prepare all data structures required for save
           operation (create related objects)"""
        ...
        (self.first, self.last) = self.maketree(document, self.mydestinations,
                                                toplevel=1)
        self.ready = 1
    ...
    def maketree(self, document, destinationtree, Parent=None, toplevel=0):
        ...
        # lastindex = nelts-1 (assigned but not used)
        ...
        for index in range(nelts):
            ...
            if isinstance(elt, dict):
                ...
                # XXXX should clean up (see count(...))
                try:
                ...
                (eltobj.First, eltobj.Last) = self.maketree(document, subsections,
                                                            eltref)
            else:
                raise ValueError(
    ...
    def AnnotationDict(self, **kw):
        ...
        for name in d.keys():
            if name not in permitted:
                raise ValueError("%s bad annotation dictionary name %s" %
...
class Destination(PDFObject):
    ...
    def format(self, document):
        ...
        if f is None:
            raise ValueError(
                "format not resolved, probably missing URL " +
                "scheme or undefined destination target for '%s'" % self.name)
        p = self.page
        if p is None:
            raise ValueError(
                "Page not bound, probably missing URL " +
                "scheme or undefined destination target for '%s'" % self.name)
        f.page = p
...
class PDFDestinationFitR(PDFObject):
    ...
    def format(self, document):
        pageref = document.Reference(self.page)
        A = PDFArray([pageref, PDFName(self.typename), self.left, self.bottom,
                      self.right, self.top])
        return format(A, document)
...
class PDFResourceDictionary(PDFObject):
    ...
    dict_attributes = ("ColorSpace", "XObject", "ExtGState", "Font", "Pattern",
                       "Properties", "Shading")
...
class PDFTrueTypeFont(PDFType1Font):
    Subtype = "TrueType"
    # local_attributes = (
    #    "FirstChar LastChar Widths Encoding ToUnicode FontDescriptor".split()) #same
...
# #    Subtype = "Type3"
# #    local_attributes = ("FirstChar LastChar Widths CharProcs FontBBox " +
# #                        "FontMatrix Resources Encoding").split()
# #
...
# #    local_attributes = ("BaseFont CIDToGIDMap CIDSystemInfo " +
# #                        "FontDescriptor DW W DW2 W2".split())
# #
...
class PDFFormXObject(PDFObject):
    ...
    # XXXX any resource used in a form must be propagated up to the page
    # that (recursively) uses the form!! (not implemented yet).
    ...
    def format(self, document):
        self.BBox = (self.BBox or
                     PDFArray([self.lowerx, self.lowery, self.upperx, self.uppery]))
        ...
        if self.compression:
            self.Contents.filters = (rl_config.useA85 and
                                     [PDFBase85Encode, PDFZCompress] or
                                     [PDFZCompress])
...
class PDFImageXObject(PDFObject):
    ...
    def loadImageFromA85(self, source):
        ...
        self.colorSpace = {'/RGB': 'DeviceRGB', '/G': 'DeviceGray',
    ...
    def _checkTransparency(self, im):
        if self.mask == 'auto':
            if im._dataA:
                self.mask = None
                self._smask = PDFImageXObject(_digester(im._dataA.getRGBData()),
                                              im._dataA, mask=None)
    ...
    def loadImageFromSRC(self, im):
        ...
        if fp:
            ...
            raw = im.getRGBData()
            # assert len(raw) == self.width*self.height,
            #   "Wrong amount of data for image expected %sx%s=%s got %s" %
            #    (self.width,self.height,self.width*self.height,len(raw))
...
class PDFSeparationCMYKColor:
    def __init__(self, cmyk):
        ...
        if not isinstance(cmyk, CMYKColor):
            ...
            raise ValueError('%s needs a CMYKColor argument with a spotName' %
                             self.__class__.__name__)
        self.cmyk = cmyk
