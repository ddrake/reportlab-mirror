...
def isNonPrimitiveInstance(x):
    return not isinstance(x, (
        float, int, type, tuple, list, dict, str, bytes, complex, bool, slice,
        _rl_NoneType, types.FunctionType, types.LambdaType, types.CodeType,
...
import builtins  # noqa  (could we just use os.exec?)
...
def char2int(s):
    return (s if isinstance(s, int) else
            ord(s if isinstance(s, str) else
                s.decode('latin1')))
...
try:
    ...
    def rl_glob(pattern, glob=glob.glob, fnmatch=fnmatch.fnmatch,  _RL_DIR=_RL_DIR,
                pjoin=os.path.join):
        c, pfn = __startswith_rl(pattern)
        r = glob(pfn)
        if c or r == []:
            r += list(map(lambda x, D=_archivepfx, pjoin=pjoin: pjoin(_archivepfx, x),
                          list(filter(
...
_importlib_invalidate_caches = getattr(importlib, 'invalidate_caches',
                                       lambda : None)
        ...
        try:
            ...
            try:
                if trustedHosts is not None:
                    purl = urlparse(name)
                    if (purl[0] and not
                            ((purl[0] in ('data', 'file') or
                              trustedHosts.match(purl[1]))
                             and (purl[0] in trustedSchemes))):
                        raise ValueError('Attempted untrusted host access')
                return BytesIO((datareader if name[:5].lower() == 'data:' else
                                rlUrlRead)(name))
            except Exception:
...
def rl_isdir(pn, os_path_isdir=os.path.isdir, os_path_normpath=os.path.normpath):
    ...
    if not pn.endswith(os.sep):
        pn += os.sep
    return len(list(filter(lambda x, pn=pn: x.startswith(pn),
                           list(__rl_loader__._files.keys())))) > 0


def rl_listdir(pn, os_path_isdir=os.path.isdir, os_path_normpath=os.path.normpath,
...
def rl_getmtime(pn, os_path_isfile=os.path.isfile, os_path_normpath=os.path.normpath,
                os_path_getmtime=os.path.getmtime, time_mktime=time.mktime):
    ...
    s = e[5]
    d = e[6]
    return time_mktime((((d >> 9) & 0x7f)+1980, (d >> 5) & 0xf, d & 0x1f,
                        (s >> 11) & 0x1f, (s >> 5) & 0x3f, (s & 0x1f) << 1, 0, 0, 0))
    ...
    try:
        try:
            ...
            if isCompactDistro():
                ...
                dir = ((dir == '.' or not dir) and _archive
        ...
        if _isPILImage(fileName):
            ...
            try:
                ...
                if imageReaderFlags != 0:
                    raise ValueError(
                        'imageReaderFlags values other than 0 are no longer ' +
                        'supported; all images are interned now')
                ...
                self._image.fileName = (fileName if isinstance(fileName, str) else
                                        repr(fileName))
        ...
        try:
            if self._data is None:
                ...
                if mode in ('LA', 'RGBA'):
                    ...
                    if (im.format == 'PNG' and im.mode == 'P'
...
class DebugMemo:
    ...
    '''
    def __init__(self, fn='rl_dbgmemo.dbg', mode='w', getScript=1, modules=(),
                 capture_traceback=1, stdout=None, **kw):
        import socket
        self.fn = fn
        if not stdout:
        ...
        for M, A in (
                ...
                (os, ('uname', 'ctermid', 'getgid', 'getuid', 'getegid',
                      'geteuid', 'getlogin', 'getgroups', 'getpgrp',
                      'getpid', 'getppid',
                      )),
                ):
            for a in A:
        ...
        for n, m in sys.modules.items():
            if (n == 'reportlab' or n == 'rlextra' or n[:10] == 'reportlab.' or
                    n[:8] == 'rlextra.'):
...
def find_locals(func, depth=0):
    '''apply func to the locals at each stack frame
    till func returns a non false value'''
    while 1:
...
def simpleSplit(text, fontName, fontSize, maxWidth):
    ...
    def SW(text, fN=fontName, fS=fontSize):
        return stringWidth(text, fN, fS)
    if maxWidth:
        L = []
        for l in lines:  # noqa
...
class RLString(str):
    '''allows specification of extra properties of a string using a dictionary
    of extra attributes
