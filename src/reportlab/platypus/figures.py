...
captionStyle = ParagraphStyle('Caption', fontName=_baseFontNameI, fontSize=10,
                              alignment=TA_CENTER)
...
class Figure(Flowable):
    ...
    def _getCaptionPara(self):
        ...
        if self._captionData != (caption, captionFont, captionSize, captionTextColor,
                                 captionBackColor, captionAlign, captionPosition):
            self._captionData = (caption, captionFont, captionSize, captionTextColor,
                                 captionBackColor, captionAlign, captionPosition)
            if isinstance(caption, Paragraph):
                ...
                self.captionStyle = ParagraphStyle(
                    ...
                    alignment=(TA_LEFT if captionAlign == 'left' else
                               TA_RIGHT if captionAlign == 'right' else TA_CENTER),
                    )
...
class FlexFigure(Figure):
    ...
    def _scale(self, availWidth, availHeight):
        ...
        if (self._scaleFactor is None or self.width > availWidth or
            ...
            if self.scaleFactor is None:
                ...
                self._scaleFactor = min(availWidth/self.width,
...
class ImageFigure(FlexFigure):
    ...
    def __init__(self, filename, caption, background=None, scaleFactor=None,
                 hAlign='CENTER', border=None):
        assert os.path.isfile(filename), 'image file %s not found' % filename
        from reportlab.lib.utils import ImageReader
        w, h = ImageReader(filename).getSize()
        self.filename = filename
        FlexFigure.__init__(self, w, h, caption, background, scaleFactor=scaleFactor,
                            hAlign=hAlign, border=border)
...
try:
    from rlextra.pageCatcher.pageCatcher import (storeForms, storeFormsInMemory,
...
if _hasPageCatcher:
    ...
    class PageCatcherCachingMixIn:
        ...
        def processPDF(self, pdfFileName, pageNo):
            formName = self.getFormName(pdfFileName, pageNo)
            storeForms(pdfFileName, formName + '.frm', prefix=formName + '_',
                       pagenumbers=[pageNo])
    ...
    class cachePageCatcherFigureNonA4(FlexFigure, PageCatcherCachingMixIn):
        ...
        def __init__(self, filename, pageNo, caption, width, height, background=None):
            ...
            self.pageNo = pageNo
            self.formName = (self.getFormName(self.filename, self.pageNo) +
                             '_' + str(pageNo))
    ...
    class cachePageCatcherFigure(cachePageCatcherFigureNonA4):
        ...
        def __init__(self, filename, pageNo, caption, width=595, height=842,
                     background=None):
            cachePageCatcherFigureNonA4.__init__(self, filename, pageNo, caption,
    ...
    class PageCatcherFigureNonA4(FlexFigure):
        ...
        def __init__(self, filename, pageNo, caption, width, height,
                     background=None, caching=None):
            fn = self.filename = filename
            self.pageNo = pageNo
            fn = (fn.replace(os.sep, '_').replace('/', '_').replace('\\', '_')
                  .replace('-', '_').replace(':', '_'))
        ...
        def drawFigure(self):
            if not self.canv.hasForm(self.formName):
                if self.filename in self._cache:
                    ...
                    pdf = f.read()
                    f.close()
                    f, data = storeFormsInMemory(pdf, pagenumbers=[self.pageNo],
                                                 prefix=self.prefix)
    ...
    class PageCatcherFigure(PageCatcherFigureNonA4):
        ...
        def __init__(self, filename, pageNo, caption, width=595, height=842,
                     background=None, caching=None):
            PageCatcherFigureNonA4.__init__(self, filename, pageNo, caption, width,
                                            height, background=background,
                                            caching=caching)
...
def demo1(canvas):
    ...
    bodyStyle = ParagraphStyle('Body', fontName=_baseFontName, fontSize=24,
                               leading=28, spaceBefore=6)
