from reportlab.lib.testutils import (setOutDir, makeSuiteForClasses, outputfile,
                                     printLocation, NearTestCase, rlSkipIf)
import unittest
import sys
...
class TextEncodingTestCase(NearTestCase):
    ...
    def testStringWidth(self):
        ...
        self.assertNear(pdfmetrics.stringWidth(testCp1252,
                                               'Courier', 10, 'cp1252'), 312.0)
    ...
    @rlSkipIf(sys.getfilesystemencoding().lower() in ('ascii', 'ansi_x3.4-1968'),
              'need better file system encoding')
    ...
    def testUtf8Canvas(self):
        ...
        d2.add(String(200, 25, testUTF8, fontName='Vera',
                      textAnchor='middle', encoding='utf-8'))
        d2.drawOn(c, 100, 100)
...
class FontEncodingTestCase(unittest.TestCase):
    ...
    Nothing really to do with character encodings;
    this is about hacking the font itself"""
    ...
    def test0(self):
        ...
        c.drawString(100, 700, 'The text below should be in a custom encoding ' +
                     'in which all vowels become "z"')
        ...
        f = pdfmetrics.Font('FontWithoutVowels', 'Helvetica-Oblique',
                            'EncodingWithoutVowels')
        ...
        c.setFont('Helvetica', 12)
        c.drawString(100, 650, "MacRoman encoding lacks a Euro.  We'll make " +
                     "a Mac font with the Euro at #219:")
        ...
        pdfmetrics.registerFont(
            pdfmetrics.Font('MacHelvNoEuro', 'Helvetica-Oblique', 'MacRomanEncoding'))
        c.setFont('MacHelvNoEuro', 12)

        # oct(219)=0333
        c.drawString(125, 600, 'Standard MacRoman, no Euro: Character 219 = "\333"')
        ...
        pdfmetrics.registerFont(
            pdfmetrics.Font('MacHelvWithEuro', 'Helvetica-Oblique', 'MacWithEuro'))

        c.setFont('MacHelvWithEuro', 12)

        # oct(219)=0333
        c.drawString(125, 575, 'Hacked MacRoman with Euro: Character 219 = "\333"')

        # now test width setting with and without _rl_accel - harder
        # make an encoding where 'm' becomes 'i'
        c.setFont('Helvetica', 12)
        c.drawString(100, 500, "Recode 'm' to 'i' and check we can measure widths. " +
                     "Boxes should surround letters.")
        sample = 'Mmmmm. ' * 6 + 'Mmmm'
        ...
        pdfmetrics.registerFont(pdfmetrics.Font('narrow', 'Helvetica-Oblique',
                                                'm-to-i'))
        c.setFont('narrow', 12)
        ...
        c.setFont('Helvetica', 12)
        c.drawString(100, 400, "Symbol & Dingbats fonts - check we still " +
                     "get valid PDF in StandardEncoding")
        c.setFont('Symbol', 12)
