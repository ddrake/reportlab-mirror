...
from reportlab.graphics.charts.piecharts import (AbstractPieChart, WedgeProperties,
                                                 _addWedgeLabel, fixLabelOverlaps)
...
class Doughnut(AbstractPieChart):
    _attrMap = AttrMap(
        x=AttrMapValue(
            isNumber, desc='X position of the chart within its container.'),
        y=AttrMapValue(
            isNumber, desc='Y position of the chart within its container.'),
        width=AttrMapValue(
            isNumber,
        ...
        slices=AttrMapValue(
            None, desc="collection of sector descriptor objects"),
    ...
    def normalizeData(self, data=None):
        ...
        return (abs(sum) >= 1e-8 and
                list(map(lambda x, f=360./sum: f*x, data)) or len(data)*[0])
        ...
        if multi:
            ...
            for sn, series in enumerate(normData):
                for i, angle in enumerate(series):
                    ...
                    # if we didn't use %stylecount here
                    # we'd end up with the later sectors all having the default style
                    ...
                    xr = xr1 + xdr
                    if len(series) > 1:
                        theSector = Wedge(cx, cy, xr, a1, a2, yradius=yr, radius1=xr1,
                                          yradius1=yr1)
                    else:
                        theSector = Wedge(cx, cy, xr, a1, a2, yradius=yr, radius1=xr1,
                                          yradius1=yr1, annular=True)
                    ...
                    if shader:
                        ...
                        if nshades > 1:
                            shader = (colors.Whiter if shader == 'lighten' else
                                      colors.Blacker)
                            ...
                            for ish in range(nshades):
                                ...
                                if len(series) > 1:
                                    shSector = Wedge(
                                        cx, cy, xr, sha1, sha2, yradius=yr,
                                        radius1=xr1, yradius1=yr1)
                                else:
                                    shSector = Wedge(
                                        cx, cy, xr, sha1, sha2, yradius=yr,
                                        radius1=xr1, yradius1=yr1, annular=True)
            ...
            for i, angle in enumerate(normData):
                ...
                if n > 1:
                    theSector = Wedge(cx, cy, xradius, a1, a2, yradius=yradius,
                                      radius1=xir, yradius1=yir)
                elif n == 1:
                    theSector = Wedge(cx, cy, xradius, a1, a2, yradius=yradius,
                                      radius1=xir, yradius1=yir, annular=True)
                ...
                if shader:
                    ...
                    if nshades > 1:
                        ...
                        for ish in range(nshades):
                            ...
                            shc = shader(shsc, shf1 + dsh*ish)
                            if n > 1:
                                shSector = Wedge(cx, cy, xradius, sha1, sha2,
                                                 yradius=yradius, radius1=xir,
                                                 yradius1=yir)
                            elif n == 1:
                                shSector = Wedge(cx, cy, xradius, sha1, sha2,
                                                 yradius=yradius, radius1=xir,
                ...
                if labels[i] and sectorStyle.visible and sectorStyle.label_visible:
                    ...
                    aveAngleRadians = averageAngle*pi/180.0
                    labelRadius = sectorStyle.labelRadius
                    labelX = centerx + (0.5 * self.width * cos(aveAngleRadians) *
                                        labelRadius)
                    labelY = centery + (0.5 * self.height * sin(aveAngleRadians) *
                                        labelRadius)
                    rx = xradius*labelRadius
                    ry = yradius*labelRadius
                    l = _addWedgeLabel(self, labels[i], averageAngle,  # noqa
...
def sample1():
    ...
    d = Drawing(400, 400)
    # g = Group()  (assigned but not used)

    s1 = Wedge(centerx=200, centery=200, radius=150, startangledegrees=0,
               endangledegrees=120, radius1=100)
    s1.fillColor = colors.red
    s1.strokeColor = None
    d.add(s1)
    s2 = Wedge(centerx=200, centery=200, radius=150, startangledegrees=120,
               endangledegrees=240, radius1=100)
    s2.fillColor = colors.green
    s2.strokeColor = None
    d.add(s2)
    s3 = Wedge(centerx=200, centery=200, radius=150, startangledegrees=240,
               endangledegrees=260, radius1=100)
    s3.fillColor = colors.blue
    s3.strokeColor = None
    d.add(s3)
    s4 = Wedge(centerx=200, centery=200, radius=150, startangledegrees=260,
               endangledegrees=360, radius1=100)
