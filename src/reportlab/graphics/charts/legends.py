...
from reportlab.lib.validators import (isNumber, OneOf, isString, isColorOrNone,
                                      isNumberOrNone, isListOfNumbersOrNone,
                                      isBoolean, EitherOr, NoneOr, AutoOr, isAuto,
                                      Auto, isBoxAnchor, SequenceOf, isInstanceOf)
...
class SubColProperty(PropHolder):
    ...
    _attrMap = AttrMap(
        ...
        underlines=AttrMapValue(
            EitherOr((NoneOr(isInstanceOf(Line)), SequenceOf(
                isInstanceOf(Line), emptyOK=0, lo=0, hi=0x7fffffff))),
...
class LegendCallout:
    def _legendValues(legend, *args):
        '''return a tuple of values from the first function up the stack
        with isinstance(self,legend)'''
...
class Legend(Widget):
    ...
    _attrMap = AttrMap(
        x=AttrMapValue(
            isNumber, desc="x-coordinate of upper-left reference point"),
        y=AttrMapValue(
            isNumber, desc="y-coordinate of upper-left reference point"),
        deltax=AttrMapValue(
        ...
        dx=AttrMapValue(
            isNumber, desc="Width of swatch rectangle"),
        dy=AttrMapValue(
            isNumber, desc="Height of swatch rectangle"),
        columnMaximum=AttrMapValue(
            isNumber, desc="Max. number of items per column"),
        ...
        fontName=AttrMapValue(
            isString, desc="Font name of the strings"),
        fontSize=AttrMapValue(
            isNumber, desc="Font size of the strings"),
        leading=AttrMapValue(
            isNumberOrNone, desc="text leading"),
        fillColor=AttrMapValue(
            isColorOrNone, desc="swatches filling color"),
        strokeColor=AttrMapValue(
            isColorOrNone, desc="Border color of the swatches"),
        ...
        swatchMarker=AttrMapValue(
            ...
            desc="None, Auto() or makeMarker('Diamond') ...", advancedUsage=1),
        callout=AttrMapValue(
            None, desc="a user callout(self,g,x,y,(color,text))", advancedUsage=1),
        boxAnchor=AttrMapValue(
            isBoxAnchor, 'Anchor point for the legend area'),
        variColumn=AttrMapValue(
            isBoolean, 'If true column widths may vary (default is false)',
            advancedUsage=1),
        ...
        dividerWidth=AttrMapValue(
            isNumber, desc="dividerLines width", advancedUsage=1),
        ...
        dividerOffsX=AttrMapValue(
            ...
            desc='divider lines X offsets', advancedUsage=1),
        dividerOffsY=AttrMapValue(
            isNumber, desc="dividerLines Y offset", advancedUsage=1),
        colEndCallout=AttrMapValue(
            None,
            desc="a user callout(self,g, x, xt, y,width, lWidth)", advancedUsage=1),
        subCols=AttrMapValue(
            None, desc="subColumn properties"),
        swatchCallout=AttrMapValue(
            None, desc="a user swatch callout(self,g,x,y,i,(col,name),swatch)",
            advancedUsage=1),
        swdx=AttrMapValue(
            isNumber, desc="x position adjustment for the swatch"),
        swdy=AttrMapValue(
            isNumber, desc="y position adjustment for the swatch"),
        )
    ...
    def _getTexts(self, colorNamePairs):
        if not isAuto(colorNamePairs):
            ...
            texts = [chart.getSeriesName(i, 'series %d' % i)
    ...
    def _calculateMaxBoundaries(self, colorNamePairs):
        ...
        M = [_getWidths(i, m, fontName, fontSize, subCols)
        ...
        for i in range(n):
                ...
                if isAuto(name):
                    name = getattr(swatchMarker, 'chart',
                                   getattr(swatchMarker, 'obj',
                                           None)).getSeriesName(i, 'series %d' % i)
            T = _getLines(name)
            S = []
            aS = S.append
            ...
            for k, lines in enumerate(T):
                ...
                if anchor == 'left':
                    ...
                    xoffs = 0.5*(x1+x2)
                for t in lines:
                    aS(String(xoffs+scdx,  y+scdy+vAdy, t, fontName=fN, fontSize=fS,
                              fillColor=fC,  textAnchor=anchor))
                    y -= fL
                yd = min(yd, y)
                y += fL
            ...
            if dividerLines:
                ...
                if (((dividerLines & 1) and i % columnMaximum) or
                    ...
                    g.add(Line(
                        thisx+dividerOffsX[0], yd, xd, yd,
                        strokeColor=dividerColor, strokeWidth=dividerWidth,
            ...
            if isAuto(col):
                ...
                if isSymbol(swatchMarker):
                    ...
                    c = self._defaultSwatch(swatchX, swatchY, dx, dy, fillColor=col,
                                            strokeWidth=strokeWidth,
                                            strokeColor=strokeColor)
...
class TotalAnnotator(LegendColEndCallout):
    def __init__(self, lText='Total', rText='0.0', fontName=_baseGFontName,
                 fontSize=10, fillColor=colors.black, strokeWidth=0.5,
                 strokeColor=colors.black, strokeDashArray=None,
    ...
    def __call__(self, legend, g, x, xt, y, width, lWidth):
        ...
        if strokeWidth and strokeColor:
            ...
            g.add(Line(
                x+self.dlx[0], yL, x+self.dlx[1]+lWidth, yL,
...
class LineSwatch(Widget):
    ...
    _attrMap = AttrMap(
        x=AttrMapValue(
            isNumber, desc="x-coordinate for swatch line start point"),
        y=AttrMapValue(
            isNumber, desc="y-coordinate for swatch line start point"),
        width=AttrMapValue(
            isNumber, desc="length of swatch line"),
        height=AttrMapValue(
            isNumber, desc="used for line strokeWidth"),
        strokeColor=AttrMapValue(
            isColorOrNone, desc="color of swatch line"),
        strokeWidth=AttrMapValue(
            isNumberOrNone, desc="thickness of the swatch"),
    ...
    def draw(self):
        l = Line(self.x, self.y, self.x+self.width, self.y)  # noqa
