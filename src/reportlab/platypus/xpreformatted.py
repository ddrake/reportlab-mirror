...
from reportlab.platypus.paragraph import (Paragraph, _handleBulletWidth, ParaLines,
                                          _getFragWords, stringWidth, getAscentDescent,
                                          imgVRange, imgNormV)
...
def _getFragWord(frags, maxWidth):
    ...
    for f in frags:
        ...
        # del f.text # we can't do this until we sort out splitting of paragraphs
    return n, s, W


class XPreformatted(Paragraph):
    def __init__(self, text, style, bulletText=None, frags=None,
                 caseSensitive=1, dedent=0):
        self.caseSensitive = caseSensitive
        # don't assign a lambda expression, use def

        def cleaner(text, dedent=dedent):
    ...
    def breakLines(self, width):
        ...
        style = self.style
        # fFontSize = float(style.fontSize)  (assigned but not used)
        requiredWidth = 0
        ...
        if nFrags == 1:
            ...
            if hasattr(f, 'text'):
                ...
                L = f.text.split('\n')
                for ll in L:
                    currentWidth = stringWidth(ll, fontName, fontSize)
                    if currentWidth > self._width_max:
                        self._width_max = currentWidth
                    requiredWidth = max(currentWidth, requiredWidth)
                    extraSpace = maxWidth-currentWidth
                    lines.append((extraSpace, ll.split(' '), currentWidth))
                    lineno = lineno+1
                    maxWidth = (lineno < len(maxWidths) and maxWidths[lineno] or
                                maxWidths[-1])
                blPara = f.clone(kind=kind, lines=lines, ascent=ascent,
                                 descent=descent, fontSize=fontSize)
            else:
                kind = f.kind
                lines = f.lines
                for L in lines:
            ...
            for L in _getFragLines(frags):
                ...
                if currentWidth > self._width_max:
                    self._width_max = currentWidth
                lines.append(ParaLines(extraSpace=extraSpace, wordCount=n, words=words,
                                       fontSize=maxSize, ascent=maxAscent,
                                       descent=minDescent, currentWidth=currentWidth,
                                       preformatted=True))
...
class PythonPreformatted(XPreformatted):
    ...
    def __init__(self, text, style, bulletText=None, dedent=0, frags=None):
        ...
        XPreformatted.__init__(self, text, style, bulletText=bulletText,
                               dedent=dedent, frags=frags)
...
if __name__ == '__main__':    # NORUNTESTS
    ...
    def dumpXPreformattedFrags(P):
        ...
        n = len(frags)
        for ll in range(n):
            print("frag%d: '%s'" % (ll, frags[ll].text))
...
combination of the <font name=courier color=green>CMY</font> pigments
generally never makes a perfect

black -- instead producing a muddy color -- so, to get black printers
don't use the <font name=courier color=green>CMY</font> pigments but use
a direct black ink.  Because
<font name=courier color=green>CMYK</font> maps more directly to the way
printer hardware works it may
be the case that &amp;| &amp; | colors specified in
<font name=courier color=green>CMYK</font> will provide better fidelity
