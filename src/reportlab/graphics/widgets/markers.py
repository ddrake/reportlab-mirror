...
class Marker(Widget):
    ...
    def _doPolygon(self, P):
        ...
        return Polygon(P, strokeWidth=self.strokeWidth, strokeColor=self.strokeColor,
...
class _isSymbol(Validator):
    def test(self, x):
        return (hasattr(x, '__call__') or isinstance(x, Marker) or
                isinstance(x, _Symbol) or (isClass(x) and issubclass(x, Widget)))
