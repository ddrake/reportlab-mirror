...
def textFieldAbsolute(canvas, title, x, y, width, height, value="",
                      maxlen=1000000, multiline=0):
    ...
    return theform.textField(canvas, title, x, y, x+width, y+height,
                             value, maxlen, multiline)


def textFieldRelative(canvas, title, xR, yR, width, height,
                      value="", maxlen=1000000, multiline=0):
    """same as textFieldAbsolute except the x and y are relative
       to the canvas coordinate transform
    """
    (xA, yA) = canvas.absolutePosition(xR, yR)
    return textFieldAbsolute(canvas, title, xA, yA, width, height,
                             value, maxlen, multiline)
...
def buttonFieldRelative(canvas, title, value, xR, yR, width=16.7704, height=14.907):
    """same as buttonFieldAbsolute except the x and y are relative
       to the canvas coordinate transform
    """
    (xA, yA) = canvas.absolutePosition(xR, yR)
...
class AcroForm(PDFObject):
    ...
    def textField(self, canvas, title, xmin, ymin, xmax, ymax,
                  value="", maxlen=1000000, multiline=0):
...
def FormResources():
    return PDFPattern(
        ...
        Encoding=PDFPattern(EncodingPattern,
...
def FormFont(BaseFont, Name):
    ...
    return PDFPattern(FormFontPattern, BaseFont=PDFName(BaseFont),
                      Name=PDFName(Name), Encoding=PDFPattern(PDFDocEncodingPattern))
...
def TextField(title, value, xmin, ymin, xmax, ymax, page,
              maxlen=1000000, font="Helvetica-Bold", fontsize=9,
              R=0, G=0, B=0.627, multiline=0):
    ...
    return PDFPattern(TextFieldPattern,
                      ...
                      fontname=PDFName(fontname), fontsize=fontsize,
...
def SelectField(title, value, options, xmin, ymin, xmax, ymax, page,
    ...
    # print "ARGS", (title, value, options, xmin, ymin, xmax, ymax,
    #                page, font, fontsize, R, G, B)
...
def ButtonField(title, value, xmin, ymin, page, width=16.7704, height=14.907):
    ...
    fontSize = (11.3086/14.907)*height  # noqa: F841
    dx = (3.6017/16.7704)*width         # noqa: F841
    dy = (3.3881/14.907)*height         # noqa: F841
    return PDFPattern(
        ButtonFieldPattern,
        Name=PDFString(title),
        ...
        APDYes=ButtonStream(('0.749 g 0 0 %(width)s %(height)s re f q 1 1 %(width)s ' +
                             '%(height)s re W n BT /ZaDb %(fontSize)s Tf 0 g 1 0 0 1 ' +
                             '%(dx)s %(dy)s Tm (4) Tj ET\r\n') % vars(),
                            width=width, height=height),
        APNYes=ButtonStream(('q 1 1 %(width)s %(height)s re W n BT /ZaDb ' +
                             '%(fontSize)s Tf 0 g   1 0 0 1 %(dx)s %(dy)s ' +
                             'Tm (4) Tj ET Q\r\n') % vars(),
                            width=width, height=height),
        Value=PDFName(value),
        Page=page)
