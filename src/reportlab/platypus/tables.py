...
a tuple of row heights and the data in row order. Drawing of the table can be
controlled by using a TableStyle instance. This allows control of the color and
weight of the lines (if any), and the font, alignment and padding of the text.

None values in the sequence of row heights or column widths, mean that the
corresponding rows or columns should be automatically sized.
...
See the test output from running this module as a script for a discussion of
the method for constructing tables and table styles.
"""
...
def _quadrantDef(xpos, ypos, corner, r, kind=0, direction='left-right', m=0.4472):
    ...
    if (direction == 'left-right' and P[0][0] > P[-1][0] or
            direction == 'bottom-top' and P[0][1] > P[-1][1]):
        P.reverse()
...
RoundingRectLine = namedtuple('RoundingRectLine',
                              'xs ys xe ye weight color cap dash join')
...
class Table(Flowable):
    def __init__(self, data, colWidths=None, rowHeights=None, style=None,
                 repeatRows=0, repeatCols=0, splitByRow=1, splitInRow=0,
                 ...
                 minRowHeights=None, cornerRadii=__UNSET__,
                 # or [topLeft, topRight, bottomLeft bottomRight]
                 renderCB=None,
                 ):
        self.ident = ident
        ...
        if not (nrows and ncols):
            if emptyTableAction == 'error':
                ...
                Preformatted.__init__(self, '%s(%d, %d)' %
                                      (self.__class__.__name__, nrows, ncols),
                                      _emptyTableStyle)
        ...
        if not _seqCW:
            ...
            if rl_config.allowShortTableRows and isinstance(colWidths, list):
                ...
                raise ValueError(f'{self.identity()} data error - {ncols} ' +
                                 f'columns in data but {len(colWidths)} column widths')
        if not _seqRH:
            rowHeights = nrows*[rowHeights]
        elif len(rowHeights) != nrows:
            raise ValueError(
                f'{self.identity()} data error - {nrows} rows in data ' +
                f'but {len(rowHeights)} row heights')
        for i, d in enumerate(data):
            ...
            if n != ncols:
                if rl_config.allowShortTableRows and isinstance(d, list):
                    ...
                    raise ValueError(f'{self.identity()} expected {ncols} ' +
                                     f'not {n} columns in row {i}!')
        ...
        # NB repeatRows can be a list or tuple eg (1,)
        # repeats only the second row of a table
        ...
        if minRowHeights is not None:
            lmrh = len(minRowHeights)
            if not lmrh:
                raise ValueError(f'{self.idenity()} Supplied mismatching minimum ' +
                                 f'row heights of length {lmrh}')
            elif lmrh < nrows:
        ...
        if isinstance(v, _ExpandedCellTuple):
            ...
            for c in self._cellListIter(C, aW, aH):
                if isinstance(c, Indenter):
                    ...
                    continue
                if frame:
                    R(LIIndenter(c, leftIndent=frame._leftExtraIndent,
    ...
    def _listValueWidth(self, V, aH=72000, aW=72000):
        if not V:
            return 0, 0
        # t = 0 (assigned but not used)
        # w = 0 (assigned but not used)
        canv = getattr(self, 'canv', None)
    ...
    def _calc_width(self, availWidth, W=None):
        ...
        if None in W:  # some column widths are not given
            # canv = getattr(self, 'canv', None)  (set but not used)
            # saved = None (set but not used)
            if self._spanCmds:
            ...
            while None in W:
                ...
                for i, Vi in enumerate(V):
                    ...
                    if ji in colSpanCells and not span:
                        ...
                        if t is None:
                            raise ValueError(
                                f'Flowable {v.identity()} in cell({i},{j}) ' +
                                f'can\'t have auto width\n{self.identity(30)}')
    ...
    def _calc_height(self, availHeight, availWidth, H=None, W=None):
        ...
        if None in H:
            ...
            self._rowHeights = H
            spanCons = {}
            # FUZZ = rl_config._FUZZ  (assigned but not used)
            while None in H:
                i = H.index(None)
                    ...
                    if ji in rowSpanCells and not span:
                        ...
                        if isinstance(v, (tuple, list, Flowable)):
                            ...
                            if w is None and not self._canGetWidth(v):
                                raise ValueError(
                                    f'Flowable {v[0].identity()} in cell({i},{j}) ' +
                                    f'can\'t have auto width\n{self.identity(30)}')
                            if canv:
                                canv._fontname, canv._fontsize, canv._leading = \
                                    s.fontname, s.fontsize, s.leading or 1.2*s.fontsize
                            if ji in colSpanCells:
                                if not span:
                                    continue
                                w = max(colpositions[span[2]+1]-colpositions[span[0]],
                                        w or 0)
                            dW, t = self._listCellGeom(v,
                                                       w or self._listValueWidth(v),
                                                       s)
                            if canv:
                            ...
                            if not rl_config.allowTableBoundsErrors and dW > w:
                                ...
                                raise LayoutError(
                                    ("Flowable %s (%sx%s points) too wide for " +
                                     "cell(%d,%d) (%sx* points) in\n%s") %
                                    (v[0].identity(30), fp_str(dW), fp_str(t), i, j,
                                     fp_str(w), self.identity(30)))
                        else:
                ...
                # If a minimum height has been specified use that,
                # otherwise allow the cell to grow
            ...
            if spanCons:
                try:
                    ...
                    annotateException(
                        ('\nspanning problem in %s hmax=%s lim=%s avail=%s x ' +
                         '%s\nH0=%r H=%r\nspanCons=%r') %
                        (self.identity(), hmax, lim, availWidth, availHeight,
                         H0, H, spanCons))
    ...
    def _calc(self, availWidth, availHeight):
        ...
        if ((None in self._colWidths or '*' in self._colWidths) and
    ...
    def _culprit(self):
        ...
        rh = self._rowHeights
        tallest = max(rh)
        # rowNum = rh.index(tallest)  (assigned but not used)
    ...
    def _calcPreliminaryWidths(self, availWidth):
        ...
        # if verbose:
        #    print(('prelim width calculation.  %d columns, %d undefined width, ' +
        #          '%0.2f units remain')
        ...
        for colNo in range(self._ncols):
            ...
            if w is None or w == '*' or _endswith(w, '%'):
                ...
                for rowNo in range(self._nrows):
                    ...
                    #     print('[%d,%d] new=%r-->%r'
                    #           % (rowNo, colNo,
                    #              new - style.leftPadding+style.rightPadding, new))
                    final = max(final, new)
        ...
        for cmd in tblstyle.getCommands():
            if len(cmd) >= 3:
                ...
                if (isinstance(sc, str) or isinstance(ec, str)
                    ...
                    raise ValueError(
                        f'''bad style command {cmd!r} illegal of invalid string
                            coordinate only rows may be strings with values in
                        {_SPECIALROWS!r}''')
    ...
    def _drawLines(self):
        ...
        try:
            for c in self._linecmds:
                op, (sc, sr), (ec, er), weight, color, cap, dash, join, count, space = c
    ...
    def _drawHLines(self, start, end, weight, color, count, space):
        ...
        if count == 1:
            ...
            # don't assign a lambda, use a def
            def lf(x0, y0, x1, y1, canvLine=canvLine, ws=weight+space, count=count):
                return _multiLine(x0, x1, y0, canvLine, ws, count)
            for y in rp:
    ...
    def _drawVLines(self, start, end, weight, color, count, space):
        ...
        # don't assign a lambda, use a def
        def canvLine(y0, x0, y1, x1, _line=self.canv.line):
    ...
    def _cr_1_1(self, n, nRows, repeatRows, cmds, doInRowSplit, _srflMode=False):
        ...
        for c in cmds:
            ...
            if cS:
                # it's a repeat row
                cS = list(cS)
                self._addCommand((c[0],)+((sc, repeatRows.index(min(cS))),
                                          (ec, repeatRows.index(max(cS))))+tuple(c[3:]))
            if er < n:
                continue
            sr = max(sr-n, 0)+nrr
            er = max(er-n, 0)+nrr
    ...
    def _splitLineCmds(self, n, doInRowSplit=0):
        ...
        for lcs in self._linecmds:
            op, (sc, sr), (ec, er), weight, color, cap, dash, join, count, space = lcs
            if isinstance(sr, strTypes) and sr in _SPECIALROWS:
                A.append((op, (sc, sr), (ec, sr), weight, color,
                          cap, dash, join, count, space))
                if sr == 'splitlast':
                    sr = er = n-1
                elif sr == 'splitfirst':
                    sr = n
                    ...
                    if ec < 0:
                        ec += ncols
                    A[-1] = (op, (sc, sr), (ec, sr), weight, color, cap,
                             dash, join, count, space)
                    continue
            ...
            if op in ('BOX', 'OUTLINE', 'GRID'):
                if (sr < n and er >= n) or (doInRowSplit and sr == n):
                    ...
                    A.append(('LINEABOVE', (sc, sr), (ec, sr), weight,
                              color, cap, dash, join, count, space))
                    ...
                    if op == 'GRID':
                        if doInRowSplit:
                            A.append(('INNERGRID', (sc, sr), (ec, n-1), weight,
                                      color, cap, dash, join, count, space))
                            A.append(('INNERGRID', (sc, n), (ec, er), weight,
                                      color, cap, dash, join, count, space))
                        else:
                    ...
                    A.append((op, (sc, sr), (ec, er), weight,
                              color, cap, dash, join, count, space))
            elif op == 'INNERGRID':
    ...
    def _splitRows(self, availHeight, doInRowSplit=0):
        ...
        if not doInRowSplit:
            ...
            for column, (value, style, width) in enumerate(zip(cellvalues, cellStyles,
                                                               cellWidths)):
                ...
                if isinstance(value, (tuple, list)):
                    ...
                    rows = value.split("\n")
                    lineHeight = 1.2 * style.fontsize
                    height = (lineHeight * len(rows) +
                              style.topPadding + style.bottomPadding)

                    # Make sure we don't try to split in the middle of
            ...
            if ((minSplit + maxSplit > curRowHeight) or
                ...
                # We can't split this row. So we should fail, and let the split get
                # retried with splitInRow = 0. However, if there is a spanned row
                # that will also fail. So first we need to check if any cell in the
                # current row is spanned.
                if not self._spanCmds:
                    # There are no spans to look at,
                    # so we can skip this and fail directly
                    return []
                ...
                for column in range(self._ncols):
                    ...
                    if (cell in self._rowSpanCells and
                        ...
                        for cell, span in self._spanRanges.items():
                            ...
                            if (column >= start_col and column <= end_col and
                                    n > start_row and n <= end_row):
                                splitCells.add(cell)
                                break
                ...
                for cell in splitCells:
                    ...
                    for cmd, start, end in spanCmds:
                        if ((span_sc, span_sr) == start and
                            ...
                            if n-1 > span_sr or span_sc != span_ec:
                                newSpanCmds.append((cmd, (span_sc, span_sr),
                                                    (span_ec, n-1)))
                            if n < span_er or span_sc != span_ec:
                                newSpanCmds.append((cmd, (span_sc, n),
                                                    (span_ec, span_er)))
                        else:
                    ...
                    for cmd, start, end, color in bkgrndcmds:
                        if start == (span_sc, span_sr):
                            ...
                            newbkgrndcmds.append((cmd, (start[0], n),
                                                  (end[0], n), color))
                        else:
                ...
                T = self.__class__(
                    data, colWidths=self._colWidths,
                ...
                for linecmd in self._linecmds:
                    op, (sc, sr), (ec, er), weight, color, \
                        cap, dash, join, count, space = linecmd
                    ...
                    if ((op == 'BOX') or (op == 'GRID' and (sr <= n or er >= n)) or
                        ...
                        if op in ('GRID', 'INNERGRID'):
                            newlinecmds.append(('INNERGRID', (sc, sr), (ec, n-1),
                                                weight, color, cap, dash, join, count,
                                                space))
                            newlinecmds.append(('INNERGRID', (sc, n), (ec, er), weight,
                                                color, cap, dash, join, count, space))
                            newlinecmds.append(('LINEBELOW', (sc, n-1), (ec, n-1),
                                                weight, color, cap, dash, join, count,
                                                space))
                        if op in ('GRID', 'BOX'):
                            # The box must be made of lines, because  otherwise
                            # it might not get split where it should, below.
                            newlinecmds.append(('LINEABOVE', (sc, sr), (ec, sr),
                                                weight, color, cap, dash, join,
                                                count, space))
                            newlinecmds.append(('LINEBELOW', (sc, er), (ec, er),
                                                weight, color, cap, dash, join,
                                                count, space))
                            newlinecmds.append(('LINEBEFORE', (sc, sr), (sc, er),
                                                weight, color, cap, dash, join,
                                                count, space))
                            newlinecmds.append(('LINEAFTER', (ec, sr), (ec, er),
                                                weight, color, cap, dash, join,
                                                count, space))
                    else:
                        ...
                        if (((op == "LINEABOVE" and er > span_sr and sr <= span_er) or
                           ...
                           (sc <= span_ec and ec >= span_sc)):
                            ...
                            if sr <= startrow:
                                ...
                                moddedcmds.append(
                                    (op, (sc, sr), (ec, startrow), weight,
                                     color, cap, dash, join, count, space)
                                )

                            # For any lines in between we need to remove the split cell
                            if span_sc > sc:
                                # The start column of the span is higher than
                                # the start column of the line. So we need a line
                                # up until but not including the start column
                                moddedcmds.append(
                                    (op, (sc, max(startrow, sr)),
                                     (span_sc-1, min(er, endrow)), weight, color,
                                     cap, dash, join, count, space)
                                )
                            if span_ec < ec:
                                # The start column of the span is lower than
                                # the end column of the line. So we need a line
                                # starting after but not including the end column
                                moddedcmds.append(
                                    (op, (span_ec+1, max(startrow, sr)),
                                     (ec, min(er, endrow)), weight, color, cap, dash,
                                     join, count, space)
                                )

                            if er >= endrow:
                                moddedcmds.append(
                                    (op, (sc, endrow), (ec, er), weight, color,
                                     cap, dash, join, count, space)
                                )
            ...
            for (value, style, width) in zip(cellvalues, cellStyles, cellWidths):
                ...
                R0.append(v[0])
                R1.append(v[1])
                h0 = (self._listCellGeom(v[0], width, style)[1] + style.topPadding +
                      style.bottomPadding)
                R0Height = max(R0Height, h0)
                h1 = (self._listCellGeom(v[1], width, style)[1] + style.topPadding +
                      style.bottomPadding)
        ...
        renderCB = getattr(self, '_renderCB', None)
        R0 = self.__class__(
            data[:n], colWidths=T._colWidths, rowHeights=splitH[:n],
        ...
        nrows = T._nrows
        # ncols = T._ncols (assigned but not used)
        ...
        if repeatRows:
            ...
            R1 = self.__class__(
                iRows+data[n:], colWidths=T._colWidths, rowHeights=iRowH+splitH[n:],
            ...
            R1._cr_1_1(n, nrows, repeatRows, T._bkgrndcmds, doInRowSplit,
                       _srflMode=True)
            ...
            if r is not None:
                ...
                if y1 != y2:
                    ...
                    while 1:
                        if y > ymax:
                            break
                        # split at position y is impossible
                        # because of overlapping rowspan
                        impossible[y] = None
                        y += 1
    ...
    def _getFirstPossibleSplitRowPosition(self, availHeight, ignoreSpans=0):
        ...
        h = 0
        n = 1
        # from this point of view 0 is the first position where the table
        # may *always* be splitted
        split_at = 0
    ...
    def _makeRoundedCornersClip(self, FUZZ=rl_config._FUZZ):
        ...
        nrows = self._nrows
        ncols = self._ncols
        ar = [min(self._rowHeights[i], self._colWidths[j], cornerRadii[k])
    ...
    def _restoreRoundingObscuredLines(self):
        ...
        line = canv.line
        # cornerRadii = self._cornerRadii  (assigned but not used)
    ...
    def draw(self):
        ...
        if not self._spanCmds:
            ...
            if renderCB:
                for rowNo, (row, rowstyle, rowpos, rowheight) \
                        in enumerate(zip(self._cellvalues, self._cellStyles,
                                         self._rowpositions[1:], self._rowHeights)):
                    renderCB(self, 'startRow', rowNo)
                    for colNo, (cellval, cellstyle, colpos, colwidth) \
                            in enumerate(zip(row, rowstyle, self._colpositions[:-1],
                                             self._colWidths)):
                        renderCB(self, 'startCell', rowNo, colNo, cellval, cellstyle,
                                 (colpos, rowpos), (colwidth, rowheight))
                ...
                for row, rowstyle, rowpos, rowheight \
                        in zip(self._cellvalues, self._cellStyles,
                               self._rowpositions[1:], self._rowHeights):
                    for colNo, (cellval, cellstyle, colpos, colwidth) \
                            in enumerate(zip(row, rowstyle,
                                             self._colpositions[:-1],
                                             self._colWidths)):
                        self._drawCell(cellval, cellstyle, (colpos, rowpos),
                                       (colwidth, rowheight))
        else:
                ...
                for rowNo in range(self._nrows):
                    ...
                    for colNo in range(self._ncols):
                        ...
                        if cellRect is not None:
                            ...
                            renderCB(self, 'startCell', rowNo, colNo, cellval,
                                     cellstyle, (x, y), (width, height))
        ...
        for cmd, (sc, sr), (ec, er), arg in self._bkgrndcmds:
            ...
            if hasattr(arg, '__call__'):
                ...
                if (arg and isinstance(arg, (list, tuple))
                        and arg[0] in ('VERTICAL', 'HORIZONTAL', 'VERTICAL2',
                                       'HORIZONTAL2', 'LINEARGRADIENT',
                                       'RADIALGRADIENT')):
                    ...
                    if arg0 == "HORIZONTAL":
                        ...
                        if 4 <= len(arg) <= 5:
                            ...
                            canv.linearGradient(ax0, ay0, ax1, ay1, C, positions=P,
                                                extend=extend)
                        else:
                            raise ValueError(
                                # undefined name op
                                # f'Wrong length for {op!a} arguments {arg!a}')
                                f'Wrong length for arguments {arg!a}')
                    elif arg0 == 'RADIALGRADIENT':
                        # the remaining arguments define the centre, radius, extend,
                        # colors, stops as
                        ...
                        if 4 <= len(arg) <= 5:
                            ...
                            if ref == 'width':
                                ...
                                raise ValueError(
                                    # undefined name op
                                    # f'Bad radius, {arg[1]!a}, ' +
                                    # f'for {op!a} arguments {arg!r}')
                                    f'Bad radius, {arg[1]!a}, for arguments {arg!r}')
                            r *= ref
                            extend = arg[2]
                            C = arg[3]
                            P = arg[4] if len(arg) == 4 else None
                            canv.radialGradient(xc, yc, r, C, positions=P,
                                                extend=extend)
                        else:
                            # undefined name op
                            # raise ValueError(
                            #    f'Wrong length for {op!a} arguments {arg}')
                            raise ValueError(f'Wrong length for arguments {arg}')
                    canv.restoreState()
                else:
    ...
    def _drawCell(self, cellval, cellstyle, pos, size):
        ...
        if self._curcellstyle is not cellstyle:
            ...
            if (cur is None or cellstyle.leading != cur.leading or
                    cellstyle.fontname != cur.fontname or
                    cellstyle.fontsize != cur.fontsize):
                self.canv.setFont(cellstyle.fontname, cellstyle.fontsize,
                                  cellstyle.leading)
        ...
        if cellstyle.href:
            # external hyperlink
            self.canv.linkURL(cellstyle.href,
                              (colpos, rowpos, colpos + colwidth, rowpos + rowheight),
                              relative=1)
        if cellstyle.destination:
            # external hyperlink
            self.canv.linkRect("", cellstyle.destination,
                               Rect=(colpos, rowpos, colpos + colwidth,
                                     rowpos + rowheight), relative=1)

    def _setCornerRadii(self, cornerRadii):
        if isListOfNumbersOrNone(cornerRadii):
            self._cornerRadii = (None if not cornerRadii else
                                 list(cornerRadii) + (max(4-len(cornerRadii), 0)*[0]))
        else:
            raise ValueError(
                'cornerRadii should be None or a list/tuple of numeric' +
                f'radii\nnot {cornerRadii!a}')
