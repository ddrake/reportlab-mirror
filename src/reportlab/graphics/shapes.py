...
from reportlab.rl_config import (shapeChecking,
                                 ...
                                 _unset_, decimalSymbol)
from reportlab.lib import logger
from reportlab.lib import colors
from reportlab.lib.validators import (NoneOr, isNumberInRange, isTransform,
                                      isListOfShapes, isBoolean, isValidChild,
                                      isNumber, isValidChildOrNone, OneOf,
                                      isStringOrNone, isNumberOrNone, SequenceOf,
                                      Validator, isListOfNumbersOrNone,
...
def _getBezierExtrema(y0, y1, y2, y3):
    ...
    The returned value is [y0,x1,x2,y3] where if found x1, x2 are any extremals
    that were found; there can be 0, 1 or 2 extremals
    '''
    a = y3-3*(y2-y1)-y0
    b = 2*(y2-2*y1+y0)
    c = y1-y0
...
class Shape(_SetKeyWordArgs, _DrawTimeResizeable):
    ...
    def copy(self):
        ...
        raise NotImplementedError("No copy method implemented for %s" %
...
class Group(Shape):
    ...
    _attrMap = AttrMap(
        transform=AttrMapValue(isTransform,
        ...
        fillOverprint=AttrMapValue(isBoolean, desc='Turn on fill overprinting',
                                   advancedUsage=1),
        overprintMask=AttrMapValue(isBoolean, desc='overprinting for ordinary CMYK',
                                   advancedUsage=1),
        )
...
def _addObjImport(obj, I, n=None):  # noqa
...
def _repr(self, I=None):  # noqa
    ...
    if isinstance(self, float):
        ...
        (args, varargs, varkw, defaults, kwonlyargs, kwonlydefaults,
         annotations) = getfullargspec(self.__init__)
        if defaults:
...
def _renderGroupPy(G, pfx, I, i=0, indent='\t\t'):  # noqa
    s = ''
...
class Drawing(Group, Flowable):
    ...
    _attrMap = AttrMap(
        ...
        formats=AttrMapValue(SequenceOf(
            OneOf(*_saveModes), lo=1, emptyOK=0), desc='One or more plot modes'),
            )
    ...
    def _renderPy(self):
        ...
        s = s + ('\n\nif __name__=="__main__": #NORUNTESTS\n\t%s().save(' +
                 'formats=[\'pdf\'],outDir=\'.\',fnRoot=None)\n') % n
        return s
    ...
    def draw(self, showBoundary=_unset_):
        ...
        renderPDF.draw(
            self, self.canv, 0, 0,
            showBoundary=(showBoundary if showBoundary is not _unset_ else
                          getattr(self, '_showBoundary', _unset_)))
    ...
    def save(self, formats=None, verbose=None, fnRoot=None, outDir=None,
        ...
        if not os.path.isabs(outDir):
            outDir = os.path.join(getattr(
                self, '_override_CWD', os.path.dirname(sys.argv[0])), outDir)
        ...
        verbose = (verbose is not None and (verbose,) or
                   (getattr(self, 'verbose', verbose),))[0]
    ...
    def resized(self, kind='fit', lpad=0, rpad=0, bpad=0, tpad=0):
        ...
        if (kind == 'fit' or
                (kind == 'expand' and
                 (xL < lpad or xH > oW-rpad or yL < bpad or yH > oH-tpad))):
...
class isStrokeDashArray(Validator):
    def test(self, x):
        return (isListOfNumbersOrNone.test(x) or
                (isinstance(x, (list, tuple)) and isNumber(x[0])
                 and isListOfNumbers(x[1])))
...
class LineShape(Shape):
    ...
    _attrMap = AttrMap(
        ...
        strokeOpacity=AttrMapValue(
            isOpacity,
            desc=("The level of transparency of the line, " +
                  "any real number betwen 0 and 1")),
        strokeOverprint=AttrMapValue(isBoolean, desc='Turn on stroke overprinting'),
...
class SolidShape(LineShape):
    ...
    _attrMap = AttrMap(
        ...
        fillOpacity=AttrMapValue(
            isOpacity,
            desc=("the level of transparency of the color, " +
...
class Path(SolidShape):
    ...
    def __init__(self, points=None, operators=None, isClipPath=0, autoclose=None,
                 fillMode=FILL_EVEN_ODD, **kw):
...
def getArcPoints(centerx, centery, radius, startangledegrees, endangledegrees,
                 yradius=None, degreedelta=None, reverse=None):
    if yradius is None:
        yradius = radius
    points = []
...
class ArcPath(Path):
    ...
    def addArc(self, centerx, centery, radius, startangledegrees, endangledegrees,
               yradius=None, degreedelta=None, moveTo=None, reverse=None):
        P = getArcPoints(centerx, centery, radius, startangledegrees, endangledegrees,
                         yradius=yradius, degreedelta=degreedelta, reverse=reverse)
...
class Ellipse(SolidShape):
    ...
    def getBounds(self):
        return (self.cx - self.rx, self.cy - self.ry, self.cx + self.rx,
                self.cy + self.ry)
...
class Wedge(SolidShape):
    ...
    def __init__(self, centerx, centery, radius, startangledegrees, endangledegrees,
                 yradius=None, annular=False, **kw):
        SolidShape.__init__(self, kw)
        while endangledegrees < startangledegrees:
            endangledegrees = endangledegrees+360
        # print "__init__"
        (self.centerx, self.centery, self.radius, self.startangledegrees,
         self.endangledegrees) = (centerx, centery, radius, startangledegrees,
                                  endangledegrees)
    ...
    # def __repr__(self):
    #        return "Wedge"+repr((self.centerx, self.centery, self.radius,
    #                self.startangledegrees, self.endangledegrees ))
    # __str__ = __repr__
...
class Hatching(Path):
    '''define a hatching of a set of polygons defined by lists
            ...
            for step in range(spanStart, spanEnd):
                ...
                for P in xyLists:
                    ...
                    for j in range(0, len(P), 2):
                        ...
                        if ((b1 < stripeY and y1 >= stripeY) or
                                y1 < stripeY and b1 >= stripeY):
...
class String(Shape):
    ...
    _attrMap = AttrMap(
        ...
        fontName=AttrMapValue(
            None,
            desc=("font name of the text - font is either acrobat standard " +
                  "or registered when using external font.")),
    ...
    def getBounds(self):
        ...
        if tA != 'start':
            if tA == 'middle':
                ...
                x -= numericXShift(tA, t, w, self.fontName, self.fontSize,
                                   self.encoding)
