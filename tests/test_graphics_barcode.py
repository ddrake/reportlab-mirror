...
from reportlab.graphics.barcode.code128 import Code128, Code128Auto
...
def run(fileName):
    ...
    # styleH = styles['Heading1'] (assigned but not used)
    story = []
    ...
    storyAdd(createBarcodeDrawing('USPS_4State', value='01234567094987654321',
                                  routing='01234567891'))
    ...
    if pylibdmtx:
        ...
        d = Drawing(100, 100)
        d.add(Rect(0, 0, 100, 100, strokeWidth=1, strokeColor=colors.red,
                   fillColor=None))
        addCross(d, 10, 10)
        ...
        d.add(Rect(0, 0, 100, 100, strokeWidth=1, strokeColor=colors.red,
                   fillColor=None))
        addCross(d, 10, 10)
        d.add(DataMatrixWidget(value='1234567890', x=10, y=10, color='black',
                               bgColor='lime'))
        storyAdd(d)
        ...
        d.add(Rect(0, 0, 100, 100, strokeWidth=1, strokeColor=colors.red,
                   fillColor=None))
        addCross(d, 90, 90)
        d.add(DataMatrixWidget(value='1234567890', x=90, y=90, color='darkblue',
                               bgColor='yellow', anchor='ne'))
        storyAdd(d)
...
def fullTest(fileName):
    ...
    story.append(Paragraph('ReportLab %s Barcode Test Suite - full output' %
                           __RL_Version__, styleH))
    story.append(Paragraph('Generated at %s' %
                           TimeStamp().asctime, styleN))

    story.append(Paragraph('About this document', styleH2))
    story.append(Paragraph('History and Status', styleH2))

    story.append(Paragraph("""
        This is the test suite and docoumentation
        for the ReportLab open source barcode API.
        """, styleN))

    story.append(Paragraph("""
        Several years ago Ty Sarna contributed a barcode module to the ReportLab
        community.  Several of the codes were used by him in hiw work and to the
        best of our knowledge this was correct.  These were written as flowable
        objects and were available in PDFs, but not in our graphics framework.
        However, we had no knowledge of barcodes ourselves and did not advertise
        or extend the package.
        """, styleN))

    story.append(Paragraph("""
        We "wrapped" the barcodes to be usable within our graphics framework; they
        are now available as Drawing objects which can be rendered to EPS files or
        bitmaps.  For the last 2 years this has been available in our Diagra and
        Report Markup Language products.  However, we did not charge separately
        and use was on an "as is" basis.
        """, styleN))

    story.append(Paragraph("""
        A major licensee of our technology has kindly agreed to part-fund proper
        productisation of this code on an open source basis in Q1 2006.  This has
        involved addition of EAN codes as well as a proper testing program.
        Henceforth we intend to publicise the code more widely, gather feedback,
        accept contributions of code and treat it as "supported".
        """, styleN))

    story.append(Paragraph("""
        This involved making available both downloads and testing resources.
        This PDF document is the output of the current test suite.  It contains
        codes you can scan (if you use a nice sharp laser printer!), and will be
        extended over coming weeks to include usage examples and notes on each
        barcode and how widely tested they are.  This is being done through
        documentation strings in the barcode objects themselves so should always
        be up to date.
        """, styleN))
    ...
    story.append(Paragraph("""
        Below we show a scannable code from each barcode, with and without
        human-readable text.  These are magnified about 2x from the natural size
        done by the original author to aid inspection.  This will be expanded to
        include several test cases per code, and to add explanations of checksums.
        Be aware that (a) if you enter numeric codes which are too short they may
        be prefixed for you (e.g. "123" for an 8-digit code becomes "00000123"),
        and that the scanned results and readable text will generally include
        extra checksums at the end.
        """, styleN))
...
class BarcodeWidgetTestCase(unittest.TestCase):
    ...
    def test0(self):
        ...
        for name, C in CN:
            ...
            for fmt in formats:
                ...
                self.assertTrue(os.path.isfile(efn),
                                msg="Expected file %r was not created" % efn)
        a('</body></html>')
        ...
        for klass in (QRNumber, QRAlphaNum):
            for c in klass.chars:
                if not klass.valid(c):
                    raise ValueError('%s.valid(%r) does not match' %
                                     (klass.__name__, c))
    ...
    def test_graphics_barcode(self):
        ...
        if renderPM:
            self.createSample('test_cbcim.png',
                              createBarcodeImageInMemory('EAN13',
                                                         value='123456789012'))
            self.createSample('test_cbcim.gif',
                              createBarcodeImageInMemory('EAN8',
                                                         value='1234567', format='gif'))
        self.createSample('test_cbcim.pdf',
                          createBarcodeImageInMemory('UPCA',
                                                     value='03600029145', format='pdf',
                                                     barHeight=40))
        if renderPM:
            self.createSample('test_cbcim.tiff',
                              createBarcodeImageInMemory('USPS_4State',
