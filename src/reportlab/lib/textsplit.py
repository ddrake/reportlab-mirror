...
in based on possible knowledge of the language and desirable 'niceness'
of the algorithm.
"""
...
CANNOT_START_LINE = [
    ...
    u'!\',.:;?!")]\u3001\u3002\u300d\u300f\u3011\u3015\uff3d\u3011\uff09',
    # middle priority e.g. continuation small vowels - wrapped
    # on two lines but one string...
    (u'\u3005\u2015\u3041\u3043\u3045\u3047\u3049' +
     '\u3063\u3083\u3085\u3087\u308e\u30a1\u30a3'),
    u'\u30a5\u30a7\u30a9\u30c3\u30e3\u30e5\u30e7\u30ee\u30fc\u30f5\u30f6',
...
def wordSplit(word, maxWidths, fontName, fontSize, encoding='utf8'):
    ...
    algorithms.  It should only be called after line breaking on spaces, which covers
    western languages and is highly optimised already.
    It works on the 'last unsplit word'.
...
def dumbSplit(word, widths, maxWidths):
    ...
    #(u'', u'Hello')
    # this says 'Nihongo wa muzukashii desu ne!' (Japanese is difficult isn't
    it?) in 12 characters
    >>> jtext = u'\u65e5\u672c\u8a9e\u306f\u96e3\u3057\u3044\u3067\u3059\u306d\uff01'
    ...
    while i < nW:
        ...
        if widthUsed > maxWidth + _FUZZ and widthUsed > 0:
            ...
            # lines.append([maxWidth-sum(widths[lineStartPos:i]),
            #               word[lineStartPos:i].strip()])
