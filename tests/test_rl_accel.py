...
from reportlab.lib.testutils import (setOutDir, makeSuiteForClasses, printLocation,
                                     rlSkipUnless)
from reportlab.lib.utils import asBytes, isPyPy, rawBytes
import unittest
...
# Be careful!  Flake cannot see imported functions being used like this!
def getFuncs(name):
    return tuple((_ for _ in ((_c_funcs.get(name, None), 'c'),
                              (_py_funcs.get(name, None), 'py')) if _[0]))
...
def checkrc(defns, rcv0):
    if isPyPy:
        return ''
    rcv1 = getrc(defns, 2)
    return ' '.join(["%s %s-->%s" % (x, v, w)
                     for x, v, w in zip(defns.split(), rcv0, rcv1)
                     if abs(v-w) > 1])
...
class RlAccelTestCase(unittest.TestCase):
    ...
    def testFpStr(self):
        ...
        for func, kind in getFuncs('fp_str'):
            assert func(1, 2, 3) == '1 2 3', (
                "%s fp_str(1,2,3)=='1 2 3' fails with value %s!" %
                (kind, ascii(func(1, 2, 3))))
            assert func(1) == '1', ("%s fp_str(1) == '1' fails with value %s!" %
                                    (kind, ascii(func(1))))
            assert func(595.275574) == '595.2756', (
                "%s fp_str(595.275574) == '595.2756' fails with value %s!" %
                (kind, ascii(func(595.25574))))
            assert func(59.5275574) == '59.52756', (
                "%s fp_str(59.5275574) == '59.52756' fails with value %s!" %
                (kind, ascii(func(59.525574))))
            assert func(5.95275574) == '5.952756', (
                "%s fp_str(5.95275574) == '5.952756' fails with value %s!" %
                (kind, ascii(func(5.9525574))))

    def testAsciiBase85Encode(self):
        for func, kind in getFuncs('asciiBase85Encode'):
            assert func('Dragan Andric') == '6ul^K@;[2RDIdd%@f~>', (
                ("%s asciiBase85Encode('Dragan Andric')=='6ul^K@;[2RDIdd%@f~>' " +
                 "fails with value %s!") % (kind, ascii(func('Dragan Andric'))))

    def testAsciiBase85Decode(self):
        for func, kind in getFuncs('asciiBase85Decode'):
            assert func('6ul^K@;[2RDIdd%@f~>') == b'Dragan Andric', (
                ("%s asciiBase85Decode('6ul^K@;[2RDIdd%@f~>')=='Dragan Andric' " +
                 "fails with value %s!") % (kind, ascii(func('Dragan Andric'))))
    ...
    def testAsciiBase85RoundTrip(self):
        ...
        for i in range(256):
            for j, (dfunc, kind) in enumerate(getFuncs('asciiBase85Decode')):
                ...
                assert decoded == asBytes(plain, 'latin1'), (
                    "Round-trip AsciiBase85 failed for " +
                    "%s & %s\nplain=%s\nencoded=%s\ndecoded=%s" % (
    ...
    def testCalcChecksum(self):
        for func, kind in getFuncs('calcChecksum'):
            assert func('test') == 1952805748, (
                "%s calcChecksum('test')==1952805748 fails with value %s!" % (
                    kind, ascii(func(rawBytes('test')))))

    @rlSkipUnless(_rl_accel, 'need working _rl_accel')
    def test_instanceStringWidth(self):
        from reportlab.pdfbase.pdfmetrics import (registerFont, getFont, _fonts)  # noqa
        from reportlab.pdfbase.ttfonts import TTFont
        from reportlab.lib.rl_accel import unicode2T1  # noqa
        ttfn = 'Vera'
        t1fn = 'Times-Roman'
        ...
        fontSize = 12
        defns = """ttfn t1fn ttf t1f testCp1252 enc senc ts utext fontSize ttf.face
 ttf.face.charWidths ttf.face.defaultWidth t1f.widths t1f.encName
 t1f.substitutionFonts _fonts"""
        F = []
        ...
        def tfunc(f, ts, fontSize, enc, funcs, i):
            ...
            if abs(w1-w2) >= 1e-10:
                F.append(
                    "stringWidth%s(%r,%r,%s,%r)-->%r != f._py_stringWidth(...)-->%r" %
                    (fontType, f, ts, fontSize, enc, w1, w2))
        for font, fontType in ((t1f, 'T1'), (ttf, 'TTF')):
            funcs = getFuncs('instanceStringWidth'+fontType)
            for i, kind in enumerate(('c', 'py')):
                # we run several times to allow the refcounts to stabilize
    ...
    def test_unicode2T1(self):
        from reportlab.pdfbase.pdfmetrics import getFont, _fonts  # noqa
        t1fn = 'Times-Roman'
        t1f = getFont(t1fn)
        enc = 'cp1252'
        # senc = 'utf8' (assigned but not used)
        testCp1252 = (
            b'copyright \xa9 trademark \x99 registered \xae ReportLab! Ol\xe9!'.
            decode(enc))
        utext = b'This is the end of the \xce\x91\xce\xb2 world. This is the end of the \xce\x91\xce\xb2 world jap=\xe3\x83\x9b\xe3\x83\x86. This is the end of the \xce\x91\xce\xb2 world. This is the end of the \xce\x91\xce\xb2 world jap=\xe3\x83\x9b\xe3\x83\x86'.decode('utf8')  # noqa
        ...
        defns = """t1fn t1f testCp1252 enc senc utext t1f.widths t1f.encName
 t1f.substitutionFonts _fonts"""
        F = []
        for func, kind in FUNCS:
            ...
            for j in (9, 8, 7, 6, 5, 4, 3, 2, 1, 0):
                ...
                if j == 0:
                    rcc = checkrc(defns, rcv)
                    if rcc:
                        F.append("%s refcount diffs (%s)" % (kind, rcc))
    ...
    def test_sameFrag(self):
        ...
        for func, kind in getFuncs('sameFrag'):
            if not func:
                continue
            a = ABag(fontName='Helvetica', fontSize=12, textColor="red", rise=0,
                     us_lines=0, link="aaaa", nobr=1)
            b = ABag(fontName='Helvetica', fontSize=12, textColor="red", rise=0,
                     us_lines=0, link="aaaa", nobr=1)
            for name in ("fontName", "fontSize", "textColor", "rise",
                         "us_lines", "link", "nobr"):
                old = getattr(a, name)
