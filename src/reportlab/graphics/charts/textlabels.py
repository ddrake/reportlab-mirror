...
from reportlab.lib.validators import (isNumber, isNumberOrNone, OneOf, isColorOrNone,
                                      isString, isTextAnchor, isBoxAnchor, isBoolean,
                                      NoneOr, isInstanceOf, isNoneOrString,
                                      isNoneOrCallable, isSubclassOf)
from reportlab.lib.attrmap import AttrMap, AttrMapValue
from reportlab.pdfbase.pdfmetrics import stringWidth, getAscentDescent
from reportlab.graphics.charts.utils import CustomDrawChanger
from reportlab.graphics.shapes import (Drawing, Group, Circle, Rect, String,
                                       STATE_DEFAULTS, DirectDraw)
...
_A2BA = {
        'x': {0: 'n', 45: 'ne', 90: 'e', 135: 'se', 180: 's',
              225: 'sw', 270: 'w', 315: 'nw', -45: 'nw'},
        'y': {0: 'e', 45: 'se', 90: 's', 135: 'sw', 180: 'w',
              225: 'nw', 270: 'n', 315: 'ne', -45: 'ne'},
        }
...
_BA2TA = {'w': 'start', 'nw': 'start', 'sw': 'start', 'e': 'end', 'ne': 'end',
          'se': 'end', 'n': 'middle', 's': 'middle', 'c': 'middle'}
...
class Label(Widget):
    ...
    _attrMap = AttrMap(
        ...
        angle=AttrMapValue(
            isNumber,
            desc=('angle of label: default (0), 90 is vertical, ' +
                  '180 is upside down, etc')),
        boxAnchor=AttrMapValue(
            isBoxAnchor, desc='anchoring point of the label'),
        boxStrokeColor=AttrMapValue(
            isColorOrNone, desc='border color of the box'),
        boxStrokeWidth=AttrMapValue(
            isNumber, desc='border width'),
        boxFillColor=AttrMapValue(
            isColorOrNone, desc='the filling color of the box'),
        boxTarget=AttrMapValue(
            OneOf('normal', 'anti', 'lo', 'hi'),
            desc="one of ('normal','anti','lo','hi')"),
        fillColor=AttrMapValue(
            isColorOrNone, desc='label text color'),
        strokeColor=AttrMapValue(
            isColorOrNone, desc='label text border color'),
        strokeWidth=AttrMapValue(
            isNumber, desc='label text border width'),
        text=AttrMapValue(
            isString, desc='the actual text to display'),
        fontName=AttrMapValue(
            isString, desc='the name of the font used'),
        fontSize=AttrMapValue(
            isNumber, desc='the size of the font'),
        leading=AttrMapValue(
            isNumberOrNone, desc=''),
        width=AttrMapValue(
            isNumberOrNone, desc='the width of the label'),
        maxWidth=AttrMapValue(
            isNumberOrNone, desc='maximum width the label can grow to'),
        height=AttrMapValue(
            isNumberOrNone, desc='the height of the text'),
        ...
        topPadding=AttrMapValue(
            isNumber, desc='padding at top of box'),
        leftPadding=AttrMapValue(
            isNumber, desc='padding at left of box'),
        rightPadding=AttrMapValue(
            isNumber, desc='padding at right of box'),
        bottomPadding=AttrMapValue(
            isNumber, desc='padding at bottom of box'),
        useAscentDescent=AttrMapValue(
            isBoolean,
            desc=("If True then the font's Ascent & Descent will be used " +
                  "to compute default heights and baseline.")),
        customDrawChanger=AttrMapValue(
            isNoneOrCallable,
            desc=("An instance of CustomDrawChanger " +
                  "to modify the behavior at draw time"), _advancedUsage=1),
        ddf=AttrMapValue(
            NoneOr(isSubclassOf(DirectDraw), 'NoneOrDirectDraw'),
            desc="A DirectDrawFlowable instance", _advancedUsage=1),
    ...
    def computeSize(self):
        ...
        if not ddfKlass:
            ...
            if not self.width:
                ...
                if self._lines:
                    self._lineWidths = [stringWidth(line, self.fontName, self.fontSize)
            ...
            if self.ddf is None:
                raise RuntimeError(
                    ('DirectDrawFlowable class is not available you need ' +
                     'the rlextra package as well as reportlab'))
            sty = dict(
    ...
    def _rawDraw(self):
        ...
        if ddfKlass:
            ...
            strokeColor, strokeWidth, (
                leading) = self.strokeColor, self.strokeWidth, self._leading
...
class PMVLabel(Label):
    ...
    def _getBoxAnchor(self):
        ...
        if self._pmv < 0:
            a = {'nw': 'se', 'n': 's', 'ne': 'sw', 'w': 'e',
                 'c': 'c', 'e': 'w', 'sw': 'ne', 's': 'n', 'se': 'nw'}[a]
        return a
...
class BarChartLabel(PMVLabel):
    ...
    _attrMap = AttrMap(
        ...
        nudge=AttrMapValue(
            isNumber, desc="Non-zero sign dependent nudge"),
        boxTarget=AttrMapValue(
...
class NA_Label(BarChartLabel):
    ...
    _attrMap = AttrMap(
        BASE=BarChartLabel,
        text=AttrMapValue(
            isNoneOrString, desc="Text to be used for N/A values"),
        )
...
class XLabel(Label):
    ...
    if False:
        def __init__(self, *args, **kwds):  # noqa
