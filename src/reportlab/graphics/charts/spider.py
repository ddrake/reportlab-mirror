...
from reportlab.lib.validators import (isNumber, isListOfNumbersOrNone,
                                      ...
                                      isBoolean, isNumberOrNone, isStringOrNone,
                                      EitherOr, isCallable)
from reportlab.lib.attrmap import AttrMap, AttrMapValue
from reportlab.graphics.shapes import (Group, Drawing, Line, Rect, Polygon,
                                       PolyLine, STATE_DEFAULTS)
...
class StrandProperty(PropHolder):
    ...
    _attrMap = AttrMap(
        ...
        symbol=AttrMapValue(EitherOr((isStringOrNone, isSymbol)),
                            desc='Widget placed at data points.', advancedUsage=1),
...
class StrandLabel(SpokeLabel):
    _attrMap = AttrMap(
        BASE=SpokeLabel,
        format=AttrMapValue(EitherOr((isStringOrNone, isCallable)),
                            desc="Format for the label"),
...
class SpiderChart(PlotArea):
    _attrMap = AttrMap(
        ...
        strands=AttrMapValue(
            None, desc="collection of strand descriptor objects"),
        spokes=AttrMapValue(
            None, desc="collection of spoke descriptor objects"),
    ...
    def makeSwatchSample(self, rowNo, x, y, width, height):
        ...
        if fillColor is None and strokeColor is not None and strokeWidth > 0:
            bg = Line(x, ym, x+width, ym, strokeWidth=strokeWidth,
                      strokeColor=strokeColor, strokeDashArray=strokeDashArray)
        elif fillColor is not None:
            bg = Rect(x, y, width, height, strokeWidth=strokeWidth,
                      strokeColor=strokeColor, strokeDashArray=strokeDashArray,
                      fillColor=fillColor)
        else:
            bg = None
        if symbol:
            # bug? undefined name color
            symbol = uSymbol2Symbol(symbol, x+width/2., ym, color)  # noqa
            if bg:
                g = Group()
                g.add(bg)
                g.add(symbol)
    ...
    def normalizeData(self, outer=0.0):
        ...
        data = self.data
        assert min(list(map(min, data))) >= 0, (
    ...
    def _innerDrawLabel(self, sty, radius, cx, cy, angle, car, sar,
    ...
    def draw(self):
        ...
        # spokes = self.spokes  (assigned but not used)
        ...
        for i in range(n):
            ...
            si = self.spokes[i]
            if si.visible:
                spoke = Line(cx, cy, cx + car, cy + sar,
            ...
            if not text:
                text = labels[i]
            if text:
                S.append(_setupLabel(WedgeLabel, text, si.labelRadius, cx, cy, angle,
                                     car, sar, sli))
            angle += angleBetween
        ...
        for row in data:
            ...
            for i in range(n):
                ...
                points.append(cx+car*r)
                points.append(cy+sar*r)
                L = self._innerDrawLabel(strandLabels[(rowIdx, i)], r, cx, cy, angle,
                                         car, sar, labelClass=StrandLabel)
                if L:
                    labs.append(L)
