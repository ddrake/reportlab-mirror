...
import unittest
from reportlab.platypus.flowables import KeepTogether, KeepTogetherSplitAtTop
...
from reportlab.platypus.doctemplate import (PageTemplate, BaseDocTemplate, Indenter,
                                            SimpleDocTemplate, LayoutError)
...
class MyDocTemplate(BaseDocTemplate):
    ...
    def __init__(self, filename, **kw):
        ...
        template = PageTemplate('normal',
                                [frame1, frame2, frame3, frame4], myMainPageFrame)
...
def _test0(self):
    ...
    def makeStory():
        ...
        for i in range(10):
            ...
            for j in range(3):
                a(Paragraph('Heading1 paragraphs should always'
                            ...
                            len(story), bt))
                a(Paragraph('Heading 2 always starts a new frame (%d)' %
                            len(story), h2))
                a(Paragraph('Heading1 paragraphs should always'
                            'have a page break before.  Heading 2 on the other hand'
                            'should always have a FRAME break before (%d)' %
                            len(story), bt))
                for j in range(3):
                    a(Paragraph(RT((i, j, 0), theme=PYTHON, sentences=2)+' (%d)' %
                                len(story), bt))
                    a(Paragraph('I should never be at the bottom of a frame (%d)' %
                                len(story), h3))
                    a(Paragraph(RT((i, j, 1), theme=PYTHON, sentences=1)+' (%d)' %
                                len(story), bt))
        ...
        for align, bts in [('left', bt), ('JUSTIFIED', btj), ('RIGHT', btr),
            ...
            a(Paragraph(_text1.replace('forms of the', 'forms of the<br/>', 1),
                        bts))
            a(Paragraph("2*&lt;br/&gt; after 'the' in line 4", h3))
            a(Paragraph(_text1.replace('forms of the', 'forms of the<br/><br/>', 1),
                        bts))
        ...
        for i in 0, 1, 2:
            P = Paragraph("""This is a paragraph with
<font color='blue'><a href='#top'>with an incredibly
...
I hope we don't, but you never do Know.</a></font>""", bt)
    ...
    for sfx, klass in (('', KeepTogether), ('_ktsat', KeepTogetherSplitAtTop)):
        doc = MyDocTemplate(
            outputfile('test_platypus_breaking%s.pdf' % sfx),
            keepTogetherClass=klass,
            displayDocTitle=(sfx == ''),
            duplex='Simplex',)
...
class RenderMeasuringPara:
    def __init__(self, canv, style, aW, measuring=True, annotations=[],
                 errors=[], length_errors=[], onDrawFuncName='_onDrawFunc'):
        ends = []
        ...
        self._state = (canv, style, aW, measuring, ends, annotations, errors,
                       length_errors)
        if measuring:
            self._end = ('<ondraw name="%s" label="end"/>' % onDrawFuncName)

    def __call__(self, x, y, text, wc, ns, n, hrep=' ', crep=' ', hdw=0, cdw=0):
        (canv, style, aW, measuring, ends, annotations,
         errors, length_errors) = self._state
        if measuring:
            text += self._end
        if '{H}' in text:
...
class BreakingTestCase(unittest.TestCase):
    ...
    def test3(self):
        ...
        bt = styleSheet['BodyText']
        btj = ParagraphStyle('bodyText1j', parent=bt, alignment=TA_JUSTIFY)
        p = Paragraph("""<a name='top'/>Subsequent pages test pageBreakBefore,
                frameBreakBefore and keepTogether attributes.  Generated at 1111.
                The number in brackets at the end of each paragraph is its position
                in the story. llllllllllllllllllllllllll bbbbbbbbbbbbbbbbbbbbbb
                ccccccccccccccccccccccc ddddddddddddddddddddd eeeeyyy""", btj)
        ...
        good = ['q', '1 0 0 1 0 0 cm', 'q',
                'BT 1 0 0 1 0 '+x+' Tm 3.59 Tw 12 TL /F1 10 Tf 0 0 0 rg (Subsequent ' +
                'pages test pageBreakBefore, frameBreakBefore and) Tj T* 0 Tw .23 ' +
                'Tw (keepTogether attributes. Generated at 1111. The number in ' +
                'brackets) Tj T* 0 Tw .299167 Tw (at the end of each paragraph is ' +
                'its position in the story. llllllllllllllllllllllllll) Tj T* 0 Tw ' +
                '66.9 Tw (bbbbbbbbbbbbbbbbbbbbbb ccccccccccccccccccccccc) Tj T* 0 Tw ' +
                '(ddddddddddddddddddddd eeeeyyy) Tj T* ET', 'Q', 'Q']
        ok = ParaCode == good
    ...
    def test4(self):
        ...
        def swc(t):
            return stringWidth(t, 'Courier', bfs)

        def swcbo(t):
            return stringWidth(t, 'Courier-BoldOblique', bfs)

        def swh(t):
            return stringWidth(t, 'Helvetica', bfs)

        def swhbo(t):
            return stringWidth(t, 'Helvetica-BoldOblique', bfs)

        def swt(t):
            return stringWidth(t, 'Times-Roman', bfs)

        def swtb(t):
            return stringWidth(t, 'Times-Bold', bfs)
        ...
        data = (
            ...
            (1, "<span fontName='Courier'>Hello</span>{H}World.",
             swc('Hello')+swh(' World.')),
            (2, " <a name='a2'/><span fontName='Courier'>Hello{C}</span> World.",
             swc('Hello ')+swh('World.')),
            (3, " <a name='a3'/><span fontName='Courier'>Hello</span>{H}World.",
             swc('Hello')+swh(' World.')),
            (4, "<span fontName='Courier'><a name='a4'/>Hello{C}</span> World.",
             swc('Hello ')+swh('World.')),
            (5, "<span fontName='Courier'><a name='a5'/>Hello</span>{H}World.",
             swc('Hello')+swh(' World.')),
            (6, "<span fontName='Courier'>Hello<a name='a6'/>{C}</span> World.",
             swc('Hello ')+swh('World.')),
            (7, "<span fontName='Courier'>Hello<a name='a7'/></span>{H}World.",
             swc('Hello')+swh(' World.')),
            (8, "<span fontName='Courier'>Hello{C}<a name='a8'/></span> World.",
             swc('Hello ')+swh('World.')),
            (9, "<span fontName='Courier'>Hello</span><a name='a9'/>{H}World.",
             swc('Hello')+swh(' World.')),
            (10, "<span fontName='Courier'>Hello{C}</span> <a name='a10'/>World.",
             swc('Hello ')+swh('World.')),
            (11, "<span fontName='Courier'>Hello</span>{H}<a name='a11'/>World.",
             swc('Hello')+swh(' World.')),
            (12, "<span fontName='Courier'>Hello{C}</span> World. <a name='a12'/>",
             swc('Hello ')+swh('World.')),
            (13, "<span fontName='Courier'>Hello</span>{H}World. <a name='a13'/>",
             swc('Hello')+swh(' World.')),
            (14, " <a name='a2'/> <span fontName='Courier'>Hello{C}</span> World.",
             swc('Hello ')+swh('World.')),
            (15, " <a name='a3'/> <span fontName='Courier'>Hello</span>{H}World.",
             swc('Hello')+swh(' World.')),
            (16, " <a name='a2'/> <span fontName='Courier'>Hello{C}<a name='b'/> " +
             "</span> <a name='b'/> World.", swc('Hello ')+swh('World.')),
            (17, " <a name='a3'/> <span fontName='Courier'>Hello</span>{H}<a " +
             "name='b'/> World.", swc('Hello')+swh(' World.')),
            (30, ("<span fontName='Courier'>He<span face='Times-Roman' " +
                  "color='red'>l</span><span face='Times-Bold' color='orange'>lo" +
                  "</span>{C}</span> World."),
             swt('l')+swtb('lo')+swc('He ')+swh('World.')),
            (31, "<span fontName='Courier'>He<span face='Times-Roman' color=" +
             "'red'>l</span><span face='Times-Bold' color='orange'>lo</span>" +
             "</span>{H}World.", swt('l')+swtb('lo')+swc('He')+swh(' World.')),
            (32, " <a name='a2'/><span fontName='Courier'>He<span face='Times-" +
             "Roman' color='red'>l</span><span face='Times-Bold' color='orange'>" +
             "lo</span>{C}</span> World.", swt('l')+swtb('lo')+swc('He ') +
             swh('World.')),
            (33, " <a name='a3'/><span fontName='Courier'>He<span face='Times-Roman'" +
             " color='red'>l</span><span face='Times-Bold' color='orange'>lo</span>" +
             "</span>{H}World.", swt('l')+swtb('lo')+swc('He')+swh(' World.')),
            (34, "<span fontName='Courier'><a name='a4'/>He<span face='Times-Roman' " +
             "color='red'>l</span><span face='Times-Bold' color='orange'>lo</span> " +
             "</span> World.", swt('l')+swtb('lo')+swc('He ')+swh('World.')),
            (35, "<span fontName='Courier'><a name='a5'/>He<span face='Times-Roman' " +
             "color='red'>l</span><span face='Times-Bold' color='orange'>lo</span>" +
             "</span>{H}World.", swt('l')+swtb('lo')+swc('He')+swh(' World.')),
            (36, ("<span fontName='Courier'>He<span face='Times-Roman' color='red'>" +
                  "l</span><span face='Times-Bold' color='orange'>lo</span>" +
                  "<a name='a6'/>" +
                  " </span> World."), swt('l')+swtb('lo')+swc('He ')+swh('World.')),
            (37, "<span fontName='Courier'>He<span face='Times-Roman' color='red'>" +
             "l</span><span face='Times-Bold' color='orange'>lo</span><a name='a7'/>" +
             "</span>{H}World.", swt('l')+swtb('lo')+swc('He')+swh(' World.')),
            (38, "<span fontName='Courier'>He<span face='Times-Roman' color='red'>" +
             "l</span><span face='Times-Bold' color='orange'>lo</span>{C}<a " +
             "name='a8'/></span> World.", swt('l')+swtb('lo')+swc('He ')+swh('World.')),
            (39, "<span fontName='Courier'>He<span face='Times-Roman' color='red'>" +
             "l</span><span face='Times-Bold' color='orange'>lo</span></span><a " +
             "name='a9'/>{H}World.", swt('l')+swtb('lo')+swc('He')+swh(' World.')),
            (40, "<span fontName='Courier'>He<span face='Times-Roman' color='red'>" +
             "l</span><span face='Times-Bold' color='orange'>lo</span>{C}</span> <a " +
             "name='a10'/>World.", swt('l')+swtb('lo')+swc('He ')+swh('World.')),
            (41, "<span fontName='Courier'>He<span face='Times-Roman' color='red'>" +
             "l</span><span face='Times-Bold' color='orange'>lo</span></span>{H}<a " +
             "name='a11'/>World.", swt('l')+swtb('lo')+swc('He')+swh(' World.')),
            (42, "<span fontName='Courier'>He<span face='Times-Roman' color='red'>" +
             "l</span><span face='Times-Bold' color='orange'>lo</span>{C}</span>" +
             "World. <a name='a12'/>", swt('l')+swtb('lo')+swc('He ')+swh('World.')),
            (43, "<span fontName='Courier'>He<span face='Times-Roman' color='red'>" +
             "l</span><span face='Times-Bold' color='orange'>lo</span></span> World."
             "{H}<a name='a13'/>", swt('l')+swtb('lo')+swc('He')+swh(' World.')),
            (44, " <a name='a2'/> <span fontName='Courier'>He<span face='Times-Roman'" +
             " color='red'>l</span><span face='Times-Bold' color='orange'>lo" +
             "</span>{C}</span> World.", swt('l')+swtb('lo')+swc('He ')+swh('World.')),
            (45, " <a name='a3'/> <span fontName='Courier'>He<span face='Times-Roman'" +
             "color='red'>l</span><span face='Times-Bold' color='orange'>lo</span>" +
             "</span>{H}World.", swt('l')+swtb('lo')+swc('He')+swh(' World.')),
            (46, " <a name='a2'/> <span fontName='Courier'>He<span face='Times-Roman'" +
             "color='red'>l</span><span face='Times-Bold' color='orange'>lo" +
             "</span>{C}<a name='b'/> </span> <a name='b'/> World.",
             swt('l')+swtb('lo')+swc('He ')+swh('World.')),
            (47, " <a name='a3'/> <span fontName='Courier'>He<span face='Times-" +
             "Roman' color='red'>l</span><span face='Times-Bold' color='orange'>" +
             "lo</span></span>{H}<a name='b'/> World.",
             swt('l')+swtb('lo')+swc('He')+swh(' World.')),
            )
        _exceptions = {
                1:  {
        ...
        doPara = RenderMeasuringPara(canv, bt, aW, measuring, annotations, errors,
                                     length_errors, onDrawFuncName)

        rep0 = '<ondraw name="%s"/>\\1' % onDrawFuncName
        for n, text, wc in data:
            if argv and str(n) not in argv:
                continue
            text0 = (apat.sub(rep0, text) if rep0 else text)
            ns = str(n)
            h = doPara(x, y, text0, wc, ns, n)
            if '<a' in text:
                text1 = apat.sub('<img width="10" height="5" src="pythonpowered.gif"/>',
                                 text0)
                doPara(x1, y, text1, wc+10+gex(1, n), ns+'.11', None)
                # text2 assigned but not used
                # text2 = apat.sub(
                ...
                text3 = apat.sub(
                    '\\1<img width="10" height="5" src="pythonpowered.gif"/>' +
                    '<img width="10" height="5" src="pythonpowered.gif"/>\\1',
                    text0)
    ...
    def test5(self):
        '''extreme test inspired by Moritz Pfeiffer
        https://bitbucket.org/moritzpfeiffer/'''
        with self.assertRaises(LayoutError):
            ...
            descriptive <span color="red">fact</span>.<br/>So far, the earlier
            discussion of deviance is not
            ...
            doc = BaseDocTemplate(
                ...
                pageTemplates=[PageTemplate(
                    'page_template',
                    [Frame(0, 0, 0, 0, leftPadding=0, rightPadding=0, topPadding=0,
                           bottomPadding=0, id='DUMMY_FRAME')],
                    )],
                )
            doc.build(story)
