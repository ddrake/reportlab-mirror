...
class _PDFRenderer(Renderer):
    ...
    def drawWedge(self, wedge):
        if wedge.annular:
            ...
            (centerx, centery, radius, startangledegrees,
             endangledegrees) = (wedge.centerx, wedge.centery, wedge.radius,
                                 wedge.startangledegrees, wedge.endangledegrees)
            ...
            if ((radius1 == 0 or radius1 is None) and
                ...
                path.arc(centerx-radius, centery-yradius, centerx+radius,
                         centery+yradius, startangledegrees, angle)
                path.arcTo(centerx-radius1, centery-yradius1, centerx+radius1,
                           centery+yradius1, endangledegrees, -angle)
            path.close()
    ...
    def drawString(self, stringObj):
        ...
        if (self._fill and needFill) or (self._stroke and needStroke):
            ...
            text_anchor, x, y, text, enc = (S['textAnchor'], stringObj.x, stringObj.y,
                                            stringObj.text, stringObj.encoding)
    ...
    def applyStateChanges(self, delta, newState):
        ...
        for key, value in (sorted(delta.items())
                           if rl_config.invariant else delta.items()):
            if key == 'transform':
