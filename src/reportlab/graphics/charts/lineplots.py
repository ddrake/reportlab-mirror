...
from reportlab.lib.validators import (isNumber, isColorOrNone, isListOfNumbersOrNone,
...
from reportlab.graphics.charts.axes import (XValueAxis, YValueAxis, AdjYValueAxis,
                                            NormalDateXValueAxis)
from reportlab.graphics.charts.linecharts import AbstractLineChart
from reportlab.graphics.charts.utils import mkTimeTuple, mktime
...
class Shader(_SetKeyWordArgs):
    _attrMap = AttrMap(
        ...
        colors=AttrMapValue(SequenceOf(
            isColorOrNone, lo=2, hi=2), desc='(AxisColor, LineColor)'),
        )
...
class LinePlot(AbstractLineChart):
    ...
    _attrMap = AttrMap(
        ...
        lineLabelNudge=AttrMapValue(
            isNumber,
            desc='Distance between a data point and its label.', advancedUsage=1),
        ...
        lineLabelArray=AttrMapValue(
            None,
            desc=('explicit array of line label values, ' +
        ...
        lines=AttrMapValue(
            None, desc='Handle of the lines.'),
        xValueAxis=AttrMapValue(
            None, desc='Handle of the x axis.'),
        yValueAxis=AttrMapValue(
            None, desc='Handle of the y axis.'),
        data=AttrMapValue(
    ...
    def makeLines(self):
        ...
        # labelFmt = self.lineLabelFormat  (assigned but not used)
        ...
        for rowNo in R:
            ...
            if self.joinedLines:
                ...
                if inFill or isinstance(row, FillPairedData):
                    ...
                    if filler:
                        ...
                        inFillG.add(
                            Polygon(fpoints, fillColor=fillColor,
                                    strokeColor=(strokeColor if strokeColor == fillColor
                                                 else None),
                                    strokeWidth=width or 0.1))
                # bug? undefined name inFillb
                if not inFill or inFillb == 2 or strokeColor != fillColor:  # noqa
                    line = PolyLine(points, strokeColor=strokeColor, strokeLineCap=0,
                                    strokeLineJoin=1)
                    if width:
            ...
            if uSymbol:
                ...
                for j, xy in enumerate(row):
                    ...
                    symbol = uSymbol2Symbol(tpcGetItem(symbol, j), xy[0], xy[1],
                                            symColor)
                    if symbol:
                        if bubblePlot:
                ...
                for j, xy in enumerate(row):
                    ...
                    if not juSymbol:
                        continue
                    symColor = getattr(
                        juSymbol, 'fillColor', getattr(juSymbol, 'strokeColor',
                                                       strokeColor))
    ...
    def addCrossHair(self, name, xv, yv, strokeColor=colors.black,
                     strokeWidth=1, beforeLines=True):
        from reportlab.graphics.shapes import Group, Line
        annotations = [a for a in getattr(self, 'annotations', [])
        ...
        def annotation(self, xScale, yScale):
            ...
            g.add(Line(xA._x, y, xA._x+xA._length, y, strokeColor=strokeColor,
                       strokeWidth=strokeWidth))
            yA = yScale.__self__   # the y axis
            g.add(Line(x, yA._y, x, yA._y+yA._length, strokeColor=strokeColor,
                       strokeWidth=strokeWidth))
            return g
    ...
    def makeLines(self):
        ...
        if self.xValueAxis.style != 'parallel_3d':
            ...
            if getattr(self, '_find_intersections', None):
                ...
                I = find_intersections(fpositions, small=tileWidth)  # noqa
                ic = None
                for i, j, x, y in I:
                    if ic != i:
                        ic = i
        ...
        for rowNo in P:
            ...
            # dash = getattr(rowStyle, 'strokeDashArray', None)  (assigned but not used)
            z0 = self._calc_z0(rowNo)
            z1 = z0 + zDepth

            # width assigned but not used
            # if hasattr(rowStyle, 'strokeWidth'):
            #     width = rowStyle.strokeWidth
...
class GridLinePlot(SimpleTimeSeriesPlot):
    ...
    def draw(self):
        ...
        # width, height = self.width, self.height (set but not used)
        scaleFactor = self.scaleFactor
        if scaleFactor and scaleFactor != 1:
            # g = Drawing(scaleFactor*width, scaleFactor*height)
            # bug? undefined name g (why commented out in line above?)
            g.transform = (scaleFactor, 0, 0, scaleFactor, 0, 0)  # noqa
        else:
            g = Group()
...
def _maxWidth(T, fontName, fontSize):
    ...
    return T and max(list(map(lambda t, sW=stringWidth, fN=fontName,
                              fS=fontSize: sW(t, fN, fS), T))) or 0
...
class ScatterPlot(LinePlot):
    ...
    _attrMap = AttrMap(
        BASE=LinePlot,
        width=AttrMapValue(
            isNumber, desc="Width of the area inside the axes"),
        height=AttrMapValue(
            isNumber, desc="Height of the area inside the axes"),
        ...
        labelOffset=AttrMapValue(
            isNumber, desc="Space between label and Axis (or other labels)",
            advancedUsage=1),
        axisTickLengths=AttrMapValue(
            isNumber, desc="Lenth of the ticks on both axes"),
        axisStrokeWidth=AttrMapValue(
            isNumber, desc="Stroke width for both axes"),
        xLabel=AttrMapValue(
            isString, desc="Label for the whole X-Axis"),
        yLabel=AttrMapValue(
            isString, desc="Label for the whole Y-Axis"),
        data=AttrMapValue(
            isAnything, desc='Data points - a list of x/y tuples.'),
        strokeColor=AttrMapValue(
    ...
    def __init__(self):
        ...
        self.y = (self.bottomPadding+_axisTickLengths+_labelOffset +
    ...
    def _getDrawingDimensions(self):
        ...
        tx += (5*_maxWidth(str(self.yValueAxis.valueMax),
                           self.yValueAxis.labels.fontName,
                           self.yValueAxis.labels.fontSize))
        tx += self.width+self.rightPadding
        t = ('%.2f%%' % self.xValueAxis.valueMax)
        tx += (_maxWidth(t, self.yValueAxis.labels.fontName,
                         self.yValueAxis.labels.fontSize))
        ty = (self.bottomPadding+self.xValueAxis.tickDown +
              (self.xValueAxis.labels.dy*2)+(self.xValueAxis.labels.fontSize*2))
        ty += self.yValueAxis.labels.fontSize+self.height+self.topPadding
        # print (tx, ty)
        return (tx, ty)
...
def sampleFillPairedData():
    ...
    chart.lines[0].filler = Filler(fillColor=colors.toColor('#9f9f9f'),
