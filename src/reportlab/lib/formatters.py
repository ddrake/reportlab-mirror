...
class DecimalFormatter(Formatter):
    ...
    def __init__(self, places=2, decimalSep='.', thousandSep=None,
    ...
    def _calcPlaces(self, V):
        '''called with the full set of values to be formatted
        so we can calculate places'''
...
if __name__ == '__main__':
    def t(n, s, places=2, decimalSep='.', thousandSep=None, prefix=None, suffix=None):
        ...
        print("places=%2d dot=%-4s comma=%-4s prefix=%-4s suffix=%-4s result=%10s %s" %
              (f.places, f.dot, f.comma, f.prefix, f.suffix,
               r, r == s and 'OK' or 'BAD'))
