...
from reportlab.pdfbase.pdfmetrics import getFont, stringWidth, unicode2T1  # font info
...
from reportlab.graphics.shapes import (STATE_DEFAULTS, numericXShift, Path)
...
''' % (self.width, self.height))
        ...
        if self._color != color:
            ...
            if color:
                if hasattr(color, "cyan"):
                    ...
                    self.code_append('%s setrgbcolor' % fp_str(
    ...
    def _textOut(self, x, y, s, textRenderMode=0):
        ...
        fill = (textRenderMode == 0 or textRenderMode == 2 or textRenderMode == 4 or
                textRenderMode == 6)
        stroke = (textRenderMode == 1 or textRenderMode == 2 or textRenderMode == 5 or
                  textRenderMode == 6)
        # addToClip = textRenderMode >= 4  (set but not used)
        if fill and stroke:
    ...
    def _issueT1String(self, fontObj, x, y, s, textRenderMode=0):
        ...
        # escape = self._escape (set but not used)
        if not isUnicode(s):
            try:
                s = s.decode('utf8')
    ...
    def drawRightString(self, text, x, y, text_anchor='end', textRenderMode=0):
        self.drawString(text, x, y, text_anchor=text_anchor,
                        textRenderMode=textRenderMode)
    ...
    def rect(self, x1, y1, x2, y2, stroke=1, fill=1):
        ...
        self.polygon(((x1, y1), (x2, y1), (x2, y2), (x1, y2)),
                     closed=1, stroke=stroke, fill=fill)
    ...
    def roundRect(self, x1, y1, x2, y2, rx=8, ry=8):
        ...
        ellipsePath = ('matrix currentmatrix %s %s translate %s %s ' +
    ...
    def drawArc(self, x1, y1, x2, y2, startAng=0, extent=360, fromcenter=0):
        ...
        # extentRadians = math.pi*extent/180.0 (assigned but not used)
        # endAngleRadians = startAngleRadians + extentRadians (assigned but not used)
        ...
        if self._strokeColor is not None:
            ...
            self.setColor(self._strokeColor)
            (startx, starty) = (cx+rx*math.cos(startAngleRadians),
    ...
    def _genArcCode(self, x1, y1, x2, y2, startAng, extent):
        """Calculate the path for an arc inscribed in rectangle
    ...
    def _drawImageLevel1(self, image, x1, y1, width=None, height=None):
        ...
        if not height:
            height = imgheight
        # print 'Image size (%d, %d); Draw size (%d, %d)' %
        ...
        # piddlePDF again
        rawimage = (myimage.tobytes if hasattr(myimage, 'tobytes') else
                    myimage.tostring)()
...
/BitsPerComponent %d""" % (imwidth, imheight, imBitsPerComponent))
        ...
        self.code.extend(['/ImageMatrix [%s 0 0 %s 0 %s]' % (imwidth, -imheight,
                                                             imheight),
...
#  _restores = {'stroke':'_stroke','stroke_width': '_lineWidth',
...
#              'font_size':'_fontSize'}
    ...
    def drawWedge(self, wedge):
        ...
        if ((radius1 == 0 or radius1 is None) and
                (yradius1 == 0 or yradius1 is None) and
                not wedge.annular):
            ...
            self._canvas.drawArc(centerx-radius, centery-yradius, centerx+radius,
                                 centery+yradius, startangledegrees, extent,
                                 fromcenter=1)
        else:
            P = wedge.asPolygon()
    ...
    def drawPath(self, path, fillMode=None):
        ...
        def fas(**kwds):
            c._fillAndStroke([], fillMode=fillMode, **kwds)
        # pathFill = lambda : c._fillAndStroke([], stroke=0, fillMode=fillMode) (unused)
        # pathStroke = lambda : c._fillAndStroke([], fill=0)  (set but not used)
        if autoclose == 'svg':
            rP()
...
def test(outDir='epsout', shout=False):
    ...
    testshapes._FONTS[:] = ['Times-Roman', 'Times-Bold', 'Times-Italic',
