...
from reportlab.graphics.shapes import (Drawing, Polygon, String, Rect, Line, Circle,
                                       Wedge, PolyLine, Ellipse, Group, translate,
                                       rotate, mmult)
...
def getDrawing07():
    """This tests the ability to translate and rotate groups.  The first set of axes
    should be near the bottom left of the drawing.  The second should be rotated
    counterclockwise by 15 degrees.  The third should be rotated by 30 degrees."""
    D = Drawing(400, 200)
...
def getDrawing08():
    """This tests the ability to scale coordinates. The bottom left set of axes should
    be near the bottom left of the drawing.  The bottom right should be stretched
    vertically by a factor of 2.  The top left one should be stretched horizontally by
    a factor of 2.  The top right should have the vertical axiss leaning over to the
    right by 30 degrees."""
    D = Drawing(400, 200)
...
def getDrawing13():
    ...
    def drawit(F, w=400, h=200, fontSize=12, slack=2, gap=5):
        ...
        for fontName in F:
            y -= th
            text = fontName+asNative(b': I should be totally horizontal and enclosed in a box and end in alphabetagamma \xc2\xa2\xc2\xa9\xc2\xae\xc2\xa3\xca\xa5\xd0\x96\xd6\x83\xd7\x90\xd9\x82\xe0\xa6\x95\xce\xb1\xce\xb2\xce\xb3')  # noqa
            textWidth = stringWidth(text, fontName, fontSize)
            maxx = max(maxx, textWidth+20)
            D.add(
                Group(Rect(8, y-gh, textWidth + 4, th, strokeColor=colors.red,
                           strokeWidth=.5, fillColor=colors.lightgrey),
...
def _evalFuncDrawing(name, D, l=None, g=None):  # noqa
    if g is None:
        g = globals()
    if l is None:
        l = locals()  # noqa
