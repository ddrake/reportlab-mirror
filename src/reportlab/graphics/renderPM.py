...
# history www.reportlab.co.uk/rl-cgi/viewcvs.cgi/rlextra/graphics/Csrc/
#                            renderPM/renderP.py
__version__ = '3.3.0'
    ...
    if backend == '_renderPM':
        try:
            ...
            try:
                ...
                raise RenderPMError(f"""Cannot import desired renderPM backend,
{backend}.  No module named _rl_renderPM
...
https://pairlist2.pair.net/mailman/listinfo/reportlab-users""")
        ...
        try:
            ...
            try:
                ...
                raise RenderPMError(f"""cannot import desired renderPM backend
{backend} Seek advice at the users list see
https://pairlist2.pair.net/mailman/listinfo/reportlab-users""")
    else:
...
def Color2Hex(c):
    ...
    if c:
        return (((0xFF & int(255*c.red)) << 16) | ((0xFF & int(255*c.green)) << 8) |
                (0xFF & int(255*c.blue)))
    return c
...
class _PMRenderer(Renderer):
    ...
    def drawImage(self, image):
        ...
        self._canvas._aapixbuf(
            ...
            # rlPyCairo has a from_pil method
            (im if self._canvas._backend == 'rlPyCairo' else
             (im.tobytes if hasattr(im, 'tobytes') else
              im.tostring)()),
            srcW, srcH, 3,
            )
...
class PMCanvas:
    def __init__(self, w, h, dpi=72, bg=0xffffff, configPIL=None, backend=None,
        ...
        # the _rl_renderPM.gstate object doesn't support hasattr so we use this
        # as a proxy test for 'isbuiltin'
        self.__dict__['_backend'] = ('_renderPM'
                                     if type(self._gs._aapixbuf) == type(pow) else
                                     'rlPyCairo')
    ...
    def _drawTimeResize(self, w, h, bg=None):
        ...
        A = {'ctm': None, 'strokeWidth': None, 'strokeColor': None, 'lineCap': None,
             'lineJoin': None, 'dashArray': None, 'fillColor': None}
        gs = self._gs
    ...
    def _saveToBMP(self, f):
        '''
        Niki Spahiev, <niki@vintech.bg>, asserts that this is a respectable way to get
        BMP without PIL.  f is a file like object to which the BMP is written
        '''
        import struct
        gs = self._gs
        pix, width, height = gs.pixBuf, gs.width, gs.height
        f.write(struct.pack('=2sLLLLLLhh24x', 'BM',
                            len(pix)+54, 0, 54, 40, width, height, 1, 24))
        rowb = width * 3
    ...
    def circle(self, cx, cy, r):
        """add closed path circle with center cx,cy and
        axes r: counter-clockwise orientation"""
        self.ellipse(cx, cy, r, r)

    def ellipse(self, cx, cy, rx, ry):
        """add closed path ellipse with center cx,cy and
        axes rx,ry: counter-clockwise orientation
...
def drawToPMCanvas(d, dpi=72, bg=0xffffff, configPIL=None,
                   showBoundary=rl_config._unset_, backend=rl_config.renderPMBackend):
...
def drawToPIL(d, dpi=72, bg=0xffffff, configPIL=None, showBoundary=rl_config._unset_,
              backend=rl_config.renderPMBackend):
    return drawToPMCanvas(d, dpi=dpi, bg=bg, configPIL=configPIL,
                          showBoundary=showBoundary, backend=backend).toPIL()
...
def drawToString(d, fmt='GIF', dpi=72, bg=0xffffff, configPIL=None,
                 showBoundary=rl_config._unset_, backend=rl_config.renderPMBackend):
    s = BytesIO()
...
def test(outDir='pmout', shout=False):
    ...
    # drawings = []  (assigned but not used)
    ...
    for (drawing, docstring,
         name) in getAllTestDrawings(doTTF=hasattr(_pmBackend, 'ft_get_face')):
        ...
        if argv and name not in argv:
            continue
        fnRoot = name
        # w = int(drawing.width) (assigned but not used)
        # h = int(drawing.height) (assigned but not used)
