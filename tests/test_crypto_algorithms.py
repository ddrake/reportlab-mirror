...
from reportlab.lib.pdfencrypt import (computeO, computeU, hexText, unHexText,
                                      encryptionkey, encodePDF, encryptCanvas)
...
class EncryptionAlgorithmTestCase(unittest.TestCase):
    ...
    def checkUserHash40Bit(self):
        ...
        self.assertEqual(
            hexText(userHash),
            '<AA154131D8FA105317F7104D2001A345D78A3DEEFA3D85D032FC9B4B35DA72A0>')

    def checkEncryptString40Bit(self):
        self.assertEqual(hexText(encodePDF(unHexText(
            '<3DC3EBDA71>'), 9, 0, 'anonymous')), '<57AC33DDEB5775982A>')
    ...
    def check0wnerHash128Bit(self):
        ...
        self.assertEqual(
            hexText(ownerHash),
            '<68E5704AC779A5F0CD89704406587A52F25BF61CADC56A0F8DB6C4DB0052534D>')
    ...
    def checkUserHash128Bit(self):
        ...
        self.assertEqual(
            hexText(userHash),
            '<A9AE45CDE827FE0B7D6536267948836A00000000000000000000000000000000>')

    def checkEncryptString128Bit(self):
        self.assertEqual(hexText(encodePDF(unHexText(
            '<3C0C5EBE0122D8EB2BDDF8A09FA8E29E>'), 9, 0, 'anonymous')),
                         '<27FB3E943FCF61878B>')
                ...
                for canCopy in (0, 1):
                    for canAnnotate in (0, 1):
                        for strength in (40, 128):
                            ...
                            canv.drawString(100, 675, 'Verify by looking at File - ' +
