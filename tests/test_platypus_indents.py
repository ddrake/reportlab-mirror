...
from reportlab.lib.colors import lightgreen, lightblue, toColor
...
from reportlab.platypus import FrameBG, FrameSplitter, Spacer, ShowBoundaryValue
...
class MyDocTemplate(BaseDocTemplate):
    ...
    def __init__(self, filename, **kw):
        ...
        templateX = PageTemplate('templateX', [
            Frame(3*cm, 7.5*cm, 14*cm, 4*cm, id='XF4', showBoundary=greenBoundary),
...
class IndentTestCase(unittest.TestCase):
    ...
    def test0(self):
        ...
        storyAdd(Paragraph(
            "This should be indented 0 points at each edge. " + ("spam " * 25), bt))
        storyAdd(Indenter(0, 0))

        storyAdd(Indenter(36, 0))
        storyAdd(Paragraph(
            "This should be indented 36 points at the left. " + ("spam " * 25), bt))
        storyAdd(Indenter(-36, 0))

        storyAdd(Indenter(0, 36))
        storyAdd(Paragraph(
            "This should be indented 36 points at the right. " + ("spam " * 25), bt))
        storyAdd(Indenter(0, -36))

        storyAdd(Indenter(36, 36))
        storyAdd(Paragraph(
            "This should be indented 36 points at each edge. " + ("spam " * 25), bt))
        storyAdd(Indenter(36, 36))
        storyAdd(Paragraph(
        ...
        storyAdd(Paragraph(
            "This should be back to normal at each edge. " + ("spam " * 25), bt))
        ...
        continue on the next page. """ + (random.randint(0, 10) * 'x') + ' ') * 20 ,
                           bt))
        storyAdd(Indenter(-36, -36))
        ...
        storyAdd(Indenter(0, 0))
        storyAdd(Paragraph("This should be indented 0 points at each edge. " +
                           ("spam " * 25), bt))
        storyAdd(Indenter(0, 0))
        storyAdd(Indenter(36, 72))
        storyAdd(Paragraph(("""This should be indented 36 points at the left
        and 72 at the right.  It should run over more than one frame and one page,
        and the indent should continue on the next page. """ +
                            (random.randint(0, 10) * 'x') + ' ') * 35 , bt))
        ...
        storyAdd(Paragraph(
            ...
            bt))
        storyAdd(FrameBG(start=True, color=lightgreen, strokeColor=toColor('red'),
                         strokeWidth=1))
        ...
        storyAdd(Paragraph("Below we split to two new frames with dark green borders",
                           bt))
        storyAdd(FrameSplitter('templateX', ['XF4', 'XF5'], adjustHeight=False))
        storyAdd(FrameBG(start=True, color=lightgreen, strokeColor=toColor('red'),
                         strokeWidth=1))
        for i in range(15):
            storyAdd(Paragraph("We should have a light green background here %d" % i,
                               bt))
        storyAdd(Spacer(6, 6))
        storyAdd(FrameBG(start=False))
        ...
        storyAdd(Paragraph(
            "We should have a light blue background here and " +
            "the whole frame should be filled in.", bt))
