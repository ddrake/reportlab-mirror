...
if simpleparse:
    # redefinition of unused parsexml above
    parsexml = parsexmlSimple  # noqa
...
def parsexml0(xmltext, startingat=0, toplevel=1,
    ...
    if firstbracket < 0:
        ...
        if toplevel is not None:
            ...
            raise ValueError("no tags at non-toplevel %s" %
    ...
    if toplevel is not None:
        ...
        raise ValueError("non top level entry should be at start tag: %s" %
                         repr(xmltext[:10]))
    # special case: CDATA
    ...
    # special case COMMENT
        ...
        if endcommentdashes < firstbracket:
            raise ValueError("unterminated comment %s" %
                             repr(xmltext[cursor:cursor+20]))
        endcomment = endcommentdashes+2
        if xmltext[endcomment] != ">":
            raise ValueError("invalid comment: contains double dashes %s" %
                             repr(xmltext[cursor:cursor+20]))
        ...
        if '=' not in tagcontent:
            ...
            if '"' in tagcontent:
                ...
                while stop is None:
                    ...
                    if noclose or len((tagcontent+".").split('"')) % 2:
                        stop = 1
            if noclose:
                raise ValueError("unclosed start tag %s" %
            ...
            # if not taglist:
            #     raise ValueError, "tag with no name %s" %
            #      repr(xmltext[firstbracket:firstbracket+20])
            taglist0 = taglist[0]
            ...
            while taglistindex < lasttaglistindex:
                ...
                try:
                    ...
                    raise ValueError("attvalue,attentry,attlist=" +
    ...
    while docontents is not None:
        ...
        if nextopenbracket < cursor:
            ...
            if name == NONAME:
                ...
                raise ValueError("no close bracket for %s found after %s" %
        ...
        # is it a close bracket?
            ...
            if nextclosebracket < nextopenbracket:
                raise ValueError("unclosed close tag %s" %
                                 repr(xmltext[nextopenbracket: nextopenbracket+20]))
            ...
            if name != closename:
                ...
                raise ValueError(
                    "at lines %s...%s close tag name doesn't match %s...%s %s" % (
                       linenum, endlinenum, repr(name), repr(closename),
        ...
        # first record any intervening content
            ...
            #    print "--- recursively parsing starting at",
            #     xmltext[nextopenbracket:nextopenbracket+20]
            (parsetree, cursor) = parsexml0(xmltext, startingat=nextopenbracket,
                                            toplevel=None,
        ...
        #    if remainder:
        #        raise ValueError, "trailing garbage at top level %s" %
        #            repr(remainder[:20])
    if ContentList:
        if entityReplacer:
            ContentList = entityReplacer(ContentList)
...
def test(dump=0):
    testparse("""<this type="xml">text &lt;&gt;<b>in</b>
                 <funnytag foo="bar"/>xml</this>
                 <!-- comment -->
                 <![CDATA[
                 <this type="xml">text <b>in</b> xml</this> ]]>
