...
class ScaleWidget(Widget):
    ...
    def draw(self):
        return shapes.Group(transform=(self.scale, 0, 0, self.scale, self.x, self.y),
                            *self.contents)
...
class TypedPropertyCollection(PropHolder):
    ...
    def __getitem__(self, x):
        ...
        try:
            ...
            for i in filter(lambda x, K=list(child.__dict__.keys()):
                            x in K, list(child._attrMap.keys())):
...
class CandleStickProperties(PropHolder):
    _attrMap = AttrMap(
        ...
        crossWidth=AttrMapValue(isNumberOrNone, desc="cross line width",
                                advancedUsage=1),
        crossLo=AttrMapValue(isNumberOrNone, desc="cross line low value",
                             advancedUsage=1),
        crossHi=AttrMapValue(isNumberOrNone, desc="cross line high value",
                             advancedUsage=1),
        boxWidth=AttrMapValue(isNumberOrNone, desc="width of the box part",
                              advancedUsage=1),
        boxFillColor=AttrMapValue(isColorOrNone, desc='fill color of box'),
        boxStrokeColor=AttrMapValue(NotSetOr(isColorOrNone),
                                    desc='stroke color of box'),
        boxStrokeDashArray=AttrMapValue(NotSetOr(isListOfNumbersOrNone),
                                        desc='Dash array of the box.'),
        boxStrokeWidth=AttrMapValue(NotSetOr(isNumber), desc='Width of the box lines.'),
        boxLo=AttrMapValue(isNumberOrNone, desc="low value of the box",
                           advancedUsage=1),
        boxMid=AttrMapValue(isNumberOrNone, desc="middle box line value",
                            advancedUsage=1),
        boxHi=AttrMapValue(isNumberOrNone, desc="high value of the box",
                           advancedUsage=1),
        boxSides=AttrMapValue(isBoolean, desc="whether to show box sides",
                              advancedUsage=1),
        position=AttrMapValue(isNumberOrNone, desc="position of the candle",
                              advancedUsage=1),
        chart=AttrMapValue(None, desc="our chart", advancedUsage=1),
        candleKind=AttrMapValue(OneOf('vertical', 'horizontal'),
                                desc="candle direction", advancedUsage=1),
        axes=AttrMapValue(SequenceOf(isString, emptyOK=0, lo=2, hi=2),
                          desc="candle direction", advancedUsage=1),
        )
    ...
    def __call__(self, _x, _y, _size, _color):
        ...
        def xScale(x):
            return _xScale(x) if x is not None else None
        ...
        def yScale(x):
            return _yScale(x) if x is not None else None
        G = shapes.Group().add
        ...
        def aLine(x0, y0, x1, y1):
            if candleKind != 'vertical':
                x0, y0 = y0, x0
                x1, y1 = y1, x1
            G(shapes.Line(x0, y0, x1, y1, strokeWidth=strokeWidth,
        ...
        if haveLine:
            ...
            if not haveBox or crossLo >= boxHi or crossHi <= boxLo:
                ...
                if crossWidth is not None:
                    aLine(position-crossWidth*0.5, crossLo, position+crossWidth*0.5,
                          crossLo)
                    aLine(position-crossWidth*0.5, crossHi, position+crossWidth*0.5,
                          crossHi)
            elif haveBox:
                if crossLo < boxLo:
        ...
        if haveBox:
            ...
            if candleKind != 'vertical':
                x, y, w, h = y, x, h, w
            G(shapes.Rect(x, y, w, h, strokeColor=boxStrokeColor
                          if self.boxSides else None,
