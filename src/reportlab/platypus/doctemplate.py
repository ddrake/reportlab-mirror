...
(for example the header, footer, page number, fixed logo graphic, watermark, etcetera)
and a set of Frames which enclose the flowing part of the page (for example the
paragraphs, tables, or non-fixed diagrams of the text).
...
from reportlab.platypus.flowables import (_ContainerSpace, Flowable, PageBreak, DocIf,
                                          DocWhile, KeepTogether, SlowPageBreak,
                                          DDIndenter, PageBreakIfNotEmpty, XBox)
...
from reportlab.lib.utils import (isSeq, encode_label, decode_label, annotateException,
                                 strTypes)
...
class LCActionFlowable(ActionFlowable):
    ...
    def wrap(self, availWidth, availHeight):
        ...
        raise NotImplementedError('%s.wrap should never be called' %
                                  self.__class__.__name__)

    def draw(self):
        '''Should never be called.'''
        raise NotImplementedError('%s.draw should never be called' %
                                  self.__class__.__name__)
...
class NextPageTemplate(ActionFlowable):
    locChanger = 1                  # we cause a frame or page change
    """When you get to the next page, use the template specified (change to two
    column, for example)  """
...
class PageTemplate:
    ...
    def __init__(self, id=None, frames=[], onPage=_doNothing, onPageEnd=_doNothing,
        ...
        if not isSeq(frames):
            frames = [frames]
        assert [x for x in frames if not isinstance(x, Frame)] == [], \
...
def _ktAllow(f):
    ...
    return (not (isinstance(f, (_ContainerSpace, DocIf, DocWhile)) or
                 getattr(f, 'locChanger', False)))
...
class BaseDocTemplate:
    ...
    - allowSplitting:  If set flowables (eg, paragraphs) may be split across frames or
      pages (default: 1)
    ...
    def __init__(self, filename, **kw):
        """create a document template bound to a filename (see class documentation
    ...
    def addPageTemplates(self, pageTemplates):
        ...
        # assert filter(lambda x: not isinstance(x,PageTemplate), pageTemplates)==[], \
        # "pageTemplates argument error"
    ...
    def _samePT(self, npt):
        ...
        if isinstance(npt, strTypes):
            return npt == (self.pageTemplates[self._nextPageTemplateIndex].id
                           if hasattr(self, '_nextPageTemplateIndex') else
    ...
    def handle_pageEnd(self):
        ...
        if self._emptyPages >= self._emptyPagesAllowed:
            if 1:
                ident = '''More than %d pages generated without content - halting
                           layout.  Likely that a flowable is too large for any
                           frame.''' % self._emptyPagesAllowed
            ...
            self.canv.setPageRotation(getattr(self.pageTemplate, 'rotation',
                                              self.rotation))
            self.canv.showPage()
            self._setPageTemplate()
    ...
    def handle_breakBefore(self, flowables):
        ...
        if hasattr(first, 'pageBreakBefore') and first.pageBreakBefore == 1:
            ...
            return
        if (hasattr(first, 'style') and hasattr(first.style, 'pageBreakBefore') and
                first.style.pageBreakBefore == 1):
            first._skipMeNextTime = 1
        ...
        if hasattr(first, 'frameBreakBefore') and first.frameBreakBefore == 1:
            ...
            return
        if (hasattr(first, 'style') and hasattr(first.style, 'frameBreakBefore') and
                first.style.frameBreakBefore == 1):
            first._skipMeNextTime = 1
            flowables.insert(0, FrameBreak())
            return
                    ...
                    if hasattr(f, '_postponed'):
                        ...
                        ident = ('''Flowable %s%s too large on page %d in frame
                                 ...
                                 (self._fIdent(f, 60, frame), _fSizeString(f),
                                  self.page, self.frame.id, self.frame._aSpaceString(),
                                  self.pageTemplate.id))
                        # leave to keep apart from the raise
                        raise LayoutError(ident)
    ...
    def build(self, flowables, filename=None, canvasmaker=canvas.Canvas):
        ...
        try:
            ...
            while len(flowables):
                if (self._hanging and self._hanging[-1] is PageBegin and
                        isinstance(flowables[0], PageBreakIfNotEmpty)):
    ...
    def notify(self, kind, stuff):
        ...
        for l in self._indexingFlowables:  # noqa
    ...
    def docExec(self, stmt, lifetime):
        ...
        try:
            if lifetime not in self._allowedLifetimes:
                raise ValueError('bad lifetime %r not in %r' %
...
class SimpleDocTemplate(BaseDocTemplate):
    ...
    def build(self, flowables, onFirstPage=_doNothing, onLaterPages=_doNothing,
        ...
        self.addPageTemplates([
            PageTemplate(id='First', frames=frameT, onPage=onFirstPage,
                         pagesize=self.pagesize),
