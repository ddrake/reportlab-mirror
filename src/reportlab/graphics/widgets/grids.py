...
from reportlab.lib.validators import (isNumber, isColorOrNone, isBoolean,
                                      isListOfNumbers, OneOf, isListOfColors,
                                      isNumberOrNone)
from reportlab.lib.attrmap import AttrMap, AttrMapValue
from reportlab.graphics.shapes import (Drawing, Group, Line, Rect, LineShape,
                                       definePath, EmptyClipPath)
...
class ShadedRect(Widget):
    ...
    def _flipRectCorners(self):
        ...
        (x, y, width, height, fillColorStart,
         fillColorEnd) = (self.x, self.y, self.width, self.height,
                          self.fillColorStart, self.fillColorEnd)
        ...
        for v in V:
            ...
            if self.cylinderMode:
                if V.index(v) >= halfNumShades:
                    col = colors.linearlyInterpolatedColor(c1, c0, V[halfNumShades],
                                                           V[-1],  v)
                else:
                    col = colors.linearlyInterpolatedColor(c0, c1, V[0],
                                                           V[halfNumShades],  v)
            else:
...
if __name__ == '__main__':  # noruntests
    ...
    D.add(ShadedPolygon(
        ...
        strokeColor=None, strokeWidth=1, angle=90,
        numShades=50, cylinderMode=0))
