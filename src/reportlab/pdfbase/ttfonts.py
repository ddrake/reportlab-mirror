...
GF_ARG_1_AND_2_ARE_WORDS        = 1 << 0   # noqa
GF_ARGS_ARE_XY_VALUES           = 1 << 1   # noqa
GF_ROUND_XY_TO_GRID             = 1 << 2   # noqa
GF_WE_HAVE_A_SCALE              = 1 << 3   # noqa
GF_RESERVED                     = 1 << 4   # noqa
GF_MORE_COMPONENTS              = 1 << 5   # noqa
GF_WE_HAVE_AN_X_AND_Y_SCALE     = 1 << 6   # noqa
GF_WE_HAVE_A_TWO_BY_TWO         = 1 << 7   # noqa
GF_WE_HAVE_INSTRUCTIONS         = 1 << 8   # noqa
GF_USE_MY_METRICS               = 1 << 9   # noqa
GF_OVERLAP_COMPOUND             = 1 << 10  # noqa
GF_SCALED_COMPONENT_OFFSET      = 1 << 11  # noqa
GF_UNSCALED_COMPONENT_OFFSET    = 1 << 12  # noqa
...
class TTFontParser:
    ...
    def readTTCHeader(self):
        ...
        if self.ttcVersion not in self.ttcVersions:
            raise TTFError('"%s" is not a %s file: can\'t read version 0x%8.8x' %
    ...
    def getSubfont(self, subfontIndex):
        if self.fileKind != 'TTC':
            raise TTFError('"%s" is not a TTC file: use this method' % (self.filename))
        try:
    ...
    def readTableDirectory(self):
        try:
            ...
            raise TTFError('Corrupt %s file "%s" cannot read Table Directory' %
    ...
    def readHeader(self):
        ...
        try:
            ...
            raise TTFError('"%s" is not a %s file: can\'t read version' %
                           (self.filename, self.fileKind))

        if version == 0x4F54544F:
            raise TTFError('%s file "%s": postscript outlines are not supported' %
                           (self.fileKind, self.filename))
    ...
    def checksumFile(self):
        ...
        if 0xB1B0AFBA != checksum:
            raise TTFError(
                ('TTF file "%s": invalid checksum %s (expected 0xB1B0AFBA) ' +
                 'len: %d &3: %d') %
...
CMapFmt2SubHeader = namedtuple('CMapFmt2SubHeader',
...
class TTFontFile(TTFontParser):
        ...
        if isStr(subfontIndex):  # bytes or unicode
            ...
            while True:
                ...
                if (isBytes(subfontIndex) and subfontIndex == self.name
                    ...
                    return
                if not sfi:
                    __dict__.update(dict(_ttf_data=self._ttf_data,
                                         filename=self.filename))
                sfi += 1
                if sfi >= numSubfonts:
                    raise ValueError('cannot find %r subfont %r' %
                                     (self.filename, subfontIndex))
                self.__dict__.clear()
    ...
    def extractInfo(self, charInfo=1):
        ...
        for i in range(numRecords):
            ...
            if platformId == 3 and encodingId == 1 and languageId == 0x409:
                ...
                try:
                    ...
                    N = TTFNameBytes(
                        self.get_chunk(string_data_offset + offset, length),
                        'utf_16_be')
                finally:
                    self._pos = opos
            # Macintosh, Roman, English, PS Name
                ...
                N = TTFNameBytes(self.get_chunk(string_data_offset + offset, length),
                                 'mac_roman')
        ...
        # Don't just assume, check for None
        # since some shoddy fonts cause crashes here...
        if not psName:
            if rl_config.autoGenerateTTFMissingTTFName:
                ...
                if not fn:
                    psName = bytestr('_RL_%s_%s_TTF' %
                                     (time.time(), self.__class__._agfnc))
        ...
        # do not assign a lambda, use a def
        def scale(x, unitsPerEm=unitsPerEm):
            return x * 1000. / unitsPerEm
        self.skip(16)
        ...
        if "OS/2" in self.table:
            ...
            if fsType == 0x0002 or (fsType & 0x0300):
                subsettingAllowed = (os.path.basename(self.filename) not in
                                     rl_config.allowTTFSubsetting)
        ...
        # There's no way to get stemV from a TTF file short of analyzing
        # actual outline data.
        ...
        # All fonts that contain characters outside the original Adobe character
        ...
        if not subsettingAllowed:
            if self.numGlyphs > 0xFF:
                raise TTFError('Font does not allow subsetting/embedding (%04X)' %
                               fsType)
            else:
        ...
        if fmt == 0:
            ...
            for unichar in range(min(256, self.numGlyphs, len(table))):  # noqa: F821
                # undefined name glyph!
                glyph = T[glyph]  # noqa: F821
            ...
            for i in range(maxSHK+1):
                ...
                SH.append(CMapFmt2SubHeader(firstCode, entryCount, idDelta,
                                            idRangeOffset))
            # number of glyph indexes to read. it is the length of the entire subtable
            # minus that bit we've read so far
    ...
    def makeSubset(self, subset):
        ...
        cmap = [0, 1,           # version, number of tables
                ...
                0,
                entryCount] + list(map(codeToGlyph.get, subset))
...
FF_FIXED        = 1 <<  1-1  # noqa
FF_SERIF        = 1 <<  2-1  # noqa
FF_SYMBOLIC     = 1 <<  3-1  # noqa
FF_SCRIPT       = 1 <<  4-1  # noqa
FF_NONSYMBOLIC  = 1 <<  6-1  # noqa
FF_ITALIC       = 1 <<  7-1  # noqa
FF_ALLCAP       = 1 << 17-1  # noqa
FF_SMALLCAP     = 1 << 18-1  # noqa
FF_FORCEBOLD    = 1 << 19-1  # noqa
...
class TTFontFace(TTFontFile, pdfmetrics.TypeFace):
    ...
    def addSubsetObjects(self, doc, fontname, subset):
        ...
        fontFileRef = doc.Reference(fontFile, 'fontFile:%s(%s)' %
...
class TTFont:
    ...
    def getSubsetInternalName(self, subset, doc):
        ...
        if subset < 0 or subset >= len(state.subsets):
            raise IndexError('Subset %d does not exist in font %s' %
    ...
    def addObjects(self, doc):
        ...
        for n, subset in enumerate(state.subsets):
            ...
            baseFontName = (b''.join((SUBSETN(n), b'+', self.face.name,
