...
# from reportlab.lib.utils import rl_safe_exec
from reportlab.lib.utils import (rl_safe_eval, annotateException,
        ...
        for kind, _data in (
            (
                ...
                (
                    ...
                    "(lambda f: lambda n: (1,(1,(1,(1,f(n-1))))) " +
                    "if n else 1)(lambda x:x)(5)",
                    "((lambda f: (lambda x: x(x))(lambda y: f(lambda *args: y(y)" +
                    "(*args)))) (lambda f: lambda n: (1,(1,(1,(1,f(n-1))))) " +
                    "if n else 1)(30))",
                    ...
                    '(getattr(testInst,"a"),getattr(testInst,"a",12),' +
                    'getattr(testInst,"xxx",13))',
                    'list(sorted([3,4,1,2,0],reverse=True))',
                    'list(reversed([3,4,1,2,0]))',
                    'list(range(1,10,3))',
                    '({1,2,3},set([4,5,6]),frozenset([7,8,9]),' +
            ...
            (
                ...
                (
                    ...
                    ("testInst.__class__.__bases__[0].__subclasses__()",
                     dict(g=dict(testInst=testInst))),
                    "10**200**200",
                    "pow(10,200)",
                    '__import__("reportlab")',
                    ('(lambda i: [i for i in ((i, 1) for j in range(1000000))][-1])(1)',
                     dict(timeout=0.1)),
                    ('[i for i in ((j, 1) for j in range(1000000))][-1]',
                     dict(timeout=0.1)),
                    ('''((lambda f: (lambda x: x(x))(lambda y: f(lambda *args: y(y)
                     (*args)))) (lambda f: lambda n: (1,(1,(1,(1,f(n-1)))))
                     if n else 1)(300))''',
                    ...
                    '"{1}{2}".format(1,2)',
                    'builtins',
                    '[ [ [ [ ftype(ctype(0, 0, 0, 0, 3, 67, b"t\\x00d\\x01\\x83' +
                    '\\x01\\xa0\\x01d\\x02\\xa1\\x01\\x01\\x00d\\x00S\\x00", (None,' +
                    ' "os", "touch /tmp/exploited"), ("__import__", "system"), (), ' +
                    '"<stdin>", "", 1, b"\\x12\\x01"), {})() for ftype in [type' +
                    '(lambda: None)] ] for ctype in [type(getattr(lambda: {None}, ' +
                    'Word("__code__")))] ] for Word in [orgTypeFun("Word", (str,), ' +
                    '{ "mutated": 1, "startswith": lambda self, x: False, "__eq__": ' +
                    'lambda self,x: self.mutate() and self.mutated < 0 and str(self) ' +
                    '== x, "mutate": lambda self: {setattr(self, "mutated", ' +
                    'self.mutated - 1)}, "__hash__": lambda self: hash(str(self)) })]' +
                    '] for orgTypeFun in [type(type(1))]] and "red"',
                )
            ),
        ):
...
class SafeEvalTestCase(unittest.TestCase):
    def works(self, expr, g=None, l=None):  # noqa
        try:
    ...
    def fails(self, expr, g=None, l=None, timeout=None, exception=BadCode):  # noqa
        try:
            ...
            self.assertEqual(True, False,
                             "rl_safe_eval(%r) raised %s: %s instead of %s" %
                             (expr, sys.exc_info()[0].__name__, str(sys.exc_info()[1]),
                              exception.__name__))
...
class ExtendedLiteralEval(unittest.TestCase):
    def test_001(self):
        ...
        C = {s: getattr(colors, s) for s in
             '''Blacker CMYKColor CMYKColorSep Color ColorType HexColor PCMYKColor
             PCMYKColorSep Whiter _chooseEnforceColorSpace _enforceCMYK _enforceError
             _enforceRGB _enforceSEP _enforceSEP_BLACK _enforceSEP_CMYK _namedColors
             _re_css asNative cmyk2rgb cmykDistance color2bw colorDistance cssParse
             describe fade fp_str getAllNamedColors hsl2rgb hue2rgb
             linearlyInterpolatedColor obj_R_G_B opaqueColor rgb2cmyk setColors
             toColor toColorOrNone'''.split()
        ...
        for expr, expected in (
                ...
                ('PCMYKColor(21,10,30,5,spotName="ABCD")',
                 colors.PCMYKColor(21, 10, 30, 5, spotName='ABCD', alpha=100)),
                ('HexColor("#ffffff")', colors.Color(1, 1, 1, 1)),
                ('linearlyInterpolatedColor(red, blue, 0, 1, 0.5)',
                 colors.Color(.5, 0, .5, 1)),
            ...
            self.assertEqual(
                showVal(expr), expected,
                f"rl_extended_literal_eval({expr!r}) is not equal " +
