...
class ColorTestCase(unittest.TestCase):
    ...
    def test2(self):
        ...
        for thing in allRed:
            assert colors.toColor(thing) == colors.red, (
                "colors.toColor(%s)-->%s != colors.red(%s)" %
    ...
    def test2a(self):
        ...
        self.assertRaises(ValueError, colors.toColor,
                          "open(%s,'w').write('dumber and dumber')" % repr(ofn))
        self.assertFalse(os.path.isfile(ofn),
                         "toColor managed to create a file %s :(" % repr(ofn))
        self.assertRaises(ValueError, colors.toColor,
                          "red.__class__.__bases__[0].__subclasses__()")
        self.assertRaises(
            ValueError, colors.toColor,
            '''(lambda fc=(lambda n: [c for c in ().__class__.
                          __bases__[0].__subclasses__()
                          if c.__name__ == n][0]): fc("function")
                          (fc("code")(0,0,0,0,"KABOOM",(), (),(),"",
                          "",0,""),{})())()''')
        self.assertEqual(colors.Blacker(colors.red, 0.5),
                         colors.toColor("Blacker(red,0.5)"))
    ...
    def test3(self):
        ...
        for name, rgbCol in rgbCols:
            ...
            # rgbCol2 = colors.Color(r2, g2, b2) (assigned but not used)
    ...
    def test4(self):
        ...
        for name, rgbCol in rgbCols:
            ...
            # colors.cmyk2rgb((c, m, y, k))
            r2, g2, b2 = cmykCol.red, cmykCol.green, cmykCol.blue
            # rgbCol2 = colors.Color(r2, g2, b2) (assigned but not used)

            # Make sure the differences for each RGB component
            # isreally small (< power(10, -N)!
            N = 16  # max. value found to work on Python2.0 and Win2K.
            deltas = [abs(x) for x in (r1-r2, g1-g2, b1-b2)]
            assert deltas < [math.pow(10, -N)] * 3
    ...
    def test5(self):
        ...
        canvas.setFont('Times-Roman', 10)
        text = ('This shows all the named colors in the HTML standard ' +
                '(plus their gray and CMYK equivalents).')
    ...
    def test6(self):
        ...
        self.ctAssertRaisesRegex(
            ValueError,
            r"invalid literal for int\(\) with base 10:.*ffffff", HexColor, b'ffffff')
        self.assertEqual(HexColor(b'#FFFFFF', htmlOnly=True), Color(1, 1, 1, 1))
        self.ctAssertRaisesRegex(
            ValueError, "not a hex string", HexColor, b'0xffffff', htmlOnly=True)
        self.ctAssertRaisesRegex(
            ValueError, "not a hex string", HexColor, b'16777215', htmlOnly=True)
        ...
        self.ctAssertRaisesRegex(
            ValueError, r"invalid literal for int\(\) with base 10:.*ffffff",
            HexColor, u'ffffff')
        self.assertEqual(
            HexColor(u'#FFFFFF', htmlOnly=True), Color(1, 1, 1, 1))
        self.ctAssertRaisesRegex(
            ValueError, "not a hex string", HexColor, u'0xffffff', htmlOnly=True)
        self.ctAssertRaisesRegex(
            ValueError, "not a hex string", HexColor, u'16777215', htmlOnly=True)
