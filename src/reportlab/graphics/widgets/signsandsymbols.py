...
from reportlab.lib.validators import (isNumber, isColorOrNone, isColor, isBoolean,
...
class _Symbol(Widget):
    ...
    def __init__(self):
        assert self.__class__.__name__ != '_Symbol', (
    ...
    def demo(self):
        ...
        D.add(shapes.String(ob.x+(s/2), (ob.y-12),
                            ob.__class__.__name__, fillColor=colors.black,
...
class ETriangle(_Symbol):
    ...
    def draw(self):
        ...
        # ae = s*0.125  # (ae = 'an eighth')  (assigned but not used)
...
class Crossbox(_Symbol):
    ...
    def draw(self):
        ...
        crossLine1 = shapes.Line(
            self.x+(s*0.15), self.y+(s*0.15), self.x+(s*0.85), self.y+(s*0.85),
        ...
        crossLine2 = shapes.Line(
            self.x+(s*0.15), self.y+(s*0.85), self.x+(s*0.85), self.y+(s*0.15),
            fillColor=self.crossColor,
            strokeColor=self.crossColor,
            strokeWidth=self.crosswidth)
        g.add(crossLine2)
...
"""
    ...
    def draw(self):
        ...
        tickLine = shapes.PolyLine(
            points=[
                self.x+(s*0.15), self.y+(s*0.35),
                self.x+(s*0.35), self.y+(s*0.15),
                self.x+(s*0.35), self.y+(s*0.15),
...
class StopSign(_Symbol):
    ...
    def draw(self):
        ...
        innerOctagon = shapes.Polygon(
            points=[self.x+athird+(s/75), self.y+(s/75),
...
class NoEntry(_Symbol):
    ...
    def draw(self):
        ...
        if self.fillColor:
            g.add(shapes.Circle(
                cx=(self.x+(s/2)), cy=(self.y+(s/2)), r=((s/2)-(s/50)),
                fillColor=self.fillColor, strokeColor=None, strokeWidth=0))

        innerBarColor = self.innerBarColor
        if innerBarColor:
            g.add(shapes.Rect(
                self.x+(s*0.1), self.y+(s*0.4), width=s*0.8, height=s*0.2,
                fillColor=innerBarColor, strokeColor=innerBarColor,
                strokeLineCap=1, strokeWidth=0))
        return g
...
class NotAllowed(_Symbol):
    ...
    def draw(self):
        ...
        outerCircle = shapes.Circle(
            cx=(self.x+(s/2)), cy=(self.y+(s/2)), r=(s/2)-(s/10),
        ...
        radius = s-(s/6)
        yradius = radius/2
        # xradius = radius/2  (assigned but not used)
        ...
        crossbar = shapes.PolyLine(
            pointslist,
            fillColor=strokeColor, strokeColor=strokeColor, strokeWidth=s/10.)
        g.add(crossbar)
        return g
    ...
    def draw(self):
        ...
        outerTriangle = shapes.Polygon(
            points=[
                ...
                self.x+s, self.y,
                self.x+(s/2), self.y+s],
            fillColor=None, strokeColor=self.strokeColor, strokeWidth=0)
        g.add(outerTriangle)
...
"""
    ...
    def demo(self):
        ...
        labelFontSize = 8
        D.add(shapes.String(
            yn.x+(yn.size/2), (yn.y-(1.2*labelFontSize)),
            'testValue=0',
            fillColor=colors.black, textAnchor='middle', fontSize=labelFontSize))
        D.add(shapes.String(
            yn2.x+(yn2.size/2), (yn2.y-(1.2*labelFontSize)),
            'testValue=1',
            fillColor=colors.black, textAnchor='middle', fontSize=labelFontSize))
        labelFontSize = 10
        D.add(shapes.String(
...
class FloppyDisk(_Symbol):
    ...
    def draw(self):
        ...
        diskBody = shapes.Rect(
            x=self.x, y=self.y+(s/100), width=s, height=s-(s/100),
            fillColor=self.diskColor, strokeColor=None, strokeWidth=0)
        g.add(diskBody)

        label = shapes.Rect(
            x=self.x+(s*0.1), y=(self.y+s)-(s*0.5), width=s*0.8, height=s*0.48,
            fillColor=colors.whitesmoke, strokeColor=None, strokeWidth=0)
        g.add(label)

        labelsplash = shapes.Rect(
            x=self.x+(s*0.1), y=(self.y+s)-(s*0.1), width=s*0.8, height=s*0.08,
            fillColor=colors.royalblue, strokeColor=None, strokeWidth=0)
        g.add(labelsplash)

        line1 = shapes.Line(
            x1=self.x+(s*0.15), y1=self.y+(0.6*s),
            x2=self.x+(s*0.85), y2=self.y+(0.6*s),
            fillColor=colors.black, strokeColor=colors.black, strokeWidth=0)
        g.add(line1)

        line2 = shapes.Line(
            x1=self.x+(s*0.15), y1=self.y+(0.7*s),
            x2=self.x+(s*0.85), y2=self.y+(0.7*s),
            fillColor=colors.black, strokeColor=colors.black, strokeWidth=0)
        g.add(line2)

        line3 = shapes.Line(
            x1=self.x+(s*0.15), y1=self.y+(0.8*s),
            x2=self.x+(s*0.85), y2=self.y+(0.8*s),
            fillColor=colors.black, strokeColor=colors.black, strokeWidth=0)
        g.add(line3)

        metalcover = shapes.Rect(
            x=self.x+(s*0.2), y=(self.y), width=s*0.5, height=s*0.35,
            fillColor=colors.silver, strokeColor=None, strokeWidth=0)
        g.add(metalcover)

        coverslot = shapes.Rect(
            x=self.x+(s*0.28), y=(self.y)+(s*0.035), width=s*0.12, height=s*0.28,
            fillColor=self.diskColor, strokeColor=None, strokeWidth=0)
        g.add(coverslot)
...
def test():
    """This function produces a pdf with examples of all the
    signs and symbols from this file.
    """
    labelFontSize = 10
    ...
    D.add(cb)
    D.add(shapes.String(
        cb.x+(cb.size/2), (cb.y-(1.2*labelFontSize)),
        cb.__class__.__name__, fillColor=colors.black,
    ...
    D.add(tb)
    D.add(shapes.String(
        tb.x+(tb.size/2), (tb.y-(1.2*labelFontSize)),
        tb.__class__.__name__, fillColor=colors.black,
        textAnchor='middle', fontSize=labelFontSize))
    ...
    D.add(shapes.String(
        yn.x+(tb.size/2), (yn.y-(1.2*labelFontSize)),
        tempstring, fillColor=colors.black,
        textAnchor='middle', fontSize=labelFontSize))
    D.add(shapes.String(
        130, 6,
        ("(The 'YesNo' widget returns a tickbox if testvalue=1, " +
         "and a crossbox if testvalue=0)"),
    ...
    D.add(ss)
    D.add(shapes.String(
        ss.x+(ss.size/2), ss.y-(1.2*labelFontSize),
        ss.__class__.__name__, fillColor=colors.black,
        textAnchor='middle', fontSize=labelFontSize))
    ...
    D.add(ne)
    D.add(shapes.String(
        ne.x+(ne.size/2), (ne.y-(1.2*labelFontSize)),
        ne.__class__.__name__, fillColor=colors.black,
        textAnchor='middle', fontSize=labelFontSize))
    ...
    D.add(sf)
    D.add(shapes.String(
        sf.x+(sf.size/2), (sf.y-(1.2*labelFontSize)),
        sf.__class__.__name__, fillColor=colors.black,
        textAnchor='middle', fontSize=labelFontSize))
    ...
    D.add(ds)
    D.add(shapes.String(
        ds.x+(ds.size/2), (ds.y-(1.2*labelFontSize)),
        ds.__class__.__name__, fillColor=colors.black,
        textAnchor='middle', fontSize=labelFontSize))
    ...
    D.add(na)
    D.add(shapes.String(
        na.x+(na.size/2), (na.y-(1.2*labelFontSize)),
    ...
    D.add(ns)
    D.add(shapes.String(
        ns.x+(ns.size/2), (ns.y-(1.2*labelFontSize)),
        ns.__class__.__name__, fillColor=colors.black,
        textAnchor='middle', fontSize=labelFontSize))
    ...
    D.add(a1)
    D.add(shapes.String(
        a1.x+(a1.size/2), (a1.y-(1.2*labelFontSize)),
        a1.__class__.__name__, fillColor=colors.black,
        textAnchor='middle', fontSize=labelFontSize))
    ...
    D.add(a2)
    D.add(shapes.String(
        a2.x+(a2.size/2), (a2.y-(1.2*labelFontSize)),
        a2.__class__.__name__, fillColor=colors.black,
        textAnchor='middle', fontSize=labelFontSize))
    ...
    D.add(fd)
    D.add(shapes.String(
        fd.x+(fd.size/2), (fd.y-(1.2*labelFontSize)),
        fd.__class__.__name__, fillColor=colors.black,
        textAnchor='middle', fontSize=labelFontSize))
