...
from reportlab.lib.validators import (isNumber, isNumberOrNone, isColorOrNone, isString,
                                      SequenceOf, isBoolean, isStringOrNone, isInt,
...
from reportlab.graphics.charts.axes import (XCategoryAxis, YValueAxis, YCategoryAxis,
                                            XValueAxis)
...
class BarChartProperties(PropHolder):
    _attrMap = AttrMap(
        ...
        swatchMarker=AttrMapValue(
            NoneOr(isSymbol),
            desc="None or makeMarker('Diamond') ...", advancedUsage=1),
...
class BarChart(PlotArea):
    ...
    _attrMap = AttrMap(
        ...
        useAbsolute=AttrMapValue(
            ...
            desc=('Flag to use absolute spacing values; use string of gsb for finer ' +
        ...
        barLabelFormat=AttrMapValue(
            None, desc=('Formatting string or function used for bar labels. ' +
                        'Can be a list or list of lists of such.')),
        barLabelCallOut=AttrMapValue(
            None, desc=('Callout function(label)\nlabel._callOutInfo = (self,g,rowNo,' +
                        'colNo,x,y,width,height,x00,y00,x0,y0)'), advancedUsage=1),
        barLabelArray=AttrMapValue(
            None,
            desc=('explicit array of bar label values, ' +
                  'must match size of data if present.')),
        reversePlotOrder=AttrMapValue(
            isBoolean, desc='If true, reverse common category plot order.',
            advancedUsage=1),
        naLabel=AttrMapValue(
            NoneOrInstanceOfNA_Label, desc='Label to use for N/A values.',
            advancedUsage=1),
        ...
        barRecord=AttrMapValue(
            None, desc=('callable(bar,label=labelText,value=value,**kwds) to record' +
                        ' bar information'), advancedUsage=1),
        zIndexOverrides=AttrMapValue(
            isStringOrNone, desc='''None (the default ie use old z ordering scheme) or a
',' separated list of key=value (int/float) for new zIndex ordering.
If used defaults are
    background=0,
    categoryAxis=1,
    valueAxis=2,
    bars=3,
    ...
    annotations=7'''),
        ...
        seriesOrder=AttrMapValue(
            SequenceOf(SequenceOf(
                isInt, emptyOK=0, NoneOK=0, lo=1), emptyOK=0, NoneOK=1, lo=1),
    ...
    def makeSwatchSample(self, rowNo, x, y, width, height):
        ...
        if swatchMarker:
            ...
            if S:
                S = uSymbol2Symbol(S, x+width/2., yh2,
    ...
    def _getConfigureData(self):
        ...
        data = self.data
        # cc = max(list(map(len, data)))   # category count (assigned but not used)
        _data = data
    ...
    def _drawBegin(self, org, length):
        ...
        return (crossesAt
                if vA.forceZero or (crossesAt >= org and crossesAt <= org+length) else
                org)
    ...
    def calcBarPositions(self):
        ...
        style = cA.style
        bars = self.bars
        lineCount = sum((int(bars.checkAttr(_, 'isLine', False))
                         for _ in range(seriesCount)))
        ...
        if useAbsolute:
            if not isinstance(useAbsolute, str):
                ...
                useAbsolute = (0 + 1*('b' in useAbsolute)+2*('g' in useAbsolute) +
                               4*('s' in useAbsolute))
        else:
            useAbsolute = 0
    ...
    def _addBarLabel(self, g, rowNo, colNo, x, y, width, height):
        ...
        if text:
            self._addLabel(text, self.barLabels[(rowNo, colNo)], g, rowNo, colNo,
                           x, y, width, height)

    def _addNABarLabel(self, g, rowNo, colNo, x, y, width, height,
                       calcOnly=False, na=None):
        if na is None:
            na = self.naLabel
        if na and na.text:
            na = copy.copy(na)
            ...
            if height is None:
                height = v
            return self._addLabel(na.text, na, g, rowNo, colNo, x, y, width, height,
                                  calcOnly=calcOnly)

    def _addLabel(self, text, label, g, rowNo, colNo, x, y, width, height,
                  calcOnly=False):
        if label.visible:
            # labelWidth = stringWidth(text, label.fontName, label.fontSize) (not used)
            flipXY = self._flipXY
            if flipXY:
            ...
            if alx:
                label._callOutInfo = (self, g, rowNo, colNo, x, y, width, height,
                                      x00, y00, x0, y0)
                alx(label)
    ...
    def _makeBars(self, g, lg):
        ...
        for rowNo, row in enumerate(BP):
            ...
            linePts = [].append
            for colNo, (x, y, width, height) in enumerate(row):
                style = ((styleIdx, colNo) in bars and bars[(styleIdx, colNo)]
                         or rowStyle)
                if None in (width, height):
                    if not catNAL or colNo in catNNA:
                        self._addNABarLabel(lg, rowNo, colNo, x, y, width, height)
                    elif catNAL and colNo not in CBL:
                        r0 = self._addNABarLabel(lg, rowNo, colNo, x, y, width, height,
                                                 True, catNAL)
                        if r0:
                            x, y, width, height = BP[rowNoL][colNo]
                            r1 = self._addNABarLabel(lg, rowNoL, colNo, x, y, width,
                                                     height, True, catNAL)
                            x = (r0[0]+r1[0])/2.0
                            y = (r0[1]+r1[1])/2.0
                            self._addNABarLabel(lg, rowNoL, colNo, x, y, 0.0001,
                                                0.0001, na=catNAL)
                        CBL.append(colNo)
                    if isLine:
                        linePts(None)
                ...
                if isLine:
                    ...
                    linePts(yL)
                    if symbol:
                        sym = uSymbol2Symbol(tpcGetItem(symbol, colNo), xL, yL,
                                             style.strokeColor or style.fillColor)
                        if sym:
                            lineSyms(sym)
                    ...
                    if br:
                        br(g.contents[-1], label=self._getLabelText(rowNo, colNo),
                           value=self.data[rowNo][colNo], rowNo=rowNo, colNo=colNo)
    ...
    def _computeBarPositions(self):
        """Information function, can be called by charts
        which want to with space around bars"""
    ...
    def _computeMaxSpace(self, size, required):
        ...
        # lenData = len(self.data)  (assigned but not used)
        BP = self._barPositions
        C = []
        aC = C.append
        if flipXY:
        ...
        for i, ci in enumerate(C):
            ...
            if alo > ahi:
                if alo > maxS:
                    maxS = alo
                    maxP = (flipXY and (lo, v0, lo+alo, v0+size, 0) or
                            (v0, lo, v0+size, lo+alo, 0))
                    if maxS >= required:
                        break
            elif ahi > maxS:
                maxS = ahi
                maxP = (flipXY and (hi-ahi, v0, hi, v0+size, 1) or
                        (v0, hi-ahi, v0+size, hi, 1))
                if maxS >= required:
                    break
        return maxS, maxP
    ...
    def _computeSimpleBarLabelPositions(self):
        """Information function, can be called by charts
        which want to mess with labels"""
        cA, vA = self.categoryAxis, self.valueAxis
        if vA:
            vA.joinAxis = cA
        if cA:
        ...
        # bars = self.bars  (assigned but not used)
        R = [].append
        BP = self._barPositions
        for rowNo, row in enumerate(BP):
            C = [].append
            for colNo, (x, y, width, height) in enumerate(row):
                if None in (width, height):
                    ...
                    if na and na.text:
                        ...
                        if height is None:
                            height = v
                        C(self._computeLabelPosition(na.text, na, rowNo, colNo, x, y,
                                                     width, height))
                    else:
                        C(None)
                else:
                    text = self._getLabelText(rowNo, colNo)
                    if text:
                        C(self._computeLabelPosition(text,
                                                     self.barLabels[(rowNo, colNo)],
                                                     rowNo, colNo, x, y, width, height))
                    else:
                        C(None)
            R(C.__self__)
        return R.__self__
...
class BarChart3D(BarChart):
    ...
    def makeBars(self):
        ...
        if cAStyle == 'stacked':
            ...
            fg_value = [_[1] for _ in sorted((((t[1], t[2], t[3], t[4]), t)
                                              for t in fg_value))]
        ...
        for t in fg_value:
            if t[0] == 0:
                ...
                _draw_3d_bar(g, x, x+width, y, y+height, dz*theta_x, dz*theta_y,
                             ...
                             strokeWidth=style.strokeWidth, shading=0.45)
        for t in fg_value:
            if t == 1:
...
class HorizontalBarChart3D(BarChart3D, HorizontalBarChart):
    def _cmpZ(self, a, b):
        # t, z0, z1, x, y = a[:5]
        return cmp((-a[1], a[4], a[0], -a[3]), (-b[1], b[4], b[0], -b[3]))
