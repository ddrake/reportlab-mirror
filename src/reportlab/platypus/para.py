...
class paragraphEngine:
    ...
    def format(self, maxwidth, maxheight, program, leading=0):
        ...
        #     raise ValueError(
        #         "attempt to format inside too small a height! "+repr(maxheight))
        ...
        while not done and cursor < maxcursor:
            ...
            if isinstance(opcode, str) or hasattr(opcode, 'width'):
                ...
                if newlength > maxLength and not first:  # always do at least one thing
                    # this word won't fit
                    # if debug:
                    #    print("WORD", opcode, "wont fit, width", width, "fullwidth",
                    #          fullwidth)
                ...
                if aopcode > TOOSMALLSPACE:
                    ...
                    if nextLength > maxLength and not first:
                        ...
                        # if debug:
                        #     print("EXPLICIT spacer won't fit", maxLength, nextLength,
                        #           opcode)
                        done = 1
                    else:
                        if aopcode > TOOSMALLSPACE:
                ...
                if indicator == "nextLine":
                    ...
                    # change fill color
                    # oldcolor = self.fontColor (oldcolor assigned but not used)
                    (i, colorname) = opcode
                    # print "opcode", opcode

                    # the local variable color is assigned, but not used
    ...
    def cleanProgram(self, line):
        ...
        while change:
            ...
            for index in rline:
                ...
                doswap = 0
                # don't swap visibles
                if (isinstance(this, str) or isinstance(next, str) or
                        hasattr(this, 'width') or hasattr(next, 'width')):
                    doswap = 0
                # only swap two tuples if the second one is an end operation and
                # the first is something else
                elif isinstance(this, tuple):
                    thisindicator = this[0]
                    if isinstance(next, tuple):
                        nextindicator = next[0]
                        doswap = 0
                        if (nextindicator == "endLineOperation" and
                                thisindicator != "endLineOperation" and
                                thisindicator != "lineOperation"):
    ...
    def runOpCodes(self, program, canvas, textobject):
        ...
        # escape = canvas._escape (escape set but not used)
        ...
        # thislinerightIndent = self.rightIndent (asigned but not used)
        indented = 0
        for opcode in program:
            if isinstance(opcode, str) or hasattr(opcode, 'width'):
                ...
                if indicator == "nextLine":
                    ...
                    newx = self.x = xstart
                    thislineindent = self.indent
                    # thislinerightIndent = self.rightIndent (assigned but not used)
                    indented = 0
                    for handler in self.lineOpHandlers:
                        # finish, eg, underlining this line
                        handler.end_at(x, y, self, canvas, textobject)
                        # handler.start_at(newx, newy, self, canvas, textobject))
                        # start underlining the next
                    ...
                    (i, colorname) = opcode
                    # print "opcode", opcode
                    # local variable color is assigned but not used
                    if isinstance(colorname, str):
                        color = self.fontColor = getattr(colors, colorname)
                    else:
                        # assume its something sensible :)
                    ...
                    if isinstance(fontsize, str):
                        ...
                        self.fontSize = size
                    # fontSize = self.fontSize (assigned but not used)
                    textobject.setFont(self.fontName, self.fontSize)
                elif indicator == "leading":
                    # change font leading
                    (i, leading) = opcode
                    ...
                    oldcolor = self.fontColor
                    # oldfont = self.fontName (assigned but not used)
                    # oldsize = self.fontSize (assigned but not used)
                    self.popTextState()
                    ...
                    if bulletmin > thislineindent:
                        # if debug:
                        #    print "BULLET IS BIG", bullet, bulletmin, thislineindent
...
class FastPara(Flowable):
    ...
    def wrap(self, availableWidth, availableHeight):
        ...
        style = self.style
        # text = self.simpletext (assigned but not used)
        ...
        if self.lines is None:
            ...
            while cursor < nwords and not done:
                ...
                # if not lines:
                #     thismaxlength = firstmaxlength (thismaxlength assigned not used)
                ...
                if not currentlength or nextlength < maxlength:
                    ...
                    # emit the line
                    lines.append((' '.join(currentline), currentlength,
                                  len(currentline)))
                    currentline = []
                    currentlength = 0
        ...
        # if debug:
        #    print "wrap is", (availableWidth, availableHeight), result, len(lines)
        return result
    ...
    def draw(self):
        ...
        firstindent = style.firstLineIndent
        c = self.canv
        # escape = c._escape (escape is assigned but not used)
        # if debug:
        ...
        # code = textobject._code  (code is assigned but not used)
...
class Para(Flowable):
    ...
    def __init__(self, style, parsedText=None, bulletText=None, state=None,
                 context=None, baseindent=0):
        # print id(self), "para", parsedText
        self.baseindent = baseindent
        self.context = buildContext(context)
        self.parsedText = parsedText
        self.bulletText = bulletText
        # make sure Flowable doesn't use this unless wanted! call it style1 NOT style
        self.style1 = style
    ...
    def wrap(self, availableWidth, availableHeight):
        ...
        if remainder:
            ...
            #           availableHeight, "remainder", len(remainder)
    ...
    def draw(self):
        ...
        # leading is assigned but not used
        # if state:
        #     leading = state["leading"]
        # else:
        #     leading = self.style1.leading
            ...
            if isinstance(e, str):
                ...
                if tagname == "dt":
                    ...
                    if content1:
                        ...
                        # raise ValueError, \
                        # "only simple strings supported in dd content currently:
                        #   "+repr(content1)
                elif tagname == "dd":
                    newatts = atts.copy()
                    if attdict1:
    ...
    def compile_bullet(self, attdict, content, extra, program):
        # ## eventually should allow things like images
        # and graphics in bullets too XXXX
    ...
    def compile_para(self, attdict, content, extra, program,
...
def handleSpecialCharacters(engine, text, program=None):
    ...
    amptext = text.split("&")
    first = 1
    # lastfrag = amptext[-1] (lastfrag assigned but not used)
...
def Paragraph(text, style, bulletText=None, frags=None, context=None):
    ...
    if "&" not in text and "<" not in text:
        ...
        return Para(style, parsedText=parsedpara, bulletText=bulletText,
                    state=None, context=context)
...
class HotLink(UnderLineHandler):
    ...
    def end_at(self, x, y, para, canvas, textobject):
        ...
        if debug:
            ...
            # canvas.rect(self.xStart, self.yStart, x-self.xStart,
            # y+fontsize-self.yStart, stroke=1)
...
more text and even more text and on and on and so forth
                ...
                <dl bulletFontName="Helvetica-BoldOblique"
                    spaceBefore="10" spaceAfter="10">
                <dt>frogs</dt> <dd>Little green slimy things.
                    Delicious with <b>garlic</b></dd>
                <dt>kittens</dt> <dd>cute, furry, not edible</dd>
                <dt>bunnies</dt> <dd>cute, furry,. Delicious with <b>garlic</b></dd>
                </dl>
...
<font color="red" size="15">R</font>ed letter. thisisareallylongword
andsoisthis andthisislonger
...
test_program = [
...
] + splitspace("HOTLINK This is the first segment of the first paragraph. " +
               "This is the first segment of the first paragraph. " +
               "This is the first segment of the first paragraph. " +
               "This is the first segment of the first paragraph. ") + [
    ...
    ('size', 12),
    ('leading', 14),
] + splitspace("This is the SECOND!!! segment of the first paragraph. " +
               "This is the first segment of the first paragraph. " +
               "This is the first segment of the first paragraph. " +
               "This is the first segment of the first paragraph. " +
               "This is the first segment of the first paragraph. ") + [
    ...
    ('size', 12),
    ('leading', 14),
] + splitspace("And this is the remainder of the paragraph indented further. " +
               "a a a a a a a a And this is the remainder of the paragraph " +
               "indented further. a a a a a a a a And this is the remainder " +
               "of the paragraph indented further. a a a a a a a a And this is " +
               "the remainder of the paragraph indented further. a a a a a a a a " +
               "And this is the remainder of the paragraph indented further. " +
               "a a a a a a a a And this is the remainder of the paragraph " +
               "indented further. a a a a a a a a And this is the remainder " +
               "of the paragraph indented further. a a a a a a a a ") + [
    ('pop',),
    ('nextLine', 0),]
...
def test():
    ...
    if 1:
        ...
        while remainder:
            ...
            maxwidth = 7*inch
            maxheight = 500
            (formattedprogram, remainder, laststate,
             height) = p.format(maxwidth, maxheight, remainder)
            if debug:
