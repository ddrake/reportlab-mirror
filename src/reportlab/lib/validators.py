...
class _isBoolean(Validator):
    ...
    def normalize(self, x):
        ...
        try:
            ...
            # undefined name s
            raise ValueError('Must be boolean not %s' % ascii(s))  # noqa
        if S in ('YES', 'TRUE'):
            return True
...
class OneOf(Validator):
    ...
    def __init__(self, enum, *args):
        ...
        if self._patterns:
            self._enum = tuple((_ for _ in self._enum
                                if not isinstance(_, _re_Pattern)))
...
class SequenceOf(Validator):
    def __init__(self, elemTest, name=None, emptyOK=1, NoneOK=0,
                 lo=0, hi=0x7fffffff):
...
isListOfListOfNoneOrNumber = SequenceOf(isListOfNoneOrNumber,
...
isNoneOrListOfNoneOrStrings = SequenceOf(isNoneOrString,
                                         'isNoneOrListOfNoneOrStrings', NoneOK=1)
isListOfNoneOrString = SequenceOf(isNoneOrString,
                                  'isListOfNoneOrString', NoneOK=0)
isNoneOrListOfNoneOrNumbers = SequenceOf(isNumberOrNone,
                                         'isNoneOrListOfNoneOrNumbers', NoneOK=1)
