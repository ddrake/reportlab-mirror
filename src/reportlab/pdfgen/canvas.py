...
from reportlab.lib.colors import (black, _chooseEnforceColorSpace, Color, CMYKColor,
                                  toColor)
from reportlab.lib.utils import (ImageReader, isSeq, isStr, isUnicode, _digester,
                                 asUnicode)
...
class Canvas(_PDFColorSetter):
    ...
    interiors), stroke color (for figure borders), line width and geometric transform,
    ...
    def __init__(self, filename,
        ...
        cropMarks may be True/False or an object with parameters borderWidth, markColor,
        markWidth and markLength

        if enforceColorSpace is in ('cmyk', 'rgb', 'sep','sep_black','sep_cmyk') then
        one of the standard _PDFColorSetter callables will be used to enforce
        appropriate color settings.  If it is a callable then that will be used.
        """
        if pagesize is None:
        ...
        self._initialFontName = (initialFontName if initialFontName else
                                 rl_config.canvas_basefontname)
        self._initialFontSize = (initialFontSize if initialFontSize is not None else
                                 12)
        self._initialLeading = (initialLeading if initialLeading is not None else
                                self._initialFontSize*1.2)
    ...
    def setEncrypt(self, encrypt):
        ...
        If encrypt is an instance of reportlab.lib.pdfencrypt.StandardEncryption,
        this object is used to encrypt the pdf. This allows more finegrained control
        over the encryption settings.
        '''
        if encrypt:
            ...
            if isStr(encrypt):  # encrypt is the password itself
                ...
                raise TypeError('Expected string or instance of reportlab.lib.' +
                                'pdfencrypt.StandardEncryption as encrypt parameter' +
                                'but got %r' % encrypt)
    ...
    STATE_ATTRIBUTES = """_x _y _fontname _fontsize _textMode _leading _currentMatrix
    ...
    def setKeywords(self, keywords):
        """write a list of keywords into the PDF file which shows in document
           properties.  Either submit a single string or a list/tuple"""
    ...
    def showPage(self):
        ...
        strm = (self._psCommandsBeforePage + [self._preamble] + code +
    ...
    def _setXObjects(self, thing):
        """for pages and forms, define the XObject dictionary for resources,
           if needed"""
    ...
    def bookmarkHorizontalAbsolute(self, key, top, left=0, fit='XYZ', **kw):
        """Bind a bookmark (destination) to the current page at a horizontal position.
           ...
           the page."""
        # This method should probably be deprecated since it is just
        # a sub-set of bookmarkPage
    ...
    def drawInlineImage(self, image, x, y, width=None, height=None,
                        preserveAspectRatio=False, anchor='c', anchorAtXY=False,
        ...
        img_obj.drawInlineImage(
            self, preserveAspectRatio=preserveAspectRatio, anchor=anchor,
            anchorAtXY=anchorAtXY, showBoundary=showBoundary, extraReturn=extraReturn)
    ...
    def drawImage(self, image, x, y, width=None, height=None, mask=None,
        ...
        x, y, width, height, scaled = aspectRatioFix(
            preserveAspectRatio, anchor, x, y, width, height, imgObj.width,
    ...
    def _restartAccumulators(self):
        if self._codeStack:
            ...
            self._code, self._formsinuse, self._annotationrefs, self._formData, \
                self._colorsUsed, self._shadingUsed = self._codeStack.pop(-1)
        else:
    ...
    def _pushAccumulators(self):
        """when you enter a form, save accumulator info not related to the form
           for page (if any)"""
        saved = (self._code, self._formsinuse, self._annotationrefs, self._formData,
                 self._colorsUsed, self._shadingUsed)
    ...
    def endForm(self, **extra_attributes):
        ...
        if uppery is None:
            uppery = h
        form = pdfdoc.PDFFormXObject(lowerx=lowerx, lowery=lowery, upperx=upperx,
                                     uppery=uppery)
    ...
    def freeTextAnnotation(self, contents, DA, Rect=None, addtopage=1, name=None,
                           relative=0, **kw):
        """DA is the default appearance string???"""
        Rect = self._absRect(Rect, relative)
        self._addAnnotation(pdfdoc.FreeTextAnnotation(Rect, contents, DA, **kw),
                            name, addtopage)

    def textAnnotation(self, contents, Rect=None, addtopage=1, name=None,
                       relative=0, **kw):
        """Experimental, but works.
        """
        Rect = self._absRect(Rect, relative)
        self._addAnnotation(pdfdoc.TextAnnotation(Rect, contents, **kw),
                            name, addtopage)
    textAnnotation0 = textAnnotation    # deprecated

    def highlightAnnotation(self, contents, Rect, QuadPoints=None,
                            Color=[0.83, 0.89, 0.95], addtopage=1,
                            name=None, relative=0, **kw):
        """
        Allows adding of a highlighted annotation.

        Rect: Mouseover area to show contents of annotation
        QuadPoints: List of four x/y points
        [TOP-LEFT, TOP-RIGHT, BOTTOM-LEFT, BOTTOM-RIGHT]
          ...
          Is in the format
          [x1, y1, x2, y2, x3, y3, x4, y4, x1, y1, x2, y2, x3, y3, x4, y4] etc
    ...
    def inkAnnotation(self, contents, InkList=None, Rect=None, addtopage=1,
                      name=None, relative=0, **kw):
        ...
        # undefined names h, w!
        if not InkList:
            InkList = ((100, 100, 100, h-100, w-100, h-100, w-100, 100),)  # noqa
        self._addAnnotation(pdfdoc.InkAnnotation(Rect, contents, InkList, **kw),
                            name, addtopage)
    inkAnnotation0 = inkAnnotation  # deprecated

    def linkAbsolute(self, contents, destinationname, Rect=None, addtopage=1,
        ...
        return self.linkRect(contents, destinationname, Rect, addtopage, name,
                             relative=0, thickness=thickness, color=color,
                             dashArray=dashArray, **kw)

    def linkRect(self, contents, destinationname, Rect=None, addtopage=1,
                 name=None, relative=1, thickness=0, color=None, dashArray=None, **kw):
        """rectangular link annotation w.r.t the current user transform.
           if the transform is skewed/rotated the absolute rectangle
           will use the max/min x/y
        """
    ...
    def setCropBox(self, size, name='crop'):
        """accepts a 2-tuple in points for name+'Box' size for this
           and subsequent pages"""
        name = name.lower()
    ...
    def resetTransforms(self):
        """I want to draw something (eg, string underlines)
           w.r.t. the default user space.
    ...
    def transform(self, a, b, c, d, e, f):
        ...
        if self._code and self._code[-1][-3:] == ' cm':
            ...
            # undefined name join (was str.join() intended?
            s = len(L) > 7 and join(L) + ' %s cm' or '%s cm'  # noqa
            self._code[-1] = s % fp_str(a0*a+c0*b, b0*a+d0*b, a0*c+c0*d,
                                        b0*c+d0*d, a0*e+c0*f+e0, b0*e+d0*f+f0)
        else:
            self._code.append('%s cm' % fp_str(a, b, c, d, e, f))

    def absolutePosition(self, x, y):
        """return the absolute position of x,y in user space
           w.r.t. default user space"""
    ...
    def cross(self, x, y, size=5, gap=1, text=None, strokeColor=None,
              strokeWidth=None, fontSize=3):
        size *= 0.5
        gap *= 0.5
        crosshairs = [(x, y-size, x, y-gap), (x, y+gap, x, y+size),
                      (x-size, y, x-gap, y), (x+gap, y, x+size, y)]
        self.saveState()
        if strokeColor:
    ...
    def rect(self, x, y, width, height, stroke=1, fill=0):
        """draws a rectangle with lower left corner at (x,y)
           and width and height as given."""
    ...
    def circle(self, x_cen, y_cen, r, stroke=1, fill=0):
        """draw a cirle centered at (x_cen,y_cen) with radius r
    ...
    def drawAlignedString(self, x, y, text, pivotChar=rl_config.decimalSymbol,
        ...
        if (len(parts) == 1 and digitPat.search(text) is not None
    ...
    def setFont(self, psfontname, size, leading=None):
        ...
        if not font._dynamicFont:
            if font.face.builtIn or not getattr(self, '_drawTextAsPath', False):
                ...
                self._code.append('BT %s %s Tf %s TL ET' %
    ...
    def _strokeAndFill(self, stroke, fill, fillMode=None):
        self._code.append(PATH_OPS[stroke, fill,
                                   fillMode if fillMode is not None else
                                   self._fillMode])
    ...
    def drawBoundary(self):
        if not hasattr(self, '_drawBoundary'):
            ...
            self._drawBoundary = (
                lambda sb, x, y, w, h: Frame._drawBoundary(self, sb, x, y, w, h))
