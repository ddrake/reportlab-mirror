...
__version__ = '3.3.0'
"""Testing to encrypt a very minimal pdf using a Canvas and a DocTemplate.
...
def testStdEnc(expectedO, expectedU, expectedKey,
    ...
    pdfencrypt.equalityCheck(pdfencrypt.hexText(enc.O), expectedO,
                             '%d bit O value' % strength)
    pdfencrypt.equalityCheck(pdfencrypt.hexText(enc.U), expectedU,
                             '%d bit U value' % strength)
    pdfencrypt.equalityCheck(pdfencrypt.hexText(enc.key), expectedKey,
                             '%d bit key value' % strength)
...
class EncryptTestCase(unittest.TestCase):
    ...
    def test_standardencryption(self):
        """Test generating an encrypted pdf by passing
        a StandardEncryption object to the Canvas."""
        encrypt = pdfencrypt.StandardEncryption(userPassword='User',
                                                ownerPassword='Owner')
    ...
    def test_doctemplate(self):
        """Test generating an encrypted pdf by setting
        a user password on the DocTemplate."""
    ...
    def test_standardencryption128(self):
        """Test generating an encrypted pdf by passing a
        StandardEncryption object to the Canvas."""
        encrypt = pdfencrypt.StandardEncryption(
            userPassword='User', ownerPassword='Owner', strength=128)
        encrypt.setAllPermissions(0)
        encrypt.canPrint = 1
    ...
    def test_calcs(self):
        testStdEnc(
            expectedO=('<FA7F558FACF8205D25A7F1ABFA02629F' +
                       '707AE7B0211A2BB26F5DF4C30F684301>'),
            expectedU=('<4E6F35A0356D6A624A72387D71FFB92EA' +
                       '4457E435307A9DD0BA9DE0618F5BABD>'),
            expectedKey='<DC96439AD9>',
            strength=40, overrideID='xxxxxxxxxxxxxxxx',
            userPass='User', ownerPass='Owner',
            )
        testStdEnc(
            expectedO=('<E60E73846B1C9EB09986B2C20DEAEF48' +
                       'BCC2210F75AE640655EDDFF8B67E7DD6>'),
            expectedU=('<D9E804E73FB37EDECF35176140304BD6' +
        ...
        testStdEnc(
            expectedO=('<FA7F558FACF8205D25A7F1ABFA02629F707' +
                       'AE7B0211A2BB26F5DF4C30F684301>'),
            expectedU=('<CB749297C89784B4BC94228F38B5FD43183' +
                       '98CA68F327A615A6CD8A5FCDB8C52>'),
            expectedKey='<1528D3A2F9>',
            strength=40,
            overrideID=(b'\xbf\x08\xdf\x96I\x959e\x8f\x03\xcb\xb4\x11 W\xa9'),
            userPass='User', ownerPass='Owner',
                )
        testStdEnc(
            expectedO=('<E60E73846B1C9EB09986B2C20DEAEF4' +
                       '8BCC2210F75AE640655EDDFF8B67E7DD6>'),
            expectedU=('<9A823CD6C39CEA508577319287621BF' +
