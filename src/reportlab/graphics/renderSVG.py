...
from reportlab.graphics.shapes import (STATE_DEFAULTS, Path, UserNode, numericXShift)
...
AREA_STYLES = '''stroke-width stroke-linecap stroke stroke-opacity fill fill-opacity
                 stroke-dasharray stroke-dashoffset fill-rule id'''.split()
LINE_STYLES = '''stroke-width stroke-linecap stroke stroke-opacity stroke-dasharray
                 stroke-dashoffset id'''.split()
TEXT_STYLES = 'font-family font-weight font-style font-variant font-size id'.split()
EXTRA_STROKE_STYLES = '''stroke-width stroke-linecap stroke stroke-opacity
                         stroke-dasharray stroke-dashoffset'''.split()
...
class SVGCanvas:
    def __init__(self, size=(300, 300), encoding='utf-8', verbose=0, bom=False, **kwds):
        ...
        useClip = False True means don't use a clipPath definition put the global
        clip into the clip property
                        to get around an issue with safari
        extraXmlDecl = ''   use to add extra xml declarations
        scaleGroupId = ''   id of an extra group to add around the drawing to allow
        easy scaling
    ...
    def _escape(self, s):
        '''I don't think this was ever needed; seems to have
        been copied from renderPS'''
        return s
    ...
    def _genArcCode(self, x1, y1, x2, y2, startAng, extent):
        ...
        codeline = ('matrix currentmatrix %s %s translate %s %s ' +
                    'scale 0 0 1 %s %s %s setmatrix')
    ...
    def drawString(self, s, x, y, angle=0, link_info=None, text_anchor='left',
                   textRenderMode=0, **_svgAttrs):
        if textRenderMode == 3:
            return    # invisible
        s = asNative(s)
        if self.verbose:
        ...
        if ((self._fillColor is not None and needFill) or
                (self._strokeColor is not None and needStroke)):
            if text_anchor not in ['start', 'inherited', 'left']:
    ...
    def drawCentredString(
        self, s, x, y, angle=0, text_anchor='middle',
        ...
        self.drawString(s, x, y, angle=angle, link_info=link_info,
                        text_anchor=text_anchor, textRenderMode=textRenderMode,
                        **_svgAttrs)

    def drawRightString(
        self, text, x, y, angle=0, text_anchor='end',
            link_info=None, textRenderMode=0, **_svgAttrs):
        if self.verbose:
            print("+++ SVGCanvas.drawRightString")
        self.drawString(text, x, y, angle=angle, link_info=link_info,
                        text_anchor=text_anchor, textRenderMode=textRenderMode,
                        **_svgAttrs)
    ...
    def comment(self, data):
        ...
        # comment = self.doc.createComment(data) (assigned but not used)
    ...
    def drawCurve(self, x1, y1, x2, y2, x3, y3, x4, y4, closed=0):
        ...
        return  # bug? return without doing anything?
    ...
    def polygon(self, points, closed=0, link_info=None):
        ...
        if self._strokeColor is not None or self._fillColor is not None:
...
class _SVGRenderer(Renderer):
    ...
    _restores = {'strokeColor': '_strokeColor', 'strokeWidth': '_lineWidth',
                 'strokeLineCap': '_lineCap', 'strokeLineJoin': '_lineJoin',
                 'fillColor': '_fillColor', 'fontName': '_font',
                 'fontSize': '_fontSize'}
    ...
    def _get_link_info_dict(self, obj):
        ...
        if url :
            ...
            # will show  SVG images embedded in HTML using a simple
            # <img src="..." /> tag.  However, the links don't work
            # (Safari 3.2.1 on the Mac).
            #
            # Therefore I use the following, which also works for Firefox, Opera, and
            # IE 6.0 with Adobe SVG Viewer 6 beta:
            # <object data="..." type="image/svg+xml" width="430" height="150"
            #    class="img">
            #
            ...
            #
            # Therefore I use target="_top" to force the links to replace the whole
            # page.
    ...
    def drawString(self, stringObj):
        ...
        text_anchor, x, y, text = (S['textAnchor'], stringObj.x, stringObj.y,
                                   stringObj.text)
    ...
    def drawWedge(self, wedge):
        ...
        if ((radius1 == 0 or radius1 is None) and
                (yradius1 == 0 or yradius1 is None) and not wedge.annular):
            centerx, centery, radius, startangledegrees, endangledegrees = (
             wedge.centerx, wedge.centery, wedge.radius, wedge.startangledegrees,
            ...
            self._canvas.drawArc(x1, y1, x2, y2, startangledegrees, extent,
                                 fromcenter=1)
        else:
            P = wedge.asPolygon()
