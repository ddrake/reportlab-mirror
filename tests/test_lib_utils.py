...
from reportlab.lib.utils import (recursiveImport, rl_isfile, isPyPy,
                                 TimeStamp, rl_get_module, recursiveGetAttr,
                                 recursiveSetAttr, recursiveDelAttr, asUnicode,
                                 asUnicodeEx, asBytes)
...
class ImporterTestCase(unittest.TestCase):
    ...
    def test2(self):
        ...
        if rl_isfile(fn) or rl_isfile(fn+'c') or rl_isfile(fn+'o'):
            recursiveImport('stdparser', baseDir=D)
    ...
    def test3(self):
        ...
        try:
            ...
            recursiveImport(self._testmodulename)
        finally:
            os.chdir(cwd)
    ...
    def test7(self):
        ...
        _rel_open_and_read('../docs/images/Edit_Prefs.gif')

    def test8(self):
        "test open and read of a relative file: URL"
        _rel_open_and_read('file:../docs/images/Edit_Prefs.gif')

    @patch('reportlab.lib.utils.rlUrlRead', mockUrlRead)
    def test9(self):
        "test open and read of an http: URL"
        from reportlab.lib.utils import open_and_read
        open_and_read('http://www.reportlab.com/rsrc/encryption.gif')
    ...
    def test11(self):
        ...
        result = _rel_open_and_read(
            'data:image/gif;base64,R0lGODdhAQABAIAAAP' +
            '///////ywAAAAAAQABAAACAkQBADs=')
        self.assertEqual(
            result,
    ...
    def testRecursiveImportErrors(self):
        ...
        try:
            ...
            recursiveImport('unimportable')
            self.fail("Imported a buggy module")
        except Exception as e:
            self.assertIn(
                ("integer division by zeroException raised while importing " +
                 "'unimportable': integer division by zero"
                 if isPyPy else 'division by zero'), str(e))
    ...
    def test17(self):
        ...
        self.myAssertRaisesRegex(
            AttributeError,
            r"asUnicode\(.*'list' object has no attribute 'decode'",
            asUnicode, ['abc'])
        ...
        self.myAssertRaisesRegex(
            AttributeError,
            r"asBytes\(.*'list' object has no attribute 'encode'",
            asBytes, ['abc'])
...
class RaccessTestCase(unittest.TestCase):
    ...
    def test_person_example(self):
        ...
        self.assertEqual(recursiveGetAttr(jack, 'friends[-1].settings["themes"][1]'),
                         'dark')

        # Setattr
        # recursiveSetAttr(bob, 'settings["style"]["width"]', 400)
        # self.assertEqual(recursiveGetAttr(
        #                  bob,
        #                  'settings["style"]["width"]') ,400)
