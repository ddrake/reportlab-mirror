__all__ = (
        'setFont',
        'pathNumTrunc',
        # 'processGlyph', (undefined name processGlyph)
        #   defined as nested function in text2PathDescription; not in module scope
        # 'text2PathDescription', (undefined name text2PathDescription)
        #   defined as nested function in __makeTextPathsCode__; not in module scope
        'text2Path',
        'RenderPMError',
        )
...
def __makeTextPathsCode__(tp=None, _TP=('freetype', '_renderPM')):
    ...
    if tp is not None:
        # redefinition of unused textPaths
        textPaths = tp  # noqa
    if textPaths == 'backend':
        ...
        raise ValueError(
            f"textPaths={textPaths!r} should be one of 'backend', " +
            "'freetype' or '_renderPM')")
    ...
    for tp in TP:
        if tp == 'freetype':
            ...
            class FTTextPath:
                ftLFlags = (freetype.FT_LOAD_DEFAULT | freetype.FT_LOAD_NO_SCALE |
                ...
                def _text2Path(self, text, x=0, y=0, fontName=_baseGFontName,
                               fontSize=1000, **kwds):
                    ...
                    if truncate:
                        def xpt(x):
                            return pathNumTrunc(scale*(x+__dx__))

                        def ypt(y):
                            return pathNumTrunc(scale*(y+__dy__))
                    else:
                        def xpt(x):
                            scale*(x + __dx__)
                    ...
                    def conic_to(a, b, ctx):
                        ...
                        x2 = xpt(b.x)
                        y2 = ypt(b.y)
                        P_append(('curveTo', x0+((x1-x0)*2)/3, y0+((y1-y0)*2)/3,
                                  x1+(x2-x1)/3, y1+(y2-y1)/3, x2, y2))

                    def cubic_to(a, b, c, ctx):
                        P_append(('curveTo', xpt(a.x), ypt(a.y), xpt(b.x), ypt(b.y),
                                  xpt(c.x), ypt(c.y)))
                    ...
                    for c in text:
                        ...
                        face.glyph.outline.decompose(
                            self, move_to=move_to, line_to=line_to, conic_to=conic_to,
                            cubic_to=cubic_to)
            ...
            def text2PathDescription(text, x=0, y=0, fontName=_baseGFontName,
                                     fontSize=1000, anchor='start', truncate=1,
                                     pathReverse=0, gs=None):
                ...
                if gs is None:
                    gs = FTTextPath()
                if font._dynamicFont:
                    P_extend(gs._text2Path(text, x=x, y=y, fontName=fontName,
                                           fontSize=fontSize, truncate=truncate,
                    ...
                    if isBytes(text):
                        try:
                            ...
                            raise UnicodeDecodeError(
                                *(e.args[:4] +
                                  ('%s\n%s-->%s<--%s' %
                                   (e.args[4], text[max(i-10, 0):i], text[i:j],
                                    text[j:j+10]),)))
                    ...
                    for i, (f, t) in enumerate(FT):
                        ...
                        P_extend(gs._text2Path(t, x=x, y=y, fontName=f.fontName,
                                               fontSize=fontSize, truncate=truncate,
                                               pathReverse=pathReverse))
                        if i != nm1:
                            x += f.stringWidth(t, fontSize)
                return P_extend.__self__
            return dict(text2PathDescription=text2PathDescription,
                        FTTextPath=FTTextPath)
        elif tp == '_renderPM':
            try:
                import _rl_renderPM  # noqa
            except ImportError:
                continue
            ...
            def text2PathDescription(text, x=0, y=0, fontName=_baseGFontName,
                ...
                if font._dynamicFont:
                    for g in gs._stringPath(text, x, y):
                        P_extend(processGlyph(g, truncate=truncate,
                                              pathReverse=pathReverse))
                else:
                    if isBytes(text):
                        try:
                ...
                return P_extend.__self__
            return dict(text2PathDescription=text2PathDescription,
                        processGlyph=processGlyph, setFont=setFont)
    else:
        def _(*args, **kwds):
            raise RuntimeError(f'''This installation of reportLab has neither
            RLPYCAIRO or RLRENDERPM extras installed.  It cannot create paths
            from text.  Could not create text2PathDescription for using backends
            from {TP!a}''')
...
def text2Path(text, x=0, y=0, fontName=_baseGFontName, fontSize=1000,
    ...
    t2pd = kwds.pop('text2PathDescription')
    return definePath(t2pd(text, x=x, y=y, fontName=fontName, fontSize=fontSize,
                           anchor=anchor, truncate=truncate, pathReverse=pathReverse,
                           gs=gs), **kwds)
