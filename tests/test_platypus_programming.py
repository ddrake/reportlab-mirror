...
"""Tests low level programming of doc templates
"""
...
class PlatypusProgrammingTestCase(unittest.TestCase):
    ...
    def test0(self):
        ...
        def func(val):
            story = [
                    ...
                    DocPara("'val=%r'" % val),
                    DocPara('"{"+", ".join(("%s=%s" % (_k,(_v.__class__.__name__ ' +
                            'if "<" in repr(_v) else repr(_v)[1:] if repr(_v) and ' +
                            'repr(_v)[0] in "ub" else repr(_v))) for _k,_v in ' +
                            'sorted(doc._nameSpace.items()) if _k not in ' +
                            '("_k","_v")))+"}"', escape=True),
                    ]
    ...
    def test1(self):
        ...
        #                         leading=19, spaceAfter=6, keepWithNext=1)
        story = [
                ...
                DocWhile('i', [DocPara('i', format='The value of i is %(__expr__)d',
                                       style=normal), DocExec('i-=1')]),
                DocPara('"{"+", ".join(("%s=%s" % (_k,(_v.__class__.__name__ if ' +
                        '"<" in repr(_v) else repr(_v)[1:] if repr(_v) and ' +
                        'repr(_v)[0] in "ub" else repr(_v))) for _k,_v in sorted' +
                        '(doc._nameSpace.items()) if _k not in ("_k","_v")))+"}"',
                        escape=True),
                DocPara('doc.canv.getPageNumber()',
                        'The current page number is %(__expr__)d', style=normal)
                ]
        doc = SimpleDocTemplate(outputfile('test_doc_programming.pdf'))
        doc.build(story)

    def test2(self):
        """This makes one long multi-page paragraph in multi-pass
        for testing docWhile etc etc"""
        from reportlab.platypus.flowables import (DocAssign, DocExec, DocPara,
                                                  DocIf, DocWhile)
        ...
        for i in range(10):
            story.append(Paragraph(
                'Chapter %d: Chapters always starts a new page' % chapterNum, h1))
            ...
            for j in range(3):
                ...
                for j in range(3):
                    story.append(Paragraph(randomText(
                        theme=PYTHON, sentences=2)+' (%d)' % len(story), bt))
                    story.append(Paragraph(
            ...
            story.extend([
                    ...
                    DocWhile('i', [DocPara(
                        'i', format='The value of i is %(__expr__)d', style=bt),
                                  DocExec('i-=1')]),
                    DocPara('"{"+", ".join(("%s=%s" % (_k,(_v.__class__.__name__ ' +
                            'if "<" in repr(_v) else repr(_v)[1:] if repr(_v) and ' +
                            'repr(_v)[0] in "ub" else repr(_v))) for _k,_v in ' +
                            'sorted(doc._nameSpace.items()) ' +
                            'if _k not in ("_k","_v")))+"}"', escape=True),
                    DocPara('doc.canv.getPageNumber()',
