...
def fontSizeNormalize(frag, attr, default):
    ...
    return ((v[1]+frag.fontSize) if isinstance(v, tuple) else
            v.normalizedValue(frag.fontSize) if isinstance(v, _PCT) else
            v)
...
class _ExValidate:
    ...
    def invalid(self, s):
        raise ValueError('<%s> invalid value %r for attribute %s' %
                         (self.tag, s, self.attr))
...
def _valignpc(s):
    s = s.lower()
    if s in ('baseline', 'sub', 'super', 'top', 'text-top', 'middle', 'bottom',
             'text-bottom'):
        return s
    if s.endswith('%'):
        n = _convnum(s[:-1])
...
def _bAnchor(s):
    s = s.lower()
    if s not in ('start', 'middle', 'end', 'numeric'):
...
_paraAttrMap = {'font': ('fontName', None),
                ...
                'underlinewidth': ('underlineWidth',
                                   _CheckUS('para', 'underlineWidth')),
                'underlinecolor': ('underlineColor', toColor),
                'underlineoffset': ('underlineOffset',
                                    _CheckUS('para', 'underlineOffset')),
...
greeks = {
    ...
    'acd': u'\u223f',                             # SINE WAVE
    'acE': u'\u223e\u0333',                       # INVERTED LAZY S with double
                                                  # underline
    'Acirc': u'\xc2',                             # LATIN CAPITAL LETTER A WITH
                                                  # CIRCUMFLEX
    ...
    'angmsdaa': u'\u29a8',                        # MEASURED ANGLE WITH OPEN ARM ENDING
                                                  # IN ARROW POINTING UP AND RIGHT
    'angmsdab': u'\u29a9',                        # MEASURED ANGLE WITH OPEN ARM ENDING
                                                  # IN ARROW POINTING UP AND LEFT
    'angmsdac': u'\u29aa',                        # MEASURED ANGLE WITH OPEN ARM ENDING
                                                  # IN ARROW POINTING DOWN AND RIGHT
    'angmsdad': u'\u29ab',                        # MEASURED ANGLE WITH OPEN ARM ENDING
                                                  # IN ARROW POINTING DOWN AND LEFT
    'angmsdae': u'\u29ac',                        # MEASURED ANGLE WITH OPEN ARM ENDING
                                                  # IN ARROW POINTING RIGHT AND UP
    'angmsdaf': u'\u29ad',                        # MEASURED ANGLE WITH OPEN ARM ENDING
                                                  # IN ARROW POINTING LEFT AND UP
    'angmsdag': u'\u29ae',                        # MEASURED ANGLE WITH OPEN ARM ENDING
                                                  # IN ARROW POINTING RIGHT AND DOWN
    'angmsdah': u'\u29af',                        # MEASURED ANGLE WITH OPEN ARM ENDING
                                                  # IN ARROW POINTING LEFT AND DOWN
    'angrt': u'\u221f',                           # RIGHT ANGLE
    'angrtvb': u'\u22be',                         # RIGHT ANGLE WITH ARC
    'angrtvbd': u'\u299d',                        # MEASURED RIGHT ANGLE WITH DOT
    'angsph': u'\u2222',                          # SPHERICAL ANGLE
    'angst': u'\xc5',                             # LATIN CAPITAL LETTER A WITH RING
                                                  # ABOVE
    'angzarr': u'\u237c',                         # RIGHT ANGLE WITH DOWNWARDS ZIGZAG
                                                  # ARROW
    ...
    'apacir': u'\u2a6f',                          # ALMOST EQUAL TO WITH CIRCUMFLEX
                                                  # ACCENT
    ...
    'Aring': u'\xc5',                             # LATIN CAPITAL LETTER A WITH RING
                                                  # ABOVE
    ...
    'Auml': u'\xc4',                              # LATIN CAPITAL LETTER A WITH
                                                  # DIAERESIS
    ...
    'bbrktbrk': u'\u23b6',                        # BOTTOM SQUARE BRACKET OVER TOP
                                                  # SQUARE BRACKET
    ...
    'boxDl': u'\u2556',                           # BOX DRAWINGS DOWN DOUBLE AND LEFT
                                                  # SINGLE
    'boxdL': u'\u2555',                           # BOX DRAWINGS DOWN SINGLE AND LEFT
                                                  # DOUBLE
    'boxdl': u'\u2510',                           # BOX DRAWINGS LIGHT DOWN AND LEFT
    'boxDR': u'\u2554',                           # BOX DRAWINGS DOUBLE DOWN AND RIGHT
    'boxDr': u'\u2553',                           # BOX DRAWINGS DOWN DOUBLE AND RIGHT
                                                  # SINGLE
    'boxdR': u'\u2552',                           # BOX DRAWINGS DOWN SINGLE AND RIGHT
                                                  # DOUBLE
    'boxdr': u'\u250c',                           # BOX DRAWINGS LIGHT DOWN AND RIGHT
    'boxH': u'\u2550',                            # BOX DRAWINGS DOUBLE HORIZONTAL
    'boxh': u'\u2500',                            # BOX DRAWINGS LIGHT HORIZONTAL
    'boxHD': u'\u2566',                           # BOX DRAWINGS DOUBLE DOWN AND
                                                  # HORIZONTAL
    'boxHd': u'\u2564',                           # BOX DRAWINGS DOWN SINGLE AND
                                                  # HORIZONTAL DOUBLE
    'boxhD': u'\u2565',                           # BOX DRAWINGS DOWN DOUBLE AND
                                                  # HORIZONTAL SINGLE
    'boxhd': u'\u252c',                           # BOX DRAWINGS LIGHT DOWN AND
                                                  #  HORIZONTAL
    'boxHU': u'\u2569',                           # BOX DRAWINGS DOUBLE UP AND
                                                  # HORIZONTAL
    'boxHu': u'\u2567',                           # BOX DRAWINGS UP SINGLE AND
                                                  # HORIZONTAL DOUBLE
    'boxhU': u'\u2568',                           # BOX DRAWINGS UP DOUBLE AND
                                                  # HORIZONTAL SINGLE
    ...
    'boxUl': u'\u255c',                           # BOX DRAWINGS UP DOUBLE AND LEFT
                                                  # SINGLE
    'boxuL': u'\u255b',                           # BOX DRAWINGS UP SINGLE AND LEFT
                                                  # DOUBLE
    'boxul': u'\u2518',                           # BOX DRAWINGS LIGHT UP AND LEFT
    'boxUR': u'\u255a',                           # BOX DRAWINGS DOUBLE UP AND RIGHT
    'boxUr': u'\u2559',                           # BOX DRAWINGS UP DOUBLE AND RIGHT
                                                  # SINGLE
    'boxuR': u'\u2558',                           # BOX DRAWINGS UP SINGLE AND RIGHT
                                                  # DOUBLE
    'boxur': u'\u2514',                           # BOX DRAWINGS LIGHT UP AND RIGHT
    'boxV': u'\u2551',                            # BOX DRAWINGS DOUBLE VERTICAL
    'boxv': u'\u2502',                            # BOX DRAWINGS LIGHT VERTICAL
    'boxVH': u'\u256c',                           # BOX DRAWINGS DOUBLE VERTICAL AND
                                                  # HORIZONTAL
    'boxVh': u'\u256b',                           # BOX DRAWINGS VERTICAL DOUBLE AND
                                                  # HORIZONTAL SINGLE
    'boxvH': u'\u256a',                           # BOX DRAWINGS VERTICAL SINGLE AND
                                                  # HORIZONTAL DOUBLE
    'boxvh': u'\u253c',                           # BOX DRAWINGS LIGHT VERTICAL AND
                                                  # HORIZONTAL
    'boxVL': u'\u2563',                           # BOX DRAWINGS DOUBLE VERTICAL AND
                                                  # LEFT
    'boxVl': u'\u2562',                           # BOX DRAWINGS VERTICAL DOUBLE AND
                                                  # LEFT SINGLE
    'boxvL': u'\u2561',                           # BOX DRAWINGS VERTICAL SINGLE AND
                                                  # LEFT DOUBLE
    'boxvl': u'\u2524',                           # BOX DRAWINGS LIGHT VERTICAL AND LEFT
    'boxVR': u'\u2560',                           # BOX DRAWINGS DOUBLE VERTICAL AND
                                                  # RIGHT
    'boxVr': u'\u255f',                           # BOX DRAWINGS VERTICAL DOUBLE AND
                                                  # RIGHT SINGLE
    'boxvR': u'\u255e',                           # BOX DRAWINGS VERTICAL SINGLE AND
                                                  # RIGHT DOUBLE
    'boxvr': u'\u251c',                           # BOX DRAWINGS LIGHT VERTICAL AND
                                                  # RIGHT
    ...
    'capcap': u'\u2a4b',                          # INTERSECTION BESIDE AND JOINED
                                                  # WITH INTERSECTION
    ...
    'Ccirc': u'\u0108',                           # LATIN CAPITAL LETTER C WITH
                                                  # CIRCUMFLEX
    'ccirc': u'\u0109',                           # LATIN SMALL LETTER C WITH CIRCUMFLEX
    'Cconint': u'\u2230',                         # VOLUME INTEGRAL
    'ccups': u'\u2a4c',                           # CLOSED UNION WITH SERIFS
    'ccupssm': u'\u2a50',                         # CLOSED UNION WITH SERIFS AND SMASH
                                                  # PRODUCT
    'Cdot': u'\u010a',                            # LATIN CAPITAL LETTER C WITH DOT
                                                  # ABOVE
    ...
    'crarr': u'\u21b5',                           # DOWNWARDS ARROW WITH CORNER
                                                  # LEFTWARDS
    ...
    'ddotseq': u'\u2a77',                         # EQUALS SIGN WITH TWO DOTS ABOVE
                                                  # AND TWO DOTS BELOW
    ...
    'dHar': u'\u2965',                            # DOWNWARDS HARPOON WITH BARB LEFT
                                                  # BESIDE DOWNWARDS HARPOON WITH BARB
                                                  # RIGHT
    'dharl': u'\u21c3',                           # DOWNWARDS HARPOON WITH BARB
                                                  # LEFTWARDS
    'dharr': u'\u21c2',                           # DOWNWARDS HARPOON WITH BARB
                                                  # RIGHTWARDS
    ...
    'disin': u'\u22f2',                           # ELEMENT OF WITH LONG HORIZONTAL
                                                  # STROKE
    ...
    'DownArrowUpArrow': u'\u21f5',                # DOWNWARDS ARROW LEFTWARDS OF UPWARDS
                                                  # ARROW
    'downdownarrows': u'\u21ca',                  # DOWNWARDS PAIRED ARROWS
    'downharpoonleft': u'\u21c3',                 # DOWNWARDS HARPOON WITH BARB
                                                  # LEFTWARDS
    'downharpoonright': u'\u21c2',                # DOWNWARDS HARPOON WITH BARB
                                                  # RIGHTWARDS
    'DownLeftRightVector': u'\u2950',             # LEFT BARB DOWN RIGHT BARB DOWN
                                                  # HARPOON
    'DownLeftTeeVector': u'\u295e',               # LEFTWARDS HARPOON WITH BARB DOWN
                                                  # FROM BAR
    'DownLeftVector': u'\u21bd',                  # LEFTWARDS HARPOON WITH BARB
                                                  # DOWNWARDS
    'DownLeftVectorBar': u'\u2956',               # LEFTWARDS HARPOON WITH BARB DOWN
                                                  # TO BAR
    'DownRightTeeVector': u'\u295f',              # RIGHTWARDS HARPOON WITH BARB DOWN
                                                  # FROM BAR
    'DownRightVector': u'\u21c1',                 # RIGHTWARDS HARPOON WITH BARB
                                                  # DOWNWARDS
    'DownRightVectorBar': u'\u2957',              # RIGHTWARDS HARPOON WITH BARB
                                                  # DOWN TO BAR
    'DownTee': u'\u22a4',                         # DOWN TACK
    'DownTeeArrow': u'\u21a7',                    # DOWNWARDS ARROW FROM BAR
    'drbkarow': u'\u2910',                        # RIGHTWARDS TWO-HEADED TRIPLE DASH
                                                  # ARROW
    ...
    'duarr': u'\u21f5',                           # DOWNWARDS ARROW LEFTWARDS OF UPWARDS
                                                  # ARROW
    'duhar': u'\u296f',                           # DOWNWARDS HARPOON WITH BARB LEFT
                                                  # BESIDE UPWARDS HARPOON WITH BARB
                                                  # RIGHT
    ...
    'Ecirc': u'\xca',                             # LATIN CAPITAL LETTER E WITH
                                                  # CIRCUMFLEX
    ...
    'eDDot': u'\u2a77',                           # EQUALS SIGN WITH TWO DOTS ABOVE AND
                                                  # TWO DOTS BELOW
    'Edot': u'\u0116',                            # LATIN CAPITAL LETTER E WITH DOT
                                                  # ABOVE
    'eDot': u'\u2251',                            # GEOMETRICALLY EQUAL TO
    'edot': u'\u0117',                            # LATIN SMALL LETTER E WITH DOT ABOVE
    'ee': u'\u2147',                              # DOUBLE-STRUCK ITALIC SMALL E
    'efDot': u'\u2252',                           # APPROXIMATELY EQUAL TO OR THE IMAGE
                                                  # OF
    ...
    'egsdot': u'\u2a98',                          # SLANTED EQUAL TO OR GREATER-THAN
                                                  # WITH DOT INSIDE
    ...
    'elsdot': u'\u2a97',                          # SLANTED EQUAL TO OR LESS-THAN WITH
                                                  # DOT INSIDE
    ...
    'Equilibrium': u'\u21cc',                     # RIGHTWARDS HARPOON OVER LEFTWARDS
                                                  # HARPOON
    ...
    'Euml': u'\xcb',                              # LATIN CAPITAL LETTER E WITH
                                                  # DIAERESIS
    ...
    'exponentiale': u'\u2147',                    # DOUBLE-STRUCK ITALIC SMALL E
    'fallingdotseq': u'\u2252',                   # APPROXIMATELY EQUAL TO OR THE
                                                  # IMAGE OF
    ...
    'gescc': u'\u2aa9',                           # GREATER-THAN CLOSED BY CURVE ABOVE
                                                  # SLANTED EQUAL
    'gesdot': u'\u2a80',                          # GREATER-THAN OR SLANTED EQUAL TO
                                                  # WITH DOT INSIDE
    'gesdoto': u'\u2a82',                         # GREATER-THAN OR SLANTED EQUAL TO
                                                  # WITH DOT ABOVE
    'gesdotol': u'\u2a84',                        # GREATER-THAN OR SLANTED EQUAL TO
                                                  # WITH DOT ABOVE LEFT
    'gesl': u'\u22db\ufe00',                      # GREATER-THAN slanted EQUAL TO OR
                                                  # LESS-THAN
    'gesles': u'\u2a94',                          # GREATER-THAN ABOVE SLANTED EQUAL
                                                  # ABOVE LESS-THAN ABOVE SLANTED EQUAL
    ...
    'glE': u'\u2a92',                             # GREATER-THAN ABOVE LESS-THAN ABOVE
                                                  # DOUBLE-LINE EQUAL
    ...
    'gne': u'\u2a88',                             # GREATER-THAN AND SINGLE-LINE NOT
                                                  # EQUAL TO
    'gneq': u'\u2a88',                            # GREATER-THAN AND SINGLE-LINE NOT
                                                  # EQUAL TO
    ...
    'gsiml': u'\u2a90',                           # GREATER-THAN ABOVE SIMILAR ABOVE
                                                  # LESS-THAN
    ...
    'gtquest': u'\u2a7c',                         # GREATER-THAN WITH QUESTION MARK
                                                  # ABOVE
    ...
    'gtreqqless': u'\u2a8c',                      # GREATER-THAN ABOVE DOUBLE-LINE
                                                  # EQUAL ABOVE LESS-THAN
    'gtrless': u'\u2277',                         # GREATER-THAN OR LESS-THAN
    'gtrsim': u'\u2273',                          # GREATER-THAN OR EQUIVALENT TO
    'gvertneqq': u'\u2269\ufe00',                 # GREATER-THAN BUT NOT EQUAL TO
                                                  # - with vertical stroke
    'gvnE': u'\u2269\ufe00',                      # GREATER-THAN BUT NOT EQUAL TO
                                                  # - with vertical stroke
    'Hacek': u'\u02c7',                           # CARON
    'hairsp': u'\u200a',                          # HAIR SPACE
    ...
    'intlarhk': u'\u2a17',                        # INTEGRAL WITH LEFTWARDS ARROW
                                                  # WITH HOOK
    ...
    'isinE': u'\u22f9',                           # ELEMENT OF WITH TWO HORIZONTAL
                                                  # STROKES
    'isins': u'\u22f4',                           # SMALL ELEMENT OF WITH VERTICAL BAR
                                                  # AT END OF HORIZONTAL STROKE
    'isinsv': u'\u22f3',                          # ELEMENT OF WITH VERTICAL BAR AT END
    ...
    'Iukcy': u'\u0406',                           # CYRILLIC CAPITAL LETTER
                                                  # BYELORUSSIAN-UKRAINIAN I
    'iukcy': u'\u0456',                           # CYRILLIC SMALL LETTER
                                                  # BYELORUSSIAN-UKRAINIAN I
    'Iuml': u'\xcf',                              # LATIN CAPITAL LETTER I WITH
                                                  # DIAERESIS
    'iuml': u'\xef',                              # LATIN SMALL LETTER I WITH DIAERESIS
    'Jcirc': u'\u0134',                           # LATIN CAPITAL LETTER J WITH
                                                  # CIRCUMFLEX
    ...
    'Lang': u'\u27ea',                            # MATHEMATICAL LEFT DOUBLE ANGLE
                                                  # BRACKET
    ...
    'lbbrk': u'\u2772',                           # LIGHT LEFT TORTOISE SHELL BRACKET
                                                  # ORNAMENT
    'lbrace': u'{',                               # LEFT CURLY BRACKET
    'lbrack': u'[',                               # LEFT SQUARE BRACKET
    'lbrke': u'\u298b',                           # LEFT SQUARE BRACKET WITH UNDERBAR
    'lbrksld': u'\u298f',                         # LEFT SQUARE BRACKET WITH TICK IN
                                                  # BOTTOM CORNER
    'lbrkslu': u'\u298d',                         # LEFT SQUARE BRACKET WITH TICK IN
                                                  # TOP CORNER
    ...
    'ldca': u'\u2936',                            # ARROW POINTING DOWNWARDS THEN
                                                  # CURVING LEFTWARDS
    'ldquo': u'\u201c',                           # LEFT DOUBLE QUOTATION MARK
    'ldquor': u'\u201e',                          # DOUBLE LOW-9 QUOTATION MARK
    'ldrdhar': u'\u2967',                         # LEFTWARDS HARPOON WITH BARB DOWN
                                                  # ABOVE RIGHTWARDS HARPOON WITH BARB
                                                  # DOWN
    ...
    'LeftArrowRightArrow': u'\u21c6',             # LEFTWARDS ARROW OVER RIGHTWARDS
                                                  # ARROW
    'leftarrowtail': u'\u21a2',                   # LEFTWARDS ARROW WITH TAIL
    'LeftCeiling': u'\u2308',                     # LEFT CEILING
    'LeftDoubleBracket': u'\u27e6',               # MATHEMATICAL LEFT WHITE SQUARE
                                                  # BRACKET
    'LeftDownTeeVector': u'\u2961',               # DOWNWARDS HARPOON WITH BARB LEFT
                                                  # FROM BAR
    'LeftDownVector': u'\u21c3',                  # DOWNWARDS HARPOON WITH BARB
                                                  # LEFTWARDS
    'LeftDownVectorBar': u'\u2959',               # DOWNWARDS HARPOON WITH BARB LEFT
                                                  # TO BAR
    'LeftFloor': u'\u230a',                       # LEFT FLOOR
    'leftharpoondown': u'\u21bd',                 # LEFTWARDS HARPOON WITH BARB
                                                  # DOWNWARDS
    ...
    'leftrightarrow': u'\u2194',                  # LEFT RIGHT ARROW
    'leftrightarrows': u'\u21c6',                 # LEFTWARDS ARROW OVER RIGHTWARDS
                                                  # ARROW
    'leftrightharpoons': u'\u21cb',               # LEFTWARDS HARPOON OVER RIGHTWARDS
                                                  # HARPOON
    ...
    'LeftTeeVector': u'\u295a',                   # LEFTWARDS HARPOON WITH BARB UP FROM
                                                  # BAR
    ...
    'LeftUpTeeVector': u'\u2960',                 # UPWARDS HARPOON WITH BARB LEFT FROM
                                                  # BAR
    'LeftUpVector': u'\u21bf',                    # UPWARDS HARPOON WITH BARB LEFTWARDS
    'LeftUpVectorBar': u'\u2958',                 # UPWARDS HARPOON WITH BARB LEFT TO
                                                  # BAR
    'LeftVector': u'\u21bc',                      # LEFTWARDS HARPOON WITH BARB UPWARDS
    'LeftVectorBar': u'\u2952',                   # LEFTWARDS HARPOON WITH BARB UP
                                                  # TO BAR
    'lEg': u'\u2a8b',                             # LESS-THAN ABOVE DOUBLE-LINE EQUAL
                                                  # ABOVE GREATER-THAN
    ...
    'lescc': u'\u2aa8',                           # LESS-THAN CLOSED BY CURVE ABOVE
                                                  # SLANTED EQUAL
    'lesdot': u'\u2a7f',                          # LESS-THAN OR SLANTED EQUAL TO WITH
                                                  # DOT INSIDE
    'lesdoto': u'\u2a81',                         # LESS-THAN OR SLANTED EQUAL TO WITH
                                                  # DOT ABOVE
    'lesdotor': u'\u2a83',                        # LESS-THAN OR SLANTED EQUAL TO WITH
                                                  # DOT ABOVE RIGHT
    'lesg': u'\u22da\ufe00',                      # LESS-THAN slanted EQUAL TO OR
                                                  # GREATER-THAN
    'lesges': u'\u2a93',                          # LESS-THAN ABOVE SLANTED EQUAL ABOVE
    ...
    'lesseqqgtr': u'\u2a8b',                      # LESS-THAN ABOVE DOUBLE-LINE EQUAL
                                                  # ABOVE GREATER-THAN
    ...
    'lgE': u'\u2a91',                             # LESS-THAN ABOVE GREATER-THAN ABOVE
                                                  # DOUBLE-LINE EQUAL
    'lHar': u'\u2962',                            # LEFTWARDS HARPOON WITH BARB UP
                                                  # ABOVE LEFTWARDS HARPOON WITH BARB
                                                  # DOWN
    'lhard': u'\u21bd',                           # LEFTWARDS HARPOON WITH BARB
                                                  # DOWNWARDS
    'lharu': u'\u21bc',                           # LEFTWARDS HARPOON WITH BARB UPWARDS
    'lharul': u'\u296a',                          # LEFTWARDS HARPOON WITH BARB UP
                                                  # ABOVE LONG DASH
    ...
    'llhard': u'\u296b',                          # LEFTWARDS HARPOON WITH BARB DOWN
                                                  # BELOW LONG DASH
    'lltri': u'\u25fa',                           # LOWER LEFT TRIANGLE
    'Lmidot': u'\u013f',                          # LATIN CAPITAL LETTER L WITH MIDDLE
                                                  # DOT
    'lmidot': u'\u0140',                          # LATIN SMALL LETTER L WITH MIDDLE DOT
    'lmoust': u'\u23b0',                          # UPPER LEFT OR LOWER RIGHT CURLY
                                                  # BRACKET SECTION
    'lmoustache': u'\u23b0',                      # UPPER LEFT OR LOWER RIGHT CURLY
                                                  # BRACKET SECTION
    'lnap': u'\u2a89',                            # LESS-THAN AND NOT APPROXIMATE
    'lnapprox': u'\u2a89',                        # LESS-THAN AND NOT APPROXIMATE
    'lnE': u'\u2268',                             # LESS-THAN BUT NOT EQUAL TO
    'lne': u'\u2a87',                             # LESS-THAN AND SINGLE-LINE NOT
                                                  # EQUAL TO
    'lneq': u'\u2a87',                            # LESS-THAN AND SINGLE-LINE NOT
                                                  # EQUAL TO
    'lneqq': u'\u2268',                           # LESS-THAN BUT NOT EQUAL TO
    'lnsim': u'\u22e6',                           # LESS-THAN BUT NOT EQUIVALENT TO
    'loang': u'\u27ec',                           # MATHEMATICAL LEFT WHITE TORTOISE
                                                  # SHELL BRACKET
    'loarr': u'\u21fd',                           # LEFTWARDS OPEN-HEADED ARROW
    'lobrk': u'\u27e6',                           # MATHEMATICAL LEFT WHITE SQUARE
                                                  # BRACKET
    ...
    'lotimes': u'\u2a34',                         # MULTIPLICATION SIGN IN LEFT HALF
                                                  # CIRCLE
    ...
    'lrarr': u'\u21c6',                           # LEFTWARDS ARROW OVER RIGHTWARDS
                                                  # ARROW
    'lrcorner': u'\u231f',                        # BOTTOM RIGHT CORNER
    'lrhar': u'\u21cb',                           # LEFTWARDS HARPOON OVER RIGHTWARDS
                                                  # HARPOON
    'lrhard': u'\u296d',                          # RIGHTWARDS HARPOON WITH BARB DOWN
                                                  # BELOW LONG DASH
    'lrm': u'\u200e',                             # LEFT-TO-RIGHT MARK
    'lrtri': u'\u22bf',                           # RIGHT TRIANGLE
    'lsaquo': u'\u2039',                          # SINGLE LEFT-POINTING ANGLE
                                                  # QUOTATION MARK
    ...
    'lsimg': u'\u2a8f',                           # LESS-THAN ABOVE SIMILAR ABOVE
                                                  # GREATER-THAN
    ...
    'ltimes': u'\u22c9',                          # LEFT NORMAL FACTOR SEMIDIRECT
                                                  # PRODUCT
    ...
    'luruhar': u'\u2966',                         # LEFTWARDS HARPOON WITH BARB UP
                                                  # ABOVE RIGHTWARDS HARPOON WITH
                                                  # BARB UP
    'lvertneqq': u'\u2268\ufe00',                 # LESS-THAN BUT NOT EQUAL TO
                                                  # - with vertical stroke
    'lvnE': u'\u2268\ufe00',                      # LESS-THAN BUT NOT EQUAL TO
                                                  # - with vertical stroke
    'macr': u'\xaf',                              # MACRON
    'male': u'\u2642',                            # MALE SIGN
    ...
    'napE': u'\u2a70\u0338',                      # APPROXIMATELY EQUAL OR EQUAL TO
                                                  # with slash
    'napid': u'\u224b\u0338',                     # TRIPLE TILDE with slash
    'napos': u'\u0149',                           # LATIN SMALL LETTER N PRECEDED BY
                                                  # APOSTROPHE
    ...
    'nbump': u'\u224e\u0338',                     # GEOMETRICALLY EQUIVALENT TO with
                                                  # slash
    ...
    'ncong': u'\u2247',                           # NEITHER APPROXIMATELY NOR ACTUALLY
                                                  # EQUAL TO
    ...
    'nesear': u'\u2928',                          # NORTH EAST ARROW AND SOUTH EAST
                                                  # ARROW
    ...
    'ngE': u'\u2267\u0338',                       # GREATER-THAN OVER EQUAL TO with
                                                  # slash
    'nge': u'\u2271',                             # NEITHER GREATER-THAN NOR EQUAL TO
    'ngeq': u'\u2271',                            # NEITHER GREATER-THAN NOR EQUAL TO
    'ngeqq': u'\u2267\u0338',                     # GREATER-THAN OVER EQUAL TO with
                                                  # slash
    'ngeqslant': u'\u2a7e\u0338',                 # GREATER-THAN OR SLANTED EQUAL TO
                                                  # with slash
    'nges': u'\u2a7e\u0338',                      # GREATER-THAN OR SLANTED EQUAL TO
                                                  # with slash
    'nGg': u'\u22d9\u0338',                       # VERY MUCH GREATER-THAN with slash
    'ngsim': u'\u2275',                           # NEITHER GREATER-THAN NOR EQUIVALENT
                                                  # TO
    ...
    'nis': u'\u22fc',                             # SMALL CONTAINS WITH VERTICAL BAR AT
    ...
    'nleqslant': u'\u2a7d\u0338',                 # LESS-THAN OR SLANTED EQUAL TO with
                                                  # slash
    'nles': u'\u2a7d\u0338',                      # LESS-THAN OR SLANTED EQUAL TO with
                                                  # slash
    ...
    'NotGreaterFullEqual': u'\u2267\u0338',       # GREATER-THAN OVER EQUAL TO with
                                                  # slash
    'NotGreaterGreater': u'\u226b\u0338',         # MUCH GREATER THAN with slash
    'NotGreaterLess': u'\u2279',                  # NEITHER GREATER-THAN NOR LESS-THAN
    'NotGreaterSlantEqual': u'\u2a7e\u0338',      # GREATER-THAN OR SLANTED EQUAL TO
                                                  # with slash
    'NotGreaterTilde': u'\u2275',                 # NEITHER GREATER-THAN NOR EQUIVALENT
                                                  # TO
    'NotHumpDownHump': u'\u224e\u0338',           # GEOMETRICALLY EQUIVALENT TO with
                                                  # slash
    'NotHumpEqual': u'\u224f\u0338',              # DIFFERENCE BETWEEN with slash
    'notin': u'\u2209',                           # NOT AN ELEMENT OF
    'notindot': u'\u22f5\u0338',                  # ELEMENT OF WITH DOT ABOVE with slash
    'notinE': u'\u22f9\u0338',                    # ELEMENT OF WITH TWO HORIZONTAL
                                                  # STROKES with slash
    ...
    'NotLeftTriangleBar': u'\u29cf\u0338',        # LEFT TRIANGLE BESIDE VERTICAL BAR
                                                  # with slash
    ...
    'NotLessSlantEqual': u'\u2a7d\u0338',         # LESS-THAN OR SLANTED EQUAL TO
                                                  # with slash
    'NotLessTilde': u'\u2274',                    # NEITHER LESS-THAN NOR EQUIVALENT TO
    'NotNestedGreaterGreater': u'\u2aa2\u0338',   # DOUBLE NESTED GREATER-THAN with
                                                  # slash
    ...
    'NotPrecedesEqual': u'\u2aaf\u0338',          # PRECEDES ABOVE SINGLE-LINE EQUALS
                                                  # SIGN with slash
    ...
    'NotRightTriangleBar': u'\u29d0\u0338',       # VERTICAL BAR BESIDE RIGHT TRIANGLE
                                                  # with slash
    'NotRightTriangleEqual': u'\u22ed',           # DOES NOT CONTAIN AS NORMAL SUBGROUP
                                                  # OR EQUAL
    ...
    'NotSucceedsEqual': u'\u2ab0\u0338',          # SUCCEEDS ABOVE SINGLE-LINE EQUALS
                                                  # SIGN with slash
    ...
    'nrtrie': u'\u22ed',                          # DOES NOT CONTAIN AS NORMAL SUBGROUP
                                                  # OR EQUAL
    'nsc': u'\u2281',                             # DOES NOT SUCCEED
    'nsccue': u'\u22e1',                          # DOES NOT SUCCEED OR EQUAL
    'nsce': u'\u2ab0\u0338',                      # SUCCEEDS ABOVE SINGLE-LINE EQUALS
                                                  # SIGN with slash
    ...
    'nsubE': u'\u2ac5\u0338',                     # SUBSET OF ABOVE EQUALS SIGN
                                                  # with slash
    ...
    'nsubseteqq': u'\u2ac5\u0338',                # SUBSET OF ABOVE EQUALS SIGN with
                                                  # slash
    'nsucc': u'\u2281',                           # DOES NOT SUCCEED
    'nsucceq': u'\u2ab0\u0338',                   # SUCCEEDS ABOVE SINGLE-LINE EQUALS
                                                  # SIGN with slash
    'nsup': u'\u2285',                            # NOT A SUPERSET OF
    'nsupE': u'\u2ac6\u0338',                     # SUPERSET OF ABOVE EQUALS SIGN
                                                  # with slash
    ...
    'nsupseteqq': u'\u2ac6\u0338',                # SUPERSET OF ABOVE EQUALS SIGN
                                                  # with slash
    ...
    'ntrianglerighteq': u'\u22ed',                # DOES NOT CONTAIN AS NORMAL SUBGROUP
                                                  # OR EQUAL
    ...
    'nVDash': u'\u22af',                          # NEGATED DOUBLE VERTICAL BAR DOUBLE
                                                  # RIGHT TURNSTILE
    'nVdash': u'\u22ae',                          # DOES NOT FORCE
    'nvDash': u'\u22ad',                          # NOT TRUE
    'nvdash': u'\u22ac',                          # DOES NOT PROVE
    'nvge': u'\u2265\u20d2',                      # GREATER-THAN OR EQUAL TO with
                                                  # vertical line
    'nvgt': u'>\u20d2',                           # GREATER-THAN SIGN with vertical line
    'nvHarr': u'\u2904',                          # LEFT RIGHT DOUBLE ARROW WITH
                                                  # VERTICAL STROKE
    'nvinfin': u'\u29de',                         # INFINITY NEGATED WITH VERTICAL BAR
    'nvlArr': u'\u2902',                          # LEFTWARDS DOUBLE ARROW WITH
                                                  # VERTICAL STROKE
    'nvle': u'\u2264\u20d2',                      # LESS-THAN OR EQUAL TO with
                                                  # vertical line
    'nvlt': u'\x3c\u20d2',                        # LESS-THAN SIGN with vertical line
    'nvltrie': u'\u22b4\u20d2',                   # NORMAL SUBGROUP OF OR EQUAL TO
                                                  # with vertical line
    'nvrArr': u'\u2903',                          # RIGHTWARDS DOUBLE ARROW WITH
                                                  # VERTICAL STROKE
    'nvrtrie': u'\u22b5\u20d2',                   # CONTAINS AS NORMAL SUBGROUP OR
                                                  # EQUAL TO with vertical line
    ...
    'nwnear': u'\u2927',                          # NORTH WEST ARROW AND NORTH EAST
                                                  # ARROW
    ...
    'Ocirc': u'\xd4',                             # LATIN CAPITAL LETTER O WITH
                                                  # CIRCUMFLEX
    ...
    'Odblac': u'\u0150',                          # LATIN CAPITAL LETTER O WITH DOUBLE
                                                  # ACUTE
    'odblac': u'\u0151',                          # LATIN SMALL LETTER O WITH DOUBLE
                                                  # ACUTE
    'odiv': u'\u2a38',                            # CIRCLED DIVISION SIGN
    'odot': u'\u2299',                            # CIRCLED DOT OPERATOR
    'odsold': u'\u29bc',                          # CIRCLED ANTICLOCKWISE-ROTATED
                                                  # DIVISION SIGN
    ...
    'otimesas': u'\u2a36',                        # CIRCLED MULTIPLICATION SIGN WITH
                                                  # CIRCUMFLEX ACCENT
    'Ouml': u'\xd6',                              # LATIN CAPITAL LETTER O WITH
                                                  # DIAERESIS
    ...
    'plus': u'+',                                 # PLUS SIGN
    'plusacir': u'\u2a23',                        # PLUS SIGN WITH CIRCUMFLEX ACCENT
                                                  # ABOVE
    ...
    'Rang': u'\u27eb',                            # MATHEMATICAL RIGHT DOUBLE ANGLE
                                                  # BRACKET
    ...
    'rarrsim': u'\u2974',                         # RIGHTWARDS ARROW ABOVE TILDE
                                                  # OPERATOR
    'Rarrtl': u'\u2916',                          # RIGHTWARDS TWO-HEADED ARROW WITH
                                                  # TAIL
    ...
    'rdca': u'\u2937',                            # ARROW POINTING DOWNWARDS THEN
                                                  # CURVING RIGHTWARDS
    'rdldhar': u'\u2969',                         # RIGHTWARDS HARPOON WITH BARB DOWN
                                                  # ABOVE LEFTWARDS HARPOON WITH BARB
                                                  # DOWN
    ...
    'ReverseEquilibrium': u'\u21cb',              # LEFTWARDS HARPOON OVER RIGHTWARDS
                                                  # HARPOON
    'ReverseUpEquilibrium': u'\u296f',            # DOWNWARDS HARPOON WITH BARB LEFT
                                                  # BESIDE UPWARDS HARPOON WITH BARB
                                                  # RIGHT
    ...
    'rHar': u'\u2964',                            # RIGHTWARDS HARPOON WITH BARB UP
                                                  # ABOVE RIGHTWARDS HARPOON WITH BARB
                                                  # DOWN
    'rhard': u'\u21c1',                           # RIGHTWARDS HARPOON WITH BARB
                                                  # DOWNWARDS
    'rharu': u'\u21c0',                           # RIGHTWARDS HARPOON WITH BARB UPWARDS
    'rharul': u'\u296c',                          # RIGHTWARDS HARPOON WITH BARB UP
                                                  # ABOVE LONG DASH
    ...
    'RightArrowLeftArrow': u'\u21c4',             # RIGHTWARDS ARROW OVER LEFTWARDS
                                                  # ARROW
    'rightarrowtail': u'\u21a3',                  # RIGHTWARDS ARROW WITH TAIL
    'RightCeiling': u'\u2309',                    # RIGHT CEILING
    'RightDoubleBracket': u'\u27e7',              # MATHEMATICAL RIGHT WHITE SQUARE
                                                  # BRACKET
    'RightDownTeeVector': u'\u295d',              # DOWNWARDS HARPOON WITH BARB RIGHT
                                                  # FROM BAR
    'RightDownVector': u'\u21c2',                 # DOWNWARDS HARPOON WITH BARB
                                                  # RIGHTWARDS
    'RightDownVectorBar': u'\u2955',              # DOWNWARDS HARPOON WITH BARB RIGHT
                                                  # TO BAR
    'RightFloor': u'\u230b',                      # RIGHT FLOOR
    'rightharpoondown': u'\u21c1',                # RIGHTWARDS HARPOON WITH BARB
                                                  # DOWNWARDS
    'rightharpoonup': u'\u21c0',                  # RIGHTWARDS HARPOON WITH BARB UPWARDS
    'rightleftarrows': u'\u21c4',                 # RIGHTWARDS ARROW OVER LEFTWARDS
                                                  # ARROW
    'rightleftharpoons': u'\u21cc',               # RIGHTWARDS HARPOON OVER LEFTWARDS
                                                  # HARPOON
    ...
    'RightTeeVector': u'\u295b',                  # RIGHTWARDS HARPOON WITH BARB UP
                                                  # FROM BAR
    ...
    'RightTriangleEqual': u'\u22b5',              # CONTAINS AS NORMAL SUBGROUP OR EQUAL
                                                  # TO
    'RightUpDownVector': u'\u294f',               # UP BARB RIGHT DOWN BARB RIGHT
                                                  # HARPOON
    'RightUpTeeVector': u'\u295c',                # UPWARDS HARPOON WITH BARB RIGHT FROM
                                                  # BAR
    'RightUpVector': u'\u21be',                   # UPWARDS HARPOON WITH BARB RIGHTWARDS
    'RightUpVectorBar': u'\u2954',                # UPWARDS HARPOON WITH BARB RIGHT
                                                  # TO BAR
    'RightVector': u'\u21c0',                     # RIGHTWARDS HARPOON WITH BARB UPWARDS
    'RightVectorBar': u'\u2953',                  # RIGHTWARDS HARPOON WITH BARB UP
                                                  # TO BAR
    'ring': u'\u02da',                            # RING ABOVE
    'risingdotseq': u'\u2253',                    # IMAGE OF OR APPROXIMATELY EQUAL TO
    'rlarr': u'\u21c4',                           # RIGHTWARDS ARROW OVER LEFTWARDS
                                                  # ARROW
    'rlhar': u'\u21cc',                           # RIGHTWARDS HARPOON OVER LEFTWARDS
                                                  # HARPOON
    'rlm': u'\u200f',                             # RIGHT-TO-LEFT MARK
    'rmoust': u'\u23b1',                          # UPPER RIGHT OR LOWER LEFT CURLY
                                                  # BRACKET SECTION
    'rmoustache': u'\u23b1',                      # UPPER RIGHT OR LOWER LEFT CURLY
                                                  # BRACKET SECTION
    'rnmid': u'\u2aee',                           # DOES NOT DIVIDE WITH REVERSED
                                                  # NEGATION SLASH
    'roang': u'\u27ed',                           # MATHEMATICAL RIGHT WHITE TORTOISE
                                                  # SHELL BRACKET
    'roarr': u'\u21fe',                           # RIGHTWARDS OPEN-HEADED ARROW
    'robrk': u'\u27e7',                           # MATHEMATICAL RIGHT WHITE SQUARE
                                                  # BRACKET
    ...
    'scpolint': u'\u2a13',                        # LINE INTEGRATION WITH
    ...
    'semi': u';',                                 # SEMICOLON
    'seswar': u'\u2929',                          # SOUTH EAST ARROW AND SOUTH WEST
                                                  # ARROW
    ...
    'simgE': u'\u2aa0',                           # SIMILAR ABOVE GREATER-THAN ABOVE
                                                  # EQUALS SIGN
    'siml': u'\u2a9d',                            # SIMILAR OR LESS-THAN
    'simlE': u'\u2a9f',                           # SIMILAR ABOVE LESS-THAN ABOVE
                                                  # EQUALS SIGN
    'simne': u'\u2246',                           # APPROXIMATELY BUT NOT ACTUALLY
                                                  # EQUAL TO
    'simplus': u'\u2a24',                         # PLUS SIGN WITH TILDE ABOVE
    'simrarr': u'\u2972',                         # TILDE OPERATOR ABOVE RIGHTWARDS
                                                  # ARROW
    'slarr': u'\u2190',                           # LEFTWARDS ARROW
    'SmallCircle': u'\u2218',                     # RING OPERATOR
    'smallsetminus': u'\u2216',                   # SET MINUS
    'smashp': u'\u2a33',                          # SMASH PRODUCT
    'smeparsl': u'\u29e4',                        # EQUALS SIGN AND SLANTED PARALLEL
                                                  # WITH TILDE ABOVE
    'smid': u'\u2223',                            # DIVIDES
    'smile': u'\u2323',                           # SMILE
    'smt': u'\u2aaa',                             # SMALLER THAN
    ...
    'submult': u'\u2ac1',                         # SUBSET WITH MULTIPLICATION SIGN
                                                  # BELOW
    ...
    'Succeeds': u'\u227b',                        # SUCCEEDS
    'SucceedsEqual': u'\u2ab0',                   # SUCCEEDS ABOVE SINGLE-LINE EQUALS
                                                  # SIGN
    'SucceedsSlantEqual': u'\u227d',              # SUCCEEDS OR EQUAL TO
    'SucceedsTilde': u'\u227f',                   # SUCCEEDS OR EQUIVALENT TO
    'succeq': u'\u2ab0',                          # SUCCEEDS ABOVE SINGLE-LINE EQUALS
                                                  # SIGN
    ...
    'supdsub': u'\u2ad8',                         # SUPERSET BESIDE AND JOINED BY DASH
                                                  # WITH SUBSET
    'supE': u'\u2ac6',                            # SUPERSET OF ABOVE EQUALS SIGN
    'supe': u'\u2287',                            # SUPERSET OF OR EQUAL TO
    'supedot': u'\u2ac4',                         # SUPERSET OF OR EQUAL TO WITH DOT
                                                  # ABOVE
    ...
    'supmult': u'\u2ac2',                         # SUPERSET WITH MULTIPLICATION SIGN
                                                  # BELOW
    ...
    'swnwar': u'\u292a',                          # SOUTH WEST ARROW AND NORTH WEST
                                                  # ARROW
    ...
    'toea': u'\u2928',                            # NORTH EAST ARROW AND SOUTH EAST
                                                  # ARROW
    'top': u'\u22a4',                             # DOWN TACK
    ...
    'tosa': u'\u2929',                            # SOUTH EAST ARROW AND SOUTH WEST
                                                  # ARROW
    ...
    'trianglerighteq': u'\u22b5',                 # CONTAINS AS NORMAL SUBGROUP OR
                                                  # EQUAL TO
    ...
    'Uarrocir': u'\u2949',                        # UPWARDS TWO-HEADED ARROW FROM
                                                  # SMALL CIRCLE
    ...
    'Ucirc': u'\xdb',                             # LATIN CAPITAL LETTER U WITH
                                                  # CIRCUMFLEX
    'ucirc': u'\xfb',                             # LATIN SMALL LETTER U WITH CIRCUMFLEX
    'Ucy': u'\u0423',                             # CYRILLIC CAPITAL LETTER U
    'ucy': u'\u0443',                             # CYRILLIC SMALL LETTER U
    'udarr': u'\u21c5',                           # UPWARDS ARROW LEFTWARDS OF
                                                  # DOWNWARDS ARROW
    'Udblac': u'\u0170',                          # LATIN CAPITAL LETTER U WITH DOUBLE
                                                  # ACUTE
    'udblac': u'\u0171',                          # LATIN SMALL LETTER U WITH DOUBLE
                                                  # ACUTE
    'udhar': u'\u296e',                           # UPWARDS HARPOON WITH BARB LEFT
                                                  # BESIDE DOWNWARDS HARPOON WITH
                                                  # BARB RIGHT
    ...
    'uHar': u'\u2963',                            # UPWARDS HARPOON WITH BARB LEFT
                                                  # BESIDE UPWARDS HARPOON WITH BARB
                                                  # RIGHT
    ...
    'UpArrowDownArrow': u'\u21c5',                # UPWARDS ARROW LEFTWARDS OF
                                                  # DOWNWARDS ARROW
    'UpDownArrow': u'\u2195',                     # UP DOWN ARROW
    'Updownarrow': u'\u21d5',                     # UP DOWN DOUBLE ARROW
    'updownarrow': u'\u2195',                     # UP DOWN ARROW
    'UpEquilibrium': u'\u296e',                   # UPWARDS HARPOON WITH BARB
                                                  # LEFT BESIDE DOWNWARDS HARPOON
                                                  # WITH BARB RIGHT
    ...
    'Uring': u'\u016e',                           # LATIN CAPITAL LETTER U WITH RING
                                                  # ABOVE
    ...
    'Uuml': u'\xdc',                              # LATIN CAPITAL LETTER U WITH
                                                  # DIAERESIS
    ...
    'varsubsetneq': u'\u228a\ufe00',              # SUBSET OF WITH NOT EQUAL TO
                                                  # - variant with stroke through
                                                  # bottom members
    'varsubsetneqq': u'\u2acb\ufe00',             # SUBSET OF ABOVE NOT EQUAL TO
                                                  # - variant with stroke through
                                                  # bottom members
    'varsupsetneq': u'\u228b\ufe00',              # SUPERSET OF WITH NOT EQUAL TO
                                                  # - variant with stroke through
                                                  # bottom members
    'varsupsetneqq': u'\u2acc\ufe00',             # SUPERSET OF ABOVE NOT EQUAL TO
                                                  # - variant with stroke through
                                                  # bottom members
    ...
    'VDash': u'\u22ab',                           # DOUBLE VERTICAL BAR DOUBLE RIGHT
                                                  # TURNSTILE
    'Vdash': u'\u22a9',                           # FORCES
    'vDash': u'\u22a8',                           # TRUE
    'vdash': u'\u22a2',                           # RIGHT TACK
    'Vdashl': u'\u2ae6',                          # LONG DASH FROM LEFT MEMBER OF
                                                  # DOUBLE VERTICAL
    ...
    'vsubnE': u'\u2acb\ufe00',                    # SUBSET OF ABOVE NOT EQUAL TO
                                                  # - variant with stroke through
                                                  # bottom members
    'vsubne': u'\u228a\ufe00',                    # SUBSET OF WITH NOT EQUAL TO
                                                  # - variant with stroke through
                                                  # bottom members
    'vsupnE': u'\u2acc\ufe00',                    # SUPERSET OF ABOVE NOT EQUAL TO
                                                  # - variant with stroke through
                                                  # bottom members
    'vsupne': u'\u228b\ufe00',                    # SUPERSET OF WITH NOT EQUAL TO
                                                  # - variant with stroke through
                                                  # bottom members
    'Vvdash': u'\u22aa',                          # TRIPLE VERTICAL BAR RIGHT TURNSTILE
    'vzigzag': u'\u299a',                         # VERTICAL ZIGZAG LINE
    'Wcirc': u'\u0174',                           # LATIN CAPITAL LETTER W WITH
                                                  # CIRCUMFLEX
    ...
    'xnis': u'\u22fb',                            # CONTAINS WITH VERTICAL BAR AT
                                                  # END OF HORIZONTAL STROKE
    ...
    'Ycirc': u'\u0176',                           # LATIN CAPITAL LETTER Y WITH
                                                  # CIRCUMFLEX
    ...
    'Yuml': u'\u0178',                            # LATIN CAPITAL LETTER Y WITH
                                                  # DIAERESIS
    ...
    'Zdot': u'\u017b',                            # LATIN CAPITAL LETTER Z WITH DOT
                                                  # ABOVE
...
#        <span name=fontfamily/fontname color=colorname backcolor=colorname
#             size=float style=stylename>
...
class ParaParser(HTMLParser):
    ...
    def __getattr__(self, attrName):
        ...
        if (attrName != attrName.lower() and attrName != "caseSensitive" and
                not self.caseSensitive and (attrName.startswith("start_") or
                                            attrName.startswith("end_"))):
    ...
    def _initial_frag(self, attr, attrMap, bullet=0):
        ...
        try:
            if bullet:
                ...
                frag.textColor = (hasattr(style, 'bulletColor') and
        ...
        self._defaultLinkUnderline = getattr(style, 'linkUnderline',
    ...
    def start_index(self, attr):
        ...
        if format is not None and format not in ('123', 'I', 'i', 'ABC', 'abc'):
            raise ValueError(
                # offset not defined yet
                # 'index tag format is %r not valid 123 I i ABC or abc' % offset)
                'index tag format is not valid 123 I i ABC or abc')
...
if __name__ == '__main__':
    ...
    def check_text(text, p=_parser):
        ...
        if rv is None:
            ...
            for ll in rv:
                sys.stdout.write(ll.fontName, ll.fontSize, ll.textColor,
                                 ll.bold, ll.rise, '|%s|' % ll.text[:25])
                if hasattr(ll, 'cbDefn'):
                    print('cbDefn', getattr(ll.cbDefn, 'name', ''),
                          getattr(ll.cbDefn, 'label', ''), l.cbDefn.kind)
                else:
                    print()
    ...
    check_text('<para font="%s" size=24 leading=28.8 spaceAfter=72>' +
               'ReportLab -- Reporting for the Internet Age</para>' % _baseFontName)
    check_text('''
    <font color=red>&tau;</font>Tell me, O muse, of that ingenious hero
...
them when the man struck up with his tune.]''')
    ...
    check_text('''Here comes <FONT FACE="Helvetica" SIZE="14pt">Helvetica 14</FONT>
                  with <STRONG>strong</STRONG> <EM>emphasis</EM>.''')
    check_text('''Here comes <font face="Helvetica" size="14pt">Helvetica 14</font>
                  with <Strong>strong</Strong> <em>emphasis</em>.''')
    check_text('''Here comes <font face="Courier" size="3cm">Courier 3cm</font>
                  and normal again.''')
    check_text('''Before the break <br/>the middle line <br/> and the last line.''')
    check_text('''This should be an inline image
                  <img src='../../../docs/images/testimg.gif'/>!''')
