...
from reportlab.lib.validators import (isColor, isString, isColorOrNone, isNumber,
                                      isBoxAnchor)
...
class DataMatrix(Barcode, _DMTXCheck):
    ...
    @size.setter
    # redefinition of previous cellSize
    def cellSize(self, v):  # noqa
        self._cellSize = v
        self._recalc = True
    ...
    def recalc(self):
        ...
        if cellWidth:
            ...
            if len(cellWidth) > 2:
                raise ValueError('cellSize needs to be distance x distance not %r' %
                                 self._cellSize)
...
class DataMatrixWidget(Widget, _DMTXCheck):
    ...
    def draw(self):
        m = DataMatrix(value=self.value,
                       **{k: getattr(self, k) for k in self._defaults})
        m.canv = self
        m.y += m.height
        g = Group()
        self._gadd = g.add
