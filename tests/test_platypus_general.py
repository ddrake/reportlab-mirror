...
from reportlab.lib.utils import (_RL_DIR, rl_isfile, open_for_read, fileName2FSEnc,
                                 asNative)
import unittest
...
if not rl_isfile(_GAPNG):
    _GAPNG = None
if _GIF:
    _GIFFSEnc = fileName2FSEnc(_GIF)
if _GAPNG:
    _GAPNGFSEnc = fileName2FSEnc(_GAPNG)
...
def getCommentary():
    ...
    spam = """
        ...
        will start to contribute them.  PINGO users - we provide a "PINGO flowable"
        object to let
        ...
        must implement two methods.  object.wrap(availWidth, availHeight) will
        be called by other parts of
    ...
    story.append(Paragraph(
        """<para color=green bcolor=red bg=pink>This is a
        <font bgcolor=yellow color=red>contrived</font> object to give an
        example of a Flowable -
    ...
    story.append(FrameBreak())
    if _GIF:
        story.append(Paragraph("""We can use images via the file name""",
                               styleSheet['BodyText']))
    ...
    if _GAPNG:
        story.append(Paragraph(
            """We can use images via the file name""", styleSheet['BodyText']))
    ...
    if _JPG:
        story.append(Paragraph(
            """JPEGs are a native PDF image format. They should
be available even if PIL cannot be used.""", styleSheet['BodyText']))
        story.append(FrameBreak())
    return story
...
def getExamples():
    ...
    story.append(Paragraph(
        ...
        styleSheet['BodyText']))
    p = Paragraph("""
        <para align=justify leading="+1.5" fg=green><font color=red>Platypus</font>
        is all about fitting objects into frames on the page.  You
        are looking at a fairly simple Platypus paragraph in Debug mode.
        It has some gridlines drawn around it to show the left and right indents,
        and the space before and after, all of which are attributes set in
        the style sheet.  To be specific, this paragraph has left and
    ...
    story.append(
        platypus.XBox(4*inch, 0.75*inch, 'This is a box with a fixed size'))
    ...
    story.append(Paragraph("This is a bullet point", styleSheet['Bullet'],
                           bulletText='O'))
    story.append(Paragraph("Another bullet point", styleSheet['Bullet'],
                           bulletText='O'))
    ...
    from reportlab.pdfbase import pdfmetrics
    fontDir = os.path.join(_RL_DIR, 'fonts')
    face = pdfmetrics.EmbeddedType1Face(os.path.join(
        fontDir, 'DarkGardenMK.afm'), os.path.join(fontDir, 'DarkGardenMK.pfb'))
    faceName = face.name  # should be 'DarkGardenMK'
    pdfmetrics.registerTypeFace(face)
    font = pdfmetrics.Font(faceName, faceName, 'WinAnsiEncoding')
    pdfmetrics.registerFont(font)
...
class AndyTemplate(BaseDocTemplate):
    ...
    def build(self, flowables1, flowables2):
        assert [x for x in flowables1
                if not isinstance(x, Flowable)] == [], "flowables1 argument error"
        assert [x for x in flowables2
                if not isinstance(x, Flowable)] == [], "flowables2 argument error"
        self._startBuild()
...
class PlatypusTestCase(unittest.TestCase):
    ...
    def test1(self):
        ...
        normal = ParagraphStyle(name='Normal', fontName='Helvetica', fontSize=8.5,
                                leading=11)
    ...
    def test2(self):
        ...
        assert bool(ShowBoundaryValue(width=1)) is True
        assert bool(ShowBoundaryValue(color=None, width=1)) is False
        assert bool(ShowBoundaryValue(width=-1)) is False

    def test3(Self):
        from reportlab.platypus import (BalancedColumns, IndexingFlowable,
                                        ShowBoundaryValue, NullDraw)
        ...
        def first():
            ...
            Platypus stands for "Page Layout and Typography Using Scripts".
            It is a high level page layout library which lets you programmatically
            create complex documents with a minimum of effort.
        ...
        def balanced(spam=None):
            L = [Paragraph("""What concepts does PLATYPUS deal with?""",
                           styleSheet['Heading2']),
                 Paragraph(
                 """The central concepts in PLATYPUS are Flowable Objects,
                 Frames, Flow
                 Management, Styles and Style Sheets, Paragraphs and Tables.  This is
                 best explained in contrast to PDFgen, the layer underneath PLATYPUS.
                 PDFgen is a graphics library, and has primitive commans to draw lines
                 and strings.  There is nothing in it to manage the flow of text down
                 the page.  PLATYPUS works at the conceptual level fo a desktop
                 publishing package; you can write programs which deal intelligently
                 with graphic objects and fit them onto the page.
                 """, styleSheet['BodyText']),
            ...
            story.append(BalancedColumns(
                L, spaceBefore=20, spaceAfter=30,
                showBoundary=ShowBoundaryValue(color=colors.lightgreen, width=2)))

        def second():
            spam = '''The concept of an integrated one box solution for
    advanced voice and data applications began with the introduction of the IMACS.
    The IMACS 200 carries on that tradition with an integrated solution
...
This should be a u with a dieresis on top &lt;unichar code=0xfc/&gt;=
"<unichar code="0xfc"/>" and this &amp;#xfc;="&#xfc;" and this
\\xc3\\xbc="\xc3\xbc".On the other hand this
...
Use scheme "pdf:" to indicate an external PDF link, "http:",
"https:" to indicate an external link eg something to open in
your browser. If an internal link begins with something that looks like a scheme,
precede with "document:".

Empty hrefs should be allowed ie <a href="">&lt;a
href=""&gt;test&lt;/a&gt;</a> should be allowed. <strike>This text should have a
strike through it.</strike>
This should be a mailto link <a href="mailto:reportlab-users@lists2.reportlab.com">
<font color="blue">reportlab-users at lists2.reportlab.com</font></a>.''')

        story.append(Paragraph("""Testing the BalancedColumns""",
                               styleSheet['Heading1']))
        first()
        balanced()
        second()
        story.append(PageBreak())
        story.append(Paragraph("""Testing the BalancedColumns Breaking""",
                               styleSheet['Heading1']))
