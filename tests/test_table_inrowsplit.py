...
class TableTestCase(unittest.TestCase):
    ...
    def test_document(self):
        ...
        It splits between two rows. """, styNormal))
        ...
        xlst = [].append
        xlst(PageBreak())
        xlst(Paragraph("""Modifying the inrow split linebelow and lineabove""",
                       styleSheet['Heading1']))
        xlst(Paragraph("""Table, with splitByRow=0 and splitInRow=1. It splits inside
        a row without split row line below and line above (the normal case).""",
                       styNormal))
        ...
        newsty = TableStyle([
                ('LINEBELOW', (0, 'inrowsplitstart'), (-1, 'inrowsplitstart'),
                 1, colors.blue, 'butt'),
                ('LINEABOVE', (0, 'inrowsplitend'), (-1, 'inrowsplitend'),
                 1, colors.red, 'butt'),
            ], parent=ministy)
        t = Table(tableData,
                  colWidths=colWidths,
        ...
        xlst(Paragraph("""Table and repeatrow, with splitByRow=0 and splitInRow=1.
It splits inside a row without split row line below and line above
(the normal case).""", styNormal))
        ...
        lst.append(Paragraph("""Long cell with multiple splits, and minimum
split size""", styleSheet['Heading2']))

        lst.append(Paragraph("With a height of 80 amd splitInRow=1 (no minimum rest) "
                             "we get a small last split.", styleSheet['Normal']))

        ministy = TableStyle([('GRID', (0, 0), (-1, -1), 1.0, colors.black),])
        cell1 = [Paragraph(
            "This is a very very tall cell to make a very very tall " +
            "row so we can split it " +
        ...
        lst.append(Paragraph(
            "With minimum split size to 40 (splitInRow=40) the last split "
        ...
        lst.append(PageBreak())
        lst.append(Paragraph(
            """Splitfirst/splitlast behavior with split rows and spans""",
            styleSheet['Heading2']))
        ...
        sty = [
                ...
                ('LINEBELOW', (0, 'splitlast'), (-1, 'splitlast'), 1, colors.grey,
                 'butt'),
               ]
        ...
        lst.append(PageBreak())
        lst.append(Paragraph("""Style handling with rowspans""",
                             styleSheet['Heading2']))
        ...
        lst.append(PageBreak())
        lst.append(Paragraph("""More Style handling with rowspans""",
                             styleSheet['Heading2']))
        ...
        lst.append(PageBreak())
        lst.append(Paragraph("""Testing for a couple of height alignment bugs""",
                             styleSheet['Heading2']))
