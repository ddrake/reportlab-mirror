...
from reportlab.lib.testutils import (setOutDir, makeSuiteForClasses, outputfile,
                                     printLocation, rlSkipUnless)
import os
import unittest
from reportlab.pdfgen.canvas import Canvas
from reportlab.pdfbase.pdfmetrics import stringWidth, registerFont, registerFontFamily
from reportlab.pdfbase.ttfonts import TTFont
from reportlab.platypus.flowables import DocAssert
from reportlab.lib.units import cm
from reportlab.lib.enums import TA_RIGHT, TA_CENTER, TA_JUSTIFY
from reportlab.lib.utils import asBytes, asUnicode, asNative
from reportlab.lib.styles import getSampleStyleSheet, ParagraphStyle
from reportlab.platypus.xpreformatted import XPreformatted
from reportlab.platypus.frames import Frame, ShowBoundaryValue
from reportlab.platypus.doctemplate import (PageTemplate, BaseDocTemplate, PageBreak,
                                            NextPageTemplate)
from reportlab.platypus.paragraph import (Paragraph, _splitWord,
...
class ParagraphCorners(unittest.TestCase):
    ...
    def test1(self):
        ...
        btC = ParagraphStyle('BodyTextTTCapitalize', parent=bt,
        ...
        story.append(Paragraph(
            '''T<i>hi</i>s shoul<font color="red">d b</font>e <b>ORDINARY</b> text.''',
            style=bt))
        story.append(Paragraph(
            '''T<i>hi</i>s shoul<font color="red">d b</font>e <b>ORDINARY</b> text.''',
            style=btN))
        story.append(Paragraph(
            '''T<i>hi</i>s shoul<font color="red">d b</font>e <b>LOWER</b> text.''',
            style=btL))
        story.append(Paragraph(
            '''T<i>hi</i>s shoul<font color="red">d b</font>e <b>upper</b> text.''',
            style=btU))
        story.append(Paragraph(
            '''T<i>hi</i>s shoul<font color="red">d b \
</font>e <b>cAPITALIZED</b> text.''',
            style=btC))
    ...
    def test2(self):
        ...
        assert h0 <= aH, (
            'Multi-frag CJK split[0] has wrong height %s >= available %s' % (h0, aH))
        w1, h1 = S[1].wrap(aW, aH)
        assert h0+h1 == h, (
            ('Multi-frag-CJK split[0].height(%s)+split[1].height(%s)' +
             ' don\'t add to original %s') % (h0, h1, h))
    ...
    def test3(self):
        ...
        y -= h+10
        p1.drawOn(canv, 72, y)
        assert canv._code[ix:] == [
            'q', '1 0 0 1 72 697.8898 cm', 'q', '0 0 0 rg',
            'BT 1 0 0 1 0 57 Tm /F2 15 Tf 18 TL (ABCD) Tj T* (EFGH) Tj T* ' +
            '(IJKL]) Tj T* (N) Tj T* ET',
            'Q', 'Q', 'q', '1 0 0 1 72 615.8898 cm', 'q',
            'BT 1 0 0 1 0 57 Tm 18 TL /F2 15 Tf 0 0 0 rg (AB) Tj 1 0 0 rg (C) ' +
            'Tj 0 0 0 rg (D) Tj T* (EFGH) Tj T* (IJKL]) Tj T* (N) Tj T* ET',
            'Q', 'Q', 'q', '1 0 0 1 72 353.8898 cm', 'q', '0 0 0 rg',
            'BT 1 0 0 1 0 237 Tm /F2 15 Tf 18 TL (A) Tj T* (B) Tj T* (C) Tj T* (D) ' +
            'Tj T* (E) Tj T* (F) Tj T* (G) Tj T* (H) Tj T* (I) Tj T* (J) Tj T* (K) ' +
            'Tj T* (L) Tj T* (]) Tj T* (N) Tj T* ET',
            'Q', 'Q', 'q', '1 0 0 1 72 91.88976 cm', 'q',
            'BT 1 0 0 1 0 237 Tm 18 TL /F2 15 Tf 0 0 0 rg (A) Tj T* (B) Tj T* 1 0 0 ' +
            'rg (C) Tj T* 0 0 0 rg (D) Tj T* (E) Tj T* (F) Tj T* (G) Tj T* (H) Tj ' +
            'T* (I) Tj T* (J) Tj T* (K) Tj T* (L) Tj T* (]) Tj T* (N) Tj T* ET',
            'Q', 'Q']
        canv.showPage()
        canv.save()

    def test4(self):
        from reportlab.platypus.paragraph import (_SHYIndexedStr, stringWidth, _SHYWord,
                                                  ABag, _shyUnsplit)
        fontName = 'Helvetica'
        fontSize = 10
        ...
        def sW(_):
            return stringWidth(_, fontName, fontSize)
        text = u'Super\xadcali\xadfragi\xadlistic\xadexpi\xadali\xaddocious'
        u = _SHYIndexedStr(text)
        self.assertEqual(u, u'Supercalifragilisticexpialidocious',
                         '_SHYIndexStr not as expected')
        self.assertEqual(u._shyIndices, [5, 9, 14, 20, 24, 27],
                         '_SHYIndexStr._shyIndices are wrong')
        shyW = _SHYWord([sW(u), (f, u)])
        hsw = shyW.shyphenate(shyW[0], 50)
        self.assertTrue(hsw, 'shyphenate failed')
        self.assertTrue(
            hsw[0][0] == 45.01
            and hsw[0][1][0].__dict__ == ABag(fontName='Helvetica',
                                              fontSize=10).__dict__
            and hsw[0][1][1] == u'Supercali-'
            and hsw[0][1][1]._shyIndices == [5, 9],
            'left part of shyphenate split failed')
        self.assertTrue(
            hsw[1][0] == 101.69000000000001
            and hsw[1][1][0].__dict__ == ABag(fontName='Helvetica',
                                              fontSize=10).__dict__
            and hsw[1][1][1] == u'fragilisticexpialidocious'
            and hsw[1][1][1]._shyIndices == [5, 11, 15, 18],
            'right part of shyphenate split failed')
        uj = _shyUnsplit(hsw[0][1][1], hsw[1][1][1])
        self.assertTrue(uj == u and uj._shyIndices == u._shyIndices,
                        '_shyUnsplit failed')
    ...
    def test5(self):
        ...
        pagesize = (80+20, 400)
        c = canvas.Canvas(outputfile('test_platypus_soft_hyphenation.pdf'),
                          pagesize=pagesize)
        f = Frame(10, 0, 68, 400,
        ...
        text = (u'<span color="red">Super</span><span color="pink">&#173;cali&#173;' +
                u'</span>fragi&#173;listic&#173;expi&#173;ali&#173;docious')
        f.addFromList([Paragraph(text, style)], c)
        c.showPage()
        c.save()
    ...
    def test_platypus_paragraphs_embedded2(self):
        ...
        texts = [asUnicode(text) for text in [
            b'UU\xc2\xadDIS\xc2\xadTU\xc2\xadSA\xc2\xadLA JA TAI\xc2\xadMIK\xc2\xadKO',
            b'NUO\xc2\xadRI KAS\xc2\xadVA\xc2\xadTUS' +
            b'\xc2\xadMET\xc2\xadSIK\xc2\xadK\xc3\x96',
            b'VART\xc2\xadTU\xc2\xadNUT KAS\xc2\xadVA' +
            b'\xc2\xadTUS\xc2\xadMET\xc2\xadSIK\xc2\xadK\xc3\x96',
            b'UU\xc2\xadDIS\xc2\xadTUS-KYP\xc2\xadS\xc3' +
            b'\x84 MET\xc2\xadSIK\xc2\xadK\xc3\x96',
            b'SIE\xc2\xadMEN- JA SUO-JUS\xc2\xadPUU-MET\xc2\xadSIK\xc2\xadK\xc3\x96',
            b'E\xc2\xadRI-I\xc2\xadK\xc3\x84\xc2\xadIS-RA\xc2\xadKEN' +
            b'\xc2\xadTEI\xc2\xadNEN MET\xc2\xadSIK\xc2\xadK\xc3\x96',
        ]]
        maxWidth = stringWidth('VARTTUNUT KAS', fontName, fontSize)+0.5

        def span(_t, _c):
            return "<span color='%s'>%s</span>" % (_c, _t)
        ...
        for eh in 0, 1, 2:
            for slw in 0, 1:
                ...
                observed.append((len(c._code),
                                 hashlib_md5(b"".join((asBytes(b, "latin1")
                                                       for b in c._code))).digest()))
                c.showPage()
        c.save()
    ...
    def test_lele_img(self):
        ...
        cx0 = len(c._code)
        p.drawOn(c, cm, 2*cm)
        xcode = ['q', '1 0 0 1 28.34646 56.69291 cm', 'q', 'q',
                 '110 0 0 44 0 -16 cm',
                 '/FormXob.00cb676cb2b2da8ec875fe2c13cf2496 Do', 'Q',
                 'BT 1 0 0 1 0 2 Tm 12 TL 110 0 Td /F1 10 Tf 12 TL  T* -110 0 Td ET',
                 'Q', 'Q']
        ...
        self.assertEqual(
            (owh, ocode), (xwh, xcode),
            ("\n(owh,ocode)=%r\nfor test_lele_img.pdf " +
             "doesn't match expected\n(xwh,xcode)=%r") % ((xwh, xcode), (owh, ocode)))


DEJAVUSANS = ('DejaVuSans', 'DejaVuSans-Bold', 'DejaVuSans-Oblique',
...
'''
        ...
        story.append(ParagraphAndImage(Paragraph(description, bt), Image(gif),
                                       side='left'))
...
see how the AdSu one might be done generically, but the O'Reilly, unsure...
I guess I'm hoping that I've missed something, and that
it's actually easy to do using platypus.We can do greek letters
<greek>mDngG</greek>. This should be a
u with a dieresis on top &lt;unichar code=0xfc/&gt;="<unichar code="0xfc"/>"
and this &amp;#xfc;="&#xfc;" and this \\xc3\\xbc="\xc3\xbc". On the other hand this
should be a pound sign &amp;pound;="&pound;" and this an alpha &amp;alpha;="&alpha;".
You can have links in the page
<link href="http://www.reportlab.com" color="blue">ReportLab</link> &amp;
<a href="http://www.reportlab.org" color="green">ReportLab.org</a>.
Use scheme "pdf:" to indicate an external PDF link, "http:", "https:" to indicate
an external link eg something to open in
your browser. If an internal link begins with something that looks like a scheme,
precede with "document:". Empty hrefs should be allowed ie
<a href="">&lt;a href=""&gt;test&lt;/a&gt;</a> should be allowed.
<u>This text should be underlined.</u><br/>
<strike>This text should have a strike through it.</strike><br/>
<span backcolor="yellow"><strike>This text should have a strike through it
and be highlighted.</strike></span><br/>
<span backcolor="yellow"><strike><u>This text should have a strike through it
and be highlighted and underlined.</u></strike></span><br/>
This should be a mailto link
<a href="mailto:reportlab-users@lists2.reportlab.com"><font color="blue">
reportlab-users at lists2.reportlab.com</font></a>.<br/>
This should be an underlined mailto link
<a underline="1" href="mailto:reportlab-users@lists2.reportlab.com">
<font color="blue">reportlab-users at lists2.reportlab.com</font></a>.<br/>
This should be a highlighted mailto link
<span backcolor="yellow"><a href="mailto:reportlab-users@lists2.reportlab.com">
<font color="blue">reportlab-users at lists2.reportlab.com</font></a></span>.<br/>
This should be a highlighted &amp; underlined mailto link
<span backcolor="yellow"><a underline="1" ucolor="red" uwidth="0.01*F"
href="mailto:reportlab-users@lists2.reportlab.com"><font color="blue">
reportlab-users at lists2.reportlab.com</font></a></span>.<br/>
...
'''
        ...
        phrase = 'This should be a paragraph spanning at least three pages. '
        description = ''.join([('%d: ' % i)+phrase for i in range(250)])
        story.append(ImageAndFlowables(Image(gif),
        ...
        story.append(ImageAndFlowables(
                        ...
                        Paragraph('''<font name="Vera">The <b>concept</b> of an
<i>integrated</i> one <b><i>box</i></b> solution for <i><b>advanced</b></i> voice and
...
providing the ultimate in ease of installation.</font>''',
        ...
        story.append(ImageAndFlowables(
                        ...
                        Paragraph('''The concept of an integrated one box solution
for advanced voice and
data applications began with the introduction of the IMACS. The
IMACS 200 carries on that tradition with an integrated solution
optimized for smaller port size applications that the IMACS could not
economically address. An array of the most popular interfaces and
...
component delimits the traditional practice of grammarians.'''
        ...
        story.append(ImageAndFlowables(
            Image(gif, width=66, height=81),
            [Paragraph(text, bt)], imageSide='left', imageRightPadding=10))
        story.append(NextPageTemplate('special'))
        story.append(PageBreak())
        story.append(Paragraph('Width 240, multi-frag free hyphenation', h3))
        story.append(ImageAndFlowables(
                        Image(gif, width=240, height=120),
                        Paragraph('''The concept of an <span color="red">integ</span>
<span color="green">rated</span> one box solution for advanced voice and
data applications began with the introduction of the IMACS. The
IMACS 200 carries on that tradition with an integrated solution
<span color="pink">optimized</span> for smaller port size applications
that the IMACS could not
<span color="lightgreen">eco</span><span color="blue">nomically</span>
address. An array of the most popular interfaces and
features from the IMACS has been bundled into a small 2U chassis
providing the ultimate in ease of installation.''',
                                  style=st,), imageSide='left',))
        ...
        story.append(ImageAndFlowables(
            Image(gif, width=240, height=120),
            Paragraph(u'''The concept of an <span color="red">integ</span>
<span color="green">\xadrated</span> one box solution for advanced voice and
data applica\xadtions began with the in\xadtroduction of the IMACS. The
IMACS 200 carries on that tradition with an integrated solution
<span color="pink">op\xadtimized</span> for smaller port size applications that
the IMACS could not
<span color="lightgreen">eco</span><span color="blue">\xadnomically</span> address.
An array of the most popular interfaces and
fea\xadtures from the IMACS has been bundled into a small 2U chassis
providing the ultimate in ease of installation.''', style=st,), imageSide='left',))

        story.append(PageBreak())
        story.append(Paragraph('Width 240, single-frag soft hyphenation', h3))
        story.append(ImageAndFlowables(
            Image(gif, width=240, height=120),
            Paragraph(u'''The concept of an integ\xadrated one box solution for
advanced voice and
data applica\xadtions began with the in\xadtroduction of the IMACS. The
IMACS 200 carries on that tradition with an integrated solution
...
providing the ultimate in ease of installation.''', style=st,), imageSide='left',))
    ...
    def test1_RTL(self):
        ...
        ParagraphStyle(
            name="base", fontName="Helvetica", leading=12, leftIndent=0,
            firstLineIndent=0, spaceBefore=9.5, fontSize=9.5, wordWrap='RTL',
            alignment=TA_RIGHT,)
        text = b'''\xd7\x94\xd7\xa0\xd7\x93\xd7\xa1\xd7\xaa \xd7\x90\xd7\x99\xd7
\xa0\xd7\x98\xd7\xa8\xd7\xa0\xd7\x98 \xd7\xa2\xd7\x9c
        \xd7\xa9\xd7\x9e\xd7\x95. \xd7\x91\xd7\x94 \xd7\xa2\xd7\x96\xd7\x94 \xd7
\x90\xd7\x97\xd7\xa8\xd7\x95\xd7\xaa
        \xd7\x9c\xd7\xa2\xd7\xa8\xd7\x99\xd7\x9b\xd7\xaa \xd7\x94\xd7\xa0\xd7
\x90\xd7\x9e\xd7\xa0\xd7\x99\xd7\x9d,
        \xd7\x94\xd7\xa7\xd7\x94\xd7\x99\xd7\x9c\xd7\x94 \xd7\x9e\xd7\x99\xd7
\x95\xd7\x97\xd7\x93\xd7\x99\xd7\x9d
        ...
        \xd7\x90\xd7\xaa\xd7\x94 \xd7\x93\xd7\xaa.'''  # noqa
        ...
        gif = os.path.join(testsFolder, 'pythonpowered.gif')
        heading = Paragraph(
            b'\xd7\x96\xd7\x95\xd7\x94\xd7\x99 \xd7' +
            b'\x9b\xd7\x95\xd7\xaa\xd7\xa8\xd7\xaa', h3)
        story.append(ImageAndFlowables(Image(gif), [heading, Paragraph(text, bt)]))
        heading = Paragraph(
            b'\xd7\x96\xd7\x95\xd7\x94\xd7\x99 \xd7' +
            b'\x9b\xd7\x95\xd7\xaa\xd7\xa8\xd7\xaa', h3)
        story.append(ImageAndFlowables(Image(gif), [heading, Paragraph(text1, bt)]))
        doc = MyDocTemplate(outputfile('test_platypus_imageandflowables_rtl.pdf'),
                            showBoundary=1)
        doc.multiBuild(story)
    ...
    def test2_RTL(self):
        ...
        text = u'\u05e9\u05dc\u05d5\u05dd! \u05d6\u05d5 \u05ea\u05d4\u05d9\u05d4 \u05e4\u05e1\u05e7\u05d4 \u05e9\u05d4\u05e9\u05d5\u05e8\u05d4 \u05d4\u05d0\u05d7\u05e8\u05d5\u05e0\u05d4 \u05e9\u05dc\u05d4 \u05dc\u05d0 \u05ea\u05d5\u05e6\u05d3\u05e7 \u05db\u05d4\u05dc\u05db\u05d4.'  # noqa
        ...
        rtext = text.replace(u'\u05db\u05d4\u05dc\u05db\u05d4',
                             u'<span color=red>\u05db\u05d4\u05dc\u05db\u05d4</span>')
    ...
    def test_splitJustBug(self):
        ...
        text = '''<para><onDraw name="_odW" label="start"/>First line<onDraw name="_odW"
label="end"/><br/><onDraw name="_odW" label="start"/>Second line<onDraw name="_odW"
label="end"/><br/><onDraw name="_odW" label="start"/>split here<onDraw name="_odW"
label="end"/><br/><onDraw name="_odW" label="start"/>Third line should not be
justified<onDraw name="_odW" label="end"/><br/></para>'''
        ...
        aH -= h
        canv.save()
        self.assertEqual(M0, measures,
                         "difference detected in justified split Paragraph rendering")
    ...
    def test_unicharCodeSafety(self):
        ...
        self.assertRaises(
            Exception,
            Paragraph,
            """<unichar code="open('/tmp/test.txt','w').
write('Hello from unichar')"/>""",
            normal)
...
class TwoFrameDocTemplate(BaseDocTemplate):
    ...
    def __init__(self, filename, **kw):
        ...
        ch -= 14*cm
        f1 = Frame(
            m, m+0.5*cm, cw-0.75*cm, ch-1*cm, id='F1',
            leftPadding=0, topPadding=0, rightPadding=0, bottomPadding=0,
            showBoundary=True
        )
        f2 = Frame(
            cw+2.7*cm, m+0.5*cm, cw-0.75*cm, ch-1*cm, id='F2',
            leftPadding=0, topPadding=0, rightPadding=0, bottomPadding=0,
            showBoundary=True
        )
        BaseDocTemplate.__init__(self, filename, **kw)
...
class SplitFrameParagraphTest(unittest.TestCase):
    ...
    def test(self):
        ...
        text = (b"Bedauerlicherweise ist ein Donaudampfschiffkapit\xc3\xa4n " +
                b"auch <font color='red'>nur</font> <font color='green'>ein</font> " +
                b"Dampfschiffkapit\xc3\xa4n.")
        # tagFormat = '%s' (assigned but not used)
        # strange behaviour when using next code line
        # (same for '<a href="http://www.reportlab.org">%s</a>'
        # tagFormat = '<font color="red">%s</font>' (assigned but not used)
        ...
        doc = TwoFrameDocTemplate(outputfile('test_paragraphs_splitframe.pdf'),
                                  pagesize=PAGESIZE)
        doc.build(story)
...
class FragmentTestCase(unittest.TestCase):
    ...
    def test2(self):
        '''test _splitWord'''
        self.assertEqual(
            _splitWord(u'd\'op\u00e9ration', 30, [30], 0, 'Helvetica', 12),
            [u'',  u"d'op\xe9", u'ratio', u'n'])
        self.assertEqual(
            _splitWord(b'd\'op\xc3\xa9ration', 30, [30], 0, 'Helvetica', 12),
            [u'', u"d'op\xe9", u'ratio', u'n'])
    ...
    def test3(self):
        ...
        nF = BF.clone
        ww = 'unused width'
        W = [ww, (nF(cbDefn=ABag(kind='index', width=0)), ''),
             (BF, 'a'), (nF(fontSize=10), 'bbb'),
             (nF(fontName='Helvetica-Bold'), 'ccccc')]
        self.assertEqual(
            _fragWordSplitRep(W),
            (u'abbbccccc',
             ((2, 0), (3, 1), (3, 1), (3, 1), (4, 4), (4, 4),
              (4, 4), (4, 4), (4, 4))))
        W[1][0].rise = 2
        self.assertEqual(_fragWordSplitRep(W), None)
        W = [ww, (nF(cbDefn=ABag(kind='img', width=1)), ''),
             (BF, 'a'), (BF, 'bbb'), (BF, 'ccccc')]
        self.assertEqual(_fragWordSplitRep(W), None)

    def test4(self):
        from reportlab.platypus.paragraph import (_hy_letters_pat, _hy_shy_letters_pat,
                                                  _hy_letters, _hy_pfx_pat, _hy_sfx_pat)
        self.assertIsNotNone(
            _hy_shy_letters_pat.match(_hy_letters),
            'pre-hyphenated word match should succeed')
        self.assertIsNone(
            _hy_letters_pat.match(_hy_letters),
            'all letters word match should fail')
        self.assertIsNotNone(
            _hy_letters_pat.match(_hy_letters.replace(u'-', u'')),
            'all letters word match should succeed')
        pfx = u'\'"([{\xbf\u2018\u201a\u201c\u201e'
        m = _hy_pfx_pat.match(pfx)
        self.assertIsNotNone(m, 'pfx pattern should match')
        self.assertEqual(len(m.group(0)), len(pfx),
                         'pfx pattern should match %d characters not %d' % (
                             len(pfx), len(m.group(0))))
        sfx = u']\'")}?!.,;:\u2019\u201b\u201d\u201f'
        m = _hy_sfx_pat.search(sfx)
        self.assertIsNotNone(m, 'sfx pattern should match')
        self.assertEqual(len(m.group(0)), len(sfx),
                         'sfx pattern should match %d characters not %d' % (
                             len(sfx), len(m.group(0))))

    def test5(self):
        from reportlab.platypus.paragraph import (_hyphenateWord, _hyphenateFragWord,
                                                  _rebuildFragWord, stringWidth, ABag)
        # w = 'https://www.reportlab.com/pypi/packages' (assigned but not used)
        fontName = 'Helvetica'
        fontSize = 12
        ...
        def applyTest(w, uriWasteReduce, embeddedHyphenation, ex, split=None):
            if not split:
                ...
                r = _hyphenateWord(None, fontName, fontSize, w, ww, ww+10, ww+5,
                ...
                ww = fw[0]
                r = _hyphenateFragWord(None, fw, ww+10, ww+5, uriWasteReduce,
                                       embeddedHyphenation)
                if r is not None:
                    _r = r
                    r = [u''.join((_[1] for _ in _fw[1:])) for _fw in _r]
            self.assertEqual(
                r, ex, 'hyphenation of w=%r u=%r e=%r ex=%r split=%r failed' % (
                    w, uriWasteReduce, embeddedHyphenation, ex, split))
        for split in (None, 0.001, 0.1, 0.5, 0.8, 0.9, 1.0):
            applyTest('https://www.reportlab.com/pypi/packages',
                      0.3, True,
                      [u'https://www.reportlab.com/pypi/', u'packages'],
                      split=split)
            applyTest('https://www.reportlab.com/pypi/packages',
                      False, True, None, split=split)
            applyTest('https://www.repor-tlab.com/pypi/packages',
                      0.3, True, [u'https://www.repor-tlab.com/pypi/', u'packages'],
                      split=split)
            # not a uri (no :) and contains - and non letters
            applyTest('https//www.repor-tlab.com/pypi/packages',
                      0.3, True, None, split=split)
            applyTest('httpsSSwwwDrepor-tlabDcomSpypiSpackages',
                      0.3, True, [u'httpsSSwwwDrepor-', u'tlabDcomSpypiSpackages'],
                      split=split)  # should succeed because '-' with no non-letters
            # fails because embeddedHyphenation=False
            applyTest('httpsSSwwwDrepor-tlabDcomSpypiSpackages',
                      0.3, False, None, split=split)
    ...
    def test6(self):
        ...
        # raw = """This is a splittable word 'disestablishment'!""" (set but not used)
        for text in ("""This is a splittable word '<span color="red">dis</span><span
color="blue">estab</span><span color="green">lish</span>
<span color="magenta">ment</span>'!""",
                     """This is a splittable word 'd<span color="red">ise</span>
<span color="blue">stabl</span><span color="green">ishm</span>
<span color="magenta">ent</span>'!""",
                     """This is a splittable word 'di<span color="red">ses</span><span
color="blue">tabli</span><span color="green">shme</span>
<span color="magenta">ent</span>'!""",
                     """This is a splittable word 'disestablishment'!""",
                     ):
            aW = stringWidth("This is a splittable word 'dis", bt.fontName,
                             bt.fontSize)
            y = t(x, y, aW)
            aW = stringWidth("This is a splittable word 'dise", bt.fontName,
                             bt.fontSize)
            y = t(x, y, aW)
            aW = stringWidth("This is a splittable word 'disestabl", bt.fontName,
                             bt.fontSize)
            y = t(x, y, aW)
            aW = stringWidth("This is a splittable word 'disestabli", bt.fontName,
                             bt.fontSize)
            y = t(x, y, aW)
            aW = stringWidth("This is a splittable word 'disestablishm", bt.fontName,
                             bt.fontSize)
            y = t(x, y, aW)

        canv.showPage()
        y = canv._pagesize[1] - 72
        nt = bt.clone('nt', fontName='Helvetica', fontSize=12, leading=16,
                      alignment=TA_JUSTIFY)
        naW, naH = 342.992125984252, 56.69291338582681
        for ntext in (b"Bedauerlicherweise ist ein Donaudampfschiffkapit\xc3\xa4n " +
                      b"auch <font color='red'>nur</font> <font color='green'>ein" +
                      b"</font> Dampfschiffkapit\xc3\xa4n.",
                      b"Bedauerlicherweise ist ein Donaudampfschiffkapit\xc3\xa4n " +
                      b"auch nur ein Dampfschiffkapit\xc3\xa4n.",
                      ):
    ...
    def test7(self):
        ...
        aW = 51.0236220472
        text = (u'\u0440\u044b\u0431\u0430 \u043f\u0438\u043b\u0430 \u0438 \u0431' +
                u'\u0430\u0431\u0443 \u0434\u0430\u0436\u0435 \u0432\u0435\u043b' +
                u'\u0430 \u043d\u0430 \u043c\u0430\u044f\u043a')
        text1 = (u'\u0440\u044b\u0431\u0430 <span color="blue">\u043f\u0438\u043b' +
                 u'\u0430</span> \u0438 \u0431\u0430\u0431\u0443 \u0434\u0430\u0436' +
                 u'\u0435 \u0432\u0435\u043b\u0430 \u043d\u0430 \u043c\u0430' +
                 u'\u044f\u043a')
        tmpls = [
        ...
        for ex, x, hymwl in [
            ...
            kwds = dict(hyphenationMinWordLength=hymwl) if hymwl is not None else {}
            template = tmpls[x]
            t = template % locals()
            p = Paragraph(
                    template % locals(),
                    style=ParagraphStyle('P10H5', fontSize=10, fontName="Vera",
                                         hyphenationLang="ru_RU", **kwds),
                    )
            w, h = p.wrap(aW, 0x7fffffff)
            self.assertEqual(
                h, ex,
                ('Russion hyphenation test failed for ex=%(ex)s ' +
                 'template=%(template)r hymwl=%(hymwl)r h=%(h)s\ntext=%(t)r') %
                locals())
    ...
    def test8(self):
        ...
        from reportlab.platypus.paragraph import Paragraph
        pW, pH = A4
        canv = Canvas(outputfile('test_platypus_paragraphs_hysplit.pdf'),
                      pagesize=(pW, pH))
        ...
        mstext = (u'<span color="red">Super\xad</span>cali\xadfragi\xadlistic' +
                  u'\xadexpi\xadali\xaddocious')
        mstext1 = (u'Super\xadcali\xadfragi\xadlistic\xadexpi\xadali\xad<span ' +
                   u'color="green">docious</span>')
        text = u'Supercalifragilisticexpialidocious'
        mtext2 = ('converted to a <b>Python</b> <font name=Courier>' +
                  '<nobr>string</nobr></font>.')
        mtext3 = ('This one uses fonts with size "14pt" and also uses the em and ' +
                  'strong tags: Here comes <font face="Helvetica" size="14pt">' +
                  'Helvetica 14</font> with <Strong>strong</Strong> <em>emphasis</em>.')
        textF = ('Figure [seq template="%(Chapter)s-%(FigureNo+)s"/] - ' +
                 'Multi-level templates')
        mtextF = ('Figure &lt;seq template="%(Chapter)s-%(FigureNo+)s"/&gt; - ' +
                  'Multi-level templates')
        sty0 = ParagraphStyle(
            name="base",
            fontName="Helvetica",
        ...
        y = doTest('Multi Frag Free Hyphenation 2', mtext2, x, y, 77,
                   st=sty1, split=False, wc=2) - 5
        y = doTest('Multi Frag Free Hyphenation 3', mtext3, x, y,
                   439.27559055118115, st=styN, split=False, wc=1) - 5
        y = doTest('Multi Frag Soft Hyphenation', mstext, x, y, aW) - 5
        y = doTest('Multi Frag Soft Hyphenation 1', mstext1, x, y, aW) - 5
        y = doTest('Single Frag Free F', textF, x, y,
                   158.13921259842525, st=styF, split=False, wc=3) - 5
        y = doTest('Multi Frag Free F', mtextF, x, y,
                   158.13921259842525, st=styF, split=False, wc=3) - 5
        c = '\n'.join(canv._code[cp0:])
        if not isBytes(c):
            c = c.encode('utf8')
        ...
        self.assertEqual(xh, h,
                         'test8 code is no longer correct %s != expected %s' % (h, xh))
...
class ULTestCase(unittest.TestCase):
    ...
    def testUl(self):
        ...
        styleSheet = getSampleStyleSheet()
        normal = ParagraphStyle(name='normal', fontName='Times-Roman', fontSize=12,
                                leading=1.2*12, parent=styleSheet['Normal'])
        normal_sp = ParagraphStyle(name='normal_sp', parent=normal,
                                   alignment=TA_JUSTIFY, spaceBefore=12)
        normal_just = ParagraphStyle(name='normal_just', parent=normal,
                                     alignment=TA_JUSTIFY)
        normal_right = ParagraphStyle(name='normal_right', parent=normal,
                                      alignment=TA_RIGHT)
        normal_center = ParagraphStyle(name='normal_center', parent=normal,
                                       alignment=TA_CENTER)
        normal_indent = ParagraphStyle(name='normal_indent', firstLineIndent=0.5*inch,
                                       parent=normal)
        normal_indent_lv_2 = ParagraphStyle(name='normal_indent_lv_2',
                                            firstLineIndent=1.0*inch, parent=normal)
        texts = ['''Furthermore, a subset of <font size="14">English sentences</font>
interesting on quite independent grounds is not quite equivalent to a stipulation
...
phonemic and morphological analysis.''']
        story = []
        a = story.append
        a(Paragraph("This should &lt;a href=\"#theEnd\" color=\"blue\"&gt;<a " +
                    "href=\"#theEnd\" color=\"blue\">jump</a>&lt;/a&gt; jump to " +
                    "the end!", style=normal))
        a(XPreformatted("This should &lt;a href=\"#theEnd\" color=\"blue\"&gt;" +
                        "<a href=\"#theEnd\" color=\"blue\">jump</a>&lt;/a&gt; " +
                        "jump to the end!", style=normal))
        a(Paragraph(
        ...
        a(XPreformatted(
            ...
            style=normal))
        a(Paragraph(
            "This <font color='CMYKColor(0,0.6,0.94,0)'>should</font> &lt;a href=\"#" +
            "thePenultimate\" color=\"blue\"&gt;<a href=\"#thePenultimate\" color=" +
            "\"blue\">jump</a>&lt;/a&gt; jump to the penultimate page!",
            style=normal))
        a(Paragraph(
            "This should &lt;a href=\"#theThird\" color=\"blue\"&gt;<a href=\"#the" +
            "Third\" color=\"blue\">jump</a>&lt;/a&gt; jump to a justified para!",
            style=normal))
        a(Paragraph(
            "This should &lt;a href=\"#theFourth\" color=\"blue\"&gt;<a href=\"#the" +
            "Fourth\" color=\"blue\">jump</a>&lt;/a&gt; jump to an indented para!",
            style=normal))
        for mode in (0, 1):
            text0 = texts[0]
            text1 = texts[1]
            if mode:
                text0 = text0.replace(
                    ...
                    '<b>English sentences</b>').replace(
                        'quite equivalent', '<i>quite equivalent</i>')
                text1 = text1.replace('the methodological work',
                                      '<b>the methodological work</b>').replace(
                                          'to impose problems',
                                          '<i>to impose problems</i>')
            for t in ('u', 'strike'):
                for n in range(6):
                    for s in (normal, normal_center, normal_right, normal_just,
                              normal_indent, normal_indent_lv_2):
                        for autoLeading in ('', 'min', 'max'):
                            if (n == 4 and s == normal_center and
                                    t == 'strike' and mode == 1):
                                a(Paragraph(
                                    "<font color=green>The second jump at the " +
                                    "beginning should come here &lt;a name=\"the" +
                                    "Penultimate\"/&gt;<a name=\"thePenultimate\"/>" +
                                    "!</font>", style=normal))
                            elif (n == 4 and s == normal_just and t == 'strike'
                                  and mode == 1):
                                a(Paragraph(
                                    "<font color=green>The third jump at the " +
                                    "beginning should come just below here to a " +
                                    "paragraph with just an a tag in it!</font>",
                                    style=normal))
                                a(Paragraph(
                                    "<a name=\"theThird\"/>", style=normal))
                            elif (n == 4 and s == normal_indent and t == 'strike' and
                                  mode == 1):
                                a(Paragraph(
                                    "<font color=green>The fourth jump at the " +
                                    "beginning should come just below here!</font>",
                                    style=normal))
                                a(AnchorFlowable('theFourth'))
                            a(Paragraph('n=%d style=%s(autoLeading=%s) tag=%s' % (
                                n, s.name, autoLeading, t), style=normal_sp))
                            a(Paragraph(
                                ('<para autoleading="%s">%s<%s>%s</%s>. ' +
                                 '%s <%s>%s</%s>. %s</para>') % (
                                    autoLeading,
                                    (s == normal_indent_lv_2 and
                                     ('<seq id="document" inc="no"/>.' +
                                      '<seq id="document_lv_2"/>') or ''),
                                     t, ' '.join((n+1)*['A']), t, text0, t,
                                     ' '.join((n+1)*['A']), t, text1),
                                style=s))
        a(Paragraph(
            "The jump at the beginning should come here &lt;a name=\"theEnd\"/&gt;" +
            "<a name=\"theEnd\"/>!",
            style=normal))
        a(Paragraph(
            'Underlining <span fontSize="11"><u color="red">A<u color="green">B</u>' +
            '<u color="blue">C</u>D<sup><strike width="0.5" color="magenta">2' +
            '</strike><sup><u color="darkgreen" width="0.2">3</u></sup></sup></u>' +
            '</span>', normal))
        a(Paragraph(
            '<para autoLeading="max" spaceAfter="10">this is in 12 <font size=30>' +
            'this is in 30</font> <u offset="-0.5" width="0.5" color="red"><u offset' +
            '="-1.5" width="0.5" color="blue">and</u></u> <link underline="1" ucolor' +
            '="blue" href="http://google.com/">the link box<sup><a color="red" ucolor' +
            '="green" underline="1" href="https://www.reportlab.com">2</a></sup> is ' +
            'right (twice).</link></para>''', normal))
        a(Paragraph(
            '<para autoLeading="max" spaceAfter="10">this is in 12 <font size=30>' +
            'this is in 30</font> and <link underline="1" ucolor="blue" href="http' +
            '://google.com/">the link box is right.</link></para>''', normal))
        a(Paragraph(
            'Underlining <u><span color="red">underlined in red? <span color="blue">' +
            '<u>or blue</u></span> or red again?</span></u>', normal))
        a(Paragraph(
            'Link <a href="#theEnd" color="blue">jump</a> to end.<br/>Underlined ' +
            'link <a href="#theEnd" underline="1" ucolor="red" color="blue">jump' +
            '</a> to end!', style=normal))
        a(Paragraph('<para autoleading=""><u>A</u>. Furthermore, a subset of <font ' +
                    'size="14">English sentences</font> interesting on quite\n' +
                    'independent grounds is not quite equivalent to a stipulation ' +
                    'to place\nthe constructions into these various categories. ' +
                    '<u>A</u>. We will bring evidence in favor of\nThe following ' +
                    'thesis: most of the methodological work in modern\nlinguistics ' +
                    'can be defined in such a way as to impose problems of\n' +
                    'phonemic and morphological analysis.</para>', normal))
        a(Paragraph(
            '<para autoleading=""><u>A</u>. Furthermore, a subset of <font size=' +
            '"14">English sentences</font> interesting on quite<br/><u>A</u>.</para>',
            normal))
        a(Paragraph(
            u"<para>This is a <sup rise=5><span color='red'>" +
            u"sup</span></sup>rise=5.</para>", normal))
        a(Paragraph(
            '<span fontSize="11"><u color="green"><strike color="blue">AAAAAA' +
            '</strike></u></span>', normal))
        a(Paragraph(
            "Underlining &amp; width proportional to first use font size ('f' " +
            "suffix) <u offset='-0.125*f' width='0.05*f'>underlined <span size=14>" +
            "underlined</span></u>!", style=normal))
        a(Paragraph(
            "Underlining &amp; width proportional to first use font size ('F' " +
            "suffix) <u offset='-0.125*F' width='0.05*F'>underlined <span size=14>" +
            "underlined</span></u>!", style=normal))
        a(Paragraph('''<para spaceBefore="10">This is underlined &lt;sup&gt;:
a<sup><u><span color="red">sup</span></u></sup></para>''', style=normal))
        a(Paragraph('''<para spaceBefore="10">This is <u>underlined</u></para>''',
                    style=normal))
        a(Paragraph('''<para spaceBefore="10">This is <u kind="double">
underlined double</u></para>''', style=normal))
        a(Paragraph('''<para spaceBefore="10">This is
<strike>striken</strike></para>''', style=normal))
        a(Paragraph('''<para spaceBefore="10">This is
<strike><u>both</u></strike></para>''', style=normal))
        a(Paragraph('''<para spaceBefore="10">This is <u width="0.5" offset="-1"
kind="double">underlined kind="double"</u></para>''', style=normal))
        a(Paragraph('''<para spaceBefore="10">This is <u width="0.25" offset="-1"
kind="double">double underlined with thinner lines</u></para>''', style=normal))
        a(Paragraph('''<para spaceBefore="10">This is <u width="0.5" offset="-0.5"
color="red">underlined in red</u></para>''', style=normal))
        a(Paragraph('''<para spaceBefore="10">This is <strike width="0.5" color="red">
overstruck in red</strike></para>''', style=normal))
        a(Paragraph('''<para spaceBefore="10">This is <strike width="0.5" color="red"
kind="double">doubly overstruck in red</strike></para>''', style=normal))
        a(Paragraph('''<para spaceBefore="10">This is <strike width="0.5" offset=
"0.125*F" color="red" kind="triple" gap="0.5">triply overstruck in
red</strike></para>''', style=normal))
        a(Paragraph('''<para autoLeading="max" spaceAfter="10" spaceBefore="30">this
is in 12 <font size="30">this is in 30</font> <u offset="-0.5" width="0.5" color="red">
<u offset="-1.5" width="0.5" color="blue">and</u></u> <link underline="1" ucolor="blue"
href="http://google.com/">the link box<sup><a color="red" ucolor="green" underline="1"
href="https://www.reportlab.com">2</a></sup> is right (twice).</link></para>''',
                    style=normal))
        a(Paragraph("", style=normal))
        # 3.5.x had a bug with leftIndent and underlines, check that
        left_indent = ParagraphStyle(name='left_indent',
                                     leftIndent=80, parent=styleSheet['Normal'])
        a(Paragraph("<u>Replicating a left indent underline bug.</u>",
                    style=left_indent))

        # testing PR 49
        a(Paragraph(
            "This paragraph contains <strike>really</strike> large <u>spaces</u> " +
            "<strike color='blue' offset='1' width='0.5'><u color='red' offset='-1' " +
            "width='0.5'>between words</u></strike>. ThisLongWordDoesntFitIntoPrevi" +
            "ousLineCausingWordsSpread", style=normal_just))
...
class AutoLeadingTestCase(unittest.TestCase):
    ...
    def testAutoLeading(self):
        ...
        texts = ['''Furthermore, a subset of <font size="14">English sentences</font>
interesting on quite independent grounds is not quite equivalent to a stipulation
to place <font color="blue">the constructions
<img src="%(testsFolder)s/../docs/images/testimg.gif"/> into these various
categories.</font>''' % dict(testsFolder=testsFolder),
                 '''We will bring <font size="18">Ugly Things</font> in favor of
The following thesis:  most of the methodological work in Modern
Linguistics can be <img src="%(testsFolder)s/../docs/images/testimg.gif"
valign="baseline" /> defined in such
<img src="%(testsFolder)s/../docs/images/testimg.gif" valign="10" />
a way as to impose problems of phonemic and <u>morphological
<img src="%(testsFolder)s/../docs/images/testimg.gif" valign="top"/>
</u> analysis.''' % dict(testsFolder=testsFolder)]
        story = []
        a = story.append
        t = 'u'
        n = 1
        for s in (normal, normal_sp):
            for autoLeading in ('', 'min', 'max'):
                a(Paragraph('style=%s(autoLeading=%s)' %
                            (s.name, autoLeading), style=normal_sp))
                a(Paragraph(
                    '<para autoleading="%s"><%s>%s</%s>. %s <%s>%s</%s>. %s</para>' % (
                        autoLeading, t, ' '.join((n+1)*['A']), t, texts[0], t,
                        ' '.join((n+1)*['A']), t, texts[1]), style=s))
        a(Paragraph(
            '''<img src="%(testsFolder)s/../docs/images/testimg.gif" valign="top"/>
image is very first thing in the line.''' % dict(testsFolder=testsFolder),
            style=normal))
        a(Paragraph('some text.... some more.... some text.... some more....', normal))
        a(Paragraph(
            ('<img src="%(testsFolder)s/../docs/images/testimg.gif" width="0.57in" ' +
             'height="0.19in" /> some text <br /> ') % dict(testsFolder=testsFolder),
            normal))
        a(Paragraph('some text.... some more.... some text.... some more....', normal))
        a(Paragraph(
            ('<img src="%(testsFolder)s/../docs/images/testimg.gif" ' +
             'width="0.57in" height="0.19in" /> <br /> ') %
            dict(testsFolder=testsFolder), normal))
        a(Paragraph('some text.... some more.... some text.... some more....', normal))

        # Volker Haas' valign tests
        fmt = '''<font color="red">%(valign)s</font>: Furthermore, a <u>subset</u>
<strike>of</strike> <font size="14">English sentences</font> interesting on quite
independent grounds is not quite equivalent to a stipulation to place
<img src="%(testsFolder)s/../docs/images/redsquare.png"
width="0.5in" height="0.5in" valign="%(valign)s"/>
the constructions into these <u>various</u> categories. We will bring
<font size="18">Ugly Things</font> in favor of The following thesis:  most
of the methodological work in Modern Linguistics can be defined in such a way
as to impose problems of phonemic and <u>morphological</u> <strike>analysis</strike>.'''
        ...
        for valign in (
            ...
            a(XPreformatted(fmt % dict(valign=valign, testsFolder=testsFolder),
                            p_style))

        a(Paragraph(
            '<br/><b>Some Paragraph tests of &lt;sup rise="pts" size="pts"</b>...',
            normal))
        a(Paragraph(
            "<br/>This is a <sup><span color='red'>sup</span></sup>.", p_style))
        a(XPreformatted(
            "This is a <sup><span color='red'>sup</span></sup>.", p_style))
        a(Paragraph(
            "This is a <sup rise=5><span color='red'>sup</span></sup>rise=5.", p_style))
        a(XPreformatted(
            "This is a <sup rise=5><span color='red'>sup</span></sup>rise=5.", p_style))
        a(Paragraph(
            "This is a <sup rise=6><span color='red'>sup</span></sup>rise=6.", p_style))
        a(XPreformatted(
            "This is a <sup rise=6><span color='red'>sup</span></sup>rise=6.", p_style))
        a(Paragraph(
            "This is a <sup rise=7><span color='red'>sup</span></sup>rise=7.", p_style))
        a(XPreformatted(
            "This is a <sup rise=7><span color='red'>sup</span></sup>rise=7.", p_style))
        a(Paragraph(
            "This is a <sup rise=8><span color='red'>sup</span></sup>rise=8.", p_style))
        a(XPreformatted(
            "This is a <sup rise=8><span color='red'>sup</span></sup>rise=8.", p_style))
        a(Paragraph(
            "This is a <sup rise=9><span color='red'>sup</span></sup>rise=9.", p_style))
        a(XPreformatted(
            "This is a <sup rise=9><span color='red'>sup</span></sup>rise=9.", p_style))
        a(Paragraph(
            "This is a <sup size=7><span color='red'>sup</span></sup>size=7.", p_style))
        a(XPreformatted(
            "This is a <sup size=7><span color='red'>sup</span></sup>size=7.", p_style))
        a(Paragraph(
            "This is a <sup rise=5 size=7><span color='red'>" +
            "sup</span></sup>rise=5 size=7.", p_style))
        a(XPreformatted("This is a <sup rise=5 size=7><span color='red'>sup" +
                        "</span></sup>rise=5 size=7.", p_style))
        a(Paragraph("This is a <sup rise=6 size=7><span color='red'>sup</span>" +
                    "</sup>rise=6 size=7.", p_style))
        a(XPreformatted(
            "This is a <sup rise=6 size=7><span color='red'>sup" +
            "</span></sup>rise=6 size=7.", p_style))
        a(Paragraph(
            "This is a <sup rise=7 size=7><span color='red'>sup" +
            "</span></sup>rise=7 size=7.", p_style))
        a(XPreformatted(
            "This is a <sup rise=7 size=7><span color='red'>sup" +
            "</span></sup>rise=7 size=7.", p_style))
        a(Paragraph(
            "This is a <sup rise=8 size=7><span color='red'>sup" +
            "</span></sup>rise=8 size=7.", p_style))
        a(XPreformatted(
            "This is a <sup rise=8 size=7><span color='red'>sup" +
            "</span></sup>rise=8 size=7.", p_style))
        a(Paragraph(
            "This is a <sup rise=9 size=7><span color='red'>sup" +
            "</span></sup>rise=9 size=7.", p_style))
        a(XPreformatted(
            "This is a <sup rise=9 size=7><span color='red'>sup" +
            "</span></sup>rise=9 size=7.", p_style))
        a(Paragraph(
            "This is a <sup rise=90% size=70%><span color='red'>sup" +
            "</span></sup>rise=90% size=70%.", p_style))
        a(Paragraph(
            "This is a <sup rise=-2 size=-2><span color='red'>sup" +
            "</span></sup>rise=-2 size=-2.", p_style))
        a(Paragraph(
            "This is a <sup rise=-4 size=-3><span color='red'>sup" +
            "</span></sup>rise=-4 size=-3.", p_style))
        a(PageBreak())

        a(Paragraph(
            '<br/><b>Some Paragraph tests of &lt;img width="x%" height="x%"</b>...',
            normal))
        a(Paragraph(
            ('H=10%% <img src="%(testsFolder)s/../docs/images/testimg.gif" ' +
             'width="0.57in" height="10%%" />') % dict(testsFolder=testsFolder),
            normal))
        a(Paragraph(
            ('H=50%% <img src="%(testsFolder)s/../docs/images/testimg.gif" ' +
             'width="0.57in" height="50%%" />') % dict(testsFolder=testsFolder),
            normal))
        a(Paragraph(
            ('H=100%% <img src="%(testsFolder)s/../docs/images/testimg.gif" ' +
             'width="0.57in" height="100%%" />') % dict(testsFolder=testsFolder),
            normal))
        a(Paragraph(
            ('H=100%% W=10%% <img src="%(testsFolder)s/../docs/images/testimg.gif" ' +
             'width="10%%" height="100%%" />') % dict(testsFolder=testsFolder),
            normal))
        a(Paragraph(
            ('H=100%% W=50%% <img src="%(testsFolder)s/../docs/images/testimg.gif" ' +
             'width="50%%" height="100%%" />') % dict(testsFolder=testsFolder),
            normal))
        a(Paragraph(
            ('H=50%% W=50%% <img src="%(testsFolder)s/../docs/images/testimg.gif" ' +
             'width="50%%" height="50%%" />') % dict(testsFolder=testsFolder),
            normal))
        a(Paragraph(
            '<br/><b>Some XPreformatted tests of &lt;img ' +
            'width="x%" height="x%"</b>...', normal))
        a(XPreformatted(
            ('H=10%% <img src="%(testsFolder)s/../docs/images/testimg.gif" ' +
             'width="0.57in" height="10%%" />') % dict(testsFolder=testsFolder),
            normal))
        ...
        a(XPreformatted(
            ('H=100%% W=50%% <img src="%(testsFolder)s/../docs/images/testimg.gif" ' +
             'width="50%%" height="100%%" />') % dict(testsFolder=testsFolder), normal))
        a(XPreformatted(
            ('H=50%% W=50%% <img src="%(testsFolder)s/../docs/images/testimg.gif" ' +
             'width="50%%" height="50%%" />') % dict(testsFolder=testsFolder), normal))
        a(Paragraph(
            '<br/><b>Some CJK Paragraph tests of &lt;img ' +
            'width="x%" height="x%"</b>...', normal))
...
phonemic and morphological analysis.'''
        ...
        for mode in (0, 1, 2, 3, 4, 5, 6, 7):
            ...
            if mode == 1:
                text = text.replace(
                    'English sentences', '<b>English sentences</b>').replace(
                        'quite equivalent', '<i>quite equivalent</i>')
                text = text.replace(
                    'the methodological work',
                    '<b>the methodological work</b>').replace(
                        'to impose problems', '<i>to impose problems</i>')
                a(Paragraph('Justified paragraph in normal/bold/italic font',
                            style=normal))
            elif mode == 2:
                text = '<b>%s</b>' % text
                ...
                text = text.replace(
                    'English ', 'English&nbsp;').replace('quite ', 'quite&nbsp;')
                text = text.replace(
                    ' methodological', '&nbsp;methodological').replace(
                        ' impose', '&nbsp;impose')
                a(Paragraph(
                    'Justified paragraph in normal font &amp; some hard spaces',
                    style=normal))
            elif mode in (5, 6, 7):
                text = text.replace(
                    'as to impose', '<br/>as to impose').replace(
                        ' most of the', '<br/>most of the')
                text = text.replace(
                    ' grounds', '<br/>grounds').replace(' various', '<br/>various')
                if mode in (6, 7):
                    msg = []
                    msg.append('justifyBreaks=1')
                    paraStyle = paraStyle.clone('paraStyle6', paraStyle,
                                                justifyBreaks=1)
                    if mode == 7:
                        msg.append('justifyLastLine=3')
                        paraStyle = paraStyle.clone('paraStyle7', paraStyle,
                                                    justifyLastLine=3)
                    msg = '(%s) ' % (' '.join(msg))
                else:
                    a(PageBreak())
                ...
                a(Paragraph('Justified%swith some &lt;br/&gt; tags' %
                            msg, style=normal))
            else:
    ...
    def testAutoPageTemplate(self):
        ...
        a(PageBreak())
        a(DocAssert('doc.pageTemplate.id=="autoFollow"',
                    'expected doc.pageTemplate.id=="autoFollow"'))
        ...
        doc = MyDocTemplate(outputfile(
            'test_platypus_paragraphs_AutoNextPageTemplate.pdf'))
        doc.build(story)
    ...
    def testParaBrFlowing(self):
        ...
        class MyDocTemplate(BaseDocTemplate):
            ...
            def __init__(self, filename, **kw):
                ...
                self.addPageTemplates([
                    PageTemplate('normal', [
                        Frame(inch, 4.845*inch, 3*inch, 3.645*inch, id='first',
                              topPadding=0, rightPadding=0, leftPadding=0,
                              bottomPadding=0,
                              showBoundary=ShowBoundaryValue(color="red")),
                        Frame(4.27*inch, 4.845*inch, 3*inch, 3.645*inch, id='second',
                              topPadding=0, rightPadding=0, leftPadding=0,
                              bottomPadding=0,
                              showBoundary=ShowBoundaryValue(color="red")),
                        Frame(inch, inch, 3*inch, 3.645*inch, id='third',
                              topPadding=0, rightPadding=0, leftPadding=0,
                              bottomPadding=0,
                              showBoundary=ShowBoundaryValue(color="red")),
                        Frame(4.27*inch, inch, 3*inch, 3.645*inch, id='fourth',
                              topPadding=0, rightPadding=0, leftPadding=0,
                              bottomPadding=0,
                              showBoundary=ShowBoundaryValue(color="red"))],),
                        ])
        styleSheet = getSampleStyleSheet()
        normal = ParagraphStyle(name='normal', fontName='Helvetica', fontSize=10,
                                leading=12, parent=styleSheet['Normal'])
        bold = ParagraphStyle(name='bold', fontName='Helvetica-Bold', fontSize=12,
                              leading=14.4, parent=normal)
        brText = u"""
...
descriptive <span color="red">fact</span>.<br/>So far,
the earlier discussion of deviance is not
...
<span color="red">order</span>.<br/>To provide a constituent structure
for T(Z,K), a subset of
...
descriptive <span color="red">fact</span>.<br/>To characterize a
linguistic level L, the notion of
...
the dominance scope of a complex <span color="red">symbol</span>.<br/>In
the discussion of
...
been defined by the paired utterance <span color="red">test</span>.<br/>From
C1, it follows that the
...
"""
    ...
    def testParaNBSP(self):
        ...
        normal = ParagraphStyle(name='normal', fontName='Helvetica', fontSize=10,
                                leading=12, parent=styleSheet['Normal'])
        bold = ParagraphStyle(name='normal', fontName='Helvetica-Bold', fontSize=10,
                              leading=12, parent=styleSheet['Normal'])
        registerFontFamily('Helvetica', 'Helvetica', 'Helvetica-Bold',
                           'Helvetica-Oblique', 'Helvetica-BoldOblique')
        story = []
        a = story.append
        a(Paragraph('Paragraph Hard Space Handling', bold))
        a(Paragraph(''' <span backcolor="pink">ABCDEFGHI</span> ''', normal))
        a(Paragraph(''' <span backcolor="palegreen">&nbsp;ABCDEFGHI&nbsp;</span> ''',
                    normal))
        a(Paragraph('''<span backcolor="lightblue">&nbsp;</span>''', normal))
        a(Paragraph('''<span backcolor="lightblue">&nbsp;</span><span
backcolor="palegreen">&nbsp;</span> ''', normal))
        a(Paragraph('''<span backcolor="pink"><b>A</b></span><span
backcolor="lightblue"> </span><span backcolor="pink"><b>B</b></span>''', normal))
        a(Paragraph('''<span backcolor="pink"><b>A</b></span> <span
backcolor="lightblue"> </span><span backcolor="pink"><b>B</b></span>''', normal))
        a(Paragraph('''<span backcolor="pink"><b>A</b></span><span
backcolor="lightblue">&nbsp;</span><span backcolor="pink"><b>B</b></span>''', normal))
...
backcolor="lightblue">&nbsp;</span><span backcolor="pink"><b>B</b></span>''', normal))
    ...
    def testParaEntities(self):
        tentities(b'unicode formatted paragraphs', False,
                  outputfile('test_platypus_unicode_paragraph_entities.pdf'))
        tentities(b'byte formatted paragraphs', True,
                  outputfile('test_platypus_bytes_paragraph_entities.pdf'))
    ...
    def testXPreUnderlining(self):
        ...
        story = [
                XPreformatted(
                    "xpre<u><font size='1' color='red'>SS</font>        </u>", bt),
                XPreformatted(
                    "xpre<u><font size='1' color='red'>SS</font>        </u>|", bt),
                XPreformatted(
                    "<u><font size='1' color='red'>SS</font>        </u>", bt),
                XPreformatted(
                    "<u><font size='1' color='red'>SS</font>        </u>|", bt),
                ]
...
# noruntests
def makeSuite():
    return makeSuiteForClasses(ParagraphCorners, SplitFrameParagraphTest,
                               FragmentTestCase, ParagraphSplitTestCase, ULTestCase,
                               JustifyTestCase, AutoLeadingTestCase)
