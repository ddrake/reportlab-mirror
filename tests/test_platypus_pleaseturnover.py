...
import sys
import unittest
from reportlab.platypus.flowables import PTOContainer, KeepInFrame
...
def _showDoc(fn, story):
    pageTemplate = PageTemplate(
        'normal', [Frame(72, 440, 170, 284, id='F1'),
...
def _ptoTestCase(self):
    ...
    def ptoblob(blurb, content, trailer=None, header=None, story=story, H1=H1):
        ...
        story.append(PTOContainer(
            [Paragraph(blurb, H1)]+list(content), trailer, header))
    ...
    ptoblob('2 PTO (inner split)', [
        ...
        PTOContainer([ColorParagraph(black, 'Inner Starts', H1),
                      ColorParagraph('yellow', text2, bt),
                      ColorParagraph('black', 'Inner Ends', H1)], t1, h1),
        ColorParagraph('magenta', text1, bt)], t0, h0)
...
def _KeepInFrameTestCase(self, mode, offset=0):
    ...
    def allModesKIF(just, ifb=True, width=170):
        if ifb:
            fbreak()
        story.append(KeepInFrame(width-offset, 284-offset, subStory(texts=(text0, )),
                                 mode=mode, hAlign=just))
        fbreak()
        story.append(KeepInFrame(width-offset, 284-offset,
