...
if 'fp_str' in _py_funcs:
    def _log_10(x, log=log, _log_e_10=log(10.0)):
        return log(x)/_log_e_10

    _fp_fmts = "%.0f", "%.1f", "%.2f", "%.3f", "%.4f", "%.5f", "%.6f"

    def fp_str(*a):
        '''convert separate arguments (or single sequence arg)
        into space separated numeric strings'''
        ...
        for i in a:
            ...
            if sa <= 1e-7:
                ...
                l = sa <= 1 and 6 or min(max(0, (6-int(_log_10(sa)))), 6)  # noqa
                n = _fp_fmts[l] % i
                if l:
                    j = len(n)
                    while j:
...
if 'sameFrag' in _py_funcs:
    def sameFrag(f, g):
        ...
        if (hasattr(f, 'cbDefn') or hasattr(g, 'cbDefn')
            ...
            return 0
        for a in ('fontName', 'fontSize', 'textColor', 'rise', 'us_lines', 'link',
                  "backColor", "nobr"):
