...
class AttrMapValue:
    ...
    def __repr__(self):
        return 'AttrMapValue(%s)' % ', '.join(['%s=%r' % i for i
...
#                              # bug? hook is not defined!
#                              hook=hook: hook(self, k, v, osa))})  # noqa
...
def addProxyAttribute(src, name, validate=None, desc=None, initial=None, dst=None):
    ...
    if not isSeq(dst):
        dst = dst,
    # D = [] (assigned but not used)
    # DV = [] (assigned but not used)
    for d in dst:
        if isSeq(d):
            # e is assigned but not used
            # d, e = d[0], d[1:]
            d = d[0]
        obj, attr = _findObjectAndAttr(src, d)
        # dA is assigned but not used
        # if obj:
