...
class RL_Codecs:
    ...
    def _makeCodecInfo(name, encoding_map, decoding_map):
        ...
        class StreamReader(Codec, codecs.StreamReader):
            pass
        C = Codec()
        return codecs.CodecInfo(C.encode, C.decode, streamreader=StreamReader,
                                streamwriter=StreamWriter, name=name)
    ...
    def __rl_codecs(name, cache=__rl_codecs_cache, data=__rl_codecs_data,
                    extension_codecs=__rl_extension_codecs, _256=True):
        try:
            return cache[name]
        except KeyError:
            if name in extension_codecs:
                ...
                e, r = data[name]
            cache[name] = c = (RL_Codecs._256_exception_codec(name, e, r) if _256 else
                               RL_Codecs._makeCodecInfo(name, e, r or {}))
        return c
